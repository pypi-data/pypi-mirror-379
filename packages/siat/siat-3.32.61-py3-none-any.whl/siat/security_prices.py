...
def get_price_stooq_0(ticker,start='MRM',end='today'):
    """
    从stooq抓取单个股价
    注意：
...
def get_price_stooq(ticker: str, start: str = 'MRM', end: str = 'today', interval: str = "d") -> pd.DataFrame:
    """
    ...
    if (len(symbol_parts) == 1) and not ('^' in symbol):
    ...
    try:
        ...
        print("  No data found in stooq for",ticker,"from",start,'to',end)
        return None   
    ...
    if num > 0:
        ...
        print("  Zero records found from stooq for",ticker,"from",start,'to',end)
