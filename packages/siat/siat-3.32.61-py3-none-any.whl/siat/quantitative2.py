...
def security_trend_highlow(ticker, start='MRY', end='today', \
    ...
    ax.annotate(text_lang(f"近{window}日最高点({srounds(high_end)})",f"Recent {window} days highest ({srounds(high_end)})"), 
    ...
    ax.annotate(text_lang(f"近{window}日最低点({srounds(low_end)})",f"Recent {window} days lowest ({srounds(low_end)})"), 
    ...
    titletxt_cn=f"证券价格高低点趋势分析：{ticker_name(ticker)}"
    #titletxt_en=f"Security Price High-Low Trend: {ticker_name(ticker)}, Recent {window} (Trading) Days"
    titletxt_en=f"Security Price High-Low Trend: {ticker_name(ticker)}"
    titletxt=text_lang(titletxt_cn,titletxt_en)
    plt.title('\n'+titletxt+'\n',fontweight='bold',fontsize=14)
    
    import datetime; todaydt = datetime.date.today()
    sourcetxt=text_lang("注：基于交易日天数。数据来源：新浪","Note: based on number of trading days. Data source: Sina")
