...
def _pairwise_atoms(params: List[Dict[str, Any]], constraints: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    atoms: List[Dict[str, Any]] = []
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            a = names[i]
            b = names[j]
            for key_a, w_a in pmap[a]:
                for key_b, w_b in pmap[b]:
                    assign = {a: key_a, b: key_b}
                    if _constraints_contradict(assign, constraints):
                        continue
                    atom_id = f"T2:{a}={key_a}|{b}={key_b}"
                    atoms.append({
                        "id": atom_id,
                        "kind": "t2",
                        "params": [a, b],
...
def build_atoms(universe: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if t_wise >= 2:
        atoms.extend(_pairwise_atoms(params, constraints))
