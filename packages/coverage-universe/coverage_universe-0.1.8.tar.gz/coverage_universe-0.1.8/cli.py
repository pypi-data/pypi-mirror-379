...
def cmd_build_universe(args: argparse.Namespace) -> int:
    ...
    issues = list(uni.get("errors") or [])
    if issues:
        err_msgs = []
        warn_msgs = []
        for it in issues:
            if isinstance(it, dict):
                p = it.get("parameter") or it.get("param") or "(unknown)"
                if "error" in it:
                    err_msgs.append(f"{p}: {it.get('error')}")
                elif "warning" in it:
                    warn_msgs.append(f"{p}: {it.get('warning')}")
            else:
                warn_msgs.append(str(it))
        if err_msgs:
            print(f"Note: {len(err_msgs)} error(s) found; see output JSON under 'universe.errors'.")
            for m in err_msgs[:5]:
                print(f" - ERROR: {m}")
        if warn_msgs:
            print(f"Note: {len(warn_msgs)} warning(s) found; see output JSON under 'universe.errors'.")
            for m in warn_msgs[:5]:
                print(f" - WARNING: {m}")
        if getattr(args, "strict", False) and warn_msgs:
            print(f"Strict mode: warnings treated as errors ({len(warn_msgs)} warning(s)). Aborting.")
            raise SystemExit(3)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    b.add_argument("--strict", action="store_true", help="Treat UDL warnings (e.g., touching numeric ranges) as errors")
