...
def _getversion():
    ...
    # lxpyodbc.__version__, plus the setup function might require it.  We also need it in the
...
def main():
    ...
    setup(
        ...
        long_description=cleandoc("""
            lxpyodbc is a fork of pyodbc, an open source Python module that makes accessing ODBC databases simple.
            This is an LX fork that adds leanXcale-specific extensions, notably for arrays.
            Otherwise, like pyodbc, it implements the [DB API 2.0](https://www.python.org/dev/peps/pep-0249)
            specification but is packed with even more Pythonic convenience."""),
        maintainer="LX Support",
        maintainer_email="support@leanxcale.com",
        url='https://gitlab.leanxcale.com',
        ext_modules=[Extension('lxpyodbc', sorted(files), **settings)],
        include_package_data=False,
        packages=[''],
        package_dir={'': 'src'},
        package_data={'': ['lxpyodbc.pyi']},  # places pyodbc.pyi alongside pyodbc.{platform}.{pyd|so} in site-packages
        license='MIT-0',
        python_requires='>=3.8',
        classifiers=[
...
def get_compiler_settings():
    ...
    if os.name == 'nt':
        ...
        cflags='-DHAVE_UNISTD_H -DHAVE_PWD_H -DHAVE_SYS_TYPES_H -DHAVE_LONG_LONG -DSIZEOF_LONG_INT=8 -I/usr/local/include'
        settings['extra_compile_args'].extend(cflags.split())
        ldflags='-L/usr/local/lib -lodbc'
        ...
        # for LX
        settings['include_dirs'].append('include')
