...
void SQLWChar::init(PyObject* src, const TextEnc& enc)
{
    ...
    if (!pb && PyUnicode_Check(src))
        pb = PyUnicode_AsEncodedString(src, enc.name, "replace");
    ...
    if (pb)
    {
        ...
        // If the encoding failed (possibly due to "replace"), it will generate an exception, but
...
{
    PyObject* bytes = PyCodec_Encode(obj, name, "replace");
...
PyObject* TextBufferToObject(const TextEnc& enc, const byte* pbData, Py_ssize_t cbData)
{
    ...
    {
        case OPTENC_UTF8:
            return PyUnicode_DecodeUTF8((char*)pbData, cbData, "replace");

        case OPTENC_UTF16: {
            int byteorder = BYTEORDER_NATIVE;
            return PyUnicode_DecodeUTF16((char*)pbData, cbData, "replace", &byteorder);
        }
        ...
        case OPTENC_LATIN1:
            return PyUnicode_DecodeLatin1((char*)pbData, cbData, "replace");
    }

    // The user set an encoding by name.
    return PyUnicode_Decode((char*)pbData, cbData, enc.name, "replace");
}
