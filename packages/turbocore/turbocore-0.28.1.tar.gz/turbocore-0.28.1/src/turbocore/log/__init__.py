...
class ReflectorDaemon:
    ...
    def process(self, logfile:str, match:Optional[str], q: queue.Queue):
        lf = LineFollow(logfile, lambda x: q.put({"logfile": logfile, "line": x}) if match == "" or match == None or match.upper() in x.upper() else None)
        lf.process()
