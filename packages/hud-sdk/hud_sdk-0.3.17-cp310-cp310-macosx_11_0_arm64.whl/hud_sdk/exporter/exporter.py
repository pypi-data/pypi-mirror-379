...
class Exporter:
    ...
    async def send_json_for_session(
        ...
        if request_type == "Logs":
            ...
            regexes = create_censor_regexes(config.investigation_censor_regexes)
            for investigation in data:
                investigation_to_upload = censor_http_investigation(
                    investigation,
                    regexes,
                    config.investigation_censor_black_list_params,
                    config.investigation_whitelist_nested_keys,
                    config.investigation_blacklist_nested_keys,
                )
