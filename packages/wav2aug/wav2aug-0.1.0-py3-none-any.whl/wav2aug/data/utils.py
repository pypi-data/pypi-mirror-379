...
def _progress(done: int, total: int):
    ...
    if total > 0:
        ...
        sys.stdout.write(f"\rProgress [{bar}] {pct*100:5.1f}%  {_human_mb(done)}/{_human_mb(total)}")
    else:
...
def download_url(url: str, dst: str, *, show_progress: bool = True, sha256: str | None = None, user_agent: str = "wav2aug/1.0"):
    ...
    with urllib.request.urlopen(req) as r, tempfile.NamedTemporaryFile(delete=False, dir=os.path.dirname(dst)) as tmp:
        total = int(r.headers.get("Content-Length") or 0)
        done = 0
        last = time.monotonic()
        tty = show_progress and sys.stdout.isatty()
        if tty: _progress(0, total)
        while True:
            ...
            if tty and time.monotonic() - last >= 0.05:
                _progress(done, total); last = time.monotonic()
        if tty:
            _progress(done, total); sys.stdout.write("\n"); sys.stdout.flush()
        tmp_path = tmp.name
