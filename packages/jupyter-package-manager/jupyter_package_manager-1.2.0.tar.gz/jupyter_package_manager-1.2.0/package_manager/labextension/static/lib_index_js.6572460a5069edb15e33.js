        ...
        future.onIOPub = (msg) => {
            ...
            if (msgType === 'stream' ||
                ...
                if (content.text.includes('NOT_INSTALLED')) {
                    ...
                    window.dispatchEvent(new CustomEvent(EVENT_PACKAGES_INSTALLED, { detail: { packages: packageName } }));
                }
...
const PackageItem = ({ pkg }) => {
    ...
    const handleDelete = async () => {
        ...
        let confirm = false;
        if (window.electron) {
            confirm = await window.electron.invoke('show-confirm-dialog', `${(0,_translator__WEBPACK_IMPORTED_MODULE_3__.t)('Click "Ok" to confirm the deletion of')} ${pkg.name}.`);
        }
        else {
            confirm = window.confirm(`${(0,_translator__WEBPACK_IMPORTED_MODULE_3__.t)('Click "Ok" to confirm the deletion of')} ${pkg.name}.`);
        }
        if (confirm) {
            setLoading(true);
            setError(false);
...
//# sourceMappingURL=lib_index_js.6572460a5069edb15e33.js.map