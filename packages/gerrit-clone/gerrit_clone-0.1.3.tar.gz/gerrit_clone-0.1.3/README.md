...
#### Move Conflicting (Recommended for Data Mining)

```bash
gerrit-clone clone --host gerrit.example.org --move-conflicting
```
...
- With `--move-conflicting`: File `test` → `test.parent`, directory created for
  child repo
...
# Move conflicting content to allow complete cloning (default: false)
--move-conflicting
```
