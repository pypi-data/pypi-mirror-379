...
def get_odf( coefficients, G_odf, symmetry, n_max ):

    nlm, shsh_basis,_,_,_ = get_symmetrized_hshs(G_odf, n_max, symmetry)
            
    return coefficients @ shsh_basis

def get_odf_batch( coefficients_batch, G_odf, symmetry, n_max ):
    
    nlm, shsh_basis,_,_,_ = get_symmetrized_hshs(G_odf, n_max, symmetry)

    odf_batch = odf_batch_parallel(coefficients_batch.astype(data_type), shsh_basis.astype(data_type))
...
def get_odf_centered( coefficients, G_odf, symmetry, n_max ):
    orders = get_orders( symmetry, n_max, info=False )
    nlm, shsh_basis,_,symmetrization_matrix, slices = get_symmetrized_hshs(G_odf, n_max, symmetry)
    odf = coefficients @ shsh_basis
    g_mu = G_odf[ np.argmax(odf) ]

    c_centered = np.empty_like(coefficients)
    c_centered[0] = coefficients[0]
    for n in range(1,orders.size):
        order = orders[n]
        R_centering = Rs_n( np.array([-g_mu[0], g_mu[1], g_mu[2]], np.float64),
                                    order, symmetrization_matrix[str(order)] )
        c_centered[slices[n,0]:slices[n,1]] = R_centering @ coefficients[slices[n,0]:slices[n,1]]

    return c_centered @ shsh_basis

def get_odf_centered_batch( coefficients_batch, G_odf, G_mu, symmetry, n_max ):
    orders = get_orders( symmetry, n_max, info=False )
    nlm, shsh_basis,_,symmetrization_matrix,slices = get_symmetrized_hshs(G_odf, n_max, symmetry)
    ...
    for n in range(1,orders.size):
        order = orders[n]
        C_centered[:, slices[n,0]:slices[n,1]] = rotate_coefficients_n_parallel(
            np.column_stack( (-G_mu[:,0], G_mu[:,1], G_mu[:,2]) ).astype(np.float64),
            coefficients_batch[:,slices[n,0]:slices[n,1]].astype(np.float64), order, 
            symmetrization_matrix[str(order)])
    ...
    odf_batch = odf_batch_parallel(C_centered.astype(data_type), shsh_basis.astype(data_type))
...
def get_symmetrized_hshs(G_odf, n_max, symmetry):
    # first calculate the HSHs, then symmetrize, then sum over coefficients
    orders = get_orders( symmetry, n_max, info=False )

    symmetrization_matrix, slices_hsh, slices_shsh = get_symmetrization_matrix(orders, symmetry)
    
    n_hsh = slices_hsh[-1,1]
    n_shsh = slices_shsh[-1,1]
    ...
    shsh_basis = np.empty((n_shsh, G_odf.shape[0]))
    for i_n, n_str  in enumerate(symmetrization_matrix.keys()):
        shsh_basis[slices_shsh[i_n,0]:slices_shsh[i_n,1]] = np.real(
            symmetrization_matrix[n_str] @ hsh_basis[slices_hsh[i_n,0]:slices_hsh[i_n,1]])
            ## why are they not purely real??

    return nlm, shsh_basis, hsh_basis, symmetrization_matrix, slices_shsh
...
def get_symmetrization_matrix( orders, symmetry ):
    ...
    return Xsn, slices_hsh, slices_shsh
