...
class SynopticS3Pipeline(Pipeline):
    ...
    async def _fetch_presigned_media_url(self, item: Dict) -> str:
        ...
        url = f"https://api.synoptic.com/v1/streams/{self._stream_id}/post"
        headers={
    ...
    async def process_item(self, item: Dict, producer: str) -> Dict:
        if "mime_type" not in item: # no media to upload
            return item

        presigned_url = await self._fetch_presigned_media_url(item)
