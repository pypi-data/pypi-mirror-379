...
class SynopticS3Pipeline(Pipeline):
    ...
    async def _fetch_presigned_media_urls(self, item: Dict) -> Dict:
        ...
        json_data = {
            ...
            'releaseAt': item['published_at'],
            'uploadMedia': [{'mimeType': mime_type} for mime_type in mime_types],
        }
        try:
            ...
            self.logger.error("fetch_presigned_media_url", exception=str(e), mime_types=mime_types, pipeline=self.name)
        else:
