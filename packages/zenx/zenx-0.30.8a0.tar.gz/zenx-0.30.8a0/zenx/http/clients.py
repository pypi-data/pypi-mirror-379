    ...
    async def request(
        ...
        **kwargs,
        ...
        try:
            ...
            session.requests +=1
            self.logger.info("response", created_at=session.created_at, expired_at=session.expired_at, age=get_time() > session.expired_at, status=response.status_code, url=url, session_id=session.id, requests=session.requests, client=self.name, requested_at=req_at, responded_at=recv_at, latency_ms=latency)
