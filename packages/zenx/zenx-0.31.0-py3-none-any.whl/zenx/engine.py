...
class Engine:
    ...
    async def _execute_spider(self, spider_name: str, settings: Settings) -> None:
        ...
        try:
            ...
            # just using it for gracefull shutdown
            all_tasks = asyncio.all_tasks()
            for t in all_tasks:
                t.cancel()
            logger.debug("waiting", tasks=len(all_tasks), belong_to="asyncio", spider=spider_name)
    ...
    async def _execute_listener(self, listener_name: str, settings: Settings) -> None:
        ...
        try:
            ...
            for t in all_tasks:
                t.cancel()
            logger.debug("waiting", tasks=len(all_tasks), belong_to="asyncio", listener=listener_name)
            await asyncio.gather(*all_tasks, return_exceptions=True)
