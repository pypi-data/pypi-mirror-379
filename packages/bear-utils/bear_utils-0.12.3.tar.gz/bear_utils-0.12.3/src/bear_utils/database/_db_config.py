...
def from_db_url(db_url: str | SecretStr) -> DatabaseConfig:
    """Create a DatabaseConfig from a database URL."""
    warn(
        "Using db_url param is deprecated and will be removed in a future version. Please use DatabaseConfig directly.",
        DeprecationWarning,
        stacklevel=2,
    )
    if isinstance(db_url, str):
        db_url = SecretStr(db_url)
    url: str = db_url.get_secret_value()
    if url.startswith("sqlite"):
        return DatabaseConfig(scheme="sqlite", name=url.removeprefix("sqlite:///"))
