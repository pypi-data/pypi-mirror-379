...
def create_gail_model_expression():
    ...
    return [
        ...
        # Convert raw patient data to model categories
        ...
        [
            ...
            [
                "If",
                [["GreaterEqual", "age_menarche", 14], 0],  # ≥14 years
                [["GreaterEqual", "age_menarche", 12], 1],  # 12-13 years
                2  # <12 years
            ]
        ],
        ...
        # Age at first birth category (handle nulliparous)
        [
            "first_birth_category",
            [
                "If",
                [["Equal", "age_first_birth", None], 2],  # Nulliparous → 25-29 category
                [["Less", "age_first_birth", 20], 0],     # <20 years
                [["LessEqual", "age_first_birth", 24], 1], # 20-24 years
                [["LessEqual", "age_first_birth", 29], 2], # 25-29 years
                3  # ≥30 years
            ]
        ],
        ...
        # Based on Costantino 1999 Table 2
        ...
        [
            ...
            [
                "If",
                [["Equal", "age_menarche_category", 0], 1.00],  # ≥14 years
                [["Equal", "age_menarche_category", 1], 1.21],  # 12-13 years
                1.47  # <12 years
            ]
        ],

        # Biopsy history relative risk (age-dependent)
        [
            "rr_biopsies",
            [
                "If",
                [["Less", "current_age", 50],
                    ...
                    [
                        "If",
                        [["Equal", "num_biopsies_capped", 0], 1.00],  # No biopsies
                        [["Equal", "num_biopsies_capped", 1], 1.11],  # 1 biopsy
                        1.23  # ≥2 biopsies
                    ]
                ],
                # Age ≥50 years
                [
                    "If",
                    [["Equal", "num_biopsies_capped", 0], 1.00],  # No biopsies
                    [["Equal", "num_biopsies_capped", 1], 1.34],  # 1 biopsy
                    1.80  # ≥2 biopsies
                ]
            ]
        ],

        # Family history and age at first birth combined relative risk
        [
            "rr_family",
            [
                "If",
                [["Equal", "num_relatives_capped", 0], 1.00],  # No first-degree relatives
                [
                    ["Equal", "num_relatives_capped", 1],  # 1 first-degree relative
                    [
                        "If",
                        [["Equal", "first_birth_category", 0], 2.13],  # First birth <20 years
                        [["Equal", "first_birth_category", 1], 2.52],  # First birth 20-24 years
                        [["Equal", "first_birth_category", 2], 3.00],  # First birth 25-29 or nulliparous
                        3.56  # First birth ≥30 years
                    ]
                ],
                3.82  # ≥2 first-degree relatives
            ]
        ],

        # Combined relative risk (before attributable risk adjustment)
        ["raw_relative_risk", ["Multiply", "rr_menarche", "rr_biopsies", "rr_family"]],
...
if __name__ == "__main__":
    ...
    print(create_gail_model_expression())
