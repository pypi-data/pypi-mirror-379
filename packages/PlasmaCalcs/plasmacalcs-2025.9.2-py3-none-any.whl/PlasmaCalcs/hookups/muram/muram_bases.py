...
class MuramBasesLoader(MhdBasesLoader):
    ...
    @known_var(dims=['snap'])
    def get_T(self):
        '''Temperature. (directly from Muram)
        Per unit volume, e.g. the SI units would be K.
        assumes single fluid mode, i.e. result corresponds to the single fluid from Muram.
        '''
        self.assert_single_fluid_mode('T')
        return self.load_maindims_var_across_dims('result_prim_eosT', u='temperature', dims=['snap'])

    @known_var(dims=['snap'])
    def get_ne(self):
        '''Electron density. (directly from Muram)
        Per unit volume, e.g. the SI units would be m^3.
        assumes single fluid mode, i.e. result corresponds to the single fluid from Muram.
        '''
        self.assert_single_fluid_mode('ne')
        return self.load_maindims_var_across_dims('result_prim_eosne', u=None, dims=['snap'])
 
    @known_var(dims=['snap'])
    def get_P(self):
        '''Pressure. (directly from Muram)
        Per unit volume, e.g. the SI units would be .
        assumes single fluid mode, i.e. result corresponds to the single fluid from Muram.
        '''
        self.assert_single_fluid_mode('P')
        return self.load_maindims_var_across_dims('result_prim_eosP', u=None, dims=['snap'])
