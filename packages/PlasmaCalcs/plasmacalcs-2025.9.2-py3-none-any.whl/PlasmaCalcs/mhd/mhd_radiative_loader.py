...
class MhdRadiativeLoader(MhdBasesLoader):
    ...
    def _extra_kw_for_quantity_loader_call(self):
        ...
        The implementation here returns ['vdem_loopdim'] + any values from super().
        '''
        return ['vdem_loopdim'] + super()._extra_kw_for_quantity_loader_call
    ...
    def rcoords_logT(self, value):
        ...
        if minval < self.vdem_logT_min:
            errmsg = (f'cannot set self.rcoords_logT to something with minimum ({minval:.3g}) less than '
                      ...
                      f'(If you entered both simultaneously as kwargs, try swapping their order.)')
    ...
    def get_Gofnt_em_interp(self, var, *, _match=None): # uhm .... not sure why, but for not sharp temperature gradient has an artifact ... 
        ...
        with self.using(units='cgs'):      
            ...
            masktg, xn = self.interp_correction(np.log10(self('T')), bins=goftn.logT, dim=x, log=True)
            emiss_interped = emiss * (1.0 - xn) + emiss1 * xn  # emiss with interp correction weighting adjacent cells

            result = masktg * goftn * emiss_interped
    ...
    def vdem_pipeline(self, *, 
        ...
        sl = slice(iz0,self.get_maindims_coords()[los_dim].size)
        self.slices = ({los_dim:sl})
        tg = self("T")
        ...
        if iz0 == None:
            # will assume that it is Bifrost snapshot and we can remove the photosphere at z=0. 
            iz0 = 100
        self.emiss_mode = 'trac_noopa'
        vdem_cut = self('vdem', 
                        slices=dict(z=slice(iz0, iz0+5)),
                        vdem_logT_min=0,
                        rcoords_logT=np.arange(3.0,4.5,0.1),
        ...
        mom.moment_1.sum(dim='logT').plot(vmin=-1e2,vmax=1e2,cmap='bwr', ax=ax[1])
