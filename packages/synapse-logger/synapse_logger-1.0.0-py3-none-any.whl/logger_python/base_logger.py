...
class Logger(object):
    def __init__(self, config_dict: dict):
        ...
        if len(config_dict.get("debug_file_suffix")) > 0:
            # 日志文件名字
            debug_log_file = (os.path.splitext(conf_file_path)[0] +
                              config_dict.get("debug_file_suffix") +
                              os.path.splitext(conf_file_path)[1])
        ...
        if len(config_dict.get("info_file_suffix")) > 0:
            # 日志文件名字
            info_log_file = (os.path.splitext(conf_file_path)[0] +
                             config_dict.get("info_file_suffix") +
                             os.path.splitext(conf_file_path)[1])
        ...
        if len(config_dict.get("wf_file_suffix")):
            # 日志文件名字
            warn_log_file = (os.path.splitext(conf_file_path)[0] +
                             config_dict.get("wf_file_suffix") +
                             os.path.splitext(conf_file_path)[1])
