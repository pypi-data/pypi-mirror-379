...
class ModelSubtractionModule(BaseModule):
    ...
    def apply(self, resources: list[BaseResource]) -> DataResource:
        ...
        counts_boot = torch.tensor(samples, dtype=data_in.dtype, device=data_in.device)
        # counts_boot += model

        # plt.imshow(counts_boot[0].cpu().numpy(), cmap='viridis', aspect='auto')
        # plt.colorbar()
        # plt.show()
        #
        ...
        r_data_out.set_data(counts_boot + model)
