...
class InlineProperty(BaseModel):
    ...
    name: str
    description: str = ""
    data_type: InlineDataType = Field(default=InlineDataType.Text)
...
class InlinePropertiesResource(BaseModel, Resource):
    ...
    def read(self, client: httpxClient, old_state: SimpleNamespace) -> None | Dict[str, Any]:
        ...
        remote = {
            ...
            for prop in sorted(data, key=lambda x: x["Key"])
        }
        return remote
    ...
    def create(self, client: httpxClient) -> None | Dict[str, Any]:
        ...
        # synthesise the remote content to get a hash
        properties_dict = {
            prop.prop_key(): {
                "Name": prop.name,
                ...
                "Description": prop.description,
                "IsMain": prop.is_main,
                "AsAt": prop.as_at.isoformat() if prop.as_at else None,
            }
