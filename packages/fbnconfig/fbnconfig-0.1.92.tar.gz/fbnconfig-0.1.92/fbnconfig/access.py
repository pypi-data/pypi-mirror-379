...
class TemplatedSelector(BaseModel):
    ...
    def serialize_selector(self, selector: Selector) -> Dict[str, Any]:
        serialized = selector.model_dump(exclude={"type_name"}, by_alias=True)
        return {
            selector.type_name: serialized
        }
...
class PolicyCollectionResource(CamelAlias, BaseModel, Resource):
    ...
    def des_model(cls, data: Dict[str, Any], info) -> Dict[str, Any]:
        # api returns a resoureceId structure, we need scope and code
        # at the top
        if info.context and info.context.get("id"):
