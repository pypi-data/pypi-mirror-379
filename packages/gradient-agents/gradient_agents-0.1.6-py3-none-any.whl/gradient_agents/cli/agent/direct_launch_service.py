...
class DirectLaunchService(LaunchService):
    ...
    def _import_entrypoint_module(self, entrypoint_file: str) -> None:
        ...
        try:
            ...
            # Verify that the module has an 'app' attribute (from @entrypoint decorator)
            if not hasattr(module, "app"):
                typer.echo("❌ No 'app' found in the module!", err=True)
                typer.echo(
                    "Make sure your entrypoint file has: app = your_entrypoint_function",
                    err=True,
                )
            ...
            if not isinstance(module.app, FastAPI):
                typer.echo("❌ 'app' is not a FastAPI application!", err=True)
                typer.echo(
                    "Make sure your entrypoint function is decorated with @entrypoint",
                    err=True,
                )
                self._show_entrypoint_example()
                raise typer.Exit(1)
    ...
    def _start_server(
        ...
        try:
            # Convert entrypoint file to module:app format for uvicorn
            module_name = (
                entrypoint_file.replace(".py", "").replace("/", ".").replace("\\", ".")
            )
            app_target = f"{module_name}:app"

            # Use uvicorn directly - it will import the module and find the app
            import uvicorn
            ...
            uvicorn.run(
                ...
                port=port,
                reload=True,
                reload_dirs=[str(Path.cwd())],  # Watch the entire project directory
                reload_includes=["*.py"],  # Watch all Python files
                reload_excludes=[  # Exclude common non-source directories
                    "venv/*",
                    "env/*",
                    ".venv/*",
                    ".env/*",
    ...
    def _show_entrypoint_example(self) -> None:
        ...
        typer.echo("  ")
        typer.echo("  # The decorator returns a FastAPI app")
        typer.echo("  app = main")
        typer.echo("")
