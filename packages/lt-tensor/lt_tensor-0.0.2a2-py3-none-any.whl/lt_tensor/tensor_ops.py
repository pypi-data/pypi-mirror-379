...
def _conv_padding_helper(k: int, d: int):
    return (max(k, 1) - 1) * max(d, 1) // 2


def get_padding_conv(kernel_size: int, dilation: int):
    ...
    if _is_int_kernel and _is_int_dilation:
        ...
        return tuple(_conv_padding_helper(k, d) for k, d in zip(kernel_size, dilation))
    elif _is_int_dilation:
        return tuple(_conv_padding_helper(k, dilation) for k in kernel_size)

    return tuple(_conv_padding_helper(kernel_size, d) for d in dilation)
