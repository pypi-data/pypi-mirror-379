...
class JobContext:
    ...
    def get_msgbox(self) -> str | bytes | None:
        """
        msgbox에서 원래의 메시지를 복원하여 반환한다.

        - type이 "ascii"이면 문자열(str)로 반환
        - type이 "binary"이면 base64 디코딩하여 bytes로 반환
        - 유효하지 않은 구조나 실패 시 None 반환
        """
        if not self.msgbox:
            return None
        ...
        try:
            ...
            if msg_type == "ascii":
                ...
                self.logger.warning(f"Unknown msgbox type: {msg_type}")
                return None

        except Exception as e:
            self.logger.warning(f"Failed to get msgbox: {e}")
            return None
