...
class DrawContext:
    ...
    def texture(
        ...
        img = _get_scaled_tile(img, tw, th)

        # 2) Farbe anwenden (aus Cache)
        img = _get_colored_tile(img, color)
        ...
        if (uW == tw and vH == th) and (u0 % tw == 0) and (v0 % th == 0):
            ...
            if size.x != uW or size.y != vH:
                render_surface = pygame.transform.scale(img, (int(size.x), int(size.y)))
        ...
        img.blits([(img, d) for d in dests])

        # 4) Auf Zielgröße bringen (Nearest Neighbor)
        if size.x != uW or size.y != vH:
            img = pygame.transform.scale(img, (int(size.x), int(size.y)))
