...
class FeatureTableClient:
    ...
    def write_table(
        ...
        self._feast_client.client.write_to_offline_store(table_name, df, allow_registry_cache=False)
    ...
    def drop_table(self, name: str, database_name: Optional[str] = None, is_auto_delete_online: bool = False) -> None:
        ...
        try:
            ...
            is_get_feature_view_success = True
            try:
                feature_view = self._feast_client.client.get_feature_view(table_name)
            except Exception as e:
                print(f"Table '{name}' is not a feature table, skip delete. {str(e)}")
                is_get_feature_view_success = False
            else:
            ...
            if is_get_feature_view_success:
                try:
