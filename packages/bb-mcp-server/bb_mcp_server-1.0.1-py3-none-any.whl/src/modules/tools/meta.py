...
from src.utils.config import REPO_SLUG
...
async def _fetch_pr_statuses(workspace: str, repo_slug: str, pr_id: str) -> list[dict[str, Any]]:
...
async def _fetch_pr_diffstat(workspace: str, repo_slug: str, pr_id: str) -> tuple[list[dict[str, Any]], Optional[str]]:
    response = await make_request(
        "GET",
...
async def _fetch_recent_comments(workspace: str, repo_slug: str, pr_id: str) -> tuple[list[dict[str, Any]], Optional[str]]:
    response = await make_request(
        "GET",
...
async def pr_overview(
    pr: str,
    repo: Optional[str] = REPO_SLUG,
    verbosity: Verbosity = Verbosity.summary,
) -> dict[str, Any]:
    """Return a composed digest of PR metadata, blockers, and recent activity."""

    workspace, repo_slug = _resolve_repo(repo)
    pr_id = str(pr).lstrip("#")

    details = await _fetch_pr(workspace, repo_slug, pr_id)
    statuses = await _fetch_pr_statuses(workspace, repo_slug, pr_id)
    diffstat_top, diffstat_cursor = await _fetch_pr_diffstat(workspace, repo_slug, pr_id)
    comments_recent, comments_cursor = await _fetch_recent_comments(workspace, repo_slug, pr_id)
...
async def pr_review(
    ...
    overview = await pr_overview(pr=pr, repo=repo, verbosity=Verbosity.full)
...
async def pipe_fail_summary(
    ...
    if pipeline:
        ...
        pipelines = [await _fetch_pipeline(workspace, repo_slug, pipeline)]
    else:
        pipelines = await _list_failed_pipelines(workspace, repo_slug, limit)
        if not pipelines:
            return {"items": []}
    ...
    for pipeline_obj in pipelines[:limit]:
        ...
        steps = await _list_pipeline_steps(workspace, repo_slug, str(build_number))
        ...
        if not step_uuid:
            continue
        try:
            logs = await _fetch_step_logs(workspace, repo_slug, str(build_number), step_uuid)
