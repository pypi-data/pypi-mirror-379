...
from src.utils.config import BASE_URL, REPO_SLUG, WORKSPACE, get_config
...
def _resolve_repo(repo: Optional[str]) -> tuple[str, str]:
...
async def _current_user_uuid() -> Optional[str]:
    try:
...
async def pr_list(
    ...
    context = get_context()
    logger.debug(f"pr_list called with context: {context}")
...
async def pr_tasks_sync(
    ...
    for item in create or []:
        ...
        task = await make_request("POST", base_endpoint, json_data=payload)
...
async def pr_upsert(
    ...
    reviewers: Annotated[
        ...
        Field(
            description=(
                f"Reviewer identifiers (uuid:{{...}}, account:{{id}}, nickname:{{name}}, or raw nickname). Find default reviewers via \"workspace://{WORKSPACE}/default-reviewers\" resource or workspace.list tool."
            ),
        ),
    ] = None,
    ...
    if not pr_id and source:
        existing = await _find_open_pr_by_branch(workspace, repo_slug, source, destination)
        if existing:
...
async def workspace_list(
    ...
    workspace = (workspace or get_config().workspace).strip()
    if not workspace:
...
async def me_whoami() -> dict[str, Any]:
