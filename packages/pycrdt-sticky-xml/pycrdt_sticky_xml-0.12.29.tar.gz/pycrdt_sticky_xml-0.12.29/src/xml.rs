...
fn sticky_index<'py>(&self, py: Python<'py>, txn: &mut Transaction, index: u32, assoc: i8) -> PyResult<Py<StickyIndex>> {
    ...
    let _assoc: Assoc = match assoc {
        0 => Assoc::After,
        _ => Assoc::Before,
    };
    
    // Get the current length
    let len = self.fragment.len(&*t);
    
    // Validate index bounds - allow index == len for "end" position
    if index > len {
        return Err(pyo3::exceptions::PyIndexError::new_err("Index out of bounds"));
    }
    
    // Try to create the sticky index normally first
    let sticky_index = self.fragment.sticky_index(t, index, _assoc);
    
    // If it fails and we're at the end position, try an alternative approach
    let sticky_index = if sticky_index.is_none() && index == len {
        // For end position, we need special handling
        if len == 0 {
            // Empty fragment - try index 0
            self.fragment.sticky_index(t, 0, _assoc)
        } else {
            // Try using the last valid index with After to represent end
            self.fragment.sticky_index(t, len - 1, Assoc::After)
        }
    } else {
        sticky_index
    };
    ...
    match sticky_index {
        Some(si) => Ok(Py::new(py, StickyIndex::from(Some(si)))?),
