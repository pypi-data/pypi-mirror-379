...
from supervisor.src.simulator.python import base_simulator
...
class GPUSlowdownSimulator(base_simulator.BaseSimulator):
  ...
  def _gpu_slowdown_fn(
      self, local_ranks: list[int], gpu_power_limit: int = 200
  ):
    ...
    slow_gpu_cmd = (
        f"/home/kubernetes/bin/nvidia/bin/nvidia-smi -pl {gpu_power_limit} -i"
        f" {local_ranks_str}"
    )
  ...
  def simulate(
    ...
    Args:
        ...
        intermittent (bool): Allows for slow GPUs to recover if they are
          re-sampled. Defaults to False.
  ...
  def report_events(self, global_ranks: list[int]):
    ...
    Args:
        global_ranks (list): A list indicating the global ranks of which GPUs
          have failed.
    """
    if not global_ranks:
      return
