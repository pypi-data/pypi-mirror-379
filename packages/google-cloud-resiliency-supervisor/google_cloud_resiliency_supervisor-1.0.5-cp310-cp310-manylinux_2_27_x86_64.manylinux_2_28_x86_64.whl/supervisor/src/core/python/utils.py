"""Core utility functions for Google Pytorch Resiliency."""

import logging
import re
from typing import Any

import supervisor.src.core.python.device_info as device_info
...
def compare_workers(
  ...
  if not isinstance(worker_info1, device_info.WorkerInfo) or not isinstance(
    ...
    raise ValueError(
        "worker_info objects must be of type device_info.WorkerInfo."
    )

  if worker_info1.worker_id != worker_info2.worker_id:
    logging.info(
        f"WorkerInfo IDs do not match: {worker_info1.worker_id} and"
        f" {worker_info2.worker_id}"
    )
    return False
  if worker_info1.worker_name != worker_info2.worker_name:
    logging.info(
        f"WorkerInfo names do not match: {worker_info1.worker_name} and"
        f" {worker_info2.worker_name}"
    )
    return False
  if worker_info1.local_rank != worker_info2.local_rank:
    if is_set(worker_info1.local_rank) != is_set(worker_info2.local_rank):
      logging.info(
          f"WorkerInfo local ranks do not match: {worker_info1.local_rank} and"
          f" {worker_info2.local_rank}"
      )
      return False
  if worker_info1.global_rank != worker_info2.global_rank:
    if is_set(worker_info1.global_rank) != is_set(worker_info2.global_rank):
      logging.info(
          "WorkerInfo global ranks do not match:"
          f" {worker_info1.global_rank} and {worker_info2.global_rank}"
      )
      return False
  if worker_info1.state.name != worker_info2.state.name:
    logging.info(
        f"WorkerInfo states do not match: {worker_info1.state} and"
        f" {worker_info2.state}"
    )
    return False
  ...
  if compare_coords:
    if worker_info1.coords != worker_info2.coords:
      logging.info(
          f"WorkerInfo coords do not match: {worker_info1.coords} and"
          f" {worker_info2.coords}"
      )
      return False
...
def compare_hosts(
  ...
  if host_info1.host_address != host_info2.host_address:
    logging.info(
        f"HostInfo addresses do not match: {host_info1.host_address} and"
        f" {host_info2.host_address}"
    )
    return False
  if host_info1.host_id != host_info2.host_id:
    logging.info(
        f"HostInfo IDs do not match: {host_info1.host_id} and"
        f" {host_info2.host_id}"
    )
    return False
  if host_info1.host_name != host_info2.host_name:
    logging.info(
        f"HostInfo names do not match: {host_info1.host_name} and"
        f" {host_info2.host_name}"
    )
    return False
  if host_info1.host_serial_number != host_info2.host_serial_number:
    logging.info(
        "HostInfo serial numbers do not match:"
        f" {host_info1.host_serial_number} and {host_info2.host_serial_number}"
    )
    return False
  if host_info1.subblock_id != host_info2.subblock_id:
    logging.info(
        f"HostInfo subblock IDs do not match: {host_info1.subblock_id} and"
        f" {host_info2.subblock_id}"
    )
    return False
  if host_info1.superblock_id != host_info2.superblock_id:
    logging.info(
        f"HostInfo superblock IDs do not match: {host_info1.superblock_id} and"
        f" {host_info2.superblock_id}"
    )
    return False
  ...
  if host_info1.state.name != host_info2.state.name:
    logging.info(
        f"HostInfo states do not match: {host_info1.state} and"
        f" {host_info2.state}"
    )
    return False
  if len(host_info1.workers) != len(host_info2.workers):
    logging.info(
        "HostInfo has different number of workers:"
        f" {len(host_info1.workers)} and {len(host_info2.workers)}"
    )
    return False
