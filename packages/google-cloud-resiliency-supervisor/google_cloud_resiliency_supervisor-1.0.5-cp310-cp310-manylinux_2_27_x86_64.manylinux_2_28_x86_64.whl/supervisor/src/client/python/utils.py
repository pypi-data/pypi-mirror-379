"""Utility functions for the client abstractions including host and worker."""
...
class AsyncSubprocess:
  ...
  def terminate(self) -> None:
    ...
    logging.info(
        "Terminating the process executing:\n%s", " ".join(self._command)
    )
    try:
...
def check_and_clean_port(port, kill_delay=5):
  ...
  try:
    ...
    if not pid_str:
      logging.info(
          f"Could not find a process using port {port} (lsof returned empty"
          " output)."
      )
      return (
    ...
    if not is_port_in_use(port):
      ...
      logging.info(
          f"Port {port} is still in use after SIGKILL.  Manual intervention may"
          " be required."
      )
      return False
    ...
    logging.exception(f"lsof returned invalid PID.")
    return False
  except OSError:
