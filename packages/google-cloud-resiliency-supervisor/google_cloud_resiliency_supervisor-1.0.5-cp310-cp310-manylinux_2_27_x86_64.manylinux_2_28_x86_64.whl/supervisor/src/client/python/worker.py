...
from supervisor.src.client.python import workload
...
class Worker:
  ...
  def __init__(
    ...
    Args:
        ...
        hang_timeout: The time elasped (seconds) between heartbeats before
          declaring a hang.
    """
    ...
    if start_method is None:
      self.logger.info("Setting start method to `spawn`.")
      mp.set_start_method("spawn")
    elif start_method != "spawn":
      self.logger.warning(
          f"Spawn method was set to {start_method} elsewhere. Resetting to"
          " spawn."
      )
      mp.set_start_method("spawn", force=True)
    else:
      self.logger.warning("Spawn method already set")
    ...
    self._heartbeat_thread = threading.Thread(
        target=self._workload_heartbeat_loop,
        name="hearbeat_thread",
        daemon=True,
    )
  ...
  def start_workload(
    ...
    """Run a workload process within the worker.

    The workload function only needs to be specified once.
    ...
    self.logger.info(f"Started workload process for {self.worker_name}")
  ...
  def stop_workload(self):
    ...
    if self.workload_proc is not None:
      try:
        self.logger.info(
            f"Terminating workload process for {self.worker_name}."
        )
        self.workload_proc.terminate()
        self.workload_proc.join()
        self.logger.info(
            f"Workload process for {self.worker_name} terminated."
        )
      except Exception:
        self.logger.warning(
            "Warning: error with terminating workload process for"
            f" {self.worker_name}. This can be expected since the workload"
      ...
      self.logger.info("No workload process to terminate.")
  ...
  def _workload_heartbeat_loop(self):
    ...
    self.logger.info(f"{self.worker_name} heartbeat thread started.")
    ...
    while self.workload_is_alive() or self._heartbeat_during_pause:
      ...
      if self._workload_intercepts.check_heartbeat():
        ...
        self.logger.warning(
            f"{self.worker_name} workload process hang detected in training"
            f" step {step}."
        )

    if self._workload_intercepts.is_complete():
      self.logger.info(f"{self.worker_name} workload successfully completed.")
      self.worker_info.state = device_info.DeviceState.COMPLETE
      self._worker.send_heartbeat(self.export_info())

    self.logger.info(f"{self.worker_name} heartbeat thread terminated.")
