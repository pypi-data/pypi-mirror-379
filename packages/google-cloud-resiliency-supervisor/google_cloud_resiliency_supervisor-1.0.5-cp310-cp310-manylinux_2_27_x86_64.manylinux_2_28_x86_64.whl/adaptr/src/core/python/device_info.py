"""Python classes to define the Mesh objects."""
...
class DeviceState(enum.Enum):
    ...
    states. ACTIVE denotes that the device is healthy and
    participating in training. SLOW denotes that the device
    is experiencing performance degradation and is participating
    in training. AVAILABLE denotes that the worker is healthy
    but not currently participating in training. UNAVAILABLE
    denotes that the worker is currently unreachable.
    """
...
class HostInfo:
    ...
    def __init__(
        ...
        Args:
            ...
            state (DeviceState, optional): The state of the host. Defaults to DeviceState.INIT.
        """
    ...
    def export_info(self) -> adaptr_core.HostInfo:
        ...
        h_info.state = adaptr_core.DeviceState(self.state.value)
        h_info.workers = [worker.export_info() for worker in self.workers]
        return h_info

    def update_info(self, host_info_proto: adaptr_core.HostInfo):
    ...
    def get_num_workers(self) -> int:
        return len(
            [worker for worker in self._workers if worker.state == DeviceState.RUNNING]
        )
...
class WorkerInfo:
    ...
    def __init__(
        ...
        Args:
            worker_id (str): The serial number of the GPU associated with the worker.
            local_rank (int | None, optional): The worker's rank respective to the host.
            global_rank (int | None, optional): The worker's rank respective to the entire workload.
            worker_name (str | None, optional): Name of worker's corresponding python process. Defaults to None.
            normalized_perf (int, optional): The worker's relative performance as a percentage in [0, 100]. Defaults to 100.
            state (DeviceState, optional): The state of the worker. Defaults to DeviceState.AVAILABLE.
        """
        # Physical Identifiers
    ...
    def export_info(self) -> adaptr_core.WorkerInfo:
        ...
        w_info.state = adaptr_core.DeviceState(self.state.value)
        return w_info

    def update_info(self, worker_info_proto: adaptr_core.WorkerInfo):
...
def host_info_from_proto(host_info_proto: adaptr_core.HostInfo) -> HostInfo:
    """Converts an instance of adaptr_core.HostInfo into a device_info.HostInfo instance."""
    host_info = HostInfo(
...
def worker_info_from_proto(worker_info_proto: adaptr_core.WorkerInfo) -> WorkerInfo:
    """Converts an instance of adaptr_core.WorkerInfo into a device_info.WorkerInfo instance."""
