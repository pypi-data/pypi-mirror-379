...
class ContextChainConfig(BaseModel):
    ...
    preprocessor_config: Optional[Dict[str, Any]] = None  # New: Config for DataPreprocessor
...
class ContextChain:
    ...
    def __init__(self, config: ContextChainConfig, llm_client: Optional[BaseLLMClient] = None):
        ...
        self.preprocessor = DataPreprocessor(config.preprocessor_config or {})  # New: Initialize DataPreprocessor
    ...
    def _classify_query(self, query: str) -> str:
        ...
        query_lower = self.preprocessor.preprocess(query).lower()  # Preprocess query
    ...
    async def _query_structured_db(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            processed_results = [
                {
                    "content": self.preprocessor.preprocess(str(result)),  # Preprocess content
    ...
    async def generate_insights(self, query: str, context: Optional[str] = None,
        ...
        try:
            ...
            # Preprocess documents if provided
            if documents:
                documents = [
    ...
    async def _retrieve_documents(self, query: str, context: ExecutionContext) -> List[Dict[str, Any]]:
        ...
        try:
            # Preprocess query before searching
            query = self.preprocessor.preprocess(query)
            search_results = await self.vector_store.search(query=query, k=5)
            return [
                {
                    'content': self.preprocessor.preprocess(result.content),  # Preprocess retrieved content
    ...
    def decide_data_destination(self, data_type: str, content: str, metadata: Optional[Dict] = None) -> DataDestination:
        ...
        content = self.preprocessor.preprocess(content)  # Preprocess content
    ...
    async def search_context(self, query: str, k: int = 5, data_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        query = self.preprocessor.preprocess(query)  # Preprocess query
    ...
    async def submit_feedback(self, session_id: str, rating: int, comments: Optional[str] = None) -> Dict[str, str]:
        ...
        try:
            if comments:
                comments = self.preprocessor.preprocess(comments)  # Preprocess comments
            if not self.storage:
