...
class McapTopic:
    def __init__(self, reader: mcap.reader.McapReader, mcap_path: Path, topic_key: int, str_limit: int = 80):
        ...
        if len(schema['properties']) == 0:
            ...
            schema = json.loads(ret_message)
            # print(json.dumps(schema, option=json.OPT_INDENT_2).decode())
    ...
    def get_data(self, start_time_ns: int = 0, num_messages: int = -1) -> Optional[np.ndarray]:
        ...
        msg_count = "100%" if num_messages == -1 or num_messages == self._message_count else f"{num_messages} messages"
        logger.info(f'Loaded "{self.topic}": {msg_count} in {time.time() - start_time:.2f} seconds.')
...
if __name__ == '__main__':
    ...
    for t in [('Start', start_time_ns), ('Stop', end_time_ns)]:
        ...
        print(f'{t[0]} time {t[1]}: {formatted}')
