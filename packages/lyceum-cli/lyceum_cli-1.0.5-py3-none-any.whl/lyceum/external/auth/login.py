...
from lyceum_cloud_execution_api_client.api.billing_credits import (
    get_user_credits_api_v2_external_billing_credits_get,
)
...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Satisfy linter by showing explicit assignment (though dict updates
        # work too)
            ...
            if parsed_url.path == "/callback":
                ...
                if "token" in query_params:
                    ...
                    try:
                        ...
                        # Browser closed connection (expected when tab
                        # auto-closes)
                        pass
...
def login(
        base_url: Optional[str] = typer.Option(
            None,
            "--url",
            help="API base URL (for development)"),
    dashboard_url: Optional[str] = typer.Option(
            "https://dashboard.lyceum.technology",
            "--dashboard",
            help="Dashboard URL"),
        manual: bool = typer.Option(
            False,
            "--manual",
            help="Use manual API key login instead of browser"),
        api_key: Optional[str] = typer.Option(
            None,
            "--api-key",
            "-k",
            help="API key for manual login"),
):
    ...
    if manual:
        ...
        try:
            ...
            response = (
                get_user_credits_api_v2_external_billing_credits_get.sync_detailed(
                    client=client))
            if response.status_code == 200:
                console.print("[green]✅ Successfully authenticated![/green]")
            else:
                console.print(
                    f"[red]❌ Authentication failed: HTTP {
    ...
    try:
        ...
        console.print(
            f"[dim]Starting callback server on port {callback_port}...[/dim]")
        ...
        console.print(
            f"[dim]If browser doesn't open, visit: {login_url}[/dim]")
        ...
        if not webbrowser.open(login_url):
            ...
            console.print(
                f"[yellow]Please manually open: {login_url}[/yellow]")

        console.print(
            "[dim]Waiting for authentication... (timeout: 120 seconds)[/dim]")

        # Wait for callback with timeout
        timeout = 120  # 2 minutes
        start_time = time.time()

        while not callback_result["received"] and (
        ...
        if callback_result["received"]:
            if callback_result["token"]:
                ...
                console.print(
                    "[green]✅ Authentication token received![/green]")

                # Test the connection
                try:
                    client = config.get_client()
                    response = get_user_credits_api_v2_external_billing_credits_get.sync_detailed(
                        client=client)
                    if response.status_code == 200:
                        console.print(
                        ...
                        if callback_result.get("user"):
                            console.print(
                                f"[dim]Logged in as: {
                                    callback_result['user']}[/dim]")
                    else:
                        console.print(
                            f"[red]❌ Token validation failed: HTTP {
                                response.status_code}[/red]")
                        raise typer.Exit(1)
                except Exception as e:
    ...
    if config.api_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[yellow]⚠️  API connection issues: HTTP {
                        response.status_code}[/yellow]")
        except Exception as e:
            console.print(f"[red]❌ API connection failed: {e}[/red]")
    else:
        console.print(
            "[red]❌ Not authenticated. Run 'lyceum login' first.[/red]")
