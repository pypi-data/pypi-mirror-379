...
sys.path.insert(0, str(Path(__file__).parent.parent.parent /
                       "lyceum-cloud-execution-api-client"))
...
class Config:
    ...
    def refresh_access_token(self) -> bool:
        ...
        try:
            ...
            if response.session and response.session.access_token:
                ...
                if error_msg:
                    console.print(
                        f"[yellow]⚠️  Token refresh failed: {
                            error_msg.message}[/yellow]")
                else:
                    console.print(
                        "[yellow]⚠️  Token refresh failed: No session returned[/yellow]"
                    )
                return False
    ...
    def get_client(self) -> AuthenticatedClient:
        ...
        if not self.api_key:
            console.print(
                "[red]Error: Not authenticated. Run 'lyceum login' first.[/red]"
            )
            raise typer.Exit(1)
        ...
        if self.is_token_expired():
            ...
            if not self.refresh_access_token():
                console.print(
                    "[red]❌ Token refresh failed. Please run 'lyceum login' again.[/red]"
                )
                raise typer.Exit(1)

        return AuthenticatedClient(base_url=self.base_url, token=self.api_key)
