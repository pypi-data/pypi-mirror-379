...
def model_to_object_schema(model: type[Model] | type[TypeModel]) -> ObjectSchema:
    ...
    has_timestamp_mixin = any(
        cls.__name__ == 'TimestampMixin' and all(hasattr(cls, attr) for attr in ('created_at', 'updated_at'))
        for cls in _mro
    )
    # we don't want to add created_at/updated_at if it's inherited from timestamp mixin
    if has_timestamp_mixin:
        if 'created_at' in object_schema.properties:
            object_schema.properties.pop('created_at')
        if 'updated_at' in object_schema.properties:
            object_schema.properties.pop('updated_at')
