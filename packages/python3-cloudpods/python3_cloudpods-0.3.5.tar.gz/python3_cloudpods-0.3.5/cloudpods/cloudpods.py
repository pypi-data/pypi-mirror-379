...
# 假设 log 已在包初始化时配置（如之前优化的 get_logger() 返回的日志器）
# 若未初始化，可添加：log = get_logger()
...
def enter_and_leave_function(func: Callable) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        # 格式化入参（避免复杂对象打印过长，简化显示）
        args_str = _format_args(args)
        kwargs_str = _format_kwargs(kwargs)

        log.info(
            f"[函数调用] 开始执行 {func.__name__}() "
            f"| 定义位置：{func_def_file}:{func_def_line} "
            f"| 入参：args={args_str}, kwargs={kwargs_str}"
        )
        ...
        try:
            ...
            # --------------------------
            # 4. 函数异常：记录完整堆栈（含函数内报错行数）
            # --------------------------
            # 获取异常堆栈信息（格式化为字符串，包含具体代码行）
            error_traceback = traceback.format_exc()

            log.error(
                f"[函数调用] 执行 {func.__name__}() 失败 "
...
def _format_args(args: tuple) -> str:
    ...
    for arg in args:
        ...
        if isinstance(arg, (str, list, tuple, dict, set)):
            arg_str = str(arg)
            formatted.append(arg_str[:100] + "..." if len(arg_str) > 100 else arg_str)
        else:
...
def _format_kwargs(kwargs: dict) -> str:
    ...
    for k, v in kwargs.items():
        if isinstance(v, (str, list, tuple, dict, set)):
            v_str = str(v)
            formatted_v = v_str[:100] + "..." if len(v_str) > 100 else v_str
        else:
            formatted_v = str(v)
        formatted.append(f"{k}={formatted_v}")
    return f"{{{', '.join(formatted)}}}"
        ...
        try:
            for i in range(600):
                ...
                if server_detail is None:
                    ...
                    return True
                log.warning(f"server_id: {server_id} is deleting")
                time.sleep(5)
            log.warning(f"Failed to wait for server {server_id} is deleted: Error.\nserver_detail is {server_detail}")
            return False
        except Exception as e:
