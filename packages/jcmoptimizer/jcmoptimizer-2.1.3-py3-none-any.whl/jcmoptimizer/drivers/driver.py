...
class Driver(Requestor):
    ...
    def __init__(self, host: str, study_id: str, session: requests.Session) -> None:
        super(Driver, self).__init__(host=host, session=session)
        self.id = study_id
    ...
    def _post(
        ...
        return super(Driver, self).post(purpose, object, operation, self.id, data)

    def _get(self, purpose:str, object: str, type: str) -> dict[str, Any]:
        return super(Driver, self).get(purpose, object, type, self.id)
