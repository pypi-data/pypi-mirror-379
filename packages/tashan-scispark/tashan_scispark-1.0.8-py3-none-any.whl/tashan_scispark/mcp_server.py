...
try:
    ...
    logger.error("请安装fastmcp: pip install fastmcp")
    sys.exit(1)
...
def search_papers(keyword: str, limit: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        formatted_papers = []
        for paper in papers:
            # 清理字符串，移除可能导致JSON解析错误的特殊字符
            def clean_string(s):
                if isinstance(s, str):
                    # 移除反引号和其他可能导致JSON解析问题的字符
            ...
            formatted_papers.append({
                ...
                "authors": clean_string(paper.get("authors", "")),
                "published": clean_string(paper.get("time", "")),  # 修正字段名
                "url": clean_string(paper.get("id", "")),  # 修正字段名，使用id作为url
                "topic": clean_string(paper.get("topic", keyword))
            })
