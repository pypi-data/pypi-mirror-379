...
from ..core.prompt import get_related_keyword_prompt, paper_compression_prompt, extract_entity_prompt, \
    extract_tec_entities_prompt, review_mechanism_prompt
from .llm_api import call_with_deepseek, call_with_deepseek_jsonout, call_with_qwenmax
from .arxiv_api import search_paper
from .scholar_download import download_all_pdfs
from .pdf_to_md import pdf2md_mineruapi
from .wiki_search import get_description, search
from ..core.config import OUTPUT_PATH, graph
import ast
...
def SearchKeyWordScore(Keywords):
    ...
    logger.info(f"关键词分数计算完成: {sorted_data}")
...
def get_related_keyword(Keyword):
    ...
    logger.info(f"相关关键词获取结果: {result}")

    logger.info("相关关键词获取完成")
