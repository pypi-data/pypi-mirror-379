...
try:
    ...
    logger.info("getaccept doesn't implement any custom " "endpoints")

try:
    from .event_handlers import register_event_handlers  # noqa
except ImportError:
    logger.info("getaccept doesn't implement any event " "handlers")

try:
    from .limeobject_classes import register_limeobject_classes  # noqa
except ImportError:
    logger.info("getaccept doesn't implement any " "limeobject classes")

try:
    from .web_components import register_web_components  # noqa
except ImportError:
    logger.info("getaccept doesn't implement any " "web-components")

try:
    from .translations import register_translations  # noqa
except ImportError:
    logger.info("getaccept doesn't contain any " "translations")
...
def register_static_content():
    ...
    for _, module_name, _ in pkgutil.walk_packages(__path__):
        ...
        if hasattr(module, "register_static_content"):
            logger.info("Registering static content for {}" "".format(module_fullname))
