...
def cube_fn(size, x, y, e, group, fps):
    size = np.concatenate((size[-1][None, ...], size)).round(2)
    fill = ["black" if s > 0 else "white" for s in size]
    size = np.abs(size)
    ...
    animf = e.dwg.animate(
        attributeName="fill", values=fill, dur=f"{len(size) / fps}s", repeatCount="indefinite", calcMode="discrete"
    )
