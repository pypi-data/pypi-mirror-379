...
def path_type_iterator(
    annotation: Any,
    key: str,
    value: Any,
    default: Any,
    metadata: ta.TickAnnotations,
) -> Iterable[tuple[FieldDataForPath, ta.TickAnnotations]]:
    """Yield value for a path annotation."""
    if isclass(annotation) and issubclass(annotation, PurePath):
        _logger.debug("annotation: Path %s", annotation)
        yield (
            FieldDataForPath.parse(annotation, key, value, default, metadata),
            metadata,
        )
