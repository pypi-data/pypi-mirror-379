...
class SettingsAPI:
    ...
    def __init__(self, api):
        self.api = api
        try:
            self.SETTINGSPATH = api.SDK.SDK_SETTINGS
            ...
            settings_path = os.path.join(current_dir, self.api.SDK_SETTINGS)
...
class CommandsAPI:
    ...
    def RunCommand(self, command):
        ...
        try:
            result = self.subprocess.run(command, shell=True, check=True, stdout=self.subprocess.PIPE, stderr=self.subprocess.PIPE, text=True)
...
class ToolAPI:
    ...
    def __init__(self, **sdk):
        """Requires sdk. Engine for Modding-Use"""
        self.SDK = SDKFramework(**sdk)
        self.Settings = SettingsAPI(self.SDK.SDK_SETTINGS)
...
class Api:
    def __init__(self, sdkpath = None, **sdk):
        """ToolAPI's API-SDK. made for general engine use."""
        self.SDK = SDKFramework(sdkpath=sdkpath, **sdk)
        self.Settings = SettingsAPI(self.SDK.SDK_SETTINGS)
        self.Cache = CacheAPI(self.Settings.CACHEPATH)
        self.Temp = TempAPI(self.Settings.TEMPPATH)
        self.Package = PackageAPI(self.Settings.PACKAGEPATH)
...
class SDKFramework:
    ...
    def __init__(self, sdkpath = None, **sdk):
        if sdkpath:
            ...
            import json
            package_dir = os.path.dirname(os.path.abspath(__file__))
            sdkpath = os.path.join(package_dir, sdkpath)
            with open(sdkpath, 'r', encoding='utf-8') as f:
                self.SDK = json.load(f)
        else:
            self.SDK = sdk
            self.SDK_VERSION = self.SDK.get("version")
            self.SDK_NAME = self.SDK.get("name")
            self.SDK_SETTINGS = self.SDK.get("settings_path")
            self.SDK_LangLib = self.SDK.get("standard_language_library", True)
