...
Analog polynomial ema inline slope filter:
...
def generate_impulse_response(
    ...
    r"""
    Creates a continuous-time-like impulse response for this
    inline slope filter. The implementation is fully analytic, rather than
    using the derivative operator DiracDelta'(t) *.

    For filter-order = 1, the leading Dirac delta function is approximated
    by 1 / dt because this context is a discretized continuous time.
    ...
    if filter_order == 1:
        ...
        h[i_t_0plus:] = -1.0 / np.power(tau, 2) * np.exp(-t_0plus / tau)
        h[i_t_0plus] += 1.0 / (dt * tau)

    else:

        h[i_t_0plus:] = (
            ((m - 1) - t_0plus / tau_m)
            / (np.power(tau_m, 2) * scipy.special.gamma(m))
            * np.power(t_0plus / tau_m, m - 2)
...
def generate_sacf_correlogram(
    ...
    # exception for polyema inline slope
    if normalize and filter_order == 1:
        sacf[:, 1] *= np.nan
...
def generate_transfer_function(
    ...
    transfer_function_builder_tools. \
        apply_leading_jw_coefficient_to_transfer_function(H)
    # fmt: on
...
def make_callable_transfer_function(
    ...
    Constructs H(jw) = jw H_ref(tau jw).
...
def generate_sscf_correlogram(
    ...
    Note: inline-slope filters have the transfer function

        s H(tau s)

    and the sscf integral covers

        int_reals s^2 H(s) H(-s sti) ds.

    The way the code is structured, `s` is attached to `H(s)` before this
    integral is evaluated, creating

        int_reals s (s sti) H(s) H(-s sti) ds.

    Therefore, the integral needs to be corrected by 1 / sti.
    ...
    # correction function
    sti_norm_correction_fcxn = lambda sti: 1.0 / sti
...
def generate_phase_spectrum(
    ...
    r"""
    Computes the phase of H_slope(f) based on the underlying reference
    filter H_ref(f):

        angle_H_slope(f) = pi / 2 + angle_H_ref(f).
    ...
    np.ndarray
        (n, 2), dtype=complex
        The phase spectrum with cols [f_axis, angle_H_slope(f)].
    """
    ...
    # fmt: off
    transfer_function_builder_tools. \
        apply_leading_jw_coefficient_to_phase_spectrum(H)
    # fmt: on
