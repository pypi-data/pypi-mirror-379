...
def dday(
    ...
    Example:
        ...
        >>> df = pd.DataFrame({"date": date, "x": x, "zone": "all_zone", "factor": "Orange"})
    ...
    if zone != 'all_zone' and 'zone' not in data.columns:
        raise ValueError("La colonne 'zone' est requise pour filtrer.")
    if factor != 'all_factor' and 'factor' not in data.columns and 'factor' not in data.columns:
        raise ValueError("La colonne 'factor' ou 'factor' est requise pour filtrer.")
    ...
    if factor == 'all_factor':
        filtered = filtered[filtered['factor'] != 'all_factor']
        pass
    else:
        col_factor = 'factor' if 'factor' in filtered.columns else 'factor'
    ...
    if mask.any():
        value = filtered.loc[mask, x].iloc[-1]
...
def wtd(
    ...
    if factor != 'all_factor':
        col_factor = 'factor' if 'factor' in data.columns else 'factor'
    ...
    if cumul:
        ...
        val = round(float(val / unite), decimal)
...
def qtd(data, date: str, x: str, q: int = 0,
    ...
    Args:
        data (pd.DataFrame): DataFrame avec colonnes 'date', 'zone', 'factor' (ou 'factor'), et la variable x.
        date (str): Date de fin de période (format 'YYYY-MM-DD').
    ...
    Example:
        >>> qtd(df, date="2023-07-06", x="revenu")  # Somme du 1er avril au 6 juillet
        >>> qtd(df, date="2023-07-06", x="user_actif", cumul=True)  # Valeur du 6 juillet
        >>> qtd(df, date="2023-07-06", x="revenu", q=-1)  # Jusqu'au 6 avril (T1)
    """
    # Validation des entrées
    if not isinstance(data, pd.DataFrame):
        raise ValueError("data doit être un DataFrame pandas.")
...
def get_summary_month(
    ...
    Args:
        data (pd.DataFrame): DataFrame avec colonnes 'date' (datetime), 'month', 'year', 'month_num', etc.
...
def get_summary_quarter(
    ...
    # Gestion robuste de l'opérateur
    if factor == 'all_factor':
        # Exclure la ligne "all_factor" si elle existe, et sommer les autres opérateurs
        df = df[df['factor'] != 'all_factor']
    else:
