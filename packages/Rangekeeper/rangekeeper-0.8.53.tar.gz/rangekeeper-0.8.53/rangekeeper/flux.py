...
class Flow:
    ...
    def resample(
        ...
        if sum:
            ...
            rk.duration.Period.include_date(
                date=origin,
                duration=frequency,
            )
    ...
    def to_periods(
        ...
        """
        print(f"Freq: {index.freq}")
        print(f"Freqstr: {index.freqstr}")
        print(f"Origin: {index[0].start_time.date()}")

        resampled = self.resample(
            frequency=rk.duration.Type.from_value(value=index.freqstr),
            origin=index[0].start_time.date(),
        )
        # print(f"Resampled: {self.name}\n {resampled.movements}")
