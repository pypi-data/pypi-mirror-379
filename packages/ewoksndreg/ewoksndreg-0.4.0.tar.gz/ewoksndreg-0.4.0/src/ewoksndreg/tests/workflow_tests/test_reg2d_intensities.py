...
def test_reg2d_intensities_without_qt():
    from orangecontrib.ewoksndreg import tutorials

    filename = resource_files(tutorials) / "reg2d_intensities.ows"
    assert_reg2d_intensities_without_qt(filename)


def test_reg2d_intensities_with_qt(ewoks_orange_canvas):
    from orangecontrib.ewoksndreg import tutorials

    filename = resource_files(tutorials) / "reg2d_intensities.ows"
    assert_reg2d_intensities_with_qt(ewoks_orange_canvas, filename)


def assert_reg2d_intensities_without_qt(filename):
...
def assert_reg2d_intensities_with_qt(ewoks_orange_canvas, filename):
