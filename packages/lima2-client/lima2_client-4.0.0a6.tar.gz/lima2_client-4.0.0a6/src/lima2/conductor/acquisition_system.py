...
class AcquisitionSystem:
    ...
    async def prepare(
        ...
        If the system state is FAULT or UNKNOWN, attempt to recover with a
        reset() on all devices first.

        NOTE: to prevent a prepare() call before the end of acquisition/processing
        while in FAULT state, this command blocks while runstate & RUNNING.
        ...
        if self.runstate & RunState.FAULT:
            ...
            while self.runstate & RunState.RUNNING:
                logger.warning("Prepare called while run is finishing: waiting...")
                if time.time() - time_start > 15:
                    raise RuntimeError("Can't recover from a failed run. Please reset.")
                await asyncio.sleep(0.5)

            logger.warning("Previous runstate was FAULT. Attempting reset()...")
            ...
            try:
                ...
                logger.error(f"Unable to reset:\n{traceback.format_exc()}")
