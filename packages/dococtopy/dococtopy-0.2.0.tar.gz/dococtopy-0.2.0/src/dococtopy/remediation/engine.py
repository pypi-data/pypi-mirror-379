...
class RemediationEngine:
    ...
    def __init__(self, options: RemediationOptions, config=None):
        ...
        self.docstring_fixer = DocstringFixer(
            self.validator, self.trivial_detector, options
        )
    ...
    def remediate_symbol(
        ...
        if self.options.verbose:
            print(f"[dim]Fixing docstring for {symbol.name} ({symbol.kind})[/dim]")
            print(f"[dim]Issues to fix: {[f.rule_id for f in symbol_findings]}[/dim]")
        ...
        try:
            ...
            if self.options.verbose:
                print(f"[dim]Applied fixes: {applied_fixes}[/dim]")
                print(f"[dim]Used LLM: {used_llm}[/dim]")
    ...
    def apply_changes(self, file_path: Path, changes: List[DocstringChange]) -> None:
        ...
        for change in sorted_changes:
            ...
            if (
                ...
                # Add docstring with proper indentation calculation
                # Calculate indentation based on the function/class definition line
                if symbol_line < len(lines):
                    # Get indentation from the function/class definition line
                    def_line = lines[symbol_line]
                    base_indent = len(def_line) - len(def_line.lstrip())
                    # Docstring should be indented one level more than the definition
                ...
                # Create docstring content with proper triple quote handling
                ...
                if start_line != end_line:
                    ...
                    # Replace last line with closing triple quotes
