...
class ValidationResult:
    ...
    trivial_fixes_applied: List[str]  # noqa: F841
    ...
    def validate_docstring(
        ...
        if self.config and hasattr(self.config, "rules"):
            ...
            for finding in all_findings:
                # Config.rules is Dict[str, str] where values are "on"/"off"
                rule_state = enabled_rules.get(finding.rule_id)
                if rule_state is None or rule_state.lower() not in {
                    "off",
                    "disabled",
                    "disable",
                }:
            ...
            try:
                ...
                if not current_docstring:
                    ...
                    current_docstring = llm_client.generate_docstring(
                        ...
                        existing_docstring="",
                        context="; ".join(
                            [f"{f.rule_id}: {f.message}" for f in remaining_findings]
                        ),
                    )

                    # Apply indentation normalization (hybrid approach)
                    from dococtopy.remediation.indentation import (
                        normalize_docstring_indentation,
                    )

                    current_docstring = normalize_docstring_indentation(
                        current_docstring
                    )
                else:
                ...
                if self.options.verbose:
                    print(
                        f"[dim]LLM attempt {attempt + 1} result: {current_docstring[:100]}...[/dim]"
                    )
                    print(
                        f"[dim]Validation result: valid={validation_result.is_valid}, findings={len(validation_result.findings)}[/dim]"
                    )
