...
class DG401TestFunctionDocstringStyle:
    ...
    def check(self, *, symbols: List[PythonSymbol]) -> List[Finding]:
        ...
        for sym in symbols:
            ...
            # Check if the docstring is missing or not descriptive enough
            if not sym.docstring:
                findings.append(
                    Finding(  # type: ignore[call-arg]
                        rule_id=self.id,
                        level=FindingLevel.WARNING,
                        message="Test function should have a docstring explaining what it tests",
                        symbol=sym.name,
                        location=Location(line=sym.lineno, column=sym.col),
                    )
                )
            elif not self._has_descriptive_docstring(sym):
                findings.append(
                    Finding(  # type: ignore[call-arg]
                        rule_id=self.id,
                        level=FindingLevel.WARNING,
...
class DG402PublicAPIFunctionDocumentation:
    ...
    def _get_missing_sections(self, sym: PythonSymbol) -> List[str]:
        ...
        # Check for Returns section (only if function has return annotation)
        if (
            hasattr(sym, "ast_node")
            and sym.ast_node
            and hasattr(sym.ast_node, "returns")
        ):
            if sym.ast_node.returns and "returns:" not in docstring_lower:
                missing.append("Returns")

        # Check for Raises section (only if function can raise exceptions)
        # For now, we'll be lenient and not require Raises section for all public APIs
        # This can be enhanced later to detect actual exceptions in the function body
        pass
