...
class DG101MissingDocstring:
    ...
    def check(self, *, symbols: List[PythonSymbol]) -> List[Finding]:
        ...
        for sym in symbols:
            if sym.kind in {"module", "function", "class"}:
                # Check if this symbol should require a docstring
                if self._should_require_docstring(sym):
                    # Check for missing or whitespace-only docstrings
                    if not sym.docstring or self._is_whitespace_only(sym.docstring):
                        message = f"{sym.kind.capitalize()} '{sym.name}' is missing a docstring"
                        if sym.docstring and self._is_whitespace_only(sym.docstring):
                            message = f"{sym.kind.capitalize()} '{sym.name}' has a whitespace-only docstring"
    ...
    def _is_whitespace_only(self, docstring: str) -> bool:
        """Check if docstring contains only whitespace characters."""
        if not docstring:
            return False
