...
def build_arima_models(processed_data, forecast_horizon=90, n_simulations=100):
    ...
    Args:
        processed_data (pd.DataFrame): DataFrame with preprocessed stock data.
                                       Expected to have columns like 'Close_AAPL'.
    ...
    Returns:
        dict: A dictionary where keys are ticker symbols and values are a dict
              containing the fitted model, forecast, confidence interval,
              simulations, and model evaluation metrics (AIC, BIC, etc.).
    """
    results = {}
    ...
    for ticker in tickers:
        try:
            ...
            # Use auto_arima to find optimal (p,d,q) with robust error handling
            auto_model = pm.auto_arima(
                ...
                d=None # Let the function determine differencing order
            )
