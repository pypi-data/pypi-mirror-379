...
def hdf5_as_os_path(func):
    @wraps(func)
    def as_os_path(*path_parts):
        args = [os.sep.join(re.split(f"[{HDF5_SEP}]+", x)) for x in path_parts]
        ret = func(*args)
        return ret.replace(os.sep, HDF5_SEP)

    return as_os_path


@hdf5_as_os_path
def hdf5_normpath(path: str) -> str:
    return os.path.normpath(path)


@hdf5_as_os_path
def hdf5_relpath(path: str, refpath: str) -> str:
    return os.path.relpath(path, refpath)


@hdf5_as_os_path
def hdf5_basename(path: str) -> str:
    return os.path.basename(path)


@hdf5_as_os_path
def hdf5_join(*args) -> str:
    return os.path.join(*args)


def joinUri(uri: str, *path_parts) -> str:
    filename, path = splitUri(uri)
    path = hdf5_join(path, *path_parts)
    return filename + "::" + path


def splitUri(uri: str) -> str:
    """
    ...
    try:
        ...
        filename = uri
        h5groupname = HDF5_SEP
    else:
        filename = uri[:i]
        h5groupname = uri[i + 2 :]
        if h5groupname[0] != HDF5_SEP:
            h5groupname = HDF5_SEP + h5groupname
    return filename, h5groupname


def normUri(uri: str) -> str:
    """Normalize uri
...
def _dereference_node(parent, name):
    ...
    try:
        ...
        return joinUri(getUri(parent), name)
    else:
        if isinstance(lnk, h5py.SoftLink):
            path = lnk.path
            if not path.startswith(HDF5_SEP):
                parts = name.split(HDF5_SEP)[:-1]
...
def h5Name(h5group):
    ...
    :returns str:
    """
    return h5group.name.split(HDF5_SEP)[-1]
...
def _isLockedError(ex):
    ...
    if sys.platform == "win32":
        # errno = 0, Win32 GetLastError() = 33
        return "unable to lock file" in str(ex)
...
def getDefault(node, signal=True):
    ...
    root = node.file
    if default and not default.startswith(HDF5_SEP):
        if node.name == HDF5_SEP:
            default = HDF5_SEP + default
        else:
            default = node.name + HDF5_SEP + default
    while default:
        try:
            node = root[default]
        except KeyError:
        ...
        if nxclass == "NXdata":
            ...
            if add.startswith(HDF5_SEP):
                default = add
            elif add:
                default += HDF5_SEP + add
            else:
                break
    return default
