...
async def ui_page():
    ...
    with ui.left_drawer(top_corner=True, bordered=True).classes('items-center'):
        with ui.column(align_items='center').classes('h-full'):
            ui.image(os.path.join(here, 'assets/iceorb.png')).props('width=150px').style('border-radius: 5%')
...
def run(host='0.0.0.0', port=8000):
    ui.run(host=host, port=port, title='Ice Orb User Interface',
