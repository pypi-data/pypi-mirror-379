...
class KnowledgeBaseQueryResult(BaseModel):
    ...
    blocks: Annotated[
        ...
        Field(
            description=(
                "List of document layout blocks containing structured information "
    ...
    @field_validator("metadata", "blocks", mode="before")
    @classmethod
    def parse_json_fields(cls, v: str | dict | list | None) -> dict | list | None:
        """Parse JSON string fields (metadata, blocks) if needed."""
        if v is None:
            return None
