...
class kartverketsAPI:
    ...
    def decode_address(self, address: str = None, lat: float = None, lon: float = None,
        ...
        try:
            ...
            if lat and lon:
                self.logger.debug(f'Søker med koordinater: {lat}, {lon}')
            ...
            if not geonorge_data and address:
                self.logger.debug(f'Søker med adresse: {address}')
                geonorge_data = self.search_address(address=address, search_type="address")

        except Exception as e:
            self.logger.error(f'En uventet feil oppstod under Geonorge-søket: {e}')
        ...
        if geonorge_data and isinstance(geonorge_data, dict) and geonorge_data.get("adresser"):
            self.logger.debug('Fikk gyldig treff fra Geonorge, transformerer data.')
            return self.transform_single_geonorge(geonorge_data)

        self.logger.warning(f'Fikk ikke gyldig treff fra Geonorge for input: addr={address}, coords=({lat},{lon})')

        # 3. Fallback til Nominatim HVIS alt annet feilet OG vi har en adresse
        if address:
            self.logger.debug(f'Prøver fallback med Nominatim for adressen: {address}')
            try:
                ...
                self.logger.error(f'En feil oppstod under Nominatim fallback: {e}')

        # 4. Hvis absolutt ingenting fungerte, gi opp
        self.logger.error(f'Klarte ikke finne adressedata for input: addr={address}, coords=({lat},{lon})')
        return None
    def _decode_address(self,address : str = None, lat = None, lon = None,radius = 50,) -> list:
        ...
        if lat and lon and not raw_output_address:
            try:
                self.logger.debug(f'Searching for coordinates {lat},{lon} with geonorge')
                raw_output_address = self.search_address(lat=lat,lon=lon,radius=radius,search_type="coordinates")
                raw_output_address = None if not raw_output_address or "adresser" not in raw_output_address.keys() else raw_output_address
                if not raw_output_address:
                    self.logger.warning(f'No result from coordinates {lat,lon}')
            except Exception as e:
                self.logger.error(f'Error getting coordinates {lat,lon}: {e}')
        ...
        if raw_output_address:
            ...
            if clean_address:
                ...
                self.logger.warning(f'No reuslts after cleaning address {address,lat,lon}')

        else:
            if address:
                self.logger.debug(f'No hit from address search {address}. Trying coordinates search')
