...
def _ensure_standard_paths(info: dict, registry_dir: str,download_directory) -> dict:
    ...
    standard = {
        ...
        "audio_path": info.get("audio_path") or os.path.join(dirbase, "audio.wav"),
        "whisper_path": info.get("whisper_path") or os.path.join(dirbase, "whisper.json"),
        "srt_path": info.get("srt_path") or os.path.join(dirbase, "captions.srt"),
        "metadata_path": info.get("metadata_path") or os.path.join(dirbase, "metadata.json"),
        "thumbnails_dir": info.get("thumbnails_dir") or os.path.join(dirbase, "thumbnails"),
        "thumbnails_path": info.get("thumbnails_path") or os.path.join(dirbase, "thumbnails.json")
    }
...
class infoRegistry(metaclass=SingletonMeta):
    ...
    def __init__(self, video_directory=None, envPath=None, info_directory=None,
        ...
        if not hasattr(self, 'initialized',True):
