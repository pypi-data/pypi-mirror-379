...
class infoRegistry:
    ...
    Thread-safe registry:
      - registry.json with:
          by_url: {url -> video_id}
          by_path: {abs_path -> video_id}
          by_id: {video_id -> {"url":..., "video_path":..., "timestamp":...}}
      - Per-video cached info at <infoRegistryDirectory>/<video_id>.json
    """

    def __init__(self, video_directory=None, envPath=None, info_directory=None,
                 temp_directory=None, data_directory=None, **kwargs):
        ...
        with _LOCK:
            ...
            if os.path.isfile(self.registry_path):
                try:
                    ...
                    pass  # corrupt registry? keep empty maps
    ...
    def get_video_info(self, url: str | None = None, video_id: str | None = None,
        ...
        """
        If video_path exists: create info from file and cache it.
        Else if url: read cached by id or fetch yt-dlp info (no download).
        """
        ...
        # fetch fresh yt-dlp info (metadata only)
        if url:
