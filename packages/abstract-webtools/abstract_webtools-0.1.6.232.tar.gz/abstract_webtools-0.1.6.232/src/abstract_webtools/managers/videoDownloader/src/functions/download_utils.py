...
class VideoDownloader:
    def __init__(self, url, download_directory=None, user_agent=None,
        ...
        self.download_direactory = get_video_root(download_directory)
    ...
    def _download(self):
        for video_url in self.video_urls:
            ...
            # --- Cached lookup first
            info = self.registry.get_video_info(
                url=video_url, force_refresh=self.force_refresh
            )
            if info and info.get("video_path") and os.path.isfile(info["video_path"]):
                logger.info(f"[VideoDownloader] Using cached: {info['video_path']}")
                return info

            # --- Prepare yt-dlp
            tmp_outtmpl = info.get('video_path')
            ...
            try:
                ...
                # --- Push into registry (this normalizes + writes info.json)
                ...
                # --- Re-read normalized info (single source of truth)
