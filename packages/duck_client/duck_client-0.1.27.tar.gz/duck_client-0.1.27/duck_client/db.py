...
class DataBase:
    ...
    def _df_okx_1s_klines(
        ...
        right, left = self._inclusive_check(inclusive)
        start_ts_expr = (
            f"DATE_PART('epoch', TIMESTAMP '{start_date}')::BIGINT"
            if start_date
            else "NULL::BIGINT"
        )
        end_ts_expr = (
            f"DATE_PART('epoch', TIMESTAMP '{end_date}')::BIGINT"
            if end_date
            else "NULL::BIGINT"
        )
        ...
        bounds AS (
            SELECT
                symbol,
                min(ts_sec) AS min_ts,
                max(ts_sec) AS max_ts
            FROM filtered_data
            GROUP BY symbol
        ),
        calendar_bounds AS (
            SELECT
                symbol,
                COALESCE({start_ts_expr}, min_ts) AS start_ts,
                COALESCE({end_ts_expr}, max_ts) AS end_ts
            FROM bounds
        ),
        calendar AS (
            SELECT
            ...
            FROM calendar_bounds cb,
                 range(cb.start_ts, cb.end_ts + 1) AS r(ts_sec)
            WHERE cb.start_ts IS NOT NULL
              AND cb.end_ts IS NOT NULL
              AND cb.end_ts >= cb.start_ts
        ),
        prepared AS (
            SELECT
                cal.symbol,
            ...
            LEFT JOIN aggregated agg
                ON cal.symbol = agg.symbol
               AND cal.ts_sec = agg.ts_sec
        ),
        tracked AS (
            SELECT
                symbol,
                ...
                close,
                volume,
                MAX(CASE WHEN close IS NOT NULL THEN ts_sec END) OVER (
                    PARTITION BY symbol
                    ORDER BY ts_sec
        ...
        finalized AS (
            SELECT
                t.symbol,
                t.ts_sec,
                COALESCE(t.open, last_trade.close) AS open,
                COALESCE(t.high, last_trade.close) AS high,
                COALESCE(t.low, last_trade.close) AS low,
                COALESCE(t.close, last_trade.close) AS close,
                COALESCE(t.volume, 0) AS volume
            FROM tracked t
            LEFT JOIN aggregated last_trade
                ON t.symbol = last_trade.symbol
               AND t.last_close_ts = last_trade.ts_sec
        )
        SELECT
            symbol,
            to_timestamp(ts_sec)::TIMESTAMPTZ AS timestamp,
