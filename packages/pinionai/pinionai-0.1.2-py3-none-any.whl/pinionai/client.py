_Bb="Twilio 'from' number not configured. Cannot send SMS."
_Ba='completedAt'
_BZ='status_code'
_BY='Error: URL template evaluation failed.'
_BX='contentType'
_BW='access_token'
_BV='token_type'
_BU='client_credentials'
_BT='function_call'
_BS='Using environment credentials/IAM role for S3.'
_BR='boto3 library is required for S3 operations.'
_BQ='Using Application Default Credentials for GCS.'
_BP='gcs_service_account'
_BO='google-cloud-storage library is required for GCS operations.'
_BN='action_action_key'
_BM='__builtins__'
_BL='destinationVariable'
_BK='sourceVariable'
_BJ='promptName'
_BI='unknown_intent'
_BH='Error: AI response was malformed (function call had no name).'
_BG='resp_mime_type'
_BF='gemini-2.5-flash-preview-04-17'
_BE='safetySettings'
_BD='mime_type'
_BC='assistant'
_BB='noInputIntentNameList'
_BA='variables'
_B9='DEEPSEEK_API_KEY'
_B8='ANTHROPIC_API_KEY'
_B7='OPENAI_API_KEY'
_B6='EMAIL_FROM_ADDRESS'
_B5='EMAIL_API_KEY'
_B4='GEMINI_API_KEY'
_B3='GCP_REGION'
_B2='GCP_PROJECT'
...
_B0='user_input'
_A_='message'
_Az='Journey token not available.'
_Ay='Accept-Encoding'
_Ax='client_secret'
_Aw='client_id'
_Av='grant_type'
...
_Ao='function'
_An='header'
_Am='gemini-2.0-flash'
_Al='model_provider'
_Ak='agentDefaultIntent'
_Aj='intentContainsPairsList'
_Ai='intentNameList'
_Ah='action_flow'
_Ag='action_type'
_Af='iframeId'
...
_AX='privacy'
_AW='prompt_type'
_AV='ragStoreVectorDistanceThreshold'
_AU='ragStoreTopK'
_AT='ragStoreResourceId'
_AS='transferPasskeyFlag'
_AR='transferAllowed'
_AQ='TWILIO_NUMBER'
_AP='TWILIO_AUTH_TOKEN'
_AO='TWILIO_ACCOUNT_SID'
_AN='parameters'
_AM='Accept'
_AL='failureResponseMessage'
_AK='gzip'
_AJ='string'
_AI='enum'
_AH='required'
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret,version=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_B1,'localhost:50051');self._gcp_project_id=gcp_project_id or os.environ.get(_B2);self._gcp_region=gcp_region or os.environ.get(_B3);self._gemini_api_key=gemini_api_key or os.environ.get(_B4);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AO);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AP);self._twilio_number=twilio_number or os.environ.get(_AQ);self._email_api_key=email_api_key or os.environ.get(_B5);self._email_from_address=email_from_address or os.environ.get(_B6);self._openai_api_key=openai_api_key or os.environ.get(_B7);self._anthropic_api_key=anthropic_api_key or os.environ.get(_B8);self._deepseek_api_key=deepseek_api_key or os.environ.get(_B9);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=_A;self._session_id=_A;self._raw_session_data=_A;self.var={};self.engagement=''
	...
	async def _initialize_session_and_vars(self):
		...
		if _C in self._raw_session_data and self._raw_session_data[_C]:
			self.var=self._extract_data_from_session(self._raw_session_data[_C]);agent_data=self._raw_session_data[_C][_E][_C][_J];self.var['sessionId']=self._session_id;self.var['agentTitle']=agent_data.get('title');self.var['agentSubtitle']=agent_data.get('subtitle');self.var['agentStart']=agent_data.get('startStatement');self.var[A]=agent_data.get(A);self.var['userImage']=agent_data.get('userImagePath','').strip();self.var['assistImage']=agent_data.get('assistantImagePath','').strip();self.var[_AR]=agent_data.get(_AR);self.var[_AS]=agent_data.get(_AS);self.var[B]=agent_data.get(B);self.var['sessionDateTime']=datetime.datetime.now(zoneinfo.ZoneInfo('UTC')).astimezone().isoformat();stores_data=agent_data.get('stores',[]);self._rag_stores_by_name={}
			...
			if self._rag_stores_by_name:first_store_cfg=next(iter(self._rag_stores_by_name.values()));self.var[_AT]=first_store_cfg.get(_m,'');self.var[_AU]=first_store_cfg.get(_n,10);self.var[_AV]=first_store_cfg.get(_o,.5)
			else:self.var[_AT]='';self.var[_AU]=10;self.var[_AV]=.5
	...
	async def process_user_input(self,user_input='',sender=_Z):
		...
		if not self.transfer_requested:
			...
			if self.next_intent:self.var['prompt_resp_var']=self.next_intent;prompt_type=self.var[_AW]=_A2;prompt_resp_var_name=self.next_intent;self.next_intent=''
			else:
			...
			intent_type=intent_data[_F];self.var[A]=intent_data[_B];self._privacy_level=self.var['privacy_resp']=intent_data[_AX];self.current_pipeline=self.var['pipeline_resp']=intent_data[_p];delivery_method=intent_data[_AY];language=intent_data[_a];intent_input_vars=intent_data.get(B,{});context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_AZ,{})
			...
			if intent_type!=_Aa:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_C,{}).get(_E,{}).get(_C,{}).get(_J,{}).get(_BA,[])
				...
				if self._class_intent_data:intent_data=self._class_intent_data;user_input=self._class_message;intent_type=intent_data[_F];self.var[A]=intent_data[_B];delivery_method=intent_data[_AY];language=intent_data[_a];intent_input_vars=intent_data.get(B,{});context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_AZ,{});self._privacy_level=intent_data[_AX];self.current_pipeline=intent_data[_p]
				original_intent_input_vars=self._class_intent_data.get(B,{})if self._class_intent_data else{}
				if action_key_for_input not in original_intent_input_vars and self._class_intent_data:final_response=f"'{user_input}' is a {intent_data[_B]} . Please respond to trigger an intent or input I am listening for such as: {self.var.get(_BB,[])}."
			if intent_input_vars:
				...
				if not final_response:
					if not self.authorized and'private'in self._privacy_level or I in self._privacy_level:
						...
						if customer_id and enrolled:
							pipeline_key=self.var[J]if I in self._privacy_level else self.var['authentication_pipeline'];json_payload={_Ab:pipeline_key,_h:{_c:delivery_method,_d:self.phone_number},_Ac:{_Ad:self.unique_id,_d:self.phone_number},_E:{_Ae:self._session_id},_a:language}
							if self.var.get(_i):json_payload[_Af]=self.var[_i]
							...
							auth_response_msg=f"You are not enrolled. We are sending a link to {self.phone_number} so that you can enroll your device.";json_payload={_Ab:self.var['enroll_pipeline'],_h:{_c:delivery_method,_d:self.phone_number},_Ac:{_Ad:self.unique_id,_d:self.phone_number},_E:{_Ae:self._session_id},_a:language}
							if self.var.get(_i):json_payload[_Af]=self.var[_i]
			...
			if not final_response:
				if intent_type=='fixed':fixed_message_template=self._clean_text(intent_data['fixedResponseMessage']);final_response=self._evaluate_f_string(fixed_message_template,self.var)
					...
					action_details_from_intent=self._map_to_standard_action_details(intent_data);action_type=action_details_from_intent.get(_Ag)
					if action_type=='journey':self.var,final_response=await self._run_journey_action(self.var,action_details_from_intent)
						...
						action_flow=action_details_from_intent.get(_Ah)
						if action_flow:
		...
		if final_response:self.chat_messages.append({_S:_BC,_Q:final_response})
	...
	async def send_grpc_message(self,message_text):
		...
		try:recipient_id=_BC if self.grpc_sender_id==_Z else _Z;request=ChatMessageRequest(thread_id=1,message=message_text,sender_id=self.grpc_sender_id,recipient_id=recipient_id,session_id=str(self._session_id));await self._grpc_stub.SendMessage(request);logger.info(f"gRPC message sent by {self.grpc_sender_id}: {message_text}")
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_C,{}).get(_J,{});global_to_self_attr_map={_B1:'_grpc_server_address',_B2:'_gcp_project_id',_B3:'_gcp_region',_B4:'_gemini_api_key',_AO:'_twilio_account_sid',_AP:'_twilio_auth_token',_AQ:'_twilio_number',_B5:'_email_api_key',_B6:'_email_from_address',_B7:'_openai_api_key',_B8:'_anthropic_api_key',_B9:'_deepseek_api_key'}
		for variable_def in agent_config.get(_BA,[]):
		...
		for intent in agent_config.get('intents',[]):
			...
			if intent[_F]==_Aa:input_intent_names.append(intent_name)
			intent_privacy_pairs.append((intent_name,intent[_AX]));intent_type_pairs.append((intent_name,intent[_F]));intent_contains_word_pairs.append((intent_name,intent.get('containsWord','')));intent_action_key_pairs_raw.append((intent_name,intent.get(_p,'')))
		var_dict[_Ai]=intent_names;var_dict[_BB]=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_Aj]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_A6]=agent_config.get(_A6);var_dict[_Ak]=agent_config.get('defaultIntent');return var_dict
	...
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		self.var[_AW]=prompt_config[_F];prompt_resp_var_name=prompt_config[_A7];prompt_body_template=self._clean_text(prompt_config[_T]);current_prompt_text=self._evaluate_f_string(prompt_body_template,self.var,user_input=user_input);model_provider=prompt_config.get(_Al,_b).lower();model_name=prompt_config.get(_U);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',self.var)
		if model_provider==_b:
			...
			if prompt_config.get(_A8):
				...
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BD]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_A9],top_p=prompt_config[_V],top_k=prompt_config[_AA],candidate_count=prompt_config[_AB],max_output_tokens=prompt_config[_AC],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BE))
			if model_name in[_BF]and prompt_config.get(_AD)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AD])
			if prompt_config.get(_AE):gen_config.response_schema=prompt_config[_AE];gen_config.response_mime_type=prompt_config.get(_BG,_M)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BH
		...
		if llm_response_text:
			if prompt_config[_F]==_A2:
				...
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_Aj,[]),self.var.get(_Ak,_BI),self.var.get(_Ai,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_B);function_args=function_call.get(_I,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BH
		...
		if llm_response_text:
			if prompt_config[_F]==_A2:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_Aj,[]),current_var.get(_Ak,_BI),current_var.get(_Ai,[]))
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_b);prompt_model=_Am
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BJ,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
			...
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_An,{}),current_var);mcp_connector=mcp_config.get(_R,'')
				if mcp_connector:
			...
			if session_context:
				async with session_context as session:
					...
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_Al,_b)if prompt_config else _b;model_to_use=_Am
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_Ao,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_An):
			try:headers.update(json.loads(api_config[_An]))
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BK)
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_BL)
	...
	def _run_script_action(self,var,script_name):
		...
		try:
			if script_type=='javascript':
				...
				local_script_vars=var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.');exec(script_code,{_BM:{}},local_script_vars)
	...
	async def _evaluate_rule(self,expr,var):
		...
		try:
			...
			try:return eval(formatted_expr,{_BM:{}},{_q:var})
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BN);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_Aq);language=action_details.get(_a);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_Ab:action_key,_h:{_c:delivery_method,_d:self.phone_number},_Ac:{_Ad:self.unique_id,_d:self.phone_number},_E:{_Ae:self._session_id},_a:language}
		if current_var.get(_i):json_payload[_Af]=current_var[_i]
		if action_details.get(_Ar)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_Ar)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_Aq)=='voice':json_payload[_h]={_c:delivery_method,_d:self.phone_number,'callOperator':'twilio'}
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		if current_var.get(_AR):
			...
			if current_var.get(_AS):passkey=self._generate_password(6);current_var['transferPasskey']=passkey;sms_message=f"Your agent will validate themselves with the following shared passkey: {passkey}"
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_B:data.get(_B),_Ag:data.get('actionType'),_BN:data.get(_p),_Aq:data.get(_AY),_a:data.get(_a),_Ar:data.get('actionEvent'),_e:data.get('actionResponseMessage'),_Ah:data.get(_AZ,{})}
	async def _run_action_action(self,current_var,action_name,final_response_from_caller=_A,user_input=_A):
		...
		action_details=self._map_to_standard_action_details(action_entity_data);action_type=action_details.get(_Ag,'general')
		...
		action_flow=action_details.get(_Ah,{});action_produced_final_response=_A
		if action_flow:
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BO)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_z)==_BP:sa_info=json.loads(conn_config.get(_A0,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_BQ)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_BR)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_z)=='aws_iam':s3_client=boto3.client('s3',aws_access_key_id=conn_config.get(_As),aws_secret_access_key=conn_config.get(_A0));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client('s3');logger.info(_BS)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_Am};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
		...
		try:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_BT)and part.function_call]
					if fc_parts:
						function_calls=[]
	...
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		http_client=httpx.AsyncClient();client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_U:model_name,_r:messages,_A1:prompt_config.get(_A9),_At:prompt_config.get(_AC),_V:prompt_config.get(_V),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(A):openai_params[A]=prompt_config.get(A)
		if tools:openai_params[_O]=tools;openai_params[_Au]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_U:model_name,_t:system_prompt,_r:messages,_A1:prompt_config.get(_A9),_At:prompt_config.get(_AC,4096),_V:prompt_config.get(_V),_AA:prompt_config.get(_AA)}
		if tools:anthropic_params[_O]=tools;anthropic_params[_Au]={_F:'auto'}
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_Y:_M};data={_Av:_BU,_Aw:client_id,_Ax:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_BV]} {auth_response[_BW]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_Ay:_AK,_l:_x,_L:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_Ay:_AK,_l:_x,_L:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_C,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_l:_x,_L:token,_Ay:_AK}
		try:
	...
	async def _post_session_api(self,host,token,session_uid,data_payload,transfer_req,transfer_acc):
		url=f"/session";headers={_Y:_M,'Content-Encoding':_AK,_L:token};full_payload={'sessionUid':session_uid,_C:data_payload,'transferRequested':transfer_req,'transferAccepted':transfer_acc}
		try:
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';url=connector_config.get(_W);client_id=connector_config.get(_As);client_secret=connector_config.get(_A0);grant_type=connector_config.get(_z,_BU);content_type=connector_config.get(_BX,_M);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_A5,_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_L]=f"Basic {client_keys_b64}";headers[_Y]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
			...
			if _y in content_type.lower():
				data_to_send={_Av:grant_type,_Aw:client_id,_Ax:client_secret}
				if scope:data_to_send[_A5]=scope
			elif'x-www-form-urlencoded'in content_type.lower():
				form_data=[(_Av,grant_type),(_Aw,client_id),(_Ax,client_secret)]
		...
		try:
			...
			async with httpx.AsyncClient()as client:response=await client.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_BV,'Bearer').capitalize();return f"{token_type} {auth_response[_BW]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		...
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_BY
		...
		try:
			...
			if response.status_code>=400 or not parsed_response:
				...
				if api_config.get(_AL):final_response_message=self._evaluate_f_string(api_config[_AL],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:raise PinionAIAPIError(f"API {method} error for {url}",status_code=getattr(e.response,_BZ,_A),details=str(e))from e
	async def _generic_api_get(self,current_var,api_config,headers):
		...
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_BY
		...
		try:
			...
			if response.status_code>=400:
				...
				if api_config.get(_AL):final_response_message=self._evaluate_f_string(api_config[_AL],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:raise PinionAIAPIError(f"API GET error for {url}",status_code=getattr(e.response,_BZ,_A),details=str(e))from e
	...
	async def _journey_lookup(self,unique_id_val,current_var,user_input=_A):
		...
		headers={_AM:_M,_L:bearer_token};url=f"https://app.journeyid.io/api/system/customers/lookup?account_id={account_id}&unique_id={unique_id_val}"
		try:
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_Az
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_Az
		url=f"https://app.journeyid.io/api/system/executions/{execution_id}";headers={_AM:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				...
				if status_data.get(_Ba):return _D,status_data.get('outcome',{}).get(_A_,'Task completed successfully.')
	...
	async def _journey_session_pipeline_status_check(self,session_id_val,current_var,pipeline_key_to_check,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_Az
		url=f"https://app.journeyid.io/api/system/sessions/lookup?external_ref={session_id_val}";headers={_AM:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				async with httpx.AsyncClient()as client:response=await client.get(url,headers=headers,timeout=30)
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_Ba):return _D,exec_item.get('outcome',{}).get(_A_,'Pipeline completed.')
						break
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_Bb);return
	...
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		...
		infobip_url='https://g9egk8.api.infobip.com/email/4/messages';headers={_L:token,_Y:_M,_AM:_M};payload={_r:[{'sender':from_address,'destinations':[{'to':[{'destination':to_address}]}],_Q:{_Ap:subject,_P:plain_text_body,'html':html_body_content}}]}
		try:
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_L:token,_Y:_M};email_payload={_A_:{_Ap:subject,_T:{_BX:'HTML',_Q:html_body_content},'toRecipients':[{'emailAddress':{'address':to_address}}]},'saveToSentItems':'true'}
		try:
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={_q:context_vars,C:context_vars,D:context_vars,_B0:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={_q:context_vars,C:context_vars,D:context_vars,_B0:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={_q:context_vars,_B0:user_input};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _create_function_declaration_from_dict(self,declaration_dict):
		...
		name=declaration_dict.get(_B);description=declaration_dict.get(_K);parameters=declaration_dict.get(_AN)
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_f:
			for fd in processed_fd_dicts:translated_tools.append({_F:_Ao,_Ao:{_B:fd.get(_B),_K:fd.get(_K),_AN:fd.get(_AN)}})
		elif provider==_s:
			for fd in processed_fd_dicts:translated_tools.append({_B:fd.get(_B),_K:fd.get(_K),'input_schema':fd.get(_AN)})
...
def twilio_sms_message(sms_body,to_phone,from_phone_twilio=_A):
	'Sends an SMS using Twilio, reading credentials from environment variables.';twilio_account_sid=os.environ.get(_AO);twilio_auth_token=os.environ.get(_AP)
	...
	_national_digits,_unique_id,to_phone_e164=format_phone(to_phone);twilio_from_number=from_phone_twilio or os.environ.get(_AQ)
	if not twilio_from_number:logger.error(_Bb);return
