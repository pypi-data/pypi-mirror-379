...
def main(url: Optional[str], quality: str, output: str, playlist_folder: bool, 
    ...
    try:
        ...
        if health:
            ...
            try:
                ...
                sys.exit(1)
            return
        # If no URL is provided and not using a URLs file, auto-start interactive mode by default
        ...
        if search:
            ...
            search_url = f"ytsearch{n}:{url}"
            click.echo(click.style("Detected search query", fg='blue'))
            process_search(extractor, converter, search_url, options, verbose, progress_tracker, lyrics)
            return
...
def process_video(extractor: YouTubeExtractor, converter: AudioConverter, 
                 url: str, options: ExtractionOptions, verbose: bool, progress_tracker: ProgressTracker, lyrics: bool = False, exit_on_failure: bool = True) -> bool:
    """
...
def process_search(extractor: YouTubeExtractor, converter: AudioConverter,
                   search_url: str, options: ExtractionOptions, verbose: bool,
                   progress_tracker: ProgressTracker, lyrics: bool = False) -> None:
    """Process yt-dlp search results (ytsearchN:query) as multiple single videos."""
    logger = logging.getLogger(__name__)
    try:
        base_info_opts = {
            'quiet': True,
            'no_warnings': True,
            # Flat extraction avoids format selection to prevent errors on info retrieval
            'extract_flat': 'in_playlist',
            'skip_download': True,
        }
        if getattr(options, 'cookie_path', None):
            base_info_opts['cookiefile'] = options.cookie_path
        with __import__('yt_dlp').YoutubeDL(base_info_opts) as ydl:
            info = ydl.extract_info(search_url, download=False)
        entries = info.get('entries', []) if info else []
        if not entries:
            progress_tracker.show_error_with_suggestions(
                'No results found', ['Try a different query', 'Increase --search-limit']
            )
            sys.exit(1)
        success_count = 0
        for entry in entries:
            ...
            if video_url and not video_url.startswith(('http://', 'https://')):
                vid = entry.get('id') or video_url
                video_url = f"https://www.youtube.com/watch?v={vid}"
            if not video_url:
                continue
            click.echo(click.style("Detected video URL", fg='blue'))
            ok = process_video(extractor, converter, video_url, options, verbose, progress_tracker, lyrics, exit_on_failure=False)
            if ok:
                success_count += 1
        if success_count == 0:
            progress_tracker.show_error_with_suggestions(
                'Search processing failed: no entries could be downloaded',
                ["Check network connection", "Try a different query", "Use --search-limit to increase results"]
            )
            sys.exit(1)
    except Exception as e:
        logger.error(f"Search processing failed: {e}")
        progress_tracker.show_error_with_suggestions(
            f"Search processing failed: {e}", ["Check network connection", "Try a different query"]
        )
        sys.exit(1)
