...
class InteractiveWizard:
    ...
    def _download_wizard(self) -> None:
        ...
        if use_filters:
            min_duration = self._ask_int_in_range("Minimum duration (seconds)", default=0, min_value=0, max_value=24*60*60)
            max_duration = self._ask_int_in_range("Maximum duration (seconds)", default=3600, min_value=0, max_value=24*60*60)
    ...
    def _batch_wizard(self) -> None:
        ...
        parallel_downloads = self._ask_int_in_range("Number of parallel downloads", default=3, min_value=1, max_value=10)
        ...
        args.extend(["--batch-size", str(parallel_downloads)])
    ...
    def _search_wizard(self) -> None:
        ...
        search_limit = self._ask_int_in_range("Number of results to download", default=10, min_value=1, max_value=50)
    ...
    def _get_format_choice(self) -> str:
        ...
        format_choices = ["mp3", "m4a", "opus", "flac", "mp4"]
        ...
        format_table.add_row("1", "MP3", "Most compatible (audio)")
        format_table.add_row("2", "M4A", "Apple devices (audio)")
        format_table.add_row("3", "Opus", "Modern, efficient (audio)")
        format_table.add_row("4", "FLAC", "Lossless (audio)")
        format_table.add_row("5", "MP4", "Video (keep video, no audio extraction)")
        
        self.console.print(format_table)
        
        choice = Prompt.ask("Select format", choices=["1", "2", "3", "4", "5"])
        return format_choices[int(choice) - 1]
    
    def _ask_int_in_range(self, prompt: str, default: int, min_value: int, max_value: int) -> int:
        """Prompt for an integer within a range, re-asking until valid."""
        while True:
            try:
                value = IntPrompt.ask(prompt, default=default)
                if value < min_value or value > max_value:
                    self.console.print(f"[red]Please enter a value between {min_value} and {max_value}.[/red]")
                    continue
                return int(value)
            except Exception:
    ...
    def _configure_auto_update(self) -> None:
        ...
        if enable_auto_update:
            check_interval = self._ask_int_in_range("Check interval (hours)", default=24, min_value=1, max_value=168)
