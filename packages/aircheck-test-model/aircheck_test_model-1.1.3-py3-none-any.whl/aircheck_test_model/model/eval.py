...
class ModelEvaluator:
    ...
    def calculate_metrics(X_test: np.ndarray, y_test: np.ndarray, y_pred: np.ndarray, y_proba: np.ndarray) -> dict:
        ...
        try:
            ...
            dp_ppv = ModelEvaluator.diverse_plate_ppv(
                y_test_array, y_pred_array, clusters=ModelEvaluator.cluster_leader_from_array(X_test).tolist()
            )
    ...
    def test_pipeline(
        ...
        try:
            updated_rows = []
            for test_model_path in model_path:
                dataset = Dataset(train_column, test_file, label)
                X_test_array = dataset.X
                Y_test_array = dataset.y

                test_metrics = ModelEvaluator.evaluate_model(test_model_path, X_test_array, Y_test_array)

                row_result = {key: value for key, value in test_metrics.items()}

                updated_rows.append(row_result)

            df_updated = pd.DataFrame(updated_rows)
            df_avg = df_updated.mean(numeric_only=True).to_frame().T
            print(df_avg.head())
            print("all columns---", df_avg.columns)
            return df_updated
        except Exception as e:
