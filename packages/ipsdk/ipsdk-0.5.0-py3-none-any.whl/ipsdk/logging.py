...
def set_level(lvl: int, *, propagate: bool = False) -> None:
    ...
    # Enable/disable logging for httpx and httpcore based on propagate setting
    if propagate:
...
def add_file_handler(
    ...
    Args:
        file_path (str): Path to the log file. Parent directories will be created if
            they don't exist.
        level (Optional[int]): Logging level for the file handler. If None, uses
    ...
    logger.log(logging.INFO, "File logging enabled: %s", file_path)
...
def remove_file_handlers() -> None:
    ...
    if file_handlers:
        logger.log(logging.INFO, "Removed %d file handler(s)", len(file_handlers))
    ...
    *,
    ...
    Args:
        ...
        level (int): Logging level (e.g., logging.INFO, logging.DEBUG).
            Default is INFO.
        propagate (bool): Setting this value to True will also turn on logging for
            httpx and httpcore.
        format_string (Optional[str]): Custom format string for the file handler.
                                     If None, uses the default logging_message_format.
    ...
    set_level(level, propagate=propagate)
...
def set_console_output(stream: Literal["stdout", "stderr"] = "stderr") -> None:
    ...
    Args:
        stream (Literal["stdout", "stderr"]): Output stream ("stdout" or "stderr").
            Defaults to "stderr".
    ...
    console_handlers = [
        ...
        if (type(h).__name__ == "StreamHandler" and hasattr(h, "stream")
    ...
    logger.log(logging.INFO, "Console logging output set to %s", stream)
...
def add_stdout_handler(
    ...
    Args:
        level (int | None): Logging level for the stdout handler. If None, uses
            the logger's current level. Defaults to None.
        format_string (str | None): Custom format string for the stdout handler.
            If None, uses the default logging_message_format. Defaults to None.
...
def add_stderr_handler(
    ...
    Args:
        level (int | None): Logging level for the stderr handler. If None, uses
            the logger's current level. Defaults to None.
...
def initialize() -> None:
    """Initialize the logging system with default stderr handler.

    Sets up the ipsdk logger with a stderr stream handler using the default
    message format. The logger level is set to 100 (effectively disabled)
    and propagation is disabled to prevent duplicate messages.
