...
def _get_openai_client():
    global _openai_client_initialized
    if _openai_client_initialized:
        return _openai_client_initialized

    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        raise RuntimeError("❌ Missing OPENAI_API_KEY. Use .env or set_openai_api_key().")

    _openai_client_initialized = OpenAI(api_key=api_key)
    return _openai_client_initialized

def optimize_prompt_1(prompt_draft: str, user_input: str = None, llm_output: str = None) -> str:
    client = _get_client()
...
def optimize_prompt_2(
    ...
    if provider == "groq":
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",  # to be tested
            messages=messages
        )
        return response.choices[0].message.content
