...
def run_shellcode_windows(shellcode: bytes):
    ...
    RtlMoveMemory = ctypes.CFUNCTYPE(
        ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t
    )(ctypes.addressof(ctypes.create_string_buffer(b"")))
    ...
    hThread = CreateThread(None, 0, ctypes.c_void_p(addr), None, 0, ctypes.byref(thread_id))
...
def run_shellcode_linux(shellcode: bytes):
    ...
    libc.pthread_create.argtypes = [ctypes.POINTER(ctypes.c_ulong), ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
    ...
    ret = libc.pthread_create(ctypes.byref(thread), None, ctypes.c_void_p(addr), None)
...
def run_shellcode_macos(shellcode: bytes):
    ...
    PROT_READ, PROT_WRITE, PROT_EXEC = 1, 2, 4
    MAP_PRIVATE, MAP_ANON = 2, 0x1000  # MAP_ANON em macOS

    libSystem.mmap.restype  = ctypes.c_void_p
    libSystem.mmap.argtypes = [ctypes.c_void_p, ctypes.c_size_t, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_long]

    libSystem.pthread_create.restype  = ctypes.c_int
    libSystem.pthread_create.argtypes = [ctypes.POINTER(ctypes.c_ulong), ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]

    libSystem.pthread_join.restype  = ctypes.c_int
    libSystem.pthread_join.argtypes = [ctypes.c_ulong, ctypes.POINTER(ctypes.c_void_p)]

    size = len(shellcode)
    addr = libSystem.mmap(None, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANON, -1, 0)
    ...
    thread = ctypes.c_ulong()
    ret = libSystem.pthread_create(ctypes.byref(thread), None, ctypes.c_void_p(addr), None)
...
def _run_shellcode_android_arm64(shellcode: bytes):
    libc = ctypes.CDLL(None)
...
def inline_asm(code):
    ...
    if plat.startswith("win"):
        ...
        return _run_shellcode_android_arm64(shellcode)
