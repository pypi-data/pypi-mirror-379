...
class Adapter(Config):
    ...
    plugins: []
        ...
        # 在plugins数组中添加harmony插件
        pluginsPattern = r'(plugins:\s*\[)(.*?)(\])'
        match = re.search(pluginsPattern, content, re.DOTALL)
        
        if match:
            existing_plugins = match.group(2).strip()
            if existing_plugins and not existing_plugins.endswith(','):
                existing_plugins += ','
            
            new_plugins = existing_plugins + '\n        ' + ',\n        '.join(harmonyPlugins)
