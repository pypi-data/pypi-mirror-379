...
class TimeTrackerGUI:
    def __init__(self):
        ...
        # Set custom icon - prefer high-quality PNG, fallback to ICO
        try:
            # Try high-quality PNG first, then ICO as fallback
            icon_paths = [
            ...
            for icon_path in icon_paths:
                if os.path.exists(icon_path):
                    if icon_path.endswith('.ico'):
                        # For ICO files, use iconbitmap immediately
                        self.root.iconbitmap(icon_path)
                        print(f"Loaded ICO icon: {icon_path}")
                        break
                    else:
                        # For PNG files, store path for later loading with PhotoImage
                        self._pending_icon_path = icon_path
                        print(f"PNG icon queued for later loading: {icon_path}")
                        # Also try iconbitmap as immediate fallback
                        try:
                            self.root.iconbitmap(icon_path)
                            print(f"Also loaded PNG with iconbitmap: {icon_path}")
                        except:
                            pass
                        break
    ...
    def _refresh_icon(self):
        ...
        try:
            # If we have a pending PNG icon, try to load it with PhotoImage
            if hasattr(self, '_pending_icon_path') and self._pending_icon_path:
                try:
                    from tkinter import PhotoImage
                    icon_image = PhotoImage(file=self._pending_icon_path)
                    self.root.iconphoto(True, icon_image)
                    print(f"Loaded PNG icon with PhotoImage: {self._pending_icon_path}")
                    # Also try iconbitmap as backup
                    try:
                    ...
                    print(f"PhotoImage failed, trying iconbitmap: {e}")
                    try:
                        self.root.iconbitmap(self._pending_icon_path)
                        print(f"Loaded PNG icon with iconbitmap: {self._pending_icon_path}")
                    except Exception as e2:
                        print(f"Both PhotoImage and iconbitmap failed: {e2}")
        except Exception as e:
