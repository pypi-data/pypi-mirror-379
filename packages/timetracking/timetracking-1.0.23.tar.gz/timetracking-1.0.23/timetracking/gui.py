...
class TimeTrackerGUI:
    def __init__(self):
        ...
        # Set custom icon - prefer PNG with PhotoImage for better macOS compatibility
        try:
            # Try PNG first (better for macOS), then ICO
            icon_paths = [
                os.path.join(os.path.dirname(__file__), 'clock_icon.png'),
                os.path.join(os.path.dirname(__file__), 'clock_icon_64.png'),
                os.path.join(os.path.dirname(__file__), 'clock_icon_64.ico'),
                os.path.join(os.path.dirname(__file__), 'clock_icon_32.ico'),
                os.path.join(os.path.dirname(__file__), 'clock_icon.ico')
            ]
            self._pending_icon_path = None
            for icon_path in icon_paths:
                if os.path.exists(icon_path):
                    if icon_path.endswith('.png'):
                        # For PNG files, use PhotoImage (better for macOS)
                        try:
                            from tkinter import PhotoImage
                            self._icon_image = PhotoImage(file=icon_path)
                            self.root.iconphoto(True, self._icon_image)
                            print(f"Loaded PNG icon with PhotoImage: {icon_path}")
                            # Try to set macOS dock icon if possible
                            self._set_macos_dock_icon(icon_path)
                        except Exception as e:
                            # Fallback to iconbitmap and queue for refresh
                            try:
                                self.root.iconbitmap(icon_path)
                                print(f"Loaded PNG icon with iconbitmap: {icon_path}")
                            except Exception as e2:
                                print(f"Icon bitmap failed for PNG: {e2}")
                        ...
                        try:
                            ...
                            print(f"ICO icon load failed: {e}")
                        break
        except Exception as e:
            print(f"Icon loading failed: {e}")  # Debug output
        ...
        self.root.after(100, self._refresh_icon)
        # Also try immediate icon setting
        self.root.after(0, self._immediate_icon_set)
    ...
    def check_for_updates(self):
        ...
        # Debug output
        print(f"Version check: current={current_version}, latest={latest}")
        print(f"Version comparison: {self._version_tuple(latest)} <= {self._version_tuple(current_version)} = {self._version_tuple(latest) <= self._version_tuple(current_version)}")
    ...
    def _restart_app(self):
        ...
        try:
            ...
            for cmd in restart_commands:
                try:
                    print(f"Attempting restart with: {' '.join(cmd)}")  # Debug output
                    _subprocess.Popen(cmd, stdout=_subprocess.DEVNULL, stderr=_subprocess.DEVNULL)
                    print("Restart command executed successfully")  # Debug output
                    break
                except Exception as e:
                    print(f"Restart command failed: {e}")  # Debug output
                    continue
                    
        except Exception as e:
            print(f"Restart failed: {e}")  # Debug output
        finally:
            self.root.quit()
    ...
    def _immediate_icon_set(self):
        """Try to set icon immediately after window creation"""
        try:
            # Try to set icon with the best available icon
            icon_paths = [
                os.path.join(os.path.dirname(__file__), 'clock_icon_64.ico'),
                os.path.join(os.path.dirname(__file__), 'clock_icon_32.ico'),
                os.path.join(os.path.dirname(__file__), 'clock_icon.ico')
            ...
            for icon_path in icon_paths:
                if os.path.exists(icon_path):
                    try:
                        self.root.iconbitmap(icon_path)
                        print(f"Immediate icon set: {icon_path}")
                        break
                    except Exception as e:
                        print(f"Immediate icon setting failed for {icon_path}: {e}")
        except Exception as e:
            print(f"Immediate icon setting failed: {e}")

    def _set_macos_icon(self, icon_path):
        """Try macOS-specific methods to set the dock icon"""
        try:
            import platform
            if platform.system() == "Darwin":  # macOS
                # Try multiple approaches for macOS
                try:
                    # Method 1: Try to set the window icon multiple times
                    for _ in range(3):
                        self.root.iconbitmap(icon_path)
                        self.root.update_idletasks()
                    
                    # Method 2: Try to force window refresh
                    self.root.withdraw()
                    self.root.deiconify()
                    self.root.iconbitmap(icon_path)
                    
                    print(f"Applied macOS icon refresh for {icon_path}")
                except Exception as e:
                    print(f"macOS icon refresh failed: {e}")
        except Exception as e:
            print(f"macOS icon method failed: {e}")

    def _set_macos_dock_icon(self, icon_path):
        """Set macOS Dock icon via AppKit if available (more reliable for Dock)."""
        try:
            import platform
            ...
            try:
                ...
                if img is not None:
                    ...
                    print(f"Set macOS Dock icon via AppKit: {icon_path}")
            except Exception as e:
                # AppKit not available or failed; ignore silently
                print(f"AppKit Dock icon set failed: {e}")
        except Exception as e:
            print(f"macOS Dock icon method failed: {e}")

    def _refresh_icon(self):
        """Refresh the window icon after window is fully created"""
        try:
            # If we have a pending PNG icon, try PhotoImage first, then bitmap
            if hasattr(self, '_pending_icon_path') and self._pending_icon_path:
                try:
                    from tkinter import PhotoImage
                    self._icon_image = PhotoImage(file=self._pending_icon_path)
                    self.root.iconphoto(True, self._icon_image)
                    print(f"Loaded PNG icon with PhotoImage (refresh): {self._pending_icon_path}")
                    # Also set Dock icon on macOS if possible
                    self._set_macos_dock_icon(self._pending_icon_path)
                    return
                except Exception as e:
                    print(f"PhotoImage refresh failed: {e}")
                # Fallback to iconbitmap
                try:
                    self.root.iconbitmap(self._pending_icon_path)
                    print(f"Loaded PNG icon with iconbitmap (refresh): {self._pending_icon_path}")
                except Exception as e2:
                    print(f"Both PhotoImage and iconbitmap failed during refresh: {e2}")
                
                # Try to force icon refresh on macOS
                try:
                    import platform
                    if platform.system() == "Darwin":  # macOS
                        # Force window to refresh icon
                        self.root.update_idletasks()
                        self.root.update()
                        # Try setting icon again after update
                        try:
                            self.root.iconbitmap(self._pending_icon_path)
                        except:
                            pass
