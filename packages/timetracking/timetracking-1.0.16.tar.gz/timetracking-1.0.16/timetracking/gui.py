...
class TimeTrackerGUI:
    def __init__(self):
        ...
        # Set custom icon
        try:
            # Try high-res PNG first, then 64x64 PNG, then ICO
            icon_paths = [
            ...
            for icon_path in icon_paths:
                if os.path.exists(icon_path):
                    if icon_path.endswith('.png'):
                        # For PNG files, try PhotoImage approach on macOS
                        try:
                            from tkinter import PhotoImage
                            icon_image = PhotoImage(file=icon_path)
                            self.root.iconphoto(True, icon_image)
                            # Also try iconbitmap as fallback for some systems
                            try:
                            ...
                            self.root.iconbitmap(icon_path)
                            print(f"Loaded PNG icon with iconbitmap: {icon_path} (fallback)")
                    else:
                ...
                if os.path.exists(icon_path):
                    if icon_path.endswith('.png'):
                        try:
                            ...
                            icon_image = PhotoImage(file=icon_path)
                            self.root.iconphoto(True, icon_image)
                            print(f"Refreshed PNG icon: {icon_path}")
                        except Exception:
                            pass
                    else:
                        try:
                            self.root.iconbitmap(icon_path)
                            print(f"Refreshed ICO icon: {icon_path}")
                        except Exception:
                            pass
                    break
        except Exception as e:
