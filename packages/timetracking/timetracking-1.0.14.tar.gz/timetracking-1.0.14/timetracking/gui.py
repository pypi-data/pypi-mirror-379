...
class TimeTrackerGUI:
    ...
    def _restart_app(self):
        ...
        try:
            # Try to restart using the same command that launched this instance
            import os
            import shutil
            
            # Check if we're running under pipx by looking for the timetracking command
            if shutil.which('timetracking'):
                _subprocess.Popen(['timetracking'])  # launch new instance via pipx
            else:
                # Fallback: try to run the module directly
                _subprocess.Popen([sys.executable, "-m", "timetracking"])
        except Exception:
            pass
        self.root.quit()
