...
from pararamio_aio._core.constants import DATETIME_FORMAT
from pararamio_aio._core.exceptions import PararamioValidationError, PararamModelNotLoadedError
...
def parse_datetime(
    ...
    if key not in data:
        return None
    return datetime.strptime(data[key], format_).replace(tzinfo=UTC)
