...
class StreamKafka:
    ...
    async def async_cache_stream_task(self, cache, groupid = 'cache-group', offset = 'earliest'):
        ...
        This coroutine subscribes to a stream with the specified consumer group and offset, then enters an infinite loop where it polls for messages asynchronously. Retrieved messages are stored in the provided cache, and the consumer group's offset is committed after each batch. Additionally, a counter tracking the number of cached messages is incremented.
        ...
        Raises:
            Exception: If subscription, polling, caching, or committing fails, an exception is logged and re-raised.
        """
        try:
    ...
    async def async_persist_stream_task(self, cache, partitioning='daily'):
        ...
        Behavior:
            ...
            - Logs and raises exceptions on subscription or persistence failures.
        """
        try:
