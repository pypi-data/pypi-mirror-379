...
class WebSocketService:
    ...
    async def _start_heartbeat(self, exchange_name, ws):
        ...
        async def heartbeat_loop():
            while exchange_name in self.connections:
                try:
                    await asyncio.sleep(20)  # Bybit recommends 20 seconds
    ...
    async def _process_message(self, exchange_name, message, message_handler):
        ...
        try:
            ...
            if parsed_message.get("ret_msg") == "pong" or parsed_message.get("op") == "ping":
                logging.debug(f"[{exchange_name}] Received pong")
                return
