...
from jingzhi.cmd import repo
from jingzhi.cmd.repo_types import RepoType
from importlib.metadata import version
from jingzhi.constants import DEFAULT_JINGZHI_ENDPOINT, DEFAULT_REVISION

app = typer.Typer(add_completion=False)

def version_callback(value: bool):
    if value:
        pkg_version = version("jingzhi")
        print(f"jingzhi-cli version {pkg_version}")
        raise typer.Exit()

OPTIONS = {
    "repoID": typer.Argument(help="The ID of the repo. (e.g. `username/repo-name`)."),
    "localPath": typer.Argument(help="Local path to the file or folder to upload. Defaults to the relative path of the file of repo of Jingzhi Hub."),
    ...
    "username": typer.Option("-u", "--username", help="Logon account of JingZhi Hub."),
    "token": typer.Option("-k", "--token", help="A User Access Token generated from https://aihub.caict.ac.cn/settings/git-token"),
    "version": typer.Option(None, "-V", "--version", callback=version_callback, is_eager=True, help="Show the version and exit."),
}

@app.command(name="download", help="Download model/dataset from https://aihub.caict.ac.cn/")
def download(
        ...
        endpoint: Annotated[Optional[str], OPTIONS["endpoint"]] = DEFAULT_JINGZHI_ENDPOINT,
...
@app.command(name="upload", help="Upload repository files to https://aihub.caict.ac.cn/.")
def upload(
        repo_id: Annotated[str, OPTIONS["repoID"]],
