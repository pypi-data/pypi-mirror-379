...
from bear_dereth.files.file_handlers.toml_file_handler import TomlFileHandler
...
class ConfigManager[ConfigType: BaseModel]:
    ...
    def _load_toml_file(self, file_path: Path) -> dict[str, Any]:
        ...
        if not file_path.exists() or not file_path.is_file():
            return {}
        try:
            return TomlFileHandler(file_path).read().get()
    ...
    def create_default_config(self, target_path: Path | None = None) -> None:
        ...
        if not default_path.exists():
            ...
            try:
                (
                    TomlFileHandler(default_path)
                    .from_pydantic(model=self._model(), exclude_none=True)
                    .to_toml_string(save=True)
                )
