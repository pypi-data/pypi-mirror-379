...
from arcos4py.tools._detect_events import ImageTracker, Linker, downscale_image
...
def track_events(
    ...
    dims: Literal["TXY", "TYX", "TZXY", "ZTYX", "XYT", "ZYXT"] = "TYX",
    ...
    if np.unique(selected_image).size > 2:
    ...
    dims_upper = dims.upper()

    if estimate_eps != "Manual":
        eps_image = selected_image
        if "T" in dims_upper:
            t_axis = dims_upper.index("T")
            if t_axis != 0:
                eps_image = np.moveaxis(eps_image, t_axis, 0)
            if downscale > 1:
                eps_image = np.asarray(
                    [downscale_image(frame, downscale) for frame in eps_image]
                )
        elif downscale > 1:
            eps_image = downscale_image(eps_image, downscale)
    ...
    pbar = progress(total=selected_image.shape[dims_upper.index("T")])
    ...
    def track_events_worker() -> list[LayerDataTuple]:
        try:
            ...
            linker = Linker(
                eps=eps,
                eps_prev=eps_prev if eps_prev > 0 else None,
                min_clustersize=min_clustersize,
                linking_method=linkin_method,
                clustering_method=clustering_method,
                min_samples=(
                    min_samples if min_samples else None
                ),
                n_prev=n_prev,
            ...
            img_tracked = np.zeros_like(image_data, dtype=np.uint16)
            img_tracked = np.moveaxis(img_tracked, dims_upper.index("T"), 0)
            for idx, timepoint in enumerate(
                tracker.track(image_data, dims)
            ):
                img_tracked[idx] = timepoint
                yield 1

            img_tracked = np.moveaxis(
                img_tracked, 0, dims_upper.index("T")
            )
