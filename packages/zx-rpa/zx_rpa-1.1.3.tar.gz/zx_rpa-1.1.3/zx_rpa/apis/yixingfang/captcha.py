...
class CaptchaHandler:
    ...
    def __init__(self, tujian_username: str, tujian_password: str):
        ...
        try:
            # 使用新的工厂类模式创建图鉴客户端
            self._tujian_client = CaptchaSolver.tujian(tujian_username, tujian_password)
    ...
    def recognize_captcha(self, image_source: str, type_id: int = 1) -> str:
        ...
        try:
            # 使用新的客户端模式，无需重复传参
            result = self._tujian_client.recognize(image_source, type_id)
            logger.debug("验证码识别成功，结果长度: {}", len(result))
            return result
        except Exception as e:
            logger.debug("验证码识别失败: {}", str(e))
    ...
    def check_balance(self) -> Dict[str, str]:
        ...
        try:
            # 使用新的客户端模式，无需重复传参
            balance_info = self._tujian_client.check_balance()
            logger.debug("余额查询成功")
            return balance_info
        except Exception as e:
            logger.debug("查询余额失败: {}", str(e))
