...
- Always process vulnerabilities in **three levels of iteration**:  
    1. Iterate through each `_new.json` file in `.scanissuefix`.  
    2. For each JSON file, iterate through every item inside the `code_list`.  
    3. For each vulnerability in `code_list`, perform fix review or automatic fix.
...
1. **Generate CSV Report:**
   ...
   * check if all the json files fixed from `_new.json` to `_finished.json` with MCP tool get_pending_vulnerability_json_files, then use MCP tool generate_csv_report generate a final CSV report (sast_fix_report.csv) in '.scanissuefix', from all files ending with '_finished.json' in the '.scanissuefix' directory.
