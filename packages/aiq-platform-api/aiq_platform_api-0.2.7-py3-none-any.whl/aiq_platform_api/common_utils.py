...
class AssessmentUtils:
    ...
    def get_run(client: AttackIQRestClient, assessment_id: str, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Getting run {run_id} for assessment {assessment_id}")
    ...
    @staticmethod
    def get_most_recent_run(client: AttackIQRestClient, assessment_id: str) -> Optional[Dict[str, Any]]:
        """Get the most recent run for an assessment."""
        logger.info(f"Getting most recent run for assessment {assessment_id}")

        runs = list(AssessmentUtils.list_assessment_runs(client, assessment_id, limit=1))
        if runs:
            run = runs[0]
            logger.info(f"Found most recent run: {run.get('id')}")
            return run
    ...
    def run_assessment(client: AttackIQRestClient, assessment_id: str) -> Optional[str]:
        ...
        endpoint = f"v{version}/assessments/{assessment_id}/run_all"
    ...
    def wait_for_run_completion(
        ...
        while time.time() - start_time < timeout:
            ...
            scenario_count = 0 if scenario_jobs is False else scenario_jobs
            integration_count = 0 if integration_jobs is False else integration_jobs

            status_msg = f"Scenarios: {scenario_count}, Integrations: {integration_count}"
            if status_msg != last_status:
                logger.info(f"Jobs in progress - {status_msg}")
