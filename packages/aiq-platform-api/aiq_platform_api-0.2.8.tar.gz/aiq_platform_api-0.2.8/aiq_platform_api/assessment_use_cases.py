...
def test_enable_detection(client: AttackIQRestClient, assessment_id: str):
    """Test enabling detection for an assessment."""
    if not assessment_id:
        logger.error("ASSESSMENT_ID required for this test")
        return

    result = AssessmentUtils.set_detection_enabled(client, assessment_id, True)
    logger.info(f"Enable detection: {'Success' if result else 'Failed'}")


def test_disable_detection(client: AttackIQRestClient, assessment_id: str):
    """Test disabling detection for an assessment."""
    if not assessment_id:
        logger.error("ASSESSMENT_ID required for this test")
        return

    result = AssessmentUtils.set_detection_enabled(client, assessment_id, False)
    logger.info(f"Disable detection: {'Success' if result else 'Failed'}")
...
def run_test(choice: "TestChoice", client: AttackIQRestClient, assessment_id: str):
    ...
    test_functions = {
        ...
        TestChoice.ENABLE_DETECTION: lambda: test_enable_detection(client, assessment_id),
        TestChoice.DISABLE_DETECTION: lambda: test_disable_detection(client, assessment_id),
...
if __name__ == "__main__":
    ...
    # choice = TestChoice.ENABLE_DETECTION
    # choice = TestChoice.DISABLE_DETECTION
