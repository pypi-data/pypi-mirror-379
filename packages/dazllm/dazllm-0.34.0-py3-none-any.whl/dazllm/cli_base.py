...
class CliProviderBase(Llm):
    ...
    def _get_timeout_seconds(self) -> int:
        """Get timeout in seconds for CLI calls"""
        return 120
    ...
    def chat_structured(
        ...
        while attempts > 0:
            try:
                ...
                raise DazLlmError(f"{self.executable_name} CLI timeout after {self._get_timeout_seconds()} seconds")
            except Exception as e:
    ...
    async def chat_async(self, conversation: Conversation, force_json: bool = False) -> LlmResponse:
        ...
        return LlmResponse(value=response, output=f"{self.executable_name} CLI async execution completed", provider=self.model)
    ...
    async def chat_structured_async(self, conversation: Conversation, schema, context_size: int = 0) -> LlmResponse:
        ...
        return LlmResponse(value=result, output=f"{self.executable_name} CLI async structured execution completed", provider=self.model)
