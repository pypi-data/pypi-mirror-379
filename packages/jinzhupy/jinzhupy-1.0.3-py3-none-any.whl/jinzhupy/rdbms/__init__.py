...
class DatabaseManager:
    ...
    async def query_one_orm(self, model, filters=None, joins=None, options=None, return_dict=True, key=None):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        record = await conn.query_one_orm(model, filters, joins, options)
        if return_dict:
            return record.as_dict()
        return record

    async def query_one_orm_with_session(self, session: AsyncSession, model, filters=None, joins=None, options=None,
                                         return_dict=True, key=None):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        record = await conn.query_one_orm_with_session(session, model, filters, joins, options)
        if return_dict:
            return record.as_dict()
        return record

    async def query_all_orm(self, model, filters: Optional[Union[Dict, List]] = None, order_by=None,
        ...
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        records = await conn.query_all_orm(model, filters, order_by, direction, joins, options, limit, offset)
        if return_dict:
            return_list = []
            for re in records:
                return_list.append(re.as_dict())
            return return_list
        return records

    async def query_all_orm_with_session(self, session: AsyncSession, model,
                                         filters: Optional[Union[Dict, List]] = None,
                                         order_by=None,
                                         direction: Optional[str] = 'asc', joins: Optional[List] = None,
                                         options: Optional[List] = None, limit: Optional[int] = None,
                                         offset: Optional[int] = None,
                                         return_dict=True, key=None):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        records = await conn.query_all_orm_with_session(session, model, filters, order_by, direction, joins, options,
                                                        limit, offset)
        if return_dict:
            return_list = []
            for re in records:
