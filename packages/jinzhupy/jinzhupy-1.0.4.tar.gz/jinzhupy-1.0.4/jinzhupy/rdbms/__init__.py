...
class DatabaseManager:
    ...
    async def query_one_orm(
            self, model, filters=None, joins=None, outerjoins=None, options=None, key=None
    ):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.query_one_orm(model, filters, joins, outerjoins, options)

    async def query_one_orm_with_session(
            self, session: AsyncSession, model, filters=None, joins=None, outerjoins=None,
            options=None, key=None
    ):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.query_one_orm_with_session(session, model, filters, joins, outerjoins, options)
    ...
    async def query_all_orm(
        ...
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.query_all_orm(model, filters, order_by, direction, joins, outerjoins, options)
    ...
    async def query_all_orm_with_session(
        ...
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.query_all_orm_with_session(session, model, filters, order_by, direction, joins, outerjoins,
                                                     options)
