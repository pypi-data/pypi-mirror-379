...
def build_markdown_docs(camb_root_dir):
    """Build the documentation in Markdown format."""
    print(f"Building documentation in Markdown format from {camb_root_dir}...")
    build_dir = os.path.join(camb_root_dir, "docs", "_build", "markdown")
    ...
    # Use absolute paths
    conf_source = os.path.join(camb_root_dir, "docs", "source", "conf.py")
    with open(conf_source, encoding="utf-8") as f:
        conf_content = f.read()
    ...
    try:
        ...
        source_dir = os.path.join(camb_root_dir, "docs", "source")
...
def extract_toctree_order(camb_root_dir):
    """Extract the order of files from the index.rst toctree."""
    try:
        index_rst_path = os.path.join(camb_root_dir, "docs", "source", "index.rst")
...
def combine_markdown_files(build_dir, exclude_files, output_file, camb_root_dir):
    ...
    ordered_docs = extract_toctree_order(camb_root_dir)
...
def convert_CAMBdemo_to_markdown(camb_root_dir):
    """Convert CAMBdemo.ipynb to markdown using jupytext."""
    print("Converting CAMBdemo.ipynb to markdown...")

    # Path to the notebook
    notebook_path = os.path.join(camb_root_dir, "docs", "CAMBdemo.ipynb")
...
def main(camb_root_dir=None):
    if camb_root_dir is None:
        args = parse_args()
        camb_root_dir = args.camb_root if hasattr(args, 'camb_root') else os.getcwd()
    
    try:
        # Get the list of files to exclude
        exclude_files = []  # Simplified for contextmaker integration

        # Build the documentation
        build_dir = build_markdown_docs(camb_root_dir)

        # Combine the files
        output_file = os.path.join(camb_root_dir, "docs", "camb_documentation.md")
        if not combine_markdown_files(build_dir, exclude_files, output_file, camb_root_dir):
        ...
        CAMBdemo_md = convert_CAMBdemo_to_markdown(camb_root_dir)
        if CAMBdemo_md:
            print(f"Appending CAMBdemo.md to {output_file}...")
        ...
        print(f"\nSuccess! Documentation has been built and combined into: {output_file}")
        return output_file
