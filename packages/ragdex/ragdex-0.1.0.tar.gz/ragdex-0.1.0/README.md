...
```bash
pdlib-cli --help                    # Show all available commands
pdlib-cli ensure-dirs                # Create necessary directories
pdlib-cli config                     # View current configuration
pdlib-cli check-indexing-status      # Check document indexing status
pdlib-cli find-unindexed             # Find documents not yet indexed
pdlib-cli manage-failed-pdfs         # Manage failed document list
pdlib-cli show-config                # Display configuration details
```
