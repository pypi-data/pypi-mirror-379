...
class GophrBridge:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Store last responses for getter methods
        self._last_quote_response: Optional[Dict[str, Any]] = None
        self._last_shipment_response: Optional[Dict[str, Any]] = None
    ...
    def get_quote(self, quote_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self._last_quote_response = result  # Store for getter methods
            return result
    ...
    def create_shipment(self, shipment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._handle_response(response)
            self._last_shipment_response = result  # Store for getter methods
            return result
        except requests.RequestException as error:
            raise self._handle_error(error)
    ...
    def get_standard_quote_id(self) -> Optional[str]:
        """Extract standard quote ID from last quote response"""
        if not self._last_quote_response:
            return None
        return self._last_quote_response.get('payload', {}).get('standard_quote', {}).get('quote_id')
    
    def get_expedited_quote_id(self) -> Optional[str]:
        """Extract expedited quote ID from last quote response"""
        if not self._last_quote_response:
            return None
        return self._last_quote_response.get('payload', {}).get('expedited_quote', {}).get('quote_id')
    
    def get_standard_quote_fee(self) -> Optional[float]:
        """Extract standard quote fee from last quote response"""
        if not self._last_quote_response:
            return None
        return self._last_quote_response.get('payload', {}).get('standard_quote', {}).get('fee')
    
    def get_expedited_quote_fee(self) -> Optional[float]:
        """Extract expedited quote fee from last quote response"""
        if not self._last_quote_response:
            return None
        return self._last_quote_response.get('payload', {}).get('expedited_quote', {}).get('fee')
    
    # Shipment Response Getter Methods
    def get_delivery_id(self) -> Optional[Union[str, int]]:
        """Extract delivery ID from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('delivery_id')
    
    def get_delivery_status(self) -> Optional[str]:
        """Extract delivery status from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('delivery_status')
    
    def get_shipping_fee(self) -> Optional[float]:
        """Extract shipping fee from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('shipping_fee')
    
    def get_vehicle_type(self) -> Optional[str]:
        """Extract vehicle type from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('vehicle_type')
    
    def get_distance(self) -> Optional[float]:
        """Extract distance from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('distance')
    
    def get_pickup_address(self) -> Optional[Dict[str, Any]]:
        """Extract pickup address from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('pick_up')
    
    def get_dropoff_address(self) -> Optional[Dict[str, Any]]:
        """Extract drop-off address from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('drop_off')
    
    def get_shipment_items(self) -> Optional[List[Dict[str, Any]]]:
        """Extract items array from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('items')
    
    def get_shipment_weight(self) -> Optional[float]:
        """Extract weight from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('weight')
    
    def is_expedited(self) -> Optional[bool]:
        """Check if last shipment is expedited"""
        if not self._last_shipment_response:
            return None
        payload = self._last_shipment_response.get('payload', {})
        return payload.get('is_expedited') if 'is_expedited' in payload else None
    
    def get_scheduled_for(self) -> Optional[str]:
        """Extract scheduled_for date from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload', {}).get('scheduled_for')
    
    # Payload Getter Methods
    def get_quote_payload(self) -> Optional[Dict[str, Any]]:
        """Extract full payload from last quote response"""
        if not self._last_quote_response:
            return None
        return self._last_quote_response.get('payload')
    
    def get_shipment_payload(self) -> Optional[Dict[str, Any]]:
        """Extract full payload from last shipment response"""
        if not self._last_shipment_response:
            return None
        return self._last_shipment_response.get('payload')
    
    # Utility Methods
    def is_successful(self, response: Optional[Dict[str, Any]] = None) -> bool:
        """Check if response is successful. Uses last quote/shipment response if no response provided."""
        if response is None:
            # Use the most recent response
            response = self._last_shipment_response or self._last_quote_response
    ...
    def get_quote_summary(self) -> Dict[str, Any]:
        """Get a formatted summary of last quote response"""
        if not self._last_quote_response or not self.is_successful(self._last_quote_response):
            error_msg = 'No quote response available'
            status_code = None
            if self._last_quote_response:
                error_msg = self._last_quote_response.get('message', 'Unknown error')
                status_code = self._last_quote_response.get('status_code')
        ...
        return {
            'success': True,
            'standard': {
                'quote_id': self.get_standard_quote_id(),
                'fee': self.get_standard_quote_fee()
            },
            'expedited': {
                'quote_id': self.get_expedited_quote_id(),
                'fee': self.get_expedited_quote_fee()
            }
        }
    
    def get_shipment_summary(self) -> Dict[str, Any]:
        """Get a formatted summary of last shipment response"""
        if not self._last_shipment_response or not self.is_successful(self._last_shipment_response):
            error_msg = 'No shipment response available'
            status_code = None
            if self._last_shipment_response:
                error_msg = self._last_shipment_response.get('message', 'Unknown error')
                status_code = self._last_shipment_response.get('status_code')
        ...
        return {
            ...
            'weight': self.get_shipment_weight(),
            'is_expedited': self.is_expedited(),
            'scheduled_for': self.get_scheduled_for()
        }
