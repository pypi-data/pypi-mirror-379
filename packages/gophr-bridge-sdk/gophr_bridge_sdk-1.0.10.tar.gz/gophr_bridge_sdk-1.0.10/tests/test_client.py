...
class TestGophrBridge:
    ...
    def test_get_quote_success(self):
        ...
        # Verify response is stored for getter methods
        assert gophr._last_quote_response == mock_response
        assert gophr.get_standard_quote_id() == 'std_123'
        assert gophr.get_expedited_quote_id() == 'exp_123'
    ...
    def test_create_shipment_success(self):
        ...
        # Verify response is stored for getter methods
        assert gophr._last_shipment_response == mock_response
        assert gophr.get_delivery_id() == 'DEL_123456'
        assert gophr.get_delivery_status() == 'confirmed'
        assert gophr.get_shipping_fee() == 15.50
...
class TestGetterMethods:
    ...
    def test_quote_getters(self):
        """Test quote response getter methods work after get_quote call"""
        # Test that getters return None before any quote request
        assert self.gophr.get_standard_quote_id() is None
        assert self.gophr.get_expedited_quote_id() is None
        
        # Mock successful quote response
        self.gophr._last_quote_response = {
            'payload': {
                'standard_quote': {
                    'quote_id': 'std_123',
                    'fee': 15.50
        ...
        # Test getters work after response is stored
        assert self.gophr.get_standard_quote_id() == 'std_123'
        assert self.gophr.get_standard_quote_fee() == 15.50
        assert self.gophr.get_expedited_quote_id() == 'exp_123'
        assert self.gophr.get_expedited_quote_fee() == 25.00
        assert self.gophr.get_quote_payload()['standard_quote']['quote_id'] == 'std_123'
    
    def test_shipment_getters(self):
        """Test shipment response getter methods work after create_shipment call"""
        # Test that getters return None before any shipment request
        assert self.gophr.get_delivery_id() is None
        assert self.gophr.get_delivery_status() is None
        
        # Mock successful shipment response
        self.gophr._last_shipment_response = {
            'payload': {
                'delivery_id': 'DEL_123456',
                'delivery_status': 'confirmed',
                'shipping_fee': 15.50,
        ...
        # Test getters work after response is stored
        assert self.gophr.get_delivery_id() == 'DEL_123456'
        assert self.gophr.get_delivery_status() == 'confirmed'
        assert self.gophr.get_shipping_fee() == 15.50
        assert self.gophr.get_vehicle_type() == 'car'
        assert self.gophr.get_distance() == 5.2
        assert self.gophr.get_shipment_weight() == 2.5
        assert self.gophr.is_expedited() is False
        assert self.gophr.get_scheduled_for() == '2025-09-25'
        assert self.gophr.get_pickup_address() == {'address': '123 Pickup St'}
        assert self.gophr.get_dropoff_address() == {'address': '456 Dropoff Ave'}
        assert self.gophr.get_shipment_items() == [{'name': 'Test Item'}]
        assert self.gophr.get_shipment_payload()['delivery_id'] == 'DEL_123456'
    
    def test_getters_with_missing_data(self):
        """Test getter methods with no stored responses"""
        # Create fresh instance to ensure no stored responses
        fresh_gophr = GophrBridge({
            'client_id': 'test_client_id',
            'client_secret': 'test_client_secret'
        })
        
        assert fresh_gophr.get_standard_quote_id() is None
        assert fresh_gophr.get_delivery_id() is None
        assert fresh_gophr.get_quote_payload() is None
        assert fresh_gophr.get_shipment_payload() is None
    ...
    def test_get_quote_summary(self):
        ...
        # Test successful quote summary
        self.gophr._last_quote_response = successful_quote
        successful_summary = self.gophr.get_quote_summary()
        ...
        # Test failed quote summary
        self.gophr._last_quote_response = failed_quote
        failed_summary = self.gophr.get_quote_summary()
    ...
    def test_get_shipment_summary(self):
        ...
        # Test successful shipment summary
        self.gophr._last_shipment_response = successful_shipment
        summary = self.gophr.get_shipment_summary()
