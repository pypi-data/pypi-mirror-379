...
def translate_makefile_to_xml_in_project(  # noqa: PLR0912, PLR0914, PLR0915
    ...
    for old_file_paths in old_file_path_groups.values():
        ...
        xml_variables = {
            translated_from_makefile_variables.get(k, k).lower(): v
        ...
        # Modify existing files
            ...
            attribute_duplicated = False
            for attribute, value in attributes.items():
                attribute = attribute.lower()  # noqa: PLW2901
                if attribute in xml_variables:
                    # If value is the same as in the XML, ignore the converted value
                    if value == xml_variables[attribute]:
                        xml_variables.pop(attribute)
                        continue
                    # Value is different, abort file conversion
                    logger.warning(
