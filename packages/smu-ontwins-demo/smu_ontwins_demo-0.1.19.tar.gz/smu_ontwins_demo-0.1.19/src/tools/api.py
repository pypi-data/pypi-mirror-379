...
def send_to(server: str,
    ...
    handled_events = [format_ts_of(e) for e in events]

    s = requests.Session()
    res = []

    def _send_order(event: dict, customer_order: dict):
        body = {
            "bucket": bucket,
    ...
    def _send_stock(event: dict, stock_status: dict | None):
        body = {
            "bucket": bucket,
    ...
    def _send_rack_status(event: dict, rack_status: list[dict]):
    ...
    for event in handled_events:
        consumable = event.get("consumable", None)
        _send_order(event, event.get("customer_order", {}))
        if consumable: _send_stock(event, event.get("stock_status", {}))
        if consumable: _send_rack_status(event, event.get("rack_status", []))
