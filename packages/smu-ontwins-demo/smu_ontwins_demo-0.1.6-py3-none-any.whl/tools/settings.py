...
products_index_label = ["품명", "분류", "가격", "주문당 수량 기대값"]
years_index_label = ["품명","1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"]
...
def prepare_settings(keyword: str,
    ...
    def _prepare_product_setting():
        ...
        if not path.exists():
            # 인덱스 포함 '빈 파일' 생성
            pd.DataFrame(columns=products_index_label).to_csv(path, index=False)
        ...
        try:
            ...
            if _looks_like_expected_columns(df, products_index_label):
    ...
    def _prepare_years_setting():
        for y in years:
            ...
            if not path.exists():
                # 인덱스 포함 '빈 파일' 생성
                pd.DataFrame(columns=years_index_label).to_csv(path, index=False)
                dfs.append(pd.DataFrame())
                msgs.append(f"[알림] 파일이 없어 빈 파일을 만들었습니다: {path}")
                continue
            ...
            try:
                ...
                if df.empty:
                    warn_bits.append("데이터가 없음")
                if _looks_like_expected_columns(df, years_index_label):
                    warn_bits.append("잘못된 형식의 설정 파일")
                if warn_bits:
                    msgs.append(f"[경고] {path} → {', '.join(warn_bits)}")
                dfs.append(df)
