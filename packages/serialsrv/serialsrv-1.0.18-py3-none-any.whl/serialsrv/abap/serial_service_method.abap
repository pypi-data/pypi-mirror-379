...
*& 
*& SerialSrv Response Types:
*& 1. Test Mode Response: {"value": "0", "msg": "hello world", "mode": "test", "result": "OK"}
*& 2. Success Response: {"value": "25.5", "msg": "Temperature reading successful", "mode": "read", "result": "OK"}
*& 3. Error Response: {"value": "-1", "msg": "Serial port error", "mode": "read", "result": "FAIL"}
*&
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS: call_serial_service
      ...
      EXCEPTIONS
        ...
        timeout_error
        parse_error.
    " Calls serialsrv service and returns full response data
    " Supports all 3 response types: Test Mode, Success, Error
    " iv_test_mode = true: Forces test mode response
    ...
    " Raises exception for Error Response (result: "FAIL")
...
CLASS ZAKIR_SERIAL_SERVICE_CLASS IMPLEMENTATION.
  ...
  METHOD call_serial_service.
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(lx_http_error).
        rs_result-success = abap_false.
    ...
    ELSEIF ls_response-message-result = 'OK'.
      ...
      " Test Mode Response: Test verisi
      rs_result-success = abap_true.
      rs_result-value = ls_response-message-value.
      rs_result-message = ls_response-message-msg.
      rs_result-mode = ls_response-message-mode.    " "read" veya "test"
  ...
  METHOD get_serial_value.
    ...
    IF ls_result-result = 'OK' AND ls_result-value IS NOT INITIAL.
      " Success Response veya Test Mode Response
...
*&
*& " 1. Call serial service and get full result (all response types)
*& TRY.
...
*&                / 'Mode:', ls_result-mode,  " "read" or "test"
...
*& ENDTRY.
*&
*& " 2. Test mode - get test data
*& TRY.
*&     ls_result = ZAKIR_SERIAL_SERVICE_CLASS=>call_serial_service(
*&       iv_host = '192.168.1.100'
*&       iv_port = '7373'
*&       iv_test_mode = abap_true  " Force test mode
*&     ).
*&     WRITE: / 'Test Value:', ls_result-value,  " "0"
*&            / 'Test Message:', ls_result-message,  " "hello world"
*&            / 'Test Mode:', ls_result-mode.  " "test"
*&   CATCH ZAKIR_SERIAL_SERVICE_CLASS=>connection_error.
*&     WRITE: / 'Test connection error occurred'.
*& ENDTRY.
*&
*& " 3. Get only the value (validates response type)
*& TRY.
...
*& ENDTRY.
*&
*& " 4. Test connection (validates Test Mode Response)
*& TRY.
