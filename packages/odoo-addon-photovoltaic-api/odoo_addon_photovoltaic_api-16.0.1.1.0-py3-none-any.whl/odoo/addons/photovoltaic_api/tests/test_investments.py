...
def id_by_name(model, name):
    return model.with_context(lang='es_ES').name_search(name)[0][0]
...
class TestInvestment(CommonCase):
    ...
    def test_investment_individual(self):
        ...
        self.assertRecordValues(partner, [{
            ...
            'city':                 form_data['city'],
            'state_id':             id_by_name(self.env['res.country.state'], form_data['state']),
            'zip':                  form_data['zip'],
            'country_id':           id_by_name(self.env['res.country'], form_data['country']),
            ...
            'participant':          True,
            'person_type':          id_by_name(self.env['res.partner.type'], 'Persona física'),
            'company_type':         'person'
        }])
