...
class InvestmentService(Component):
    ...
    def create(self, form_data):
        ...
        if form_data.type == 'minor' and form_data.vat == form_data.vat2:
        ...
        if form_data.promotional_code:
            if self._is_new_participant(form_data.vat):
                ...
                return {
                    'code': 400,
                    'name': 'Bad Request',
                    'description': 'Código promocional sólo válido para nuevos participantes'
                }
    ...
    def _create_or_update_partner(self, form_data, partner):
        ...
        if not partner:
            partner = self.env['res.partner'].create(write_data)
        else:
            partner.write(write_data)
    ...
    def _is_new_participant(self, vat):
        # either no partner exists or if they exist and have contracts,
        # they are not in state Active or Payment pending
        ...
        if partner:
            active_contracts_count = self.env['contract.participation'].search_count([
                ...
                '|',
                    ...
                    ('stage_id', '=', self.env['contract.participation.stage'].search([('name', '=', 'Payment pending')]).id)
            ])
    ...
    @restapi.method(
        [(['/check_code'], 'GET')],
        input_param=restapi.CerberusValidator('_validator_check_code'),
        auth='api_key'
    )
    def check_code(self, code, vat):
        if not self._partner_from_code(code):
            return {
                'valid': False,
                'message': 'Código promocional inválido'
            }

        if not self._is_new_participant(vat):
            return {
                'valid': False,
                'message': 'Código promocional sólo válido para nuevos participantes'
            }
    ...
    def _validator_check_code(self):
        return {
            'code': {'type': 'string'},
            'vat': {'type': 'string'}
        }
