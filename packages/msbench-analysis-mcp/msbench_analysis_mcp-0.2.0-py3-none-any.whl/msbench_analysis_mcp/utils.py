...
COMPARISON_TOOL_COLUMNS = ['Tool Name', 'Usage Count Zip1', 'Usage Count Zip2', 'Usage Difference']

def generate_report_filename(zip_path: 'Path', report_type: str) -> str:
    """
    Generate a unique report filename based on the zip file name and report type.
    
    Args:
        zip_path: Path to the original zip file
        report_type: Either 'cls' or 'trajectory'
    
    Returns:
        String filename for the report
    """
    zip_stem = zip_path.stem  # Get filename without extension
    return f"msbench_{report_type}_analysis_report_{zip_stem}.xlsx"
