...
from .utils import (
    TRAJECTORY_COLUMNS, TRAJECTORY_OUTPUT_REPORT, TRAJECTORY_TOOL_CALL_ERROR_PREFIX, 
...
def process_trajectory_file(trajectory_file: Path) -> Dict[str, any]:
    ...
    Iterates across all step snapshots and, for each snapshot, only counts the
    most recently added assistant+tool pair. Pairs are deduplicated primarily by
    `tool_call_id` to avoid double-counting across snapshots or when earlier
    messages are truncated. Also aggregates tool usage and errors.
    """
    try:
    ...
    # Walk all snapshots; per snapshot, count only the most recent tool pair
    for snap_index, snapshot in enumerate(data):
        if not isinstance(snapshot, dict) or 'messages' not in snapshot:
            continue
        messages = snapshot['messages']
        if not isinstance(messages, list) or len(messages) == 0:
            continue

        # Find the last tool message in this snapshot
        last_tool_idx = -1
        ...
        # Use tool_call_id primarily; fallback to a stable surrogate if missing
        fallback_id = f"{snap_index}:{last_tool_idx}:{hash(str(tool_msg.get('content')))}"
        tool_call_id = tool_msg.get('tool_call_id') or fallback_id
        if tool_call_id in seen_tool_call_ids:
            continue

        # Mark as seen and locate the nearest preceding assistant with tool_calls
        seen_tool_call_ids.add(tool_call_id)

        tool_name = 'unknown'
        for idx in range(last_tool_idx - 1, -1, -1):
            am = messages[idx]
        ...
        # Count this pair as a unique step and tool call
        step_counter += 1
        total_tool_calls += 1
        ...
        if isinstance(tool_content, str) and TRAJECTORY_TOOL_CALL_ERROR_PREFIX in tool_content:
            ...
            err = clean_error_message(tool_content[start + len(TRAJECTORY_TOOL_CALL_ERROR_PREFIX):].strip())
            tool_call_errors.append({
                "failed_step": step_counter,
                "failed_tool_call_id": clean_error_message(str(tool_call_id)),
                "failed_tool_name": clean_error_message(str(tool_name)),
                "error_message": err
            })
...
def generate_trajectory_error_report(zip_path: Path, need_write: bool = False) -> dict:
    ...
    if need_write and rows:  # Only write if there are rows to write
        report_filename = generate_report_filename(zip_path, "trajectory")
