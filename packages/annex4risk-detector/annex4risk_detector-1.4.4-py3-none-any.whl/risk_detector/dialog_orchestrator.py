    ...
    try:
        with Session() as db:
            ...
            def _compute_obligations_dynamic(outcome: dict) -> list[str]:
                ...
                Returns compact 'code: label [anchor]' strings for UI; keep data-driven only.
                """
                ctx = dict(answers)
                ...
                items = []
                try:
                    rows = db.execute(sql_text("SELECT name, trigger_dsl, legal_bases, actions FROM obligation_rules")).fetchall()
                    for name, trig_json, bases_json, acts_json in rows:
                        try:
                        ...
                        if evaluate_rule(trig, ctx):
                            ...
                            for a in acts:
                                ...
                                if code and label:
                                    items.append(f"{code}: {label}" + (f" [{anchor}]" if anchor else ""))
                except Exception:
                    pass
