...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.abion: AbionClient | None = None
    ...
    def _setup_credentials(self) -> None:
        self.credentials = self._configure_credentials(
            ...
            {
                "api_key": "API key for Abion",
                "api_url": "Base URL for Abion API (default: https://api.abion.com/)",
            },
        )
        self.abion = AbionClient(
            api_key=self.credentials.conf("api_key"),
            api_url=self.credentials.conf("api_url") or "https://api.abion.com/",
        )

    def _perform(self, domain: str, validation_name: str, validation: str) -> None:
        try:
            logger.debug("Adding TXT record %s = %s", validation_name, validation)
            assert self.abion is not None
