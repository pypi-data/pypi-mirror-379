...
class OAICompatLargeLanguageModel(_CommonOaiApiCompat, LargeLanguageModel):
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            # Use higher max_tokens for openai/gpt-5 model due to Azure API minimum requirement
            max_tokens = 16 if model == "openai/gpt-5" else 5
            data = {"model": credentials.get("endpoint_model_name", model), "max_tokens": max_tokens}
            ...
            logger.info(curl.parse(response))
            if response.status_code != 200:
                raise CredentialsValidateFailedError(
                    f"Credentials validation failed with status code {response.status_code} "
                    f"and response body {response.text}"
