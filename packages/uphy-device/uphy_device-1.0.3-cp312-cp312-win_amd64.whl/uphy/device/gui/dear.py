...
from uphy.device.api import up_dtype_t, up_core_status_t
...
datatype_to_min_max = {
    up_dtype_t.UP_DTYPE_INT8: (-(2**7), 2**7 - 1),
    up_dtype_t.UP_DTYPE_INT16: (-(2**15), 2**15 - 1),
    up_dtype_t.UP_DTYPE_INT32: (-(2**31), 2**31 - 1),
    up_dtype_t.UP_DTYPE_UINT8: (0, 2**8 - 1),
    up_dtype_t.UP_DTYPE_UINT16: (0, 2**16 - 1),
    up_dtype_t.UP_DTYPE_UINT32: (0, 2**31 - 1), # Incorrect max but larger values do not work with dpg
    up_dtype_t.UP_DTYPE_REAL32: (-math.inf, math.inf),
}
...
def gui(device: api.Device):
    ...
    def _render(*, status=""):
        ...
        status = int(status)
        dpg.set_value("is_running", (status & up_core_status_t.UP_CORE_RUNNING) != 0)
        dpg.set_value("is_configured", (status & up_core_status_t.UP_CORE_CONFIGURED) != 0)
        dpg.set_value("is_connected", (status & up_core_status_t.UP_CORE_CONNECTED) != 0)
        for plot in plots:
            plot.update()
