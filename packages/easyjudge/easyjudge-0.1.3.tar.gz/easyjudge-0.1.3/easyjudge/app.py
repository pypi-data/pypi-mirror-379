...
PKG_PARTICIPANTS_FILE = os.path.join(ROOT_DIR, 'participants.json')
...
def get_participants():
    if not os.path.exists(PARTICIPANTS_FILE):
        # 优先用包内默认 participants.json 初始化
        default_data = {'admin': 'xedupro'}
        try:
            if os.path.exists(PKG_PARTICIPANTS_FILE):
                with open(PKG_PARTICIPANTS_FILE, 'r', encoding='utf-8') as pf:
                    default_data = json.load(pf)
        except Exception:
            pass
...
def admin_get_leaderboard():
    try:
        df = pd.read_csv(leaderboard_file) if os.path.exists(leaderboard_file) else pd.DataFrame(columns=['username','phone','rmse','timestamp'])
        if not df.empty:
            df = df.sort_values('rmse', ascending=True)
        df = df.fillna({'phone': '', 'rmse': 999.999, 'username': '未知用户', 'timestamp': '未知时间'})
        if 'rmse' in df.columns:
            df['rmse'] = pd.to_numeric(df['rmse'], errors='coerce').fillna(999.999)
...
def admin_clear_leaderboard():
    try:
        # 重建空的排行榜CSV
        pd.DataFrame(columns=['username', 'phone', 'rmse', 'timestamp']).to_csv(leaderboard_file, index=False)
        return jsonify({'success': True, 'message': '排行榜已清空'})
    except Exception as e:
