...
MARK_LIST = [item for sublist in settings.MARK_LIST.values() for item in sublist]
...
def pytest_generate_tests(metafunc):
    ...
    if not set(m.lower() for m in marks).issubset(m.lower() for m in MARK_LIST):
        logger.warning(f"{func_name} 使用非规范的用例标签:{marks}")
    ...
    if scenarios:
        ...
        for index, item in enumerate(scenarios):
            ...
            if level not in settings.CASE_LEVEL_LIST:
                logger.warning(f"{func_name} 使用非规范的用例级别 {level}")
                if settings.STRICT_MARKS:
                    continue
...
def pytest_runtest_call(item):
    ...
    if not ignore_failed:
        ...
        prev_match = pattern.match(item.nodeid)
