on run argv
    ...
    if (count of argv) > 1 then
        set limitArg to item 2 of argv
    ...
    tell application "Mail"
        ...
        repeat with acc in targetAccounts
            ...
            repeat with mbx in targetMailboxes
                ...
                set candidateMessages to messages of mbx

                repeat with msg in candidateMessages
                    if my message_matches(msg, searchTerm) then
...
on message_matches(msg, queryText)
    if queryText is "" then return false
    ...
    tell application "Mail"
        ...
        set contentText to ""
        try
            set contentText to (content of msg as text)
        on error
            set contentText to ""
        end try
    end tell
    ...
    ignoring case
        if subjectText contains queryText then return true
        if senderText contains queryText then return true
        if contentText contains queryText then return true
    end ignoring
