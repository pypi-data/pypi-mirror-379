    ...
    def get_fields(self):
        ...
        for model_field in self.Meta.model._meta.get_fields():
            ...
            if fk_name in fields:
                ...
                # Create a new field instance with required=False
                if hasattr(original_field, '__class__'):
                    # Copy the field but make it not required
                    field_kwargs = {
                        'required': False,
                        'allow_null': getattr(original_field, 'allow_null', model_field.null),
                    }
                    # Copy other relevant attributes
                    for attr in ['read_only', 'write_only', 'default', 'help_text', 'label']:
                        if hasattr(original_field, attr):
                            field_kwargs[attr] = getattr(original_field, attr)
                    
                    fields[fk_name] = original_field.__class__(**field_kwargs)
