    ...
    def to_internal_value(self, data):
        ...
        for model_field in model._meta.get_fields():
            ...
            if fk_name in internal_data and fk_attname not in internal_data:
                ...
                logger.info(f"BulkModelSerializer - Converted '{fk_name}': {internal_data[fk_attname]} -> '{fk_attname}' (kept original for validation)")

        logger.info(f"BulkModelSerializer - Final internal_data: {internal_data}")
        
        # Debug: Check field required status right before validation
        current_fields = self.get_fields()
        for fk_name in ['payment_schedule', 'financial_account']:
