        ...
        for batch_start in range(0, len(data_list), batch_size):
            ...
            for index, item_data in enumerate(batch_data):
                try:
                    ...
                    logger.info(f"BulkViewSet - validated_data before model creation: {validated_data}")
                    # Create instance directly from validated data
                    new_instance = model_class(**validated_data)
                    logger.info(f"BulkViewSet - Created model instance: business_id={getattr(new_instance, 'business_id', 'NOT_SET')}")
