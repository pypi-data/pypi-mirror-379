...
from labels.model.ecosystem_data.alpine import ApkDBEntry, ApkFileRecord
...
def _build_lookup_table(pkgs: list[Package]) -> dict[str, list[Package]]:
    ...
    for pkg in pkgs:
        ...
        apkg: ApkDBEntry = cast("ApkDBEntry", pkg.ecosystem_data)
...
def discover_package_dependencies(
    ...
    for pkg in pkgs:
        if pkg.ecosystem_data is None:
            continue
        apkg = cast("ApkDBEntry", pkg.ecosystem_data)
