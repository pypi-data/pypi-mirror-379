...
class ArchiveParser(BaseModel):
    ...
    def parse(self) -> tuple[list[Package] | None, list[Relationship] | None]:
        ...
        if self.detect_nested:
            nested_pkgs, nested_relationships = self.discover_pkgs_from_nested_archives(parent_pkg)
    ...
    def discover_pkgs_from_nested_archives(
        ...
        for archive_path in nested_archives:
            try:
                ...
                nested_pkgs, nested_rels = self._process_nested_archive(archive_path, parent_pkg)
                packages.extend(nested_pkgs)
    ...
    def _process_nested_archive(
        self, archive_path: str, parent_pkg: Package | None
    ) -> tuple[list[Package], list[Relationship]]:
        if not self.archive_path:
            return [], []
        ...
        try:
            ...
            try:
                ...
                if nested_pkgs and parent_pkg:
                    for pkg in nested_pkgs:
                        if hasattr(pkg, "metadata") and hasattr(pkg.metadata, "parent"):
                            pkg.metadata.parent = parent_pkg
    ...
    def discover_main_package(self) -> Package | None:
        ...
        try:
            authoritative_group_id = group_id_from_java_metadata(name, metadata) or group_id
            return Package(
...
def new_package_from_maven_data(
    ...
    authoritative_group_id = (
        group_id_from_java_metadata(artifact_id, metadata) or pom_properties.group_id
    )
