...
class PureChart:
    ...
    def add_grid(self, alpha: float = 0.3):
        """Add grid to chart."""
        # Determine data bounds if we have data
    ...
    def add_legend(self):
        """Add legend."""
        if hasattr(self, 'lines') and self.lines:
            legend_x = self.width - 120  # 120 pixels from right
            legend_y = 50  # Start 50 pixels from top

            for i, line in enumerate(self.lines):
                if line.get('label'):
                    # Draw legend line sample
                    line_start_x = legend_x
                    line_start_y = legend_y + i * 20
                    line_end_x = legend_x + 20
                    line_end_y = line_start_y

                    # Draw small line sample
                    self.renderer.canvas.set_stroke_color(line['color'])
                    self.renderer.canvas.draw_line(line_start_x, line_start_y, line_end_x, line_end_y)

                    # Draw label text
                    text_x = legend_x + 25
                    text_y = line_start_y + 5
                    self.renderer.canvas.draw_text(line['label'], text_x, text_y, font_size=10, color='black')
