...
class PureChart:
    ...
    def set_title(self, title: str):
        """Set chart title."""
        self.title = title
        # Draw title at top center of chart
        title_x = self.width / 2
        title_y = 30  # 30 pixels from top
        self.renderer.canvas.draw_text(title, title_x, title_y, font_size=16, color='black')
        return self
    ...
    def set_labels(self, xlabel: str = "", ylabel: str = ""):
        ...
        if xlabel:
            ...
            self.renderer.canvas.draw_text(xlabel, label_x, label_y, font_size=12, color='black')

        if ylabel:
            # Y-axis label at left center (rotated would be better, but simplified for now)
            label_x = 20  # 20 pixels from left
            label_y = self.height / 2
            self.renderer.canvas.draw_text(ylabel, label_x, label_y, font_size=12, color='black')
    ...
    def add_legend(self, position: str = 'upper_right'):
        ...
        for i, series in enumerate(labeled_series):
            ...
            text_x = legend_x + 28
            text_y = item_y + 4
            label_text = f'<text x="{text_x}" y="{text_y}" font-size="12" fill="black" font-family="Arial, sans-serif">{series["label"]}</text>'
            self.renderer.canvas.svg_elements.append(label_text)
