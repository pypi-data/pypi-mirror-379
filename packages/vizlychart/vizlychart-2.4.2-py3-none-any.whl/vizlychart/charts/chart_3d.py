...
class Scatter3D(Chart3D):
    ...
    def render(self):
        ...
        for data in self.scatter_data:
            ...
            # Project points and collect for depth sorting
            for i in range(len(x)):
                screen_x, screen_y, depth = self._project_3d_simple(x[i], y[i], z[i])
                ...
                all_points_with_depth.append((depth, screen_x, screen_y, color, sizes[i], alpha))

        # Sort by depth (back to front for proper layering)
        all_points_with_depth.sort(key=lambda p: p[0], reverse=True)
        ...
        # X-axis (red)
        svg_content += f'<line x1="{center_x - 120}" y1="{center_y}" x2="{center_x + 120}" y2="{center_y}" stroke="rgb(200,50,50)" stroke-width="2"/>\n'
        svg_content += f'<text x="{center_x + 130}" y="{center_y + 5}" font-size="12" fill="rgb(150,50,50)">X</text>\n'

        # Y-axis (green)
        svg_content += f'<line x1="{center_x}" y1="{center_y - 120}" x2="{center_x}" y2="{center_y + 120}" stroke="rgb(50,200,50)" stroke-width="2"/>\n'
        svg_content += f'<text x="{center_x + 10}" y="{center_y - 130}" font-size="12" fill="rgb(50,150,50)">Y</text>\n'

        # Z-axis (blue, diagonal for perspective)
        svg_content += f'<line x1="{center_x}" y1="{center_y}" x2="{center_x - 85}" y2="{center_y - 85}" stroke="rgb(50,50,200)" stroke-width="2"/>\n'
        svg_content += f'<text x="{center_x - 105}" y="{center_y - 95}" font-size="12" fill="rgb(50,50,150)">Z</text>\n'

        svg_content += f'</g>\n'

        # Draw scatter points
        svg_content += f'<g id="3d-scatter-points">\n'
        for depth, screen_x, screen_y, color, size, alpha in all_points_with_depth:
    ...
    def _project_3d_simple(self, x: float, y: float, z: float) -> Tuple[float, float, float]:
        """Simplified 3D projection for scatter plot."""
        ...
        if hasattr(self, 'x_min'):
            ...
            z_norm = (z - self.z_min) / (self.z_max - self.z_min) if self.z_max != self.z_min else 0.5
        else:
            x_norm = y_norm = z_norm = 0.5
        ...
        # Apply simple rotation and perspective
        angle_x, angle_y = 0.3, 0.5  # Fixed viewing angles

        # Rotate around Y axis
        x_rot = x_norm * math.cos(angle_y) - z_norm * math.sin(angle_y)
        z_rot = x_norm * math.sin(angle_y) + z_norm * math.cos(angle_y)

        # Rotate around X axis
        y_rot = y_norm * math.cos(angle_x) - z_rot * math.sin(angle_x)
        z_final = y_norm * math.sin(angle_x) + z_rot * math.cos(angle_x)

        # Project to 2D screen coordinates
        screen_x = center_x + (x_rot - 0.5) * scale * 2
        screen_y = center_y - (y_rot - 0.5) * scale * 2  # Flip Y
