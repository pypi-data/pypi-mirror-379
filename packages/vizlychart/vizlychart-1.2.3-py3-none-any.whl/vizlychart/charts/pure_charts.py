...
class PureChart:
    ...
    def set_title(self, title: str):
        """Set chart title."""
        self.title = title
        # Draw title at top center of chart using direct pixel coordinates
        title_x = self.width / 2
        title_y = 30  # 30 pixels from top
        self.renderer.canvas.draw_text_direct(title, title_x, title_y, font_size=16, color='black', text_anchor='middle')
        return self
    ...
    def _get_data_bounds(self):
        """Get data bounds from all chart data."""
        if hasattr(self, 'lines') and self.lines:
            x_min = min(min(line['x']) for line in self.lines)
            x_max = max(max(line['x']) for line in self.lines)
            y_min = min(min(line['y']) for line in self.lines)
            y_max = max(max(line['y']) for line in self.lines)
        elif hasattr(self, 'points') and self.points:
            x_min = min(min(point['x']) for point in self.points)
            x_max = max(max(point['x']) for point in self.points)
            y_min = min(min(point['y']) for point in self.points)
            y_max = max(max(point['y']) for point in self.points)
        elif hasattr(self, 'bars') and self.bars:
            x_min = min(min(bar['x']) for bar in self.bars)
            x_max = max(max(bar['x']) for bar in self.bars)
            y_min = 0  # Bar charts typically start from 0
            y_max = max(max(bar['height']) for bar in self.bars)
        else:
            # Default bounds
            x_min, x_max = -1, 1
            y_min, y_max = -1, 1
    ...
    def add_axes(self):
        """Add axes with proper scaling and tick marks."""
        x_min, x_max, y_min, y_max = self._get_data_bounds()
        self.renderer.canvas.draw_axes(x_min, x_max, y_min, y_max)
        return self
    ...
    def add_grid(self, alpha: float = 0.3):
        ...
        # Draw grid
        self.renderer.canvas.draw_grid(x_min, x_max, y_min, y_max, alpha=alpha)
        return self

    def set_labels(self, xlabel: str = "", ylabel: str = ""):
        """Set axis labels."""
        if xlabel:
            # X-axis label at bottom center using direct pixel coordinates
            label_x = self.width / 2
            label_y = self.height - 20  # 20 pixels from bottom
            self.renderer.canvas.draw_text_direct(xlabel, label_x, label_y, font_size=12, color='black', text_anchor='middle')

        if ylabel:
            # Y-axis label at left center using direct pixel coordinates
            label_x = 20  # 20 pixels from left
            label_y = self.height / 2
            self.renderer.canvas.draw_text_direct(ylabel, label_x, label_y, font_size=12, color='black', text_anchor='start')

        return self

    def add_legend(self, position: str = 'upper_right'):
        """Add legend with better positioning and styling."""
        # Get all data series with labels
        labeled_series = []
        ...
        legend_height = len(labeled_series) * 25 + 20  # 25px per item + padding
        legend_width = 140  # Fixed width

        # Position legend based on preference
        ...
        else:  # default to upper_right
            legend_x = self.width - legend_width - 20
            legend_y = 60

        # Draw legend background
        bg_rect = f'<rect x="{legend_x - 10}" y="{legend_y - 10}" width="{legend_width}" height="{legend_height}" fill="white" stroke="black" stroke-width="1" opacity="0.9"/>'
        self.renderer.canvas.svg_elements.append(bg_rect)

        # Draw legend items
        for i, series in enumerate(labeled_series):
            item_y = legend_y + i * 25

            if series['type'] == 'line':
                # Draw line sample
                line_sample = f'<line x1="{legend_x}" y1="{item_y}" x2="{legend_x + 20}" y2="{item_y}" stroke="{series['color']}" stroke-width="2"/>'
                self.renderer.canvas.svg_elements.append(line_sample)
            elif series['type'] == 'point':
                # Draw circle sample
                circle_sample = f'<circle cx="{legend_x + 10}" cy="{item_y}" r="4" fill="{series['color']}" stroke="{series['color']}" stroke-width="1"/>'
                self.renderer.canvas.svg_elements.append(circle_sample)
            elif series['type'] == 'bar':
                # Draw rectangle sample
                rect_sample = f'<rect x="{legend_x}" y="{item_y - 6}" width="20" height="12" fill="{series['color']}" stroke="{series['color']}" stroke-width="1"/>'
                self.renderer.canvas.svg_elements.append(rect_sample)

            # Draw label text using direct coordinates
            text_x = legend_x + 28
            text_y = item_y + 4
            self.renderer.canvas.draw_text_direct(series["label"], text_x, text_y, font_size=12, color='black', text_anchor='start')
