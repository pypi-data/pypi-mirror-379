...
class PureCanvas:
    ...
    def draw_text(self, text: str, x: float, y: float, font_size: int = 12, color: str = 'black'):
        """Draw text at specified position (applies coordinate transformation for data space)."""
        # Convert coordinates using transformation
        tx, ty = self._transform_point(x, y)
    ...
    def draw_text_direct(self, text: str, x: float, y: float, font_size: int = 12, color: str = 'black', text_anchor: str = 'start'):
        """Draw text at direct pixel coordinates (no coordinate transformation for UI elements)."""
        ...
        svg_text = f'<text x="{tx}" y="{ty}" font-size="{font_size}" fill="{svg_color}" font-family="Arial, sans-serif"{anchor_attr}>{text}</text>'
        self.svg_elements.append(svg_text)
    ...
    def draw_axes(self, x_min: float, x_max: float, y_min: float, y_max: float):
        ...
        for i in range(num_x_ticks):
            ...
            # Tick label using direct coordinate method
            label_text = f"{x_val:.1f}"
            self.draw_text_direct(label_text, x_pixel, chart_bottom + 20, font_size=12, color='black', text_anchor='middle')
        ...
        for i in range(num_y_ticks):
            ...
            self.draw_text_direct(label_text, chart_left - 10, y_pixel + 4, font_size=12, color='black', text_anchor='end')
