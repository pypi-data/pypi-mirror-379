...
from epyr.lineshapes import (
    Lineshape, gaussian, lorentzian, voigtian, pseudo_voigt, lshape, convspec,
    create_gaussian, create_lorentzian, create_voigt, create_pseudo_voigt
)
...
class TestGaussian:
    ...
    def test_gaussian_basic(self):
        ...
        # Check peak position
        assert np.argmax(y) == 500  # Center at x=0
...
class TestNumericalAccuracy:
    ...
    def test_symmetry_properties(self):
        ...
        for func in [gaussian, lorentzian]:
            ...
            assert np.allclose(y, y[::-1], atol=1e-10), f"{func.__name__} absorption not symmetric"
        
        # Dispersion modes should be antisymmetric
        for func in [gaussian, lorentzian]:
            y = func(x, 0, 4, phase=np.pi/2)
            assert np.allclose(y, -y[::-1], atol=1e-10), f"{func.__name__} dispersion not antisymmetric"
