...
def _compute_gaussian_components(k, sigma, derivative):
    ...
    if derivative == -1:
        ...
        prefactor = 1 / (sigma * np.sqrt(2*np.pi)) * (-1/sigma)**derivative * 2**(-derivative/2)
        ...
        dawson_prefactor = 2 / (sigma * np.sqrt(np.pi)) * (-1/sigma)**derivative * 2**(-derivative/2)
...
def _dawson_derivative(x, n):
    ...
    if n == 0:
        ...
        return -2*n*prev_deriv + 2*x*_dawson_derivative(x, n-1) if n > 0 else F
...
def demo():
    ...
    try:
        ...
        ax.plot(x, lorentz, color=colors[1], linewidth=2.5, label='Lorentzian', linestyle='--')
    except ImportError:
