...
def create_region_mask_1d(x: np.ndarray, regions: List[Tuple[float, float]], 
...
def create_region_mask_2d(X: np.ndarray, Y: np.ndarray, 
...
def create_center_exclusion_mask_1d(x: np.ndarray, center_fraction: float = 0.3) -> np.ndarray:
    """
...
def create_center_exclusion_mask_2d(X: np.ndarray, Y: np.ndarray, 
                                   center_fraction: float = 0.3) -> np.ndarray:
    """
    ...
    x_mask = (X < (x_center - x_exclude_half_width)) | (X > (x_center + x_exclude_half_width))
    y_mask = (Y < (y_center - y_exclude_half_width)) | (Y > (y_center + y_exclude_half_width))
...
def create_edge_exclusion_mask_1d(x: np.ndarray, exclude_initial: int = 0, 
                                 exclude_final: int = 0) -> np.ndarray:
    """
...
def get_baseline_regions_1d(x: np.ndarray, y: np.ndarray, 
                           ...
                           region_mode: str = 'exclude') -> np.ndarray:
    """
...
def get_baseline_regions_2d(X: np.ndarray, Y: np.ndarray, Z: np.ndarray,
                           exclude_center: bool = True,
                           center_fraction: float = 0.3,
                           manual_regions: Optional[List[Tuple[Tuple[float, float], 
...
def validate_regions_1d(regions: List[Tuple[float, float]], x_min: float, x_max: float) -> bool:
    """
    ...
    for i, (x1, x2) in enumerate(regions):
        if not (x_min <= x1 <= x_max and x_min <= x2 <= x_max):
            raise ValueError(f"Region {i} ({x1}, {x2}) is outside data bounds ({x_min}, {x_max})")
        if x1 == x2:
...
def validate_regions_2d(regions: List[Tuple[Tuple[float, float], Tuple[float, float]]],
    ...
    for i, ((x1, x2), (y1, y2)) in enumerate(regions):
        if not (x_min <= x1 <= x_max and x_min <= x2 <= x_max):
            raise ValueError(f"Region {i} X bounds ({x1}, {x2}) outside data bounds ({x_min}, {x_max})")
        if not (y_min <= y1 <= y_max and y_min <= y2 <= y_max):
            raise ValueError(f"Region {i} Y bounds ({y1}, {y2}) outside data bounds ({y_min}, {y_max})")
        if x1 == x2 or y1 == y2:
