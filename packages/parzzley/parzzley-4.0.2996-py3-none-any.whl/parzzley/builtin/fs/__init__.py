...
class ShellBasedBackend(parzzley.fs.Backend, abc.ABC):
    ...
    class _SiteBackend(parzzley.fs.Backend.SiteBackend):
        ...
        async def cookie(self, item, stream_name):
            path = self.__path(item)
            with self.__shell_pool.shell() as shell:
                cmd_result = (await shell.exec(("/bin/stat", "-c", "%A\n%y", path),
                                               fail_on_error=False)).out.decode().split("\n")
            ...
            if stream_name == "":
                item_type = {"-": parzzley.fs.ItemType.FILE,
                             "d": parzzley.fs.ItemType.DIRECTORY,
                             "l": parzzley.fs.ItemType.SYMLINK}.get(info_str[0], parzzley.fs.ItemType.ALIEN)
        ...
        async def child_names(self, item):
            ...
            result = []
            with self.__shell_pool.shell() as shell:
                for line in (await shell.exec(("/bin/ls", "-A", "--quoting-style=c", "-1",
                                               path))).out.decode().split("\n"):
                    if line:
            ...
            if stream_name == "":
                if self.item_type_by_cookie(await self.cookie(item, "")) == parzzley.fs.ItemType.SYMLINK:
                    return parzzley.fs.stream.MemoryWriteStreamable(
                        functools.partial(self.__set_symlink_target, path))
        ...
        async def wait_for_changes(self, on_changed):
            with self.__shell_pool.shell() as shell:
                if (await shell.exec('inotifywait -h >/dev/null; [ "$?" = "1" ]', fail_on_error=False)).exit_code != 0:
                    _logger.info("No change monitoring possible for %s (because 'inotifywait' is not available on that"
                                 " system)", self)
                    while True:
        ...
        async def __set_posix(self, path, content):
            with self.__shell_pool.shell() as shell:
                await shell.exec(("/bin/touch", "-c", "-h", "-m", "-d",
                                  f"@{json.loads(content)["mtime"] / 1000 ** 3:f}", path))

        async def __get_xattrs(self, path) -> dict[bytes, bytes]:
            xattrs = {}
            with self.__shell_pool.shell() as shell:
                for line in (await shell.exec(("/usr/bin/getfattr",
                                               "-h", "-e", "base64", "-d", path))).out.decode().split("\n"):
                    line = line.strip()
        ...
        async def __set_xattrs(self, path, content):
            ...
            for key in {*xattrs, *await self.__get_xattrs(path)}:
                if key.startswith(b"user."):
                    val = xattrs.get(key)
                    setfattr_args = (("-x", key.decode()) if val is None
                    ...
                    with self.__shell_pool.shell() as shell:
                        ...
                        while True:
                            ...
                            if not block:
                                break
                            i_block += 1
                            await shell.exec(b"/usr/bin/dd bs=" + str(len(block)).encode() + b" count=1 iflag=fullblock"
                                             b" >> " + _shell_quote(self.__path), input=block)
            ...
            class _ReadStream(parzzley.fs.stream.ReadStream):
                ...
                async def read(self, max_len):
                    if self.__finished:
                        return None
                    with self.__shell_pool.shell() as shell:
                        content_chunk = (await shell.exec(
                            b"/usr/bin/dd if="+_shell_quote(self.__path)+b" skip="+str(self.__position).encode()+b"B "
    ...
    class Shell(abc.ABC):
        ...
        def __enter__(self) -> None:
            ...
            try:
                self.__process = subprocess.Popen(
                    self._shell_cmdline(), start_new_session=True,
        ...
        async def exec(self, cmd_line: str|bytes|t.Iterable[str|bytes], *, fail_on_error: bool = True,
                       input: bytes = b"") -> ExecutionResult:  # pylint: disable=redefined-builtin
            """
            ...
            if isinstance(cmd_line, str):
                ...
                cmd_line_str = b" ".join(_shell_quote(_) for _
                                         in (_.encode() if isinstance(_, str) else _ for _ in cmd_line))

            cmd_line_str = (b"/bin/echo begin; { (" + cmd_line_str + b") 2>/dev/null ; /bin/echo -n '\n'$?; }"
