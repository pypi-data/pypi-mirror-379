...
class DirectRemove(parzzley.sync.aspect.Aspect):
    ...
    @parzzley.sync.aspect.event_handler(parzzley.sync.aspect.only_if.ItemExistsHere(),
                                        parzzley.sync.aspect.only_if.ItemHereIsType(
                                            no=[parzzley.fs.ItemType.DIRECTORY]),
    ...
    async def detect_changes(self, event: parzzley.sync.aspect.events.item.stream.DetectChanges):
        ...
        if event.item_type_after_last_sync() != parzzley.fs.ItemType.NONE:
            ...
            for site in event.all_sites:
                ...
                if (event.item_type_after_last_sync(site) == parzzley.fs.ItemType.NONE
                        or event.stream_cookie_after_last_sync(site) != event.stream_cookie(site)):
                    changed_on_other_sites = True
                    break
