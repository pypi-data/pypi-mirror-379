...
class AttachSites(parzzley.sync.aspect.Aspect):
    ...
    async def __check_whether_to_stay_attached(self, event: parzzley.sync.aspect.events.sync_run.Prepare) -> bool:
        ...
        if ((await control_site.read_bytes(self._MANAGED_BY_FILE)).decode().strip()
                != f"{event.sync_run.manager_id}/{event.sync_run.volume_name}"):
            return False

        if (site_last_sync_run_sn := await self.__read_last_sync_run_sn(
                control_site, (self._EARLY_LAST_SYNC_RUN_SN_FILE, self._LAST_SYNC_RUN_SN_FILE))) is None:
            return False

        if (last_sync_run_sn := await self.__read_last_sync_run_sn(
                await event.sync_run.manager_control_site(AttachSites._EXCEPTED_LAST_SYNC_RUN_SN_PER_SITE_KEY),
                (early_site_file, site_file))) is None:
            return False

        return site_last_sync_run_sn >= last_sync_run_sn

    async def __read_last_sync_run_sn(self, site: parzzley.fs.Site,
                                      from_items: t.Iterable[parzzley.fs.Item]) -> int|None:
    ...
    async def __take_over_site_control(self, event: parzzley.sync.aspect.events.sync_run.Prepare) -> None:
        ...
        removed_old = False
        for file in (AttachSites._MANAGED_BY_FILE, AttachSites._EARLY_LAST_SYNC_RUN_SN_FILE,
        ...
        await control_site.write_bytes(AttachSites._MANAGED_BY_FILE,
                                       f"{event.sync_run.manager_id}/{event.sync_run.volume_name}".encode())

        if removed_old:
            event.log.warning("This Parzzley instance takes over control from an earlier one for site %s.",
                              repr(event.site.name))
        else:
    ...
    async def __store_current_sync_run(self, event: parzzley.sync.aspect.events.sync_run.Prepare) -> None:
        await self.__write_last_sync_run_sn(await event.site.control_site(),
                                            (self._WORKING_LAST_SYNC_RUN_SN_FILE, self._EARLY_LAST_SYNC_RUN_SN_FILE,
                                             self._LAST_SYNC_RUN_SN_FILE), event.sync_run.sn)
