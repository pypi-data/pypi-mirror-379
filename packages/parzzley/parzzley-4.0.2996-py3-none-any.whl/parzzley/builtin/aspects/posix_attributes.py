...
class PosixAttributesSynchronization(parzzley.sync.aspect.Aspect):
    ...
    async def resolve_conflicts_on_directories(self,
                                               event: parzzley.sync.aspect.events.item.stream.TryResolveConflicts):
        """
    ...
    @parzzley.sync.aspect.event_handler(parzzley.sync.aspect.only_if.IsStream("posix"))
    async def disable_change_detection(self,
                                       event: parzzley.sync.aspect.events.item.stream.CollectDestinationStreamables):
        """
