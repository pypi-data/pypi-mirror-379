...
class SkipItemIfNoUpToDateSitesAreConnected(parzzley.sync.aspect.Aspect):
    ...
    @parzzley.sync.aspect.event_handler(beforehand_optional=[
        parzzley.builtin.aspects.exclude_paths.ExcludePaths.decide_to_skip])
...
class GlobalStreamSupport(parzzley.sync.aspect.Aspect):
    ...
    async def validate_stream_support(self, event: parzzley.sync.aspect.events.sync_run.ValidateStreamSupport):
        ...
        """
        stream_support = {stream_name: (tuple(event.supported_item_types(stream_name, for_site=event.site)),
...
class SetItemsBookEntry(parzzley.sync.aspect.Aspect):
    ...
    async def refresh_items_book(self, event: parzzley.sync.aspect.events.item.RefreshItemsBook):
        ...
        Refresh items book.
        """
        cookies = {stream_name: event.updated_cookie(stream_name)
...
class UpdateTransfer(parzzley.sync.aspect.Aspect):
    ...
    @parzzley.sync.aspect.event_handler(parzzley.sync.aspect.only_if.ItemExistsOnMasterSite(of_stream=""),
    ...
    async def transfer_update(self, event: parzzley.sync.aspect.events.item.stream.TransferUpdate):
        ...
        if source_streamable and destination_streamables:
            ...
            async with contextlib.AsyncExitStack() as stack:
                destination_streams = [await stack.enter_async_context(destination_streamable.stream())
                ...
                if (event.is_change_detection_disabled()
                        or event.stream_cookie() == await event.site.cookie(event.item, event.stream_name)):
    ...
    async def log_update(self, event: parzzley.sync.aspect.events.item.RefreshItemsBook):  # TODO odd event
        ...
        if update_log_info := self._DATA__UPDATE_LOG_INFO.get(event):
            ...
            event.log.info("Updated %s", ", ".join(f"{", ".join(_ or "main" for _ in stream_names)} from {site_name}"
...
class WorkingItem(parzzley.sync.aspect.Aspect):
    ...
    @parzzley.sync.aspect.event_handler(parzzley.sync.aspect.only_if.ItemExistsOnMasterSite(),
    ...
    @parzzley.sync.aspect.event_handler(parzzley.sync.aspect.only_if.IsItemTypeOnStreamMasterSiteSupportedByStream(),
    ...
    async def collect_destination(self, event: parzzley.sync.aspect.events.item.stream.CollectDestinationStreamables):
        ...
        if self.__master_site_is_more_recent(event) or self._DATA__TEMP_ITEM.get(event):
            ...
            for working_item in working_items:
                if (event.item_type(event.to_stream_event("").master_site)
    ...
    async def commit_update(self, event: parzzley.sync.aspect.events.item.ApplyUpdate):
        ...
        cookies = {stream_name: await event.site.cookie(temp_item or event.item, stream_name)
        ...
        cookies.update({stream_name: await event.site.cookie(event.item, stream_name)
                        for stream_name in streams_with_destination_streamables if stream_name not in cookies})
    ...
    def __master_site_is_more_recent(self, event) -> bool:
        ...
        return event.content_version(only_past=True) < event.content_version(only_past=False,
                                                                             for_site=event.master_site)
