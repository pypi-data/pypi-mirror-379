...
class Formatter(parzzley.sync.logger.Formatter):
    ...
    def __header(self, sync_run: parzzley.sync.run.SyncRun|None) -> str:
        ...
        return (f"<h1>{html.escape(self.__title(sync_run))}</h1>"
    ...
    def __entries(self, entries: t.Iterable[parzzley.sync.logger.Entry]) -> str:
        severity_to_class = {parzzley.sync.logger.Severity.DEBUG: "debug",
        ...
        table_rows = ((
            ...
            f"</td>"
            f"</tr>") for entry in entries)

        return (f"<table class='report_table'>"
                f"    {"".join(table_rows)}"
                f"</table>")
    ...
    def __style(self) -> str:
        return ("h1, .aux_info { padding: 0; margin: 0; text-align: right; }\n"
