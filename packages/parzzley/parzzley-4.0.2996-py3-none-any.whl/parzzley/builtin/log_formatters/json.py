...
class Formatter(parzzley.sync.logger.Formatter):
    ...
    def format(self, sync_run, entries):
        return json.dumps({
            ...
            "started_at": sync_run.started_at.astimezone(datetime.UTC).strftime("%Y-%m-%d %H:%M:%S")
                          if sync_run else None,
            "entries": [(_.severity.name, _.message, repr(_.item.path)[2:-1] if _.item else None, _.stream)
                        for _ in entries]},
            indent=1)
