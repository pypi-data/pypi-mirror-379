...
class _WithSourceStreamableGetters(abc.ABC):
    ...
    @abc.abstractmethod
    def source_streamable(self, for_site: "parzzley.fs.TSiteInput|None" = None
    ...
    @abc.abstractmethod
    def stream_pipes(self, for_site: "parzzley.fs.TSiteInput|None" = None
...
class DetermineComparator(_Event, abc.ABC):
    ...
    @property
    @abc.abstractmethod
    def comparator(self) -> t.Callable[[parzzley.fs.stream.ReadStreamable,
                                        parzzley.fs.stream.ReadStreamable], bool]|None:
        """
...
class DetectChanges(_EventAfterItemInfoCollected, _WithStreamCookie, abc.ABC):
    ...
    def mark_all_last_involved_sites_changed_if_this_is_not_one_of_them(
            self, site: "parzzley.fs.TSiteInput|None" = None) -> None:
        """
...
class CollectSourceStreamables(_EventAfterMasterSiteDetermined, _WithStreamCookie,
                               _WithSourceStreamableGetters, _item_events._WithWorkingItems, abc.ABC):
    """
    ...
    @abc.abstractmethod
    def set_source_streamable(self, source_streamable: "parzzley.fs.stream.ReadStreamable", *,
                              for_site: "parzzley.fs.TSiteInput|None" = None) -> None:
        """
    ...
    @abc.abstractmethod
    def add_stream_pipe(self, pipe: type[Pipe], *,
                        for_site: "parzzley.fs.TSiteInput|None" = None) -> None:
        """
...
class CollectDestinationStreamables(_EventAfterMasterSiteDetermined, _WithStreamCookie,
                                    _WithDestinationStreamableGetters, _item_events._WithWorkingItems, abc.ABC):
    """
...
class TransferUpdate(_EventAfterMasterSiteDetermined, _WithStreamCookie, _WithSourceStreamableGetters,
             _WithDestinationStreamableGetters, abc.ABC):
    """
