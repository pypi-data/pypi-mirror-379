    ...
    def _all_event_handlers(self):
        ...
        for aspect_subclass in type(self).mro():
            for aspect_attr_name in aspect_subclass.__dict__:
                ...
                aspect_attr_event_handler_spec = getattr(aspect_attr, "_parzzley_aspect_event_handler_for",
                                                         None)

                if aspect_attr_event_handler_spec:
                    func_args = [x for x in inspect.signature(aspect_attr).parameters.values()
                    ...
                    if len(func_args) > 1 and func_args[1].annotation:
                        ...
                        result.append((functools.partial(aspect_attr, self), conditions, func_args[1].annotation,
                                       [aspect_attr.__qualname__, *more_names], *dependencies))
...
def event_handler(*conditions,
    ...
    """
    def decorator(func_):
        func_._parzzley_aspect_event_handler_for = (conditions, tuple(more_names),
                                                   _dep_list(beforehand), _dep_list(beforehand_optional),
                                                   _dep_list(afterwards), _dep_list(afterwards_optional))
        return func_
