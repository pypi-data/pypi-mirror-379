...
class Runner:
    ...
    class _DependencyControlledHandlerCollection:
        ...
        def __init__(self, handler_nodes: list["Runner._HandlerNode"]):
            ...
            self.__open_dependencies = {node: {n.handler for n in node.depends_on_handlers}
    ...
    async def run_event(self, event_type: type["parzzley.sync.aspect.events._Event"],
                        event: "parzzley.sync.aspect.events._Event") -> None:
        """
        Run an event.
    ...
    def __handlers_for_event_type(
            self, event_type: type[parzzley.sync.aspect.events._Event]) -> "_DependencyControlledHandlerCollection":
        return self._DependencyControlledHandlerCollection([h for h in self.__all_handler_nodes()
                                                            if h.handler.event_type is event_type])
