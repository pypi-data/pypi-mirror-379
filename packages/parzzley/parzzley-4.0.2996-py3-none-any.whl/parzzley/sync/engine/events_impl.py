...
class SyncRunEvent(parzzley.sync.aspect.events.sync_run.DetermineStreamSupport,
    ...
    def __init__(self, site: "parzzley.fs.Site|None", sync_run: "parzzley.sync.run.SyncRun",
                 per_sync_run_data, items_books, logger: "_Logger|None" = None):
        super().__init__()
        self.__site = site
    ...
    def _get_data(self, data_obj, per_site, per_item, per_stream, key_data, initial_value):
        event_data = self.__event_data_wrapper(data_obj, initial_value, per_site=per_site, per_item=per_item,
...
class ItemEvent(SyncRunEvent,
    ...
    def __init__(self, site, sync_run, per_sync_run_data, items_books,
                 item: "parzzley.fs.Item", per_item_data, logger: "_Logger|None" = None):
        super().__init__(site, sync_run, per_sync_run_data, items_books,
                         logger or _Logger(self, item, None))
        self.__item = item
        self.__per_item_data = per_item_data

    def to_dir_event(self):
        return DirEvent(self.site, self.sync_run, self._per_sync_run_data,
                        self._items_books, self.__item, self.__per_item_data)
    ...
    def to_stream_event(self, stream_name):
        ...
        return StreamEvent(self.site, self.sync_run, self._per_sync_run_data,
                           self._items_books, self.__item, self.__per_item_data,
                           stream_name, per_stream_data)
    ...
    def streams_with_destination_streamables(self, for_site=None):
        ...
        for stream_name in self.stream_support_info.supported_streams:
            ...
            if any(True for _ in stream_event._DATA__DESTINATION_STREAMABLES.get(stream_event)
                   if _[1].name == site_name):
...
class StreamEvent(ItemEvent,
    ...
    def __init__(self, site, sync_run, per_sync_run_data, items_books, item: "parzzley.fs.Item",
                 per_item_data, stream_name: str, per_stream_data, logger: "_Logger|None" = None):
        super().__init__(site, sync_run, per_sync_run_data, items_books, item,
                         per_item_data, logger or _Logger(self, item, stream_name))
    ...
    def changed_dangerously_late(self, for_site=None):
        return (self.content_version(self.master_site, only_past=True)
...
class _Logger(parzzley.sync.logger.Logger):
    ...
    def _log(self, severity, message, message_args):
        ...
        self.__event.all_log_entries.append(parzzley.sync.logger.Entry(severity=severity, message=message,
                                                                       message_args=message_args, item=self.__item,
