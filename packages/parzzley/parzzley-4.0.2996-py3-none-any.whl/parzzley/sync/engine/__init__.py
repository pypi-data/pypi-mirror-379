...
class Engine:
    ...
    def __init__(self, *, sync_run: "parzzley.sync.run.SyncRun",
        ...
        self.__event_runner = parzzley.sync.engine.event_runner.Runner({site: sync_run.aspects(site.name)
                                                                        for site in sync_run.sites})
    ...
    async def _exec(self) -> None:
        ...
        try:
            ...
            parzzley.sync.utils.run_coroutine_in_new_thread(self.__exec__post(sync_run_event),
                                                            thread_name_postfix="post")
    ...
    async def __exec__post(self, sync_run_event) -> None:
        try:
            ...
            self.__sync_control.set_finished(was_successful=not self.__had_critical_problems,
                                             was_effective=sync_run_event.was_effective)
        ...
        with self.__catch_exceptions(sync_run_event, "due to internal engine problems"):
            ...
            try:
                with self.__catch_exceptions(sync_run_event, "due to problems"):
                    ...
                    await self.__event_runner.run_event(parzzley.sync.aspect.events.sync_run.DetermineStreamSupport,
                                                        sync_run_event)
                    await self.__event_runner.run_event(parzzley.sync.aspect.events.sync_run.ValidateStreamSupport,
                                                        sync_run_event)
                ...
                # the final steps run in a new thread, so they are safe from asyncio.CancelledError.
                parzzley.sync.utils.run_coroutine_in_new_thread(self.__sync__post(sync_run_event, sync_run_label),
                                                                thread_name_postfix="post")
            ...
            if not item_event.is_item_marked_to_skip:
                ...
                if item_event.all_conflicts:
                    ...
                    for conflict in item_event.all_conflicts:
                        conflict_str = (f"{conflict.description}. "
    ...
    async def __sync_item__collect_info_and_determine_master_sites_for_streams(
            self, item_event: "parzzley.sync.engine.events_impl.ItemEvent") -> None:
        ...
        for stream_name in item_event.stream_support_info.supported_streams:
            ...
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.DetermineComparator,
                                                stream_event)
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.DetermineCookie,
                                                stream_event)
            if stream_name == "":
            ...
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.DetermineMasterSiteTable,
                                                stream_event)
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.MasterSiteDetermined,
                                                stream_event)
    ...
    async def __sync_item__source_streamables(self, item_event: "parzzley.sync.engine.events_impl.ItemEvent") -> None:
        for stream_name in item_event.stream_support_info.supported_streams:
            stream_event = item_event.to_stream_event(stream_name)
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.CollectSourceStreamables,
                                                stream_event)
    ...
    async def __sync_item__conflicts(self, item_event: "parzzley.sync.engine.events_impl.ItemEvent") -> None:
        for stream_name in item_event.stream_support_info.supported_streams:
            ...
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.DetermineConflicts,
                                                stream_event)

            if stream_event.conflicts:
                stream_event.log.debug("Found conflicts")
                await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.TryResolveConflicts,
                                                    stream_event)
                await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.ApplyConflictResolution,
                                                    stream_event)

            self.__check_canceled()

        item_event.set_all_conflicts([
            parzzley.sync.aspect.events.item.SkipDueToConflicts._Conflict(stream_name, conflict.description,
                                                                          conflict.details)
            for stream_name in item_event.stream_support_info.supported_streams
    ...
    async def __sync_item__update_streams(self, item_event: "parzzley.sync.engine.events_impl.ItemEvent") -> None:
        ...
        for stream_name in item_event.stream_support_info.supported_streams:
            stream_event = item_event.to_stream_event(stream_name)
            await self.__event_runner.run_event(parzzley.sync.aspect.events.item.stream.CollectDestinationStreamables,
                                                stream_event)
    ...
    async def __prepare_items_book(self) -> None:
        ...
        for site_name, items_book_data in [
                (site.name, await items_books_variable.value(key=site.name))
                for site in self.__sync_run.sites]:
            self.__items_books[site_name] = _PerSiteItemsBook.from_serializable_data(items_book_data,
                                                                                     self.__sync_run.sn)
    ...
    async def __store_items_book(self) -> None:
        items_books_variable = self.__items_books_variable()
        for site in self.__sync_run.sites:
            await items_books_variable.set_value(self.__items_books[site.name].to_serializable_data(),
                                                        key=site.name)
...
class _ItemsBookBase[ItemInfoT](abc.ABC):
    ...
    def from_serializable_data(cls, data: t.Iterable|None, *args) -> t.Self:
        ...
        return cls({ast.literal_eval(item_path): cls._item_info_from_serializable_data(data_)
                    for item_path, *data_ in per_item_data},
                   *args, *cls._state_args_from_serializable_data(state_data))
...
class _GlobalItemsBook(_ItemsBookBase["_GlobalItemsBook._ItemInfo"]):
    ...
    def __init__(self, after_last_sync_item_data: dict[str, "_GlobalItemsBook._ItemInfo"], sync_run_no: int,
        ...
        connected_site_names_by_sync_run_no = {k: v for k, v in connected_site_names_by_sync_run_no.items()
                                               if k in all_used_sync_run_nos}

        self.__connected_site_names_by_sync_run_no = {**connected_site_names_by_sync_run_no,
...
class _PerSiteItemsBook(_ItemsBookBase["_PerSiteItemsBook._ItemInfo"]):
    ...
    def store_item_info(self, item: parzzley.fs.TItemInput, item_type: parzzley.fs.ItemType,
                        stream_cookies: dict[str, object]) -> None:
        """
        Store item info.
    ...
    def _item_info_from_serializable_data(cls, data):
        ...
        return _PerSiteItemsBook._ItemInfo(changed_in_sync_run_no, parzzley.fs.ItemType[item_type_name], {
