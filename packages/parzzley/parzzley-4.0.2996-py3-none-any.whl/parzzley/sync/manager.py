...
class Manager:
    ...
    async def __aenter__(self):
        ...
        var_root_dir = (pathlib.Path("/var/local/parzzley")
                        if (os.geteuid() == 0) else pathlib.Path("~/.local/parzzley").expanduser())

        if not var_root_dir.exists():
            var_root_dir.mkdir(parents=True, mode=0o700)
            (var_root_dir / "README.txt").write_text("This is the local Parzzley state data storage.\n"
    ...
    async def start_sync(self,
                         prepared_sync_setup: "Manager.PreparedSyncSetup") -> "parzzley.sync.control.SyncControl|None":
        """
        Start a sync (and return a control object for watching or cancelling it). The actual sync run will happen in
        background.
        ...
        if len(prepared_sync_setup.connected_sites) >= 2:
            ...
            new_sync_run = parzzley.sync.run.SyncRun(manager=self, volume=prepared_sync_setup.volume,
                                                     sites=sites.keys(), started_at=datetime.datetime.now())
            engine = parzzley.sync.engine.Engine(sync_run=new_sync_run, sites=sites,
                                                 loggings=prepared_sync_setup.loggings)
            engine.start()
            return engine.sync_control

    def run_sync(self, s: "parzzley.config.Volume|Manager.PreparedSyncSetup"
    ...
    async def prepare_sync(
            self, volume_config: "parzzley.config.Volume", *,
            require_one_of: t.Iterable[parzzley.fs.TSiteInput]|None = None) -> t.AsyncGenerator["PreparedSyncSetup"]:
        """
        ...
        require_one_of_site_setups = [site_setup for site_setup in volume.site_setups
                                      if require_one_of is None or site_setup.name in require_one_of]
        require_one_of_site_other_setups = [site_setup for site_setup in volume.site_setups
                                            if site_setup not in require_one_of_site_setups]
        ...
        try:
            ...
            if len(sites) > 0:
                sites.update(dict(await asyncio.gather(*[self.__try_connect(site_setup)
                                                         for site_setup in require_one_of_site_other_setups])))
    ...
    def volume_state_variable(self, volume_name: str, name: str, *,
                              initial_value: t.Any = None) -> "Manager.VolumeStateVariable":
        """
        ...
        if (result_tuple := self.__volume_state_variables.get(key, self)) is self:
            ...
            if settings != result_tuple[1]:
                raise ValueError(f"volume state variable {name!r} already exists on {volume_name!r} with different"
                                 f" settings")
    ...
    async def __run_sync__async(self, s: "parzzley.config.Volume|Manager.PreparedSyncSetup"
                                ) -> "parzzley.sync.control.SyncControl|None":
    ...
    async def __emit_not_synced_warnings(self, volume_config: parzzley.config.Volume,
                                         loggings: t.Iterable["parzzley.sync.logger.Logging"]) -> None:
        ...
        now = time.time()
        site_success_info = {site.name: old_site_success_info.get(site.name, (now, False))
        ...
        for site in volume_config.sites:
            ...
            if site.warn_after is not None and last_ago > site.warn_after.total_seconds():
                if last_warned + site.warn_after.total_seconds() < now:
                    ...
                    warn_entries.append(parzzley.sync.logger.Entry(
                        message=f"The site {site.name!r} in volume {volume_config.name!r} was not synchronized for"
                                f" {not_synced_for_str}.", message_args=(),
                        severity=parzzley.sync.logger.Severity.WARNING, item=None, stream=None))
    ...
    class VolumeStateVariable:
        ...
        def __init__(self, manager_control_site: t.Callable[[str], t.Coroutine[t.Any, t.Any, "parzzley.fs.Site"]],
                     volume_name: str, name: str, initial_value: t.Any):
            """
        ...
        async def __control_site(self):
            ...
            try:
                await control_site.create_item(parzzley.fs.item(self.__volume_name.encode()),
                                               parzzley.fs.ItemType.DIRECTORY)
