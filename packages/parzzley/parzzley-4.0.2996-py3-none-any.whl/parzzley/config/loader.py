...
def load_logging(logger_config: "parzzley.config.Logging") -> "parzzley.sync.logger.Logging":
    ...
    return parzzley.sync.logger.Logging(
        min_severity=(parzzley.sync.logger.Severity.INFO if logger_config.min_severity is None
                      else parzzley.config.file_formats.log_severity(logger_config.min_severity)),
        max_severity=(None if logger_config.max_severity is None
...
def load_log_exclusion_condition(log_exclusion_condition_config: "parzzley.config.Logging.Exclude.BaseCondition"
    ...
    if isinstance(log_exclusion_condition_config, parzzley.config.Logging.Exclude.CombinedCondition):
        return (parzzley.sync.logger.conditions.ConditionAllOf if log_exclusion_condition_config.combination == "AND"
    ...
    if isinstance(log_exclusion_condition_config, parzzley.config.Logging.Exclude.NegateCondition):
        return parzzley.sync.logger.conditions.ConditionNegate(condition=load_log_exclusion_condition(
            log_exclusion_condition_config.condition))
