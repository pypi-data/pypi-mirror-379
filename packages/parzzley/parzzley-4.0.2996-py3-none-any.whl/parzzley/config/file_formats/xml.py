...
from parzzley.config.file_formats import (register_file_format as _register_file_format, FileFormat as _FileFormat,
                                          timedelta)
...
class XmlFileFormat(_FileFormat):
    ...
    def __logging(self, xml_elem: lxml.etree.Element, config_file) -> "parzzley.config.Logging":
        ...
        return parzzley.config.Logging(min_severity=min_severity, max_severity=max_severity, formatter=formatter,
    ...
    def __logger_exclusion(self, xml_elem: lxml.etree.Element, config_file) -> "parzzley.config.Logging.Exclude":
        return parzzley.config.Logging.Exclude(
            conditions=(self.__logger_exclusion_condition(xml_child_elem, config_file)
                        for xml_child_elem in xml_elem.getchildren()))

    def __logger_exclusion_condition(self, xml_elem: lxml.etree.Element,
                                     config_file) -> "parzzley.config.Logging.Exclude.BaseCondition":
        ...
        if xml_elem.tag == "{urn:parzzley}ConditionNegate":
            conditions = tuple(self.__logger_exclusion_condition(xml_child_elem, config_file)
                               for xml_child_elem in xml_elem.getchildren())
            if len(conditions) != 1:
                raise RuntimeError(f"invalid Parzzley config xml file: {config_file}")
