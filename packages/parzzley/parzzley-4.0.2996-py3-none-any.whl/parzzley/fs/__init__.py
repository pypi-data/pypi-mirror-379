...
class Site:  # pylint: disable=too-many-public-methods
    ...
    async def child_names(self, item_: TItemInput) -> t.Sequence[str]:
        ...
        item_ = item(item_)
        with Site.__translate_arbitrary_exception_to_site_exception():
            return sorted(_ for _ in await self.__site_backend.child_names(item_)
        ...
        with self.__lock:
            if self.__connect_counter == 0:
                ...
                if site_backend is not None:
                    try:
                        if ((root_cookie := await site_backend.cookie(parzzley.fs.item(b""), "")) is None
                                or site_backend.item_type_by_cookie(root_cookie) == parzzley.fs.ItemType.NONE):
...
class _SubTreeSiteBackend(Backend.SiteBackend):
    ...
    async def move_item(self, item, to_item):
        return await self.__origin_site_backend.move_item(self.__root_item(item.path),
                                                          self.__root_item(to_item.path))
