...
class SlackBot:
    ...
    def id_from_name(self, name):
        slack_id = self.user_channel_id_mapping.get(name)
        return slack_id
    ...
    def connect(self, token: Optional[str] = None):
        if not token:
            ...
            if token is None:
                raise ValueError("Slack bot token is not set, please provide slack token or set SLACK_BOT_TOKEN env var")
    ...
    def add_user_channel_id(self, recipient_id: str, recipient_name: str):
        self.user_channel_id_mapping[recipient_id] = recipient_name
