...
def create_workflow_files(work_dir):
    ...
    # Copy Snakefile from package directory
    snakefile_src = package_dir / "Snakefile"
...
Examples:
  ...
  rlooper-workflow --init-project /path  # Initialize project in directory
    ...
    if args.command == "dag":
        ...
        # Fallback: Generate DAG programmatically
        try:
            from . import dag_fallback
            ...
            print("❌ Fallback DAG generation not available")
            return 1
        except Exception as e:
            print(f"❌ Fallback DAG generation failed: {e}")
            return 1
    
    return run_snakemake(snakemake_args, work_dir)
