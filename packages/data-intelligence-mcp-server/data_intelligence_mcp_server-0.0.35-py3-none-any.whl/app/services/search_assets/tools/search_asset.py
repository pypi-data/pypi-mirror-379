...
@service_registry.tool(
        ...
        description="""Understand user's request about searching data assets and return list of retrieved assets.
                       ...
                       Args:
                            request: search_prompt (str): The search prompt from the user about data assets potentially with additional searching details.
                            container_type (str): The container type for which to search assets.
                       Returns:
                            List[SearchAssetResponse] : A list of SearchAssetResponse objects.

                       """)
@auto_context
async def search_asset(search_prompt:str, auth_scope:str, ctx=None) -> List[SearchAssetResponse]:
    auth_scope = "catalog"

    LOGGER.info("Starting asset search with prompt: '%s' and container_type: '%s'", search_prompt, auth_scope)

    payload = {
        "operation_type": "search",
        "search_request": {"text": search_prompt, "auth_scope": auth_scope},
    }
