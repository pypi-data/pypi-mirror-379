...
class WriteTool:
    ...
    def write_file(self, file_path: str, content: str) -> Dict[str, Any]:
        ...
        WHEN TO USE:
        - Creating new files: Scripts, configs, documentation
        - Complete file replacement: Major rewrites, templates
        - Use edit_file instead for: Small targeted changes, line modifications

        SAFETY REQUIREMENTS:
        - Must read existing non-empty files first before writing
        - Creates automatic .backup files before overwriting
        - Can freely write to new or empty files

        LIMITATIONS:
        - Requires read permission on existing files for safety
        - May fail on write-protected files or directories
        - Fixed UTF-8 encoding

        BEST PRACTICES:
        - Read existing files first to understand what you're replacing
        - Check 'success' field before assuming write completed
        - Consider edit_file for small changes instead of full rewrites

        COMMON ERRORS:
        - "Must read file first": Read existing file content before overwriting
        - "Permission denied": Check file/directory permissions
        - "Directory not found": Create parent directory first
