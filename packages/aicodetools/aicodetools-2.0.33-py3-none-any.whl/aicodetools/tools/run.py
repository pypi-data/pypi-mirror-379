...
class RunTool:
    ...
    def run_command(
        ...
        USAGE:
        - Non-interactive: Quick commands, builds, tests - auto-kills on timeout
        - Interactive: Long tasks, user prompts - returns control choices on timeout

        WHEN TO USE INTERACTIVE:
        - Commands needing input: installations, prompts, debugging
        - Long/uncertain duration: servers, monitoring, large builds
        - When you need to see output before completion

        CONTROL COMMANDS (when RUNNING):
        - '' (empty): Continue waiting, get more output
        - 'C-c': Kill the running process
        - '>>> input': Send input to process (note the >>> prefix)
        - New commands: Rejected until current process finishes

        TIMEOUTS:
        - Quick commands: 30-60s
        - Builds/tests: 300-600s
        - Interactive cycles: 60-120s

        LIMITATIONS:
        - Only one process at a time
        - Exact string matching for control commands
        ...
        - "Permission denied": Need appropriate permissions for command
    ...
    def _collect_available_output(self):
        ...
        try:
            ...
            # On Windows, select doesn't work with pipes, so use a simple timeout approach
            if sys.platform == "win32":
                # Windows: just try to read with very short timeout
                try:
    ...
    def _send_input_and_wait(self, input_text: str, timeout: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Small delay to let input be processed
            time.sleep(0.1)
