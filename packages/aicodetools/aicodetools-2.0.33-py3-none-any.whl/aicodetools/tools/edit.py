...
class EditTool:
    ...
    def edit_file(self, file_path: str, old_string: str, new_string: str, replace_all: bool = False) -> Dict[str, Any]:
        ...
        WHEN TO USE:
        - Targeted changes: Bug fixes, variable updates, specific line modifications
        - Code refactoring: Renaming functions/variables throughout file
        - Use write_file instead for: Complete rewrites, new files, major changes
        ...
        - Match exact capitalization and punctuation

        STRING MATCHING:
        - Exact match required: Even small differences cause failure
        - Include surrounding context: "def function_name(param):" not just "function_name"
        - Preserve indentation: Copy spaces/tabs exactly as shown in file
        - Multi-line: Include \n line breaks and proper indentation

        REPLACE OPTIONS:
        - replace_all=False: Changes first occurrence only
        - replace_all=True: Changes all occurrences throughout file

        BEST PRACTICES:
        - Always read file first to understand structure and formatting
        - Copy old_string exactly from read output
        - Verify changes by reading modified section after edit

        COMMON ERRORS:
        - "String not found": old_string doesn't match exactly (check whitespace)
        - "Must read file first": Required for safety
        - "Multiple matches": Be more specific with context
