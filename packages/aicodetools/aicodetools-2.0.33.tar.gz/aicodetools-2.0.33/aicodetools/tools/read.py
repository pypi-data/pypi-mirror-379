...
class ReadTool:
    ...
    def read_file(
        ...
        USAGE MODES:
        - Full read (no args): Small-medium files, initial exploration
        - Line ranges (LineArgs): Specific sections, around error locations
        - Regex search (RegexArgs): Find patterns, function definitions

        WHEN TO USE:
        - Always read before editing files for safety and context
        - Code exploration and understanding structure
        - Debugging specific sections or error locations
        ...
        - Very long files: Content may be truncated with warning

        BEST PRACTICES:
        - Use line ranges for large files when you know target location
        - Use regex for pattern searching: r'def function_name'
        - Check 'success' field before processing content
        - Read around error lines (±10-20 lines for context)
