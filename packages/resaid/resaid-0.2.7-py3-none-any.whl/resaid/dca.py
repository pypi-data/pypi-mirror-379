        ...
        with open(file_path, "w") as file:
            for index, row in oneline_df.iterrows():
                ...
                if row['OIL_QI'] > 0:
                    ...
                    if row['OIL_B'] > 0.01 and round(row['OIL_revised_aries_de'], 2) > dmin and oil_val > 0:
                ...
                if row['GAS_QI'] > 0:
                    ...
                    if row['GAS_B'] > 0.01 and round(row['GAS_revised_aries_de'], 2) > dmin and gas_val > 0:
                ...
                if write_water and row['WATER_QI'] > 0:
                    ...
                    if row['WATER_B'] > 0.01 and round(row['WATER_revised_aries_de'], 2) > dmin and water_val > 0:
                        water_line = f"  WATER      {water_val} X M/M {dmin} EXP B/{round(row['WATER_B'], 2)} {round(row['WATER_revised_aries_de'], 2)}"
                        ...
                        water_line = f"  WATER      {water_val} X M/M 99 YRS EXP {round(row['WATER_revised_aries_de'], 2)}"
                        water_padding = " " * (93 - len(water_line) - len(scenario))
                        file.write(water_line + water_padding + scenario + "\n")
                        
                    elif water_val > 0:
                        water_line = f"  WATER      {water_val} X M/M 99 YRS EXP {dmin}"
                        water_padding = " " * (93 - len(water_line) - len(scenario))
                        file.write(water_line + water_padding + scenario + "\n")
                        
                    else:
                        water_line = f"  WATER      {water_val} X M/M 1 YRS FLAT 0"
                        water_padding = " " * (93 - len(water_line) - len(scenario))
                        file.write(water_line + water_padding + scenario + "\n")
