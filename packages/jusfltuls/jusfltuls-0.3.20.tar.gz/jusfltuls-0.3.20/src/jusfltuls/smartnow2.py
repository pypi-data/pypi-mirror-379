...
def check_btrfs_scrub_status(device):
    ...
    if status_match:
        ...
        if status == "running":
            # For running scrub, show "running" as status and preserve error summary
...
def get_device_size(device):
    ...
    rc, out = run_cmd(f"lsblk -b -n -d -o SIZE {device}")
    if rc != 0:
        return ""
...
def get_smart_test_status(device):
    ...
    if rc == 0:
        ...
        # Look for current test execution status
    ...
    def loop(stdscr):
        ...
        while True:
            ...
            if k == ord('q'):
                ...
                selected_row = rows[cursor_row]
                if selected_row.get("device") and selected_row["type"] in ("block", "nvme"):
