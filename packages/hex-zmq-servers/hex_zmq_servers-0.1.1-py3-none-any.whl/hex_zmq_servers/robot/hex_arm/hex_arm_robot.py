...
class HexHexArmRobot(HexRobotBase):
    ...
    def __init__(
        ...
        try:
            ...
            self.__cmd_type = robot_config["cmd_type"]
    ...
    def __set_cmds(self, cmds: np.ndarray) -> bool:
        ...
        if self.__cmd_type == "pos":
            tar_pos = self._apply_pos_limits(
                cmds,
                self._limits[:, 0, 0],
                self._limits[:, 0, 1],
            )
            self.__arm_archer.motor_command(CommandType.POSITION, tar_pos)
        elif self.__cmd_type == "eff":
            tar_eff = np.clip(
                cmds,
                self._limits[:, 2, 0],
                self._limits[:, 2, 1],
            )
            self.__arm_archer.motor_command(CommandType.TORQUE, tar_eff)
        else:
            raise ValueError(f"unknown arm CommandType: {self.__cmd_type}")
