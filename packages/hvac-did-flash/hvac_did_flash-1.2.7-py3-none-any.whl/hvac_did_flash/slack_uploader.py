...
def load_env_with_fallback():
    """
    Load .slack_channel file from current directory, or fallback to ~/.slack_channel if not found.
    """
    # First try to load .slack_channel from current directory
    if os.path.exists('.slack_channel'):
        load_dotenv('.slack_channel')
        return '.slack_channel'

    # Fallback to home directory .slack_channel
    home_env = Path.home() / '.slack_channel'
    if home_env.exists():
        load_dotenv(home_env)
        return str(home_env)

    # No .slack_channel file found, but still try to load (might use system env vars)
    load_dotenv()
    return None
...
def upload_to_slack(file_path: str, channel_id: Optional[str] = None, initial_comment: Optional[str] = None) -> bool:
    ...
    """
    # Load environment variables with fallback to ~/.slack_channel
...
def validate_slack_config() -> bool:
    ...
    if env_file:
        ...
        print("No .slack_channel file found, checking system environment variables")
