...
def replace_serial_in_isotp_user(src_dir, did_serial_number):
    ...
    """
    target_file = os.path.join(src_dir, 'lib\hvac\include', 'hvac_ccm_infos.h')
...
def display_build_info():
    """
    Display build information and ask user to continue.

    Returns:
        bool: True if user wants to continue, False otherwise
    """
    home_dir = os.path.expanduser("~")
    dev_env_path = os.environ.get('DEV')

    # Check DEV environment
    if dev_env_path and os.path.exists(dev_env_path):
        work_dir = os.path.join(dev_env_path, "upstream", "hvac-main-stm32f103_pio")
    else:
        work_dir = os.path.join(home_dir, "hvac-main-stm32f103_pio")

    # Check if work directory exists
    if not os.path.exists(work_dir):
        print(f"[에러] 작업 디렉토리가 존재하지 않습니다: {work_dir}")
        print(f"hvac-main-stm32f103_pio 프로젝트를 {work_dir}에 설치해주세요.")
        return False
    ...
    # 1. Core/Inc/main.h의 Private defines 정보 읽기
    main_h_path = os.path.join(work_dir, "Core", "Inc", "main.h")
    first_private_define = None
    if os.path.exists(main_h_path):
        with open(main_h_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            found_private_defines = False
            for line in lines:
                if "/* Private defines" in line:
                    found_private_defines = True
                    continue
                if found_private_defines and "#define" in line:
                    first_private_define = line.strip()
                    break

    # 2. platformio.ini의 build_flags 정보 읽기
    platformio_ini_path = os.path.join(work_dir, "platformio.ini")
    build_flags = None
    if os.path.exists(platformio_ini_path):
        with open(platformio_ini_path, 'r', encoding='utf-8') as f:
            for line in f:
                if line.startswith("build_flags"):
                    build_flags = line.split("=", 1)[1].strip() if "=" in line else None
                    break

    # 3. Git SHA 정보 가져오기
    try:
        sha_result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=work_dir, text=True, capture_output=True, check=True)
        sha = sha_result.stdout.strip()[:7]  # 처음 7자리만 사용
    except subprocess.CalledProcessError:
        sha = "unknown"

    # 4. 모든 정보 출력
    print(f"1. main.h Private define: {first_private_define if first_private_define else 'Not found'}")
    print(f"2. platformio.ini build_flags: {build_flags if build_flags else 'Not found'}")
    print(f"3. Git 태그: {tag}")
    print(f"4. Git SHA: {sha}")
    print("==================\n")

    # 사용자에게 계속할지 물어보기
    user_input = input("빌드를 계속하시겠습니까? (y/n): ").strip().lower()
    if user_input != 'y':
        print("빌드를 취소합니다.")
        return False
...
# Use temp directory for serial file storage
TEMP_BASE_DIR = tempfile.gettempdir()
SERIAL_FILE_DIR = os.path.join(TEMP_BASE_DIR, 'hvac-firmware')
SERIAL_FILE_NAME = os.path.join(SERIAL_FILE_DIR, "did_serial.csv")
...
def main():
    ...
    # Display build info and ask for continuation BEFORE loop
    if not display_build_info():
        print("사용자가 빌드를 취소했습니다.")
        return
        ...
        if zip_file:
            ...
            if args.slack:
                ...
                if validate_slack_config():
                    ...
                    print("Slack 설정이 올바르지 않습니다. .slack_channel 파일을 확인해주세요.")
            else:
