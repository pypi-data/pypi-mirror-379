...
from kxy.framework.logger import logger
class HttpClient:
    @staticmethod
    ...
    async def request(cls, method, url, **kwargs):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(f'{method} {url} success {result.status_code}',extra={'Category':'http'})
                return result
        except Exception as ex:
            logger.error(f'{method} {url} error:{ex}',extra={'Category':'http'})
            raise ex
