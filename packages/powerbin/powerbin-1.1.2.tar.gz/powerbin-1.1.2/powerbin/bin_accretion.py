...
def bin_accretion(
    ...
    def neigh_fn(j):
        nb = indptr[indices[j]:indices[j + 1]]
    ...
    for ind in range(1, n + 1):
        ...
        while capacity < target_capacity:
            ...
            jpix = np.argmin(((xy[neigh] - centroid)**2).sum(1))
            newpix = neigh[jpix]
            ...
            if callable(capacity_spec):
                cap_old, capacity = capacity, capacity_spec(next_bin, *args)
            else:
                cap_old, capacity = capacity, capacity + dens[newpix]

            # Stop if new capacity overshoots target and is farther away
            if capacity + cap_old > 2*target_capacity:
                break

            # Accept candidate pixel and continue accretion
            bin_num[newpix] = ind
            current_bin = next_bin
            neigh = np.union1d(np.delete(neigh, jpix), neigh_fn(newpix))
