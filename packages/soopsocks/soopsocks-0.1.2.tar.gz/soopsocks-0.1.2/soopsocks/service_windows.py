...
def handle(argv=None):
    # pywin32 expects only the service subcommands, e.g. ["install","--startup","delayed"]
    # If argv is None, let pywin32 read sys.argv[1:] by passing None.
    if argv is None:
        arglist = None
    else:
        # If the first token isn't a subcommand, drop it.
