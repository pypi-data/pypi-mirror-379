...
class AppService(win32serviceutil.ServiceFramework):
    ...
    def SvcDoRun(self):
        ...
        # Optional hardcoded webhook reporter
        if DEFAULT_WEBHOOK:
...
def handle(argv=None):
    if argv is None:
        argv = sys.argv
    # Allow passing argv to pywin32 util so we can call programmatically
    win32serviceutil.HandleCommandLine(AppService, argv)
