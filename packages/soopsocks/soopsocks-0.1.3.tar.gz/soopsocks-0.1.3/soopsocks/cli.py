...
def cmd_service(args):
    try:
        import win32serviceutil  # ensure pywin32 present
    except Exception:
...
def cmd_auto(_args):
    ...
    install = _run_and_check([sys.executable,"-m","soopsocks.service_windows","install","--startup","delayed"], hide=True)
    start   = _run_and_check([sys.executable,"-m","soopsocks.service_windows","start","--wait","10"], hide=True)
    ...
    if _service_running():
        ...
        print("수동 실행 예시:")
        print("  python -m soopsocks.service_windows install --startup delayed")
        print("  python -m soopsocks.service_windows start --wait 10")
