...
class Core:
    ...
    def get_built_in_dataset(self, labelNumber: str) -> None:
        ...
        zip_url = "https://github.com/olcruzgonzalez/pyhom/tree/main/pyhom/built-in/dataset.zip"

        # --- Directory setup (no changes here) ---
        cwd = Path(os.getcwd())
        destination_dir = cwd.joinpath("built-in_input")
        # Create the folder
        destination_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            # --- Use the downloaded content directly from memory ---
            # standard_io.BytesIO treats the downloaded bytes as a file-like object
            zip_file_bytes = standard_io.BytesIO(response.content)

            # --- Extract the specific folder (no changes here) ---
