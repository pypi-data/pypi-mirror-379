...
class TestInstallInstrumentations:
    ...
    def test_install_instrumentations_basic(self, mock_requests_instrumentor, mock_azure_settings):
        ...
        # Act - all instrumentations are now enabled by default
        _install_instrumentations()
    ...
    def test_install_instrumentations_with_openai(
        ...
        """Test instrumentations installation with OpenAI module present."""
        # Arrange
        mock_requests_instance = Mock()
        mock_requests_instrumentor.return_value = mock_requests_instance
        
        # Mock OpenAI module being imported
        mock_openai_module = MagicMock()
        
        with patch.dict('sys.modules', {'openai': mock_openai_module}):
            # Act - OpenAI instrumentation now automatic if module present
            _install_instrumentations()
    ...
    def test_install_instrumentations_database_disabled(
        ...
        """Test that database instrumentation works automatically when modules are present."""
        # Arrange
        mock_requests_instance = Mock()
        mock_requests_instrumentor.return_value = mock_requests_instance
        
        # Act - database instrumentation is now always enabled if modules are present
        _install_instrumentations()
    ...
    def test_install_instrumentations_parameter_combinations(
        ...
        """Test multiple calls to instrumentation function."""
        # Arrange
        mock_requests_instance = Mock()
        mock_requests_instrumentor.return_value = mock_requests_instance
        
        # Act - Test multiple calls (all instrumentations are automatic now)
        test_calls = 4
        ...
        assert mock_requests_instance.instrument.call_count == test_calls
    ...
    @patch('azure.core.settings.settings')
    @patch('opentelemetry.instrumentation.requests.RequestsInstrumentor')
    def test_install_instrumentations_azure_ai_modules_check(
        self, mock_requests_instrumentor, mock_azure_settings
    ):
        """Test that Azure AI instrumentation only runs when modules are imported."""
        # Arrange
        mock_requests_instance = Mock()
        mock_requests_instrumentor.return_value = mock_requests_instance
        
        # Mock Azure AI modules being imported
        mock_azure_ai_modules = {
            'azure.ai.inference': MagicMock(),
            'azure.ai.agents': MagicMock(),
            'openai': MagicMock()
        }
        
        with patch.dict('sys.modules', mock_azure_ai_modules):
            # Act - Should attempt to instrument AI modules since they're "imported"
            _install_instrumentations()
            
            # Assert - Function should complete successfully
            mock_requests_instance.instrument.assert_called_once()
            # Note: We can't easily mock the AI instrumentors without their actual packages,
            # but the sys.modules check prevents import errors when modules aren't present
