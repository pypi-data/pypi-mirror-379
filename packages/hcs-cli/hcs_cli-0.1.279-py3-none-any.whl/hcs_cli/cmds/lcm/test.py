...
from hcs_cli.support.exec_util import hcs_cli
...
def _clear():
    lcm_template_ids = hcs_cli("lcm template list --ids", output_json=True)
    for tid in lcm_template_ids:
        hcs_cli("lcm template delete -y --force " + tid)
    for tid in lcm_template_ids:
        hcs_cli("lcm template delete -y --force --wait 5m " + tid)
...
def test():
    _safe_check()
    _clear()
    hcs_cli("lcm template create -p zero-new -w2m")
    hcs_cli("lcm template get")
    _clear()
