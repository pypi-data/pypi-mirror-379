...
# Local imports
from hcs_cli.cmds.advisor.config.advisor_config import *
...
def get_vm_cost_per_hour(template_info: dict) -> float:
    ...
    try:
        ...
        return AZURE_VM_COSTS.get(vm_sku, AZURE_VM_COSTS["DEFAULT"])
    except Exception:
        return AZURE_VM_COSTS["DEFAULT"]
...
def get_disk_cost_per_hour(template_info: dict, disk_type: str) -> float:
    ...
    Returns:
        float: Cost per hour in USD
    """
    return AZURE_DISK_COSTS.get(disk_type, AZURE_DISK_COSTS["DEFAULT"])
...
def calculate_gradient_based_decrease(peak_utilized: int, max_vms: int, utilization: float) -> int:
    ...
    try:
        ...
        scaling_factor = (MAX_UTILIZATION_THRESHOLD - utilization) / (MAX_UTILIZATION_THRESHOLD - MIN_UTILIZATION_THRESHOLD)
        scaling_factor = max(0.0, min(1.0, scaling_factor))  # Clamp between 0 and 1

        # Calculate denominator
        denominator = MAX_REDUCTION_FACTOR + scaling_factor * MAX_REDUCTION_FACTOR
...
def estimate_cost_savings_max_vms_reduction(old_max_vms: int, new_max_vms: int, template_info: dict) -> float:
    ...
    try:
        ...
        vm_savings = (delta_vms * min_ratio * HOURS_PER_MONTH) * vm_cost_per_hour

        # Calculate disk cost savings
        # delta_total_pool_hours_per_month = (Old_Max_VMs - New_Max_VMs) × hours_per_month
        delta_total_pool_hours_per_month = delta_vms * HOURS_PER_MONTH
...
def calculate_scheduled_powered_on_hours(template_info: dict, power_schedules: list) -> float:
    ...
    try:
        ...
        start_datetime = datetime.datetime.fromtimestamp(start_timestamp / 1000, tz=datetime.timezone.utc)
...
def estimate_cost_savings_min_available_vms(old_min: float, new_min: float, template_info: dict) -> float:
    ...
    try:
        ...
        # Calculate savings
        savings = limit * (old_min - new_min) * HOURS_PER_MONTH * vm_cost_per_hour
        return round(savings, 2)
    except Exception:
        return 0.0
...
def estimate_cost_savings_power_management_mode(template_info: dict) -> float:
    ...
    try:
        ...
        if not min_ratio:
            min_ratio = MIN_AVAILABLE_VMS_MIN_BOUND / 100.0  # Convert percentage to decimal
        ...
        for i in range(cutoff_index, min(len(powered_on_assigned), len(powered_on_unassigned))):
            ...
            if powered_on_vm_count > min_baseline:
                ...
                point_savings = surplus_vms * POWER_MANAGEMENT_REDUCTION_FACTOR * sampling_period_hours * vm_cost_per_hour
...
def rule_1a_decrease_maximum_vms(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if not max_vms or max_vms < MIN_POOL_SIZE_FOR_RECOMMENDATIONS:
            return None
            ...
            if peak_utilized <= max_vms * 0.1:
                ...
                if new_max_vms > 0 and new_max_vms < max_vms:
                    ...
                    return {
                        ...
                        "justification": f"Pool has consistently low usage — peak usage is under 10% and average utilization is under {UNDER_UTILIZATION_THRESHOLD}%, potential cost savings: ${cost_savings}/month",
...
def rule_1b_increase_maximum_vms(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if utilization > OVER_UTILIZATION_THRESHOLD:
            ...
            if utilized_to_pool_ratio > UTILIZED_TO_POOL_RATIO_THRESHOLD:
                # Gradient increase based on how close utilization is to 100%
                try:
                    scaling = (utilization - OVER_UTILIZATION_THRESHOLD) / (100.0 - OVER_UTILIZATION_THRESHOLD)
                    scaling = max(0.0, min(1.0, scaling))
                    new_max_vms = math.ceil(max_vms * (1.0 + scaling * MAX_INCREASE_ADJUSTMENT_FACTOR))
                except Exception:
                    new_max_vms = max_vms
                ...
                if new_max_vms > max_vms:
                    return {
                        "action": "Adjust Maximum VMs",
                        "justification": (
                            f"Resource Utilization > {OVER_UTILIZATION_THRESHOLD}% and "
                            f"Utilized Capacity Hours / Total Pool Hours > {UTILIZED_TO_POOL_RATIO_THRESHOLD}%. "
...
def rule_2_auto_scale_disk_type(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if pool_size < MIN_POOL_SIZE_FOR_AUTO_SCALE_DISK_TYPE:
            return None
        ...
        if powered_off_ratio >= POWERED_OFF_RATIO_THRESHOLD:
            ...
            return {
                ...
                "justification": f"Pool has high powered-off time — VMs are powered off more than {POWERED_OFF_RATIO_THRESHOLD*100}% of the time, disk cost optimization possible, potential cost savings: ${cost_savings}/month",
                "current_settings": {
...
def rule_3_power_schedule_removal(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if has_weekend or schedule_utilization < utilization:
            ...
            if has_weekend:
                justification = f"Resource Utilization ({utilization:.1f}%) < {UNDER_UTILIZATION_THRESHOLD}% and power schedule includes weekend days, potential cost savings: ${cost_savings}/month"
            else:
                justification = f"Resource Utilization ({utilization:.1f}%) < {UNDER_UTILIZATION_THRESHOLD}% and schedule utilization ({schedule_utilization:.1f}%) < overall utilization ({utilization:.1f}%), potential cost savings: ${cost_savings}/month"
...
def calculate_gradient_based_min_reduction(utilization: float, current_min: float) -> float:
    ...
    try:
        ...
        # Apply minimum bound
        new_min = max(MIN_AVAILABLE_VMS_MIN_BOUND, new_min)
...
def rule_4_minimum_available_vms(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if idle_vm_ratio < IDLE_VM_THRESHOLD:
            return None
        ...
        if utilized_to_powered_ratio >= UTILIZED_TO_POWERED_RATIO_THRESHOLD:
            return None

        # Condition 4: template["powerPolicy"]["min"] > 50%
        if current_min <= MIN_AVAILABLE_VMS_THRESHOLD:
            return None
        ...
        return {
            ...
            "justification": (
                f"Resource Utilization ({utilization:.1f}%) < {UNDER_UTILIZATION_THRESHOLD}%, "
                f"idle VMs > utilized VMs in {idle_vm_ratio:.1f}% of time points, "
                f"utilized/powered ratio ({utilized_to_powered_ratio:.1f}%) < {UTILIZED_TO_POWERED_RATIO_THRESHOLD}%, "
                f"current min ({current_min}%) > {MIN_AVAILABLE_VMS_THRESHOLD}%, "
...
def rule_5_power_management_mode(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        lowest_power_management_thresholds = OCCUPANCY_THRESHOLDS.get("OPTIMIZED_FOR_PERFORMANCE")
...
def rule_6_reevaluate_provisioning_all_at_once(org, resource_id, resource_type, template_info):
    ...
    try:
        ...
        if powered_on_to_allocated_ratio >= POWERED_ON_TO_ALLOCATED_RATIO_THRESHOLD:
            return None
        ...
        return {
            ...
            "justification": (
                ...
                f"Powered-on to allocated ratio ({powered_on_to_allocated_ratio:.1f}%) is below {POWERED_ON_TO_ALLOCATED_RATIO_THRESHOLD}%. "
