...
def _run_mongo_commands(command: str):
    ...
    cp = exec(kubectl_cmd, input=command, show_command=False)
...
def _get_redis_stat():
    cp = exec("kubectl exec -it redis-standalone-0 -- redis-cli info stats", show_command=False)
    stats = _parse_redis_info(cp.stdout)

    cp = exec("kubectl exec -it redis-standalone-0 -- redis-cli info memory", show_command=False)
