...
try:
    ...
    from quark.experimental.cli.torch_llm.llm_eval.evaluation import eval_model
    from quark.experimental.cli.torch_llm.llm_ptq.configuration_preparation import get_config, get_export_config
    from quark.experimental.cli.torch_llm.llm_ptq.customized_configuration import SUPPORTED_QUANT_SCHEME
    from quark.experimental.cli.torch_llm.llm_utils.data_preparation import get_calib_dataloader
    from quark.experimental.cli.torch_llm.llm_utils.model_preparation import (
        get_model,
        get_model_type,
        get_tokenizer,
...
class TorchLLM_PTQ_CLI(base_cli.BaseQuarkCLICommand):
    ...
    def register_subcommand(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            type=lambda s: s.split(","),
            metavar="file1,file2",
            help="The JSON file path for the corresponding quant_alg.",
        )
        parser.add_argument(
            "--exclude_layers",
            type=str,
        ...
        parser.add_argument(
            "--metrics_output_dir", default="metrics_output_dir", help="Output path of csv with metrics."
        )
        parser.add_argument(
            "--tasks",
            default=None,
    ...
    def run(self) -> None:
        ...
        if args.params_load:
            ...
            importer = ModelImporter(
                model_info_dir=args.import_model_dir,
                saved_format=args.import_file_format,
        ...
        if args.use_tp:
            if TPDeviceManager._tp_mesh is not None:
                ...
                device = TPDeviceManager._device  # type: ignore[assignment]
                tp_mesh: str = TPDeviceManager._tp_mesh  # type: ignore[assignment]
            ...
            if "hf_format" in args.model_export:
                ...
                with torch.no_grad():
                    ...
                    exporter.export_safetensors_model(
                        ...
                        tokenizer=tokenizer if not multimodal else None,
                    )
