...
def get_calib_dataloader(
    dataset_name: str, processor: ProcessorMixin | None = None, **kwargs: Any
...
class ConcatDataset(Dataset):
    def __init__(self, dataset: Any, max_length: int = 4096):
...
def get_trainer_dataset(
    ...
    """Return datasets for trainer with collator.

    Returns a dict with keys: train_dataset, data_collator, eval_dataset
    """

    def tokenize_add_label(sample: dict[str, Any]) -> dict[str, Any]:
        if path in ["wikitext"]:
...
def get_dataset(path: str, subset: str, tokenizer: PreTrainedTokenizer, seqlen: int) -> TensorDataset:
    if path in ["wikitext"]:
...
def get_scienceqa(
    ...
    from vlmeval.dataset import build_dataset  # type: ignore[import-not-found]
    ...
    for i in tqdm(range(lt)):
        ...
        inputs = processor(image, input_text, return_tensors="pt").to(device)  # type: ignore[operator]
...
def message_to_promptimg(message: list[dict[str, Any]], dataset: str | None = None) -> tuple[str, str | None]:
    ...
    if num_images == 0:
        ...
        if dataset == "BLINK":
            try:
                from vlmeval.dataset import concat_images as concat_images_vlmeval  # type: ignore[import-not-found]
            except Exception as e:
                raise ImportError("concat_images_vlmeval is not available. Please install vlmeval.")
