...
def error_handler(func):
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(f"[chatbot] ⚠️ Registro no encontrado en {operation_type}: {model_name}")
            raise

        except exc.IntegrityError as e:
            error_msg = str(e.orig) if hasattr(e, 'orig') else str(e)
            logger.error(f"[chatbot] ❌ Error de integridad en {operation_type}: {error_msg}")

            if "UNIQUE constraint failed" in error_msg or "duplicate key" in error_msg.lower():
                logger.error(f"[chatbot] ❌ Ya existe un {model_name} con esos datos")
                raise
            elif "FOREIGN KEY constraint failed" in error_msg or "foreign key" in error_msg.lower():
                if operation_type == "delete":
                    logger.error(f"[chatbot] ❌ No se puede eliminar el {model_name} porque está siendo referenciado por otros registros")
                    raise
                else:
                    logger.error(f"[chatbot] ❌ Error de referencia en los datos del {model_name}")
                    raise
            else:
                logger.error(f"[chatbot] ❌ Error de integridad al {operation_verbs.get(operation_type, 'procesar')} {model_name}")
                raise

        except exc.SQLAlchemyError as e:
            logger.error(f"[chatbot] ❌ SQLAlchemy error en {operation_type}: {str(e)}")
            logger.error(f"[chatbot] ❌ Error en la base de datos al {operation_verbs.get(operation_type, 'procesar')} {model_name}")
            raise

        except Exception as e:
            error_msg = str(e)
            logger.error(f"[chatbot] ❌ Error inesperado en {operation_type}: {error_msg}")
            logger.error(f"[chatbot] ❌ Error inesperado al {operation_verbs.get(operation_type, 'procesar')} {model_name}")
            raise
