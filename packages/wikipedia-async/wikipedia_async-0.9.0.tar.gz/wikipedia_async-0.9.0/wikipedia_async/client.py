...
from wikipedia_async.helpers.section_helpers import SectionResult, get_summary
...
class WikipediaClient:
    ...
    async def search(
        ...
        try:
            ...
            return SearchResults(results, suggestion=suggestion_text)
    ...
    async def get_page(
        ...
        try:
            ...
            wiki_page = WikiPage(
                ...
                summary=get_summary(page_data.get("extract")),
                sections=SectionResult.from_content(page_data.get("extract", "")),
            ...
            for page_data in pages:
                try:
                    ...
                    wiki_page = WikiPage(
                        ...
                        namespace=page_data.get("ns", 0),
                        summary=get_summary(page_data.get("extract")),
                        sections=SectionResult.from_content(
    ...
    async def get_page_content(
        ...
        Args:
            title: Page title (mutually exclusive with page_id)
            page_id: Page ID (mutually exclusive with title)
            plain_text: If True, return plain text; if False, return raw wikitext
        ...
        if plain_text:
            # For plain text, use extracts API
            params = {
                "prop": "extracts",
                "explaintext": "",
            }
        else:
            # For wikitext, use revisions API
            params = {
                "prop": "revisions",
                "rvprop": "content",
                "rvlimit": 1,
    ...
    async def get_page_html(
        self, title: Optional[str] = None, page_id: Optional[int] = None
    ) -> str:
        """
