...
def download_cbp_data():
    ...
    Args:
        year (str): The year of the CBP data
    """
    print("Downloading employer size data from 2016 county file (more complete than other years)")
    cbp_dir = here("work")
...
def download_ct_puma_crosswalk(main_year):
    """Function for downloading Census Tract to PUMA crosswalk file
    
    Args:
        year (str): The year of the crosswalk file
    """
...
def geocore_files():
    ...
    dest_dir = os.path.join(script_dir, "geo")
...
def download_school_data(main_year):
    """Function for downloading school data
    
    Args:
        main_year (str): The year of the school data
    """
    school_dir = here("school")
    os.makedirs(school_dir, exist_ok=True)
    
    print(f"Downloading school data for year {main_year} to {school_dir}")
    ...
    print(f"Downloading school location data")
    response = requests.get(url, stream=True)
...
class DownloadData:
    ...
    def __init__(self, config=None, config_path=None, base_dir=None):
        """Create a downloader.
    ...
    def run_all(self):
        ...
        # Download Census Tract to PUMA crosswalk file
        # Note: Commented out because links are temporary. Need to download manually or keep in folder.
        # download_ct_puma_crosswalk(main_year)
        print("Copying geocore files to geo folder")
        geocore_files()
        ...
        download_school_data(main_year)
