...
class StageWriterHelper:
    ...
    def drop_done_file(self):
        object_name:str=f"s3://{self.bucket}/{self.base_path}/{self.source_system}/done.txt"
        empty_data = io.BytesIO(b"")
        self.storage_client.put_object(bucket_name=self.bucket,object_name=object_name,data=empty_data,length=0,content_type="text/plain")
