        ...
        try:
            ...
            if "NotAuthorizedException" in error_code:
                if "Incorrect username or password" in error_message:
                    raise UserException(HTTPStatus.BAD_REQUEST, self.__class__.__name__, "Invalid email or password")
                elif "User does not exist" in error_message:
                    raise UserException(HTTPStatus.BAD_REQUEST, self.__class__.__name__, "User not found")
            elif "UserNotFoundException" in error_code:
                raise UserException(HTTPStatus.BAD_REQUEST, self.__class__.__name__, "User not found")

            raise UserException(HTTPStatus.BAD_REQUEST, self.__class__.__name__, f"Authentication failed: {error_message}")
