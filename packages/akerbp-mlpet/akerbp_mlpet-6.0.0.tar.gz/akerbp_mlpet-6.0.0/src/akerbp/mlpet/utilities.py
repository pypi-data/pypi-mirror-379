...
def normalize_zonation_columns(
    ...
    is_unknown = (
        df[zonation_columns_to_normalize]  # noqa: PD013, PD010
        .astype(str)
        .stack()
        .str.upper()
        .str.strip()
        .str.contains(r"UNKNOWN|UNDEFINED|NO\s+FORMAL")
        .unstack()
    )

    with pd.option_context("future.no_silent_downcasting", True):
        # 2) Create a unknown-free merged_zonation column
        merged_clean_zonations = (
            df[zonation_columns_to_normalize]
            .where(~is_unknown, np.nan)
            .bfill(axis=1)
...
def estimate_parameter(
    ...
    **kwargs,
    ...
    df_query.loc[:, parameter_column_name] = np.nan
    aggregation_axis = 0
        ...
        if nearest_neighbours is not None or aggregation_method == "weighted_mean":
            ...
            if scale_coordinate_columns:
                if num_train_samples < 2:
                    ...
                    x_query = scaler.transform(zone_df[coordinate_columns].to_numpy())
            else:
                x_query = zone_df[coordinate_columns].to_numpy()
