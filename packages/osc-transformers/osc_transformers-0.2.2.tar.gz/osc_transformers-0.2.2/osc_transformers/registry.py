...
class Registry(confection.registry):
    architecture = catalogue.create(
        "osc", "transformers", "architecture", entry_points=True
    )
    attention = catalogue.create("osc", "transformers", "attention", entry_points=True)
    feedforward = catalogue.create(
        "osc", "transformers", "feedforward", entry_points=True
    )
    normalization = catalogue.create(
        "osc", "transformers", "normalization", entry_points=True
    )
