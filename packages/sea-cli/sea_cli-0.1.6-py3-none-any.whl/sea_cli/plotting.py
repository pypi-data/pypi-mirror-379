...
def plot_binary_heatmap(binary_streams, show=True, savepath=None):
    ...
    for ax, (name, stream) in zip(axes, binary_streams.items()):
        ...
        ax.imshow(stream_matrix, cmap="gray_r",
...
def plot_bit_distribution(binary_streams, show=True, savepath=None):
...
def plot_byte_distribution(binary_streams, show=True, savepath=None):
    ...
    for ax, (name, stream) in zip(axes, binary_streams.items()):
        ...
        ax.set_xticklabels(
            [f"{x:02X}" for x in range(0, 256, 16)], rotation=90)
        ax.grid()
...
def plot_entropy_summary(entropies, show=True, savepath=None):
...
def plot_randomness_statistics(randomness_stats, show=True, savepath=None):
    features = list(randomness_stats.keys())

    runs_actual = [r['runs']['runs'] for r in randomness_stats.values()]
    runs_expected = [r['runs']['expected'] for r in randomness_stats.values()]
    chi_sqs = [r['chi_square']['chi_sq'] for r in randomness_stats.values()]
...
def plot_entropy_fingerprint(entropies, show=True, savepath=None):
    labels = ['Shannon', 'Min']
