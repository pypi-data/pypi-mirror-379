...
def host_options(parser: argparse.ArgumentParser, host_required: bool, os_exists: OsExists) -> None:
    ...
    # must specify host.
    if os_exists('/opt/qumulo/lib'):
        arg_group.add_argument('--host', default='localhost')
    else:
...
def rest_client_factory(
    ...
    return RestClient(
        args.host, args.port, credentials=credentials, timeout=timeout, user_agent=user_agent
    )
