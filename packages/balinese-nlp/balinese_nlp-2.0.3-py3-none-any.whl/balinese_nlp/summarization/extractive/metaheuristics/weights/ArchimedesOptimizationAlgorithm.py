...
class ArchimedesOptimizationAlgorithm(BaseMetaSummarizerWeights):
    ...
    def solve(self):
        ...
        for idx_iteration in range(MAX_ITERATIONS):
            ...
            # 7. check konvergensi
            current_best_fitness = best_object['fitness']
            self.LIST_BEST_FITNESS.append(current_best_fitness)
            is_break, best_fitness_previous, convergence = self._check_convergence(
                current_best_fitness, best_fitness_previous, convergence, idx_iteration)
    ...
    def __update_object_position(self, agents, best_object, iteration):
        ...
        for idx_agent, agent in enumerate(agents):
            ...
            agents[idx_agent]['densities'] = agent['densities'] + np.random.random(N_DIMENSION) * \
                (best_object['densities'] - agent['densities'])
            agents[idx_agent]['volume'] = agent['volume'] + \
                np.random.random(N_DIMENSION) * \
                (best_object['volume'] - agent['volume'])

            # update transfer and density decreasing factors TF and d
            t = iteration + 1
            TF = math.exp((t - MAX_ITERATIONS) / MAX_ITERATIONS)
            d = math.exp((MAX_ITERATIONS - t) / MAX_ITERATIONS) - \
                (t / MAX_ITERATIONS)
            ...
            if TF <= EXPLORATION_RATE:
                ...
                # update position (Eq. 14) - PERBAIKAN: GUNAKAN VEKTOR ACAK
                rand_vector = np.random.random(N_DIMENSION)
                agents[idx_agent]['position'] = random_agent['position'] + C1 * rand_vector * \
                ...
                # update position (Eq. 15) - PERBAIKAN: GUNAKAN VEKTOR ACAK
                rand_vector = np.random.random(N_DIMENSION)
                T = C3 * TF
                agents[idx_agent]['position'] = best_object['position'] + F * C2 * rand_vector * \
                    agents[idx_agent]['acceleration'] * d * \
