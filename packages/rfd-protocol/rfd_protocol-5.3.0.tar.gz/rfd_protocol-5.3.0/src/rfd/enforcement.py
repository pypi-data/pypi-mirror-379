...
class WorkflowEnforcer:
    ...
    def stop_enforcement(self, feature_id: str) -> Dict[str, Any]:
        ...
        try:
            conn.execute(
                "UPDATE enforcement_status SET active = 0 WHERE feature_id = ?",
                (feature_id,)
            )
            conn.commit()
    ...
    def validate_change(self, file_path: str, feature_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            status = conn.execute(
                "SELECT active, scope_baseline FROM enforcement_status WHERE feature_id = ?",
                (feature_id,)
            ).fetchone()
            ...
            in_scope = any(
                file_p.match(pattern) or str(file_p).startswith(pattern) 
            ...
            if not in_scope:
                ...
                return {
                    "allowed": False,
                    "reason": f"File {file_path} is out of scope for feature {feature_id}"
                }
...
class ScopeDriftDetector:
    ...
    def detect_drift(self, feature_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get baseline
            status = conn.execute(
                "SELECT scope_baseline FROM enforcement_status WHERE feature_id = ?",
                (feature_id,)
            ).fetchone()
