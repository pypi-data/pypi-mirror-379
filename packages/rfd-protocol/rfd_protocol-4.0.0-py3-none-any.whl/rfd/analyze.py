        ...
        for feature in spec.get("features", []):
            ...
            if not row:
                alignment["misalignments"].append({
                    "feature": feature["id"],
                    "issue": "Feature in spec but not in database"
                })
    ...
    def _check_task_consistency(self) -> Dict[str, Any]:
        ...
        for row in cursor.fetchall():
            consistency["inconsistencies"].append({
                "feature": row["id"],
                "issue": f"Feature marked complete but has {row['total_tasks'] - row['completed_tasks']} incomplete tasks"
            })
        ...
        for row in cursor.fetchall():
            consistency["inconsistencies"].append({
                "task": row["id"],
                "issue": f"Orphaned task: {row['description']}"
            })
            consistency["status"] = "inconsistent"
        ...
        for feature in spec.get("features", []):
            ...
            for test_file in test_files:
                try:
                    ...
                    if feature["id"].lower() in content or \
                       any(term.lower() in content for term in acceptance.split()[:3]):
                        found = True
                        break
                except:
                    continue
            ...
            if found:
                ...
                coverage["untested_criteria"].append({
                    "feature": feature["id"],
        ...
        for principle in principles:
            if "no mock" in principle["principle"].lower():
                ...
                for pattern in ["**/*.py", "**/*.js"]:
                    files = [f for f in self.project_root.glob(pattern) 
                ...
                for j in range(i):
                    if phases[j]["status"] not in ["complete"]:
                        dependencies["violations"].append({
                            "phase": phase["name"],
                            "issue": f"Started before completing {phases[j]['name']}"
                        })
