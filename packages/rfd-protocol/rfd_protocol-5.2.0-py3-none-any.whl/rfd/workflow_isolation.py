...
class WorkflowIsolation:
    ...
    def create_isolated_worktree(self, feature_id: str, agent_type: str = "coding") -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run([
                ...
                str(worktree_path), 
                "-b", branch_name
            ], cwd=str(self.rfd.rfd_dir.parent), capture_output=True, text=True)
            ...
            if worktree_path.exists():
                subprocess.run(["git", "worktree", "remove", str(worktree_path)], 
                             capture_output=True)
            raise RuntimeError(f"Failed to create isolated worktree: {e}")
    ...
    def get_session_worktree(self, session_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            session = conn.execute(
                "SELECT feature_id FROM sessions WHERE id = ?", 
                (session_id,)
            ).fetchone()
    ...
    def cleanup_worktree(self, worktree_id: int) -> bool:
        ...
        try:
            # Get worktree info
            worktree = conn.execute(
                "SELECT worktree_path, branch_name FROM git_worktrees WHERE id = ?",
                (worktree_id,)
            ).fetchone()
