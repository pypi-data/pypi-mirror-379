  ...
  ($name : ident, $class:ty) => {
    ...
    impl $name {
      ...
      fn add_crfile(&mut self, filename : String, source : String) {
        warn!("Hic sunt dracones - this might be buggy!");
        ...
        for frame in reader {
          if frame.has(address) {
            ...
            if source == "TofPacket" {
              let source_string = String::from("TofPacket") + stringify!($class);
              let moni_res = frame.get::<TofPacket>(&source_string).unwrap().unpack::<$class>();
              match moni_res {
                Err(err) => {
            ...
            if source == "TelemetryPacket" || try_from_telly {
              let source_string = "TelemetryPacketType::AnyTofHK"; 
              let hk_res = frame.get::<TelemetryPacket>(&source_string.clone());
              match hk_res {
                Err(err) => {
                  println!("Error unpacking! {err}");
                }
