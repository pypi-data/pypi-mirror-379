...
class orderclass:
    ...
    def dynamic_sendorder(self, dforderplan = None, timestart = None, timeend =None ):
        ...
        for index, orderrow in self.orderplan.iterrows():
            ...
            timedaly0 = float(( timeend_nano_e  - timestart_nano_s) /1000000000 )
            if timedaly0<-10:
                self.sendmailto(body='',
                                subject="%s 重大错误，剩余时间不足，请手动查看账户" % datetime.now().strftime(
                                    '%Y%m%d %H:%M:%S'))
    ...
    def tradeorder(self, timestart = None, timeend = None , delaydt = 60):
        ...
        if len(self.unfinishorder) == 0:
            ...
            self.sendmailto(body='', subject="%s 重大错误，可能存在未成交订单，请手动查看账户" % datetime.now().strftime('%Y%m%d %H:%M:%S'))
