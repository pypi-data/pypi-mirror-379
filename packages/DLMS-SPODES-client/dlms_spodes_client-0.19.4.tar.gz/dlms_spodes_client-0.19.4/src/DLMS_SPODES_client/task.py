...
@dataclass
class ClientBlocking(SimpleCopy, Base):
...
class Sequence[T: result.Result, U: Base[result.Result]](Subtasks[U], List[T]):
    ...
    def __init__(self, *tasks: Base[T], msg: str = None, err_ignore: bool = True):
...
@dataclass
class ReadAttributes(SimpleCopy, List[cdt.CommonDataType]):
    ln: collection.LNContaining
    indexes: tuple[int, ...]
    msg: str = ""
...
@dataclass
class WriteParDatas(SimpleCopy, List[result.Ok]):
