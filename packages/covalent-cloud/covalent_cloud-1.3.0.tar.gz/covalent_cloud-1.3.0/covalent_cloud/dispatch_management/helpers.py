...
def _upload_asset(local_uri: str, remote_uri: str, task: TaskID, progress: Progress) -> None:
    ...
    # Get auth headers using existing APIClient pattern
    dummy_client = APIClient("http://dummy", settings=settings)
    auth_headers = dummy_client.get_global_headers()
    ...
    try:
        if os.path.getsize(local_path) == 0:
            # Include auth headers along with Content-Length for empty uploads
            headers = {**auth_headers, "Content-Length": "0"}
            r = session.put(remote_uri, headers=headers, data="")
            r.raise_for_status()
        else:
            with open(local_path, "rb") as f:
                r = session.put(remote_uri, headers=auth_headers, data=f)
                r.raise_for_status()
    except Exception:
        raise
