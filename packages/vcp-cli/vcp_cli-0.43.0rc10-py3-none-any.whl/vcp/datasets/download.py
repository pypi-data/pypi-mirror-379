...
from .api import Location, LocationModel, get_credentials_for_datasets
...
def s3_stream_batch(
    ...
    """
    # create the s3 client (as S3Client)
...
def download_locations(
    ...
    for loc in locations:
        ...
        path, scheme = (
            ...
            else (str(loc), urlparse(loc).scheme)
        )

        if scheme in ("http", "https", "cellxgene"):
            http_links.append(path)
        elif scheme in ("s3"):
            s3_links.append(path)
        else:
            console.print(f"⚠️  unknown scheme {scheme}: {path}")

    if not http_links and not s3_links:
        console.print("⚠️  no downloadable links found.")
        return
    ...
    def download_single_dataset(dataset_id: str) -> tuple:
        ...
        try:
            ...
            if has_s3_locations:
                ...
                dataset_creds = S3Credentials.model_construct(**actual_creds)
