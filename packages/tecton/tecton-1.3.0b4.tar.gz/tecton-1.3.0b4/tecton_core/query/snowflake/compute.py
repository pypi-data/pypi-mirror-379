...
def _get_snowflake_auth_config(
    ...
    for source in sources:
        if source.user:
            ...
            break
        elif source.connection_provider:
            connection_provider = source.connection_provider
            break

    if connection_provider:
        return SnowflakeAuthConfig(connection_provider=connection_provider)
...
def create_snowflake_connection(
    ...
    if auth_config.connection_provider:
        snowflake_connection = auth_config.connection_provider()
        if not isinstance(snowflake_connection, snowflake.connector.SnowflakeConnection):
            msg = "Snowflake `connection_provider` function must return a snowflake connection"
            raise TectonValidationError(msg, can_drop_traceback=True)
