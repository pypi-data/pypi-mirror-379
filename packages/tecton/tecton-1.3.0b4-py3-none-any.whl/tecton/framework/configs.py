...
class SnowflakeConfig(BaseBatchConfig):
    ...
    def __init__(
        ...
        *,
        ...
        connection_provider: Optional[Callable[[], "snowflake.connector.SnowflakeConnection"]] = None,
    ):
        """
        ...
        :param connection_provider: (Only supported in Rift) A function that returns a Snowflake connection which Tecton will use to connect to Snowflake. This is useful for cases where you need to use custom logic to connect to Snowflake, and using Tecton's default connection parameters is not sufficient. This function must return a `snowflake.connector.SnowflakeConnection` object.
        ...
        if post_processor is not None and function_serialization.should_serialize_function(post_processor):
            args.common_args.post_processor.CopyFrom(function_serialization.to_proto(post_processor))
        if connection_provider is not None and function_serialization.should_serialize_function(connection_provider):
            args.connection_provider.CopyFrom(function_serialization.to_proto(connection_provider))
        if timestamp_field:
            args.common_args.timestamp_field = timestamp_field
        ...
        self._data_delay = data_delay
        self.post_processor = post_processor
        self.data_source_auth_function = connection_provider
