...
class MarkdownProcessor:
    ...
    def _extract_document_title(self, markdown_text: str) -> Optional[str]:
        ...
        优先级策略:
        1. 如果文档开头（前5行非空内容）有一级标题，使用它作为主标题
        2. 如果文档开头没有一级标题，但整个文档只有一个一级标题，使用它
        3. 如果有多个一级标题，使用第一个作为主标题，其他标题作为章节标题
        ...
        # 如果文档开头（前5行非空内容）有一级标题，优先使用
        for i, title in zip(h1_positions, h1_titles):
            if i < 5:  # 前5行非空内容
                return title
    ...
    def _extract_all_document_titles(self, markdown_text: str) -> List[str]:
        """
        提取文档中所有的一级标题（排除代码块中的内容）
    ...
    def parse_markdown_text(self, markdown_text: str) -> List[BaseNode]:
        ...
        all_titles = self._extract_all_document_titles(markdown_text)
        ...
        if self.include_metadata:
            for node in markdown_nodes:
                ...
                # 添加所有一级标题信息
                if all_titles:
                    node.metadata['all_h1_titles'] = all_titles
                    node.metadata['h1_count'] = len(all_titles)
            ...
            if len(md_node.get_content()) > self.chunk_size:
                ...
                for sub_node in sub_nodes:
                    ...
                    if self.include_metadata:
                        ...
                        if all_titles:
                            sub_node.metadata['all_h1_titles'] = all_titles
                            sub_node.metadata['h1_count'] = len(all_titles)
