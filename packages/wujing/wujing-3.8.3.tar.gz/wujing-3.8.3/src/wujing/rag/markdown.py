    ...
    def parse_markdown_text(self, markdown_text: str) -> List[BaseNode]:
        ...
        if self.include_metadata:
            for node in markdown_nodes:
                ...
                # 添加当前片段标题
                section_title = self._extract_section_title(node.get_content())
                if section_title:
            ...
            if len(md_node.get_content()) > self.chunk_size:
                ...
                for sub_node in sub_nodes:
                    ...
                    if self.include_metadata:
                        ...
                        # 为子节点添加片段标题（优先使用子节点自己的标题，如果没有则使用父节点的）
                        sub_section_title = self._extract_section_title(sub_node.get_content())
                        if sub_section_title:
                            sub_node.metadata['section_title'] = sub_section_title
                        elif hasattr(md_node, 'metadata') and md_node.metadata and 'section_title' in md_node.metadata:
                            sub_node.metadata['section_title'] = md_node.metadata['section_title']
