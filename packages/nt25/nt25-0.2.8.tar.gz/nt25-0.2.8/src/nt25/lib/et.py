...
def _shrink(file, output, maxWidth=None, merge=True):
  shrink = False
  ...
  # print(f"shrink: {shrink} ({times:.1%})")
  return shrink


def shrinkFile(
  file, optimizeWidth=OPTIMIZE_WIDTH, thumbnailWidth=THUMBNAIL_WIDTH, override=True
):
  name, ext = os.path.splitext(file)
  result = False

  if override:
    result = _shrink(file, file)
  else:
    result = _shrink(file, name + "-new" + ext)

  _shrink(file, name + "-o" + ext, maxWidth=optimizeWidth, merge=False)
  _shrink(file, name + "-thumbnail" + ext, maxWidth=thumbnailWidth, merge=False)

  return result


def main():
  parser = argparse.ArgumentParser(description="EXIF tool with pillow")
  ...
  if args.file:
      ...
      for d, _, f in os.walk(args.shrink):
        for file in f:
          ...
          if e.lower() in IMAGE_EXT:
            ...
            if shrinkFile(path, override=args.override):
              shrink += 1

      result = {"total": total, "shrink": shrink, "override": args.override}

    elif os.path.isfile(args.shrink):
      result = {
        "result": shrinkFile(args.shrink, override=args.override),
    ...
    print("usage: et [-h] [-v] [-f FILE] [[-o] -s FILE]")
    return
