...
def data_package_search():
    ...
    # Gets data packages incorrectly uploaded by iTAK
    mission_packages: List[MissionContent] = db.session.execute(db.select(MissionContent)).scalars()
    for dp in mission_packages:
        if dp.mission is None:
            res['results'].append(
                {'UID': dp.hash, 'Name': dp.filename, 'Hash': dp.hash, 'CreatorUid': dp.creator_uid,
                 "SubmissionDateTime": dp.submission_time.strftime('%Y-%m-%dT%H:%M:%S.000Z'), "EXPIRATION": "-1",
                 "Keywords": ["missionpackage"],
                 "MIMEType": dp.mime_type, "Size": "{}".format(dp.size), "SubmissionUser": dp.submitter,
                 "PrimaryKey": "{}".format(dp.id),
                 "Tool": dp.tool if dp.tool else "public"
                 })
            res['resultCount'] += 1
