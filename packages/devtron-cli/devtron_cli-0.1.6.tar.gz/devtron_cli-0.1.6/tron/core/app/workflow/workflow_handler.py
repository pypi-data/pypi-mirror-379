...
class Workflow:
                ...
                for ci in workflows_to_update:
                    for wf in config_data.get("workflows", []):
                        if wf.get("ci_pipeline", {}).get("name") == ci:
                            ...
                            update_result = self.update_workflow(wf, app_id, ci_pipeline_id)
    ...
    def update_workflow(self, w, app_id, ci_pipeline_id):
        try:
            # Update CI pipeline
            ...
            if pipeline_plugin_ids:
                plugin_metadata_response = self.DevtronUtils.get_plugin_details_by_id(pipeline_plugin_ids, app_id)
            ...
            updated_ci_pipeline = CiPipelineHandlers.update_current_ci_pipeline(self.base_url, self.headers, current_ci_pipeline, ci_config, plugin_metadata)
                ...
                if get_cd_pipelines.status_code == 200:
                    ...
                    for cd_config in cd_pipelines_config:
                        ...
                        if environment_name in existing_cd_by_env:
                            ...
                            if deployment_strategies_config:
                                ...
                                if strategies_differ:
                                    ...
                                    updated_pipeline_data["strategies"] = self._build_updated_strategies(existing_strategies, deployment_strategies_config)
    ...
    def _compare_deployment_strategies(self, existing_strategies: list, config_strategies: list) -> bool:
        ...
        try:
            ...
            config_strategy_map = {strategy.get("name", "ROLLING").upper(): strategy for strategy in config_strategies}
            ...
            for template_name, config_strategy in config_strategy_map.items():
                if template_name in existing_strategy_map:
                    ...
                    if "deployment" in existing_config and "strategy" in existing_config["deployment"]:
                        ...
                        strategy_key = template_name.lower()
                        
                        # Handle special cases like BLUE-GREEN
                        if template_name == "BLUE-GREEN":
                            strategy_key = "blueGreen"
                        elif template_name == "RECREATE":
                            strategy_key = "recreate"
                        elif template_name == "CANARY":
                            strategy_key = "canary"
    ...
    def _build_updated_strategies(self, existing_strategies: list, config_strategies: list) -> list:
        """
        ...
        for template_name, existing_strategy in existing_strategy_map.items():
            updated_strategy = existing_strategy.copy()
            
            # If there's a config for this strategy, update it
            if template_name in config_strategy_map:
                config_strategy = config_strategy_map[template_name]
                config_strategy_details = config_strategy.get("strategy", {})
