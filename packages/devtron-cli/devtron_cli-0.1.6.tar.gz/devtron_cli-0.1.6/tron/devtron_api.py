                                            ...
                                            if env_level_cm_cs_list.get('cm_list'):
                                                for cm_name in env_level_cm_cs_list['cm_list']:
                                                    ...
                                                    if env_cm_details_result['success']:
                                                        cm_type = env_cm_details_result.get('config_map').get('type')
                                                        cm_config_type = 'override' if env_cm_details_result.get('config_map').get('overridden') else 'env-level' # please check for cm created at env-leve
                                                        cm_merge_strategy = env_cm_details_result.get('config_map').get('mergeStrategy')
                                                        is_external = env_cm_details_result.get('config_map').get('external')
                                                        cm_override_template_yaml = {} 
                                                        if cm_merge_strategy == 'patch' and not is_external:
                                                            cm_override_template_yaml = env_cm_details_result.get('config_map').get('patchData')
                                                        elif cm_merge_strategy == 'replace' and not is_external:
                                                            cm_override_template_yaml = env_cm_details_result.get('config_map').get('data')
                                                        else:
                                                            cm_override_template_yaml = env_cm_details_result.get('config_map').get('data')

                                                        # Create filename based on app name and env name
                                                        cm_override_values_filename = f"override-cm-{cm_name}-{app_name}-{env_name}-values.yaml"
                                                        
                                                        # Write the values to the file
                                                        with open(cm_override_values_filename, 'w') as f:
                                                            yaml.dump(cm_override_template_yaml, f, default_flow_style=False, allow_unicode=True)
                                                        if cm_type == 'environment':
                                                            cm_config = {
                                                                "name": cm_name,
                                                                "config_type": cm_config_type,
                                                                "type": "environment",
                                                                "merge_strategy" : cm_merge_strategy,
                                                                "external": is_external,
                                                                "from_file": cm_override_values_filename
                                                            }
                                                        elif cm_type == 'volume':
                                                            cm_config = {
                                                                "name": cm_name,
                                                                "config_type": cm_config_type,
                                                                "type": "volume",
                                                                "mountPath": env_cm_details_result.get('config_map').get('mountPath'),
                                                                "filePermission": env_cm_details_result.get('config_map').get('filePermission'),
                                                                "subPath": env_cm_details_result.get('config_map').get('subPath'),
                                                                "merge_strategy" : cm_merge_strategy,
                                                                "external": is_external,
                                                                "from_file": cm_override_values_filename
                                                            }
                                                        env_configuration['config_maps'].append(cm_config)
                                    except Exception as e:
                                        print(f"Warning: Could not fetch Env level config maps and secrets: {cm_name} {str(e)}")
