...
class BotsLogger:
    ...
    def _log(self, level: str, message: str, context: dict):
        ...
        timestamp_sp = datetime.now(datetime.UTC).astimezone(SAO_PAULO_TZ)
    ...
    def _classify_error(self, error: Exception, error_type: str, context: str):
        ...
        if "Problemas no login" in context:
            return "CRITICAL", "stop"
        ...
        if error_type.upper() == "STRUCTURAL" or isinstance(error, (KeyError, TypeError, AttributeError, NameError)):
            return "WARNING", "log_and_stop" # Não adianta tentar de novo

        # Erros de Retorno de API (ex: status code 5xx)
        if error_type.upper() == "RETORNO_API":
            # Supondo que o erro tenha um atributo 'response'
            status_code = getattr(getattr(error, 'response', None), 'status_code', 0)
            if 500 <= status_code < 600:
                return "WARNING", "retry" # O servidor pode estar temporariamente fora
