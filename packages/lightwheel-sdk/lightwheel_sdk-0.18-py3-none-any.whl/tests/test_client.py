...
class TestLightwheelClient:
    ...
    def test_update_header(self):
        ...
        client = LightwheelClient()
        client.update_header("Authorization", "Bearer token")
    ...
    def test_post_success(self, mock_requests):
        ...
        mock_post.assert_called_once_with("https://api.lightwheel.net/test/path", headers={}, json={"key": "value"}, timeout=10)
    ...
    def test_post_with_custom_timeout(self, mock_requests):
        ...
        mock_post.assert_called_once_with("https://api.lightwheel.net/test/path", headers={}, json={"key": "value"}, timeout=30)
    ...
    def test_post_401_error(self, mock_requests):
        ...
        assert "for url path: /test/path" in str(exc_info.value)
    ...
    def test_login_from_cache(self, temp_cache_dir):
        ...
        cache_data = {"username": "cached_user", "password": "cached_pass", "headers": {"Authorization": "Bearer cached_token", "UserName": "cached_user"}}
    ...
    def test_get_login_headers_from_cache(self, temp_cache_dir):
        """Test getting login headers from cache."""
        cache_data = {"headers": {"Authorization": "Bearer cached_token", "UserName": "cached_user"}}
        ...
        headers = client.get_login_headers()

        assert headers["Authorization"] == "Bearer cached_token"
        assert headers["UserName"] == "cached_user"
        assert client.headers["Authorization"] == "Bearer cached_token"
        assert client.headers["UserName"] == "cached_user"

    def test_get_login_headers_from_memory(self):
        """Test getting login headers from memory."""
        client = LightwheelClient()
        client.headers = {"Authorization": "Bearer memory_token", "UserName": "memory_user"}

        headers = client.get_login_headers()

        assert headers["Authorization"] == "Bearer memory_token"
        assert headers["UserName"] == "memory_user"
