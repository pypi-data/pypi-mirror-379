...
class TestObjectLoader:
    ...
    def test_list_registry(self, mock_client, mock_requests):
    ...
    def test_acquire_object_deprecated(self, mock_client, mock_requests):
        """Test acquire_object (deprecated method)."""
        mock_post, mock_get = mock_requests

        # Mock the API response
        mock_response = Mock()
        mock_response.json.return_value = {"fileUrl": "https://s3.amazonaws.com/test/file.zip", "objectName": "bowl008", "fileVersionId": "version_123"}
        mock_post.return_value = mock_response
        ...
        with patch("zipfile.ZipFile"), patch("builtins.open", mock_open()), patch("pathlib.Path.exists", return_value=True), patch("pathlib.Path.mkdir"):
            ...
            result = loader.acquire_object("objects/lightwheel/bowl/Bowl008", "USD")

        # Should return the USD path
        assert isinstance(result, str)
        assert result.endswith(".usd")

    def test_acquire_object_with_version(self, mock_client, mock_requests):
        """Test acquire_object with version parameter."""
        mock_post, mock_get = mock_requests
        ...
        with patch("zipfile.ZipFile"), patch("builtins.open", mock_open()), patch("pathlib.Path.exists", return_value=True), patch("pathlib.Path.mkdir"):
            ...
            result = loader.acquire_object("objects/lightwheel/bowl/Bowl008", "USD", version="version_123")

        assert isinstance(result, str)
        assert result.endswith(".usd")

    def test_acquire_object_invalid_path_length(self, mock_client):
        """Test acquire_object with invalid path length."""
        loader = ObjectLoader(mock_client)

        with pytest.raises(ValueError) as exc_info:
            loader._acquire_object("a/b/c/d/e/f/g/h", "USD")  # pylint: disable=protected-access

        assert "Invalid levels number: 8" in str(exc_info.value)

    def test_acquire_object_empty_path(self, mock_client):
        """Test acquire_object with empty path."""
        loader = ObjectLoader(mock_client)

        with pytest.raises(ValueError) as exc_info:
            loader._acquire_object("", "USD")  # pylint: disable=protected-access

        assert "Invalid levels number: 0" in str(exc_info.value)

    def test_acquire_object_invalid_file_type(self, mock_client):
        """Test acquire_object with invalid file type."""
        loader = ObjectLoader(mock_client)

        with pytest.raises(ValueError) as exc_info:
            loader._acquire_object("objects/lightwheel/bowl/Bowl008", "INVALID")  # pylint: disable=protected-access
