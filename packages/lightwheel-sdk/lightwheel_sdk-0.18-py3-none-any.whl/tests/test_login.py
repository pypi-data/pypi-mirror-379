...
class TestLogin:
    ...
    def test_init(self, mock_client):
        ...
        mock_client.get_login_headers.return_value = {"Authorization": "Bearer token"}

        login = Login(mock_client)

        assert login.client == mock_client
        mock_client.get_login_headers.assert_called_once()
    ...
    def test_get_headers(self, mock_client):
        """Test getting login headers."""
        expected_headers = {"Authorization": "Bearer token", "UserName": "testuser"}
        mock_client.get_login_headers.return_value = expected_headers

        login = Login(mock_client)
        result = login.get_headers()
    ...
    def test_get_headers_with_client_exception(self, mock_client):
        """Test get_headers when client raises an exception."""
        mock_client.get_login_headers.side_effect = Exception("Headers failed")
        ...
        assert str(exc_info.value) == "Headers failed"
        mock_client.get_login_headers.assert_called_once()
