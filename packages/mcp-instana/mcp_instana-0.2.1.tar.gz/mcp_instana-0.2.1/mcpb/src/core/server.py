...
try:
    ...
    try:
        # For MCPB bundle, try alternative import path
        from src.core.utils import MCP_TOOLS, register_as_tool
    except ImportError:
...
def import_module_with_fallback(module_path):
    """Import a module with fallback for MCPB bundle structure"""
    try:
        return __import__(module_path, fromlist=[''])
    except ImportError:
        # Try MCPB bundle path
        mcpb_path = module_path.replace('src.', 'mcp_instana.')
        return __import__(mcpb_path, fromlist=[''])
...
def get_client_categories():
    ...
    try:
        # Try src imports first, then fallback to mcp_instana for MCPB bundle
        try:
