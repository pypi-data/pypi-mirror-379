...
class IterativeSolverController(object):
    r"""Proxy of C++ mfem::IterativeSolverController class."""

    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self):
        r"""__init__(IterativeSolverController self) -> IterativeSolverController"""
        if self.__class__ == IterativeSolverController:
            _self = None
        else:
            _self = self
        _solvers.IterativeSolverController_swiginit(self, _solvers.new_IterativeSolverController(_self, ))
    __swig_destroy__ = _solvers.delete_IterativeSolverController

    def HasConverged(self):
        r"""HasConverged(IterativeSolverController self) -> bool"""
        return _solvers.IterativeSolverController_HasConverged(self)
    HasConverged = _swig_new_instance_method(_solvers.IterativeSolverController_HasConverged)

    def Reset(self):
        r"""Reset(IterativeSolverController self)"""
        return _solvers.IterativeSolverController_Reset(self)
    Reset = _swig_new_instance_method(_solvers.IterativeSolverController_Reset)

    def MonitorResidual(self, it, norm, r, final):
        r"""MonitorResidual(IterativeSolverController self, int it, mfem::real_t norm, Vector r, bool final)"""
        return _solvers.IterativeSolverController_MonitorResidual(self, it, norm, r, final)
    MonitorResidual = _swig_new_instance_method(_solvers.IterativeSolverController_MonitorResidual)

    def MonitorSolution(self, it, norm, x, final):
        r"""MonitorSolution(IterativeSolverController self, int it, mfem::real_t norm, Vector x, bool final)"""
        return _solvers.IterativeSolverController_MonitorSolution(self, it, norm, x, final)
    MonitorSolution = _swig_new_instance_method(_solvers.IterativeSolverController_MonitorSolution)

    def SetIterativeSolver(self, solver):
        r"""SetIterativeSolver(IterativeSolverController self, IterativeSolver solver)"""
        return _solvers.IterativeSolverController_SetIterativeSolver(self, solver)
    SetIterativeSolver = _swig_new_instance_method(_solvers.IterativeSolverController_SetIterativeSolver)
    def __disown__(self):
        self.this.disown()
        _solvers.disown_IterativeSolverController(self)
        return weakref.proxy(self)

# Register IterativeSolverController in _solvers:
_solvers.IterativeSolverController_swigregister(IterativeSolverController)
class IterativeSolver(mfem._ser.operators.Solver):
    ...
    def SetMonitor(self, m):
        r"""SetMonitor(IterativeSolver self, IterativeSolverController m)"""
...
class OperatorChebyshevSmoother(mfem._ser.operators.Solver):
    ...
    def __init__(self, *args):
        ...
        __init__(OperatorChebyshevSmoother self, Operator oper_, Vector d, intArray ess_tdof_list, int order, mfem::real_t max_eig_estimate) -> OperatorChebyshevSmoother
        __init__(OperatorChebyshevSmoother self, Operator oper_, Vector d, intArray ess_tdof_list, int order, int power_iterations=10, mfem::real_t power_tolerance=1e-8, int power_seed=12345) -> OperatorChebyshevSmoother
...
class ResidualBCMonitor(IterativeSolverController):
