...
def validate_analysis(data: dict) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if data["schema_version"] != "analysis.v1.1":
            return False, [
                f"Schema version mismatch: expected 'analysis.v1.1', got '{data['schema_version']}'"
            ]
        ...
        for error in validator.iter_errors(data):
            ...
            if error.validator == "required":
                ...
                errors.append(
                    f"Invalid value at {path}: expected one of {error.validator_value}, got {error.instance}"
                )
            elif error.validator == "type":
                errors.append(
                    f"Wrong type at {path}: expected {error.validator_value}, got {type(error.instance).__name__}"
                )
            elif error.validator == "minimum":
                errors.append(
                    f"Value at {path} too small: minimum {error.validator_value}, got {error.instance}"
                )
            elif error.validator == "maximum":
                errors.append(
                    f"Value at {path} too large: maximum {error.validator_value}, got {error.instance}"
                )
            else:
...
def coerce_answer(
    value: str, question_type: str, enum_choices: List[str] = None
) -> Tuple[Any, bool]:
    """
