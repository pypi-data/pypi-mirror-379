...
class DatasetScanner:
    ...
    def scan(
        ...
        logger.info(
            f"Scanning {len(sampled_files)}/{total_files} files "
        ...
        signals = {
            ...
            "license": {"summary": {"spdx": {}, "noassertion": 0}, "flags": []},
            "pii": {
                "coverage": {
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for domain in domains:
                ...
                future = executor.submit(
                    self._check_domain, domain, registry, no_network, network_tracker
                )
        ...
        if self.config.pii_mode != "off":
            pii_findings = self.pii_detector.scan_files(
                sampled_files, budget_seconds * 0.1
            )
            ...
            if origin:
                ...
                if len(signals["pdf"]["examples"]) < 5:
                    signals["pdf"]["examples"].append(
                        {"file": str(pdf.relative_to(dataset_path)), "origin": origin}
                    )
    ...
    def _extract_domains(
        self, files: List[Path], budget: float, start_time: float
    ) -> Set[str]:
