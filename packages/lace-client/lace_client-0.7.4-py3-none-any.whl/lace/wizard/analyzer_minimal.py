...
class MinimalAnalyzer:
    ...
    def _analyze_impl(
        self, dataset_paths: List[str], profile: str, send_domains: str
    ) -> Dict[str, Any]:
        """
        ...
        all_domains = (
            Counter() if profile == "enhanced" and send_domains != "none" else None
        )
        ...
        for i, path_str in enumerate(dataset_paths):
            ...
            datasets.append(
                {
                    ...
                    "bytes": byte_total,
                    "mtime_latest": (
                        latest_mtime.isoformat() + "Z" if latest_mtime else None
                    ),
                }
            )
            ...
            if (
                profile == "enhanced"
                and send_domains != "none"
                and all_domains is not None
            ):
        ...
        result = {
            "schema_version": (
                "analysis.enhanced.v1" if profile == "enhanced" else "analysis.min.v1"
            ),
            "datasets": datasets,
    ...
    def _analyze_path(self, path: Path) -> Tuple[int, int, Optional[datetime], Counter]:
        ...
        if root_laceignore.exists():
            ...
            logger.debug(
                f"Loaded {len(root_ignore_patterns)} patterns from root .laceignore"
            )
        ...
        try:
            ...
            for root, dirs, files in os.walk(path, followlinks=False):
                ...
                if root_ignore_patterns:
                    ...
                    dirs[:] = self._filter_by_patterns(
                        dirs, root_ignore_patterns, is_dir=True
                    )
            ...
            assert (
                sum(extensions.values()) == file_count
            ), f"Extension count mismatch: {sum(extensions.values())} != {file_count}"
    ...
    def _apply_k_anonymity(
        self, extensions: Counter, k: int = 3, top_n: int = 15
    ) -> Dict[str, int]:
        """
        ...
        if total_before > 0 and total_after != total_before:
            logger.warning(
                f"Mass not conserved in k-anonymity: {total_before} → {total_after}"
            )
    ...
    def _generate_evidence_files(self, dataset_paths: List[str]) -> Dict[str, str]:
        ...
        return {"top_domains_csv": "./evidence/eu_sds_top_domains.csv"}
    ...
    def _filter_by_patterns(
        self, names: List[str], patterns: List[str], is_dir: bool = False
    ) -> List[str]:
