...
class PrivacyGuard:
    ...
    def redact_pii(cls, text: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            logger.warning(
                f"PII redaction error (failing closed): {e.__class__.__name__}"
            )
            return "", False
...
class DatasetAnalyzer:
    ...
    def __init__(self, allow_external_ai: bool = False):
        ...
        logger.info(
            f"DatasetAnalyzer initialized (external_ai={'ENABLED' if allow_external_ai else 'DISABLED'})"
        )

    def analyze_dataset(
        self, dataset_path: str, sample_rate: float = DEFAULT_SAMPLE_RATE
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Starting analysis of {PrivacyGuard.sanitize_for_logging(dataset_path)}"
        )
    ...
    def _analyze_local(self, dataset_path: str, sample_rate: float) -> Dict[str, Any]:
        ...
        provenance_summary = self._calculate_provenance_summary(
            domain_analysis.get("top_10_percent_domains", [])
        )
    ...
    def _enhance_with_ai(
        self, dataset_path: str, sample_rate: float
        ...
        try:
            ...
            for sample in samples:
                ...
                if not is_safe:
                    logger.info(
                        "Sample failed safety check - falling back to local only"
                    )
                    return None
            ...
            if total_tokens > MAX_EXTERNAL_TOKENS:
                logger.info(
                    f"Token limit exceeded ({total_tokens} > {MAX_EXTERNAL_TOKENS}) - truncating"
                )
                redacted_samples = self._truncate_to_token_limit(
                    redacted_samples, MAX_EXTERNAL_TOKENS
                )
            ...
            logger.warning(
                f"AI enhancement failed (falling back to local): {e.__class__.__name__}"
            )
            return None
    ...
    def _extract_dates(self, files: List[Path]) -> Dict[str, Any]:
        ...
        if dates:
            return {
                "values": {"min_year": min(dates), "max_year": max(dates)},
                "confidence": 0.80,
                "source": "automated",
            }

        return {"values": "unknown", "confidence": 0.0, "source": "automated"}
    ...
    def _create_safe_samples(self, dataset_path: str, sample_rate: float) -> List[str]:
        ...
        path = Path(dataset_path)
        files = self._get_files(path)
        sampled = self._sample_files(
            files, min(sample_rate, 0.01)
        )  # Max 1% for external
    ...
    def _truncate_to_token_limit(
        self, samples: List[str], max_tokens: int
    ) -> List[str]:
    ...
    def _create_fingerprint(
        ...
        structure_str = (
            f"{path.name}_{path.stat().st_size if path.is_file() else 'dir'}"
        )
    ...
    def _calculate_top_domains_accurate(self, path: Path) -> Dict[str, Any]:
        ...
        """
        logger.info(
            "Starting accurate domain calculation (full pass for EU compliance)"
        )
        ...
        for file_idx, file in enumerate(files):
            if file_idx % 1000 == 0 and file_idx > 0:
                logger.info(
                    f"Domain calculation progress: {file_idx}/{total_files} files processed"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Error processing {file} for domains: {e.__class__.__name__}"
                )
                continue

        logger.info(
            f"Processed {files_processed} files, found {len(domain_bytes)} unique domains"
        )
        ...
        coverage_percentage = (
            (top_domains_bytes / total_bytes * 100) if total_bytes > 0 else 0
        )
    ...
    def _detect_dataset_origin(self, path: Path) -> str:
        ...
        if (
            "huggingface" in path_str
            or "hf_" in path_str
            or path.name.startswith("tatsu-")
        ):
            return "huggingface"
            ...
            if any(
                (path / f).exists() for f in ["README.md", "LICENSE", "dataset_card.md"]
            ):
                return "local"
        elif path.is_file():
    ...
    def _extract_dataset_locator(self, path: Path, origin: str) -> str:
        ...
        for filename in ["README.md", "dataset_card.md", "DATASET_CARD.md"]:
            ...
            if card_path.exists():
                try:
                    content = card_path.read_text(encoding="utf-8", errors="ignore")[
                        :5000
                    ]
                    ...
                    # Look for GitHub URL
                    gh_match = re.search(
                        r"github\.com/([a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+)", content
                    )
                    if gh_match:
    ...
    def _detect_license_enhanced(self, path: Path) -> Dict[str, Any]:
        ...
        return {"spdx_id": "UNKNOWN", "source": "none", "confidence": 0.0}

    def _detect_languages_with_percentages(
                ...
                with open(file, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    for lang, (start, end, stop_words) in language_patterns.items():
                        if stop_words:
                            ...
                            char_count = sum(
                                1 for c in content if start <= ord(c) <= end
                            )
                            if char_count > 10:
        ...
        if total > 0:
            for lang, count in language_counts.most_common(5):
                ...
                if pct > 0.05:  # Only include if >5%
                    results.append({"code": lang, "pct": pct})
    ...
    def _get_temporal_coverage(self, path: Path) -> Dict[str, Any]:
        ...
        return {"earliest_mtime": None, "latest_mtime": None}
    ...
    def _find_dataset_card(self, path: Path) -> Dict[str, Any]:
        ...
        return {"present": False, "path": None, "title": None}
    ...
    def _estimate_lines_and_tokens(self, files: List[Path]) -> Dict[str, int]:
        ...
        return {"total_lines": total_lines, "est_tokens": est_tokens}
    ...
    def _calculate_provenance_summary(self, domains: List[str]) -> Dict[str, float]:
        ...
        categories = {"github": 0, "edu": 0, "org": 0, "com": 0, "gov": 0, "other": 0}
