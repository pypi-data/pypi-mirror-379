...
def visualize_neuron_state(mobius_activation, point, num_sphere_points=500):
    ...
    k_proxy = mobius_activation.realities[0]['k']
...
def visualize_transformation_flow(
    ...
    if activation_layer.learnable:
        if framework == 'torch':
            final_k = [p.detach().numpy() for p in activation_layer.k_params]
            final_w = [p.detach().numpy() for p in activation_layer.w_params]
        elif framework == 'tensorflow':
            final_k = [p.numpy() for p in activation_layer.k_params]
            final_w = [p.numpy() for p in activation_layer.w_params]
        final_axes = activation_layer.axes
    else: # Handle fixed realities
        final_k = [np.array([r['k']]) for r in activation_layer.fixed_realities]
        final_w = [np.array([r['w']]) for r in activation_layer.fixed_realities]
        final_axes = [r['axis'] for r in activation_layer.fixed_realities]
