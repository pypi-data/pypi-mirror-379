    ...
    def process_pair(self,symbol,pair_config):
        ...
        try:
            ...
            if htf_cache_key not in self.htf_cache:
              ...
              tf_HTF = self.htf_cache.get(htf_cache_key, None)  
              if tf_HTF is None:
                self.logger.info(f"{symbol} : {step}. HTF {htf} 缓存不存在 {htf_cache_key}。")
                return
            ...
            if atf_cache_key not in self.atf_cache:
                ...
                htf_resistance_timestamp = tf_HTF.resistance_timestamp
                htf_support_timestamp = tf_HTF.support_timestamp
                htf_side = tf_HTF.side
                ...
                tf_ATF = self.atf_cache.get(atf_cache_key, None)
                if tf_ATF is None:
                    self.logger.info(f"{symbol} : {step}. ATF {atf} 缓存不存在 {atf_cache_key}。")
                    return
