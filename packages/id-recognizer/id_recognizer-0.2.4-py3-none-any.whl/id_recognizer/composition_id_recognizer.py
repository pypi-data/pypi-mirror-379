...
class CompositionIdRecognizer(object):
    ...
    def _do_recognize(self, input_im: RecInputType) -> RecognitionResult:
        ...
        return RecognitionResult(status=status, sid=self.sid, id_roi=self._id_box_roi)
...
if __name__ == '__main__':
    ...
    APP_SECRET = ''
    t_debug = True
    t_im = cv2.imread("20250926-1721_001.jpg", flags=cv2.IMREAD_COLOR)
    t_start = time.time()
    t_recognizer = CompositionIdRecognizer(key=APP_KEY, secret=APP_SECRET)
    t_ret = t_recognizer.recognize(input_im=t_im, debug=t_debug, enable_ocr=True)
