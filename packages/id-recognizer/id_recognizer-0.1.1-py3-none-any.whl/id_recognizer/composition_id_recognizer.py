...
_logger = Logger(name="ECARD", level=logging.DEBUG)
...
def do_clahe(image: _MatLike, limit: float = 2.0, size: _typing.Tuple[int, int] = (8, 8)) -> _MatLike:
    if limit == 0:
        return image
...
def do_rotate(im: _MatLike, code: int) -> _MatLike:
    if im is None:
        return im
...
def do_find_cts(
    ...
    for c in cts:
        ...
        if min_size[0] <= rect_[2] <= max_size[0] and min_size[1] <= rect_[3] <= max_size[1]:
            ...
            if whr1 <= whr <= whr2:
                ct_ = CtInfo(rect=rect_, ct=c, area=rect_[2] * rect_[3], sides=0)
        ...
        if sides == -1 or circle:
            if sides == -1:
                ...
                if perimeter == 0 or area == 0:
                    ...
                    cond3 = True if 1.15 >= circularity >= 0.85 else False
        else:
...
def do_shrink(im: _MatLike, line_size: float = 1) -> _typing.Tuple[_MatLike, float]:
    ...
    if 0.5 <= whr <= 1.3 and p_wwr >= 0.5 and p_hhr >= 0.55:
        ...
        do_show(f"{seed}_none1", roi_zone)
        roi_zone = do_erode(im=roi_zone, k=(ks, ks))
        do_show(f"{seed}_none2", roi_zone)
        non_count = cv2.countNonZero(roi_zone)
        ratio1 = non_count / max_area
        ratio2 = non_count / im.shape[0] / im.shape[1]
        _logger.debug(f"shrink ratio:{ratio1}, {ratio2}")
    ...
    do_show(f"shrink_ret_{seed}", ret_im)
...
def do_ocr(
    ...
    b64_str = mat_to_b64(bk, quality=90, with_header=False)
    _logger.debug(f"im size: {bk.shape}")
    result = _ocr_slave(b64_str, key=key, secret=secret)
    if result[-1] in ["13003", "12001", "10004", "5004", "1004"]:
        _logger.info(f"ocr fail: {result[-1]}. retry")
...
class CompositionIdRecognizer(object):
    ...
    def _id_container_width(self) -> float:
        return self._radius * 520 / 11

    @property
    def _id_container_height(self) -> float:
        return self._id_container_width * 114 / 520

    @property
    def _id_box_width(self) -> float:
        return self._radius * 233 / 11

    @property
    def _id_box_height(self) -> float:
        return self._id_box_width * 32 / 233

    @property
    def _side_anchor_height(self) -> float:
        return self._radius * 27 / 11
    ...
    def recognize(self, input_im: _MatLike | str | bytes, debug: bool = False) -> RecognitionResult:
        self._debug = debug
        if not self._ocr_key or not self._ocr_secret:
            return RecognitionResult(status=StatusCode.ERR_OCR_AUTH_FAIL)
        try:
    ...
    def _do_recognize(self, input_im: _MatLike | str | bytes) -> RecognitionResult:
        ...
        ocr_status = self._do_ocr()
        if self._debug:
            cv2.imshow("canvas", self._canvas)

        return RecognitionResult(status=ocr_status, sid=self.sid)
    ...
    def _extract_info(
        ...
        for s_idx, sp in enumerate(self._sharpen_parameter):
            for c_idx, cp in enumerate(self._clahe_parameters):
                ...
                c_im = do_clahe(image=s_im, limit=cp[0], size=(cp[1], cp[2]))
    ...
    def _search_id_box_in_container(self):
        ...
        for p in self._im_parameters:
            ...
            refer_w = roi_w * 233 / 520
            refer_h = refer_w * 32 / 233
            cts = do_find_cts(
                im=roi,
            ...
            if ret:
                ...
                self._id_box_roi = do_warp(full_im=roi, ct=boxes[-1], whr=7.28125)
                return
    ...
    def _find_id_container(self, cts: _typing.List[CtInfo]) -> _QueryInfo:
        mask = self._make_contour_mask(cts=cts, refer_im=self._base_im)
        do_show("tmp mask", mask)
    ...
    def _rebuild_id_im(self) -> _typing.Tuple[_MatLike, _MatLike]:
        id_im = None
        if self._id_box_roi is not None:
            ls = self._query_line_size()
            ...
            subs_shrinks = [do_shrink(s, line_size=ls//2) for s in subs]
            subs = [s[0] for s in subs_shrinks]
            gray_val = np.mean(np.array([s[1] for s in subs_shrinks]))
            _logger.debug(f"gray_val: {gray_val}")
        ...
        for r in range(row):
            for c in range(col):
                ...
                box[dy:dy+h, dx:dx+w] = id_im

        do_show("id im", box)
        _logger.debug(f"box shape: {box.shape}")
        return box, id_im
    ...
    def _do_ocr(self) -> StatusCode:
        ...
        self.sid = collections.Counter(ids).most_common()[0][0]
        return StatusCode.OK


if __name__ == '__main__':
    t_debug = True
    t_recognizer = CompositionIdRecognizer(key='', secret='')
    t_im = cv2.imread("/Users/asan/Documents/epson/scan/img20250924_14373225_bk.jpg", flags=cv2.IMREAD_COLOR)
    if t_im is not None:
        t_start = time.time()
        t_ret = t_recognizer.recognize(input_im=t_im, debug=t_debug)
