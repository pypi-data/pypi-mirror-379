...
def read_qr(im: _MatLike) -> _typing.Tuple[str, _typing.Any]:
    if im is None:
        return "", []
    im_h, im_w = im.shape[:2]
    try:
...
if __name__ == '__main__':
    ...
    APP_SECRET = ''
    t_debug = False
    t_im = cv2.imread("/Users/asan/Documents/epson/scan/img20250925_11451413.jpg", flags=cv2.IMREAD_COLOR)
    b64_str1 = mat_to_b64(im=t_im, quality=95, with_header=True)
    b64_str2 = mat_to_b64(im=t_im, quality=95, with_header=False)
    t_start = time.time()
    t_recognizer = CompositionIdRecognizer(key=APP_KEY, secret=APP_SECRET)
    t_ret = t_recognizer.recognize(input_im=b64_str2, debug=t_debug)
