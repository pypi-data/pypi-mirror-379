...
import json
import time
#TODO Schauen wo der Fehler ist bei Copy to IFS
...
class Library:
    ...
    def getInfoForLibrary(self, library:str, wantJson=True) -> None:
        ...
        Example:
            iLibrary.Library(conn).getInfoForLibrary(library='QGPL')
        """
    ...
    """
        ...
        #ToDO Schauen ob der letze char vom Path noch ein slash ist oder nicht

        #starting with mem main Sorucecode of saveLLibrary
        if self.__crtsavf(saveFileName, library, description):
            ...
            try:
                with self.conn.cursor() as cursor:
                    ...
                    if getZip:
                        try:
                            remote_temp_savf_path = f"/home/{self.db_user}/{saveFileName.upper()}.savf"
                            # This is the full path on your local computer
                            ...
                            if not self.__getZipFile(localFilePath=destination_local_path,
                                ...
                                # The __getZipFile method handles the SFTP download and returns a status.
                                raise ValueError("Something went wrong. With downloading the Save File.")



                        except Exception as e:
                            # A generic catch-all for any errors in the process.
                            print(f"An error occurred during the transfer process: {e}")
                            self.conn.rollback()  # assuming you want to roll back on any failure
                            # return False # Return a failure status if this is in a function
