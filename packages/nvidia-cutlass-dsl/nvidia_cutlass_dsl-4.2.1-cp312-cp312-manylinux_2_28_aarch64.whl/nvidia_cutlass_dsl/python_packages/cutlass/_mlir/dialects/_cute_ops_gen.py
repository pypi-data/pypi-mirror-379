...
@_ods_cext.register_operation(_Dialect)
class AtomOp(_ods_ir.OpView):
  OPERATION_NAME = "cute.atom"
...
def atom(atom, *, loc=None, ip=None) -> _ods_ir.Value:
  return AtomOp(atom=atom, loc=loc, ip=ip).result
...
class MakeFragmentLikeOp(_ods_ir.OpView):
  ...
  def __init__(self, src, *, elem_type=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(src))
    _ods_context = _ods_get_default_loc_context(loc)
    if elem_type is not None: attributes["elem_type"] = (elem_type if (
  ...
  @builtins.property
  def elem_type(self):
    if "elem_type" not in self.operation.attributes:
      return None
  ...
  def elem_type(self, value):
    ...
    elif "elem_type" in self.operation.attributes:
      del self.operation.attributes["elem_type"]
...
def make_fragment_like(src, *, elem_type=None, loc=None, ip=None) -> _ods_ir.Value:
  return MakeFragmentLikeOp(src=src, elem_type=elem_type, loc=loc, ip=ip).result
