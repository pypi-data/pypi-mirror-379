...
/**
  ...
  *@param packet ptr to ${proto.namespace}_packet
  *@param data ptr to copy data to
...
%endif
%if field.isArray:
uint32_t ${proto.namespace}_get${field.camel()}(${proto.namespace}_packet_t* packet, ${field.getParamType()} data, uint32_t len)
{
  return poly_packet_get_field(packet, ${field.globalName}, data, len);
}
% else:
${field.getParamType()} ${proto.namespace}_get${field.camel()}(${proto.namespace}_packet_t* packet)
{
  ${field.getParamType()} val;
  poly_packet_get_field(packet, ${field.globalName}, &val, sizeof(val));
  return val;
}
% endif
...
/*******************************************************************************
  Struct to packet and packet to struct functions
*******************************************************************************/
...
void ${proto.namespace}_packet_fill_${packet.name}_struct(${proto.namespace}_packet_t* packet, ${proto.namespace}_${packet.name}_t* strct)
{
  ...
  %for field in packet.fields:
  %if field.isArray:
  strct->${field.name}.len = ${proto.namespace}_get${field.camel()}(packet, strct->${field.name}.data,sizeof(strct->${field.name}.data));
  %else:
...
HandlerStatus_e ${proto.namespace}_sendPing(int iface)
{
  ...
  ${proto.namespace}_setIcd(&packet,  ${proto.namespace.upper()}_ICD_VERSION );
...
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_Ping_handler(${proto.namespace}_packet_t* ping, ${proto.namespace}_packet_t* ack)
{
  ...
  ${proto.namespace}_setIcd(ack, ${proto.namespace.upper()}_ICD_VERSION);
