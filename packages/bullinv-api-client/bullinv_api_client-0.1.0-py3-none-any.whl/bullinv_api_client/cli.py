...
async def run_indicators(url: str, subs: Iterable[Subscription]) -> None:
    async for msg in stream_indicators(url, subs):
        print(msg)


async def run_aggregates(url: str, subs: Iterable[Subscription]) -> None:
    async for msg in stream_aggregates(url, subs):
        print(msg)
...
def main() -> int:
    ...
    p_ind.add_argument("--url", default="ws://localhost:8080")
    p_ind.add_argument("--subs", default="15m.*")

    p_agg = sub.add_parser("aggregates", help="Stream aggregates")
    p_agg.add_argument("--url", default="ws://localhost:8080")
    ...
    if args.cmd == "indicators":
        asyncio.run(run_indicators(args.url, subs))
    elif args.cmd == "aggregates":
        asyncio.run(run_aggregates(args.url, subs))
    else:
        parser.print_help()
        return 2
    return 0
