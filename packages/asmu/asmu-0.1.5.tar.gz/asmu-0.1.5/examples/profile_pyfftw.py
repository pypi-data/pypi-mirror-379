...
# @profile
def main():
    indata32 = np.empty((BLOCKSIZE, 1), dtype=np.float32)

    outdata64_numpy = np.empty((int(BLOCKSIZE/2+1), 1), dtype=np.complex64)
    outdata64_scipy = np.empty((int(BLOCKSIZE/2+1), 1), dtype=np.complex64)
    outdata64_fftw = pyfftw.empty_aligned((int(BLOCKSIZE/2+1), 1), dtype=np.complex64)
    outdata64_ducc0 = np.empty((int(BLOCKSIZE/2+1), 1), dtype=np.complex64)
