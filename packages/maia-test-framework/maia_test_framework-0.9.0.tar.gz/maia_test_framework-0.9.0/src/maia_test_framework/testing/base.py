...
class Participant:
    id: str
    name: str
    type: Literal["user", "agent", "tool", "mcp_server"]
...
class MaiaTest(ABC, ProviderMixin):
    ...
    def setup_mcp_servers(self):
        """Override this method to define MCP servers for test suite"""
        pass
    ...
    def create_agent(self, name, provider, system_message="", ignore_trigger_prompt="", tools: List[str] = None, mcp_servers: List[str] = None):
        agent_tools = [self.tools[tool_name] for tool_name in tools] if tools else []
        agent_mcp_servers = [self.mcp_servers[server_name] for server_name in mcp_servers] if mcp_servers else []
        agent = Agent(name, provider, system_message, ignore_trigger_prompt, tools=agent_tools, mcp_servers=agent_mcp_servers)
    ...
    def create_mcp_server(self, name: str, server_class: BaseMCPServer, *args, **kwargs) -> BaseMCPServer:
        """Creates an MCP server and registers it with the test."""
        server = server_class(name=name, *args, **kwargs)
    ...
    def get_mcp_server(self, name: str) -> BaseMCPServer:
        """Get a MCP server by its name."""
        if name not in self.mcp_servers:
            raise ValueError(f"MCP server with name '{name}' not found.")
