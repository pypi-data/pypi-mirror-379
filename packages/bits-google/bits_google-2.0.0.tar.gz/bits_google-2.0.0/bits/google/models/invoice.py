...
class Invoice:
    ...
    def check_content_type(self):
        ...
        if self.object['contentType'] != contentType:
            print('   Updating contentType: {} -> {}'.format(
    ...
    def check_invoices(self, bucket, project, dataset, table):
        ...
        for invoice_id in sorted(invoices, key=lambda x: invoices[x].get_date()):
            ...
            if not bigquery_total:
                print(f'{invoice_month} - {i.id}: {invoice_total}')
            else:
                diff = bigquery_total - invoice_total
                print(f'{invoice_month} - {i.id}: {invoice_total} [bq diff: {diff}]')
    ...
    def from_gcs_object(self, o):
        ...
        # set the object uri
        invoice.uri = f'gs://{invoice.bucket}/{invoice.name}'
    ...
    def update_all(self, bucket):
        ...
        for i in all_invoices:
            if i.type == 'csv':
                ...
                print(f'ERROR: Unknown type [{i.type}]: {i.name}')

        # display details
        print(f'Objects: {len(all_invoices)}')
        print(f'  CSV Invoices: {len(csv_invoices)}')
        print(f'  JSON Invoices: {len(json_invoices)}')
    ...
    def update_json_invoices(self, csv_invoices, json_invoices):
        ...
        for invoice_id, invoice in csv_invoices_dict.items():
            if invoice_id not in json_invoices_dict:
                print(f' + Add {invoice_id}')
                invoice.update_object_data()

        # find invoices to delete
        for invoice_id, invoice in json_invoices_dict.items():
            if invoice_id not in csv_invoices_dict:
                print(f' + Delete {invoice_id}')
                invoice.delete_json_invoice()
    ...
    def update_object_data(self):
        ...
        name = f'json/{self.get_date()} Invoice {self.get_id()}.json'
