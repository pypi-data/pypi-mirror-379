...
class Google:
    ...
    def check_billing_labels(self, projects, projects2, billing_accounts):
        ...
        for projectId in sorted(projects):
            ...
            if billing_account:
                billing_account = '{}'.format(
                    billing_account.replace('billingAccounts/', '').lower(),
                )

            # check labels
            if billing_label != billing_account:
                print(f'   * Updating billing label for {projectId}: {billing_label} -> {billing_account}')

                # get project
                # project = self.crm().get_project(projectId)
                if projectId not in projects2:
                    print(f'ERROR: Project not found in current list: {projectId}')
                    continue
            ...
            if billingaccount:
                ...
                if billingaccount in billing_accounts:
                    ...
                    if re.match('Broad Institute - [0-9]{7}$', displayName):
                        billing_co = 'broad-{}'.format(displayName.replace('Broad Institute - ', ''))
                    else:
                        billing_co = 'broad-split'

            # check labels
            if costobject != billing_co:
                print(f'   * Updating costobject label for {projectId}: {costobject} -> {billing_co}')
    ...
    def check_project_folders(self, projects, projects2, folders, billing_accounts):  # noqa: PLR0912,PLR0915
        ...
        for folder in sorted(folders, key=lambda x: folders[x]['displayName']):
            ...
            if displayName in foldernames:
                print(f'Duplicate folder name: {displayName} [{folder}]')
                continue
        ...
        for projectId in sorted(projects):
            ...
            broad_orgname = f'organizations/{broad_orgid}'
            ...
            # check projects with a non-broad billing account
                ...
                if billingAccount not in billing_accounts:
                    print('  * Project in {} has non-Broad billing: {}'.format(
                        projectId,
            ...
            if parent['id'] == broad_orgid:
                print(f'  * Project in broadinstitute.org top-level: {projectId}')
                move = True
            ...
            if folder and broad_orgname in p['ancestry'] and broad_billing:
                if folder not in p['ancestry']:
                    ...
                    if current_foldername != foldername:
                        print(f'  * Project with wrong parent: {projectId} [{current_foldername} -> {foldername}]')
                        move = True

            if move and folder:
                print(f'    - Moving project {projectId} to folder {foldername}')
    ...
    def getAllGroupsSettings(self):
        ...
        for groupKey in sorted(groups):
            ...
            try:
                ...
                group_settings = {}
                print(f'ERROR: Failed to get settings for {groupKey}.')
                print(e)
        # if self.verbose:
        #     progress.finish()
    ...
    def removeFromGroups(self, userKey):
        ...
        for groupKey in groups:
            try:
                ...
                print(f'      - Unsubscribed from group: {groupKey}')
            except Exception as e:
    ...
    def subscribeToGroups(self, userKey, groups):
        ...
        for groupKey in groups:
            try:
                ...
                success.append(groupKey)
                print(f'      + Subscribed to group: {groupKey}')
            except Exception as e:
                print(e, file=sys.stderr)
    ...
    def _remove_members(self, groupKey, remove, owners, deleteOwners):
        ...
        for email in sorted(remove):
            if email not in owners or (email in owners and deleteOwners):
                ...
                print(f'   o {email} (not deleted - OWNER)')
    ...
    def updateGroupMembers(
        ...
        if not group:
            print(f'ERROR group not found: {groupKey}')
            return group

        if 'members' not in group or not group['members']:
            print(f'ERROR no members retrieved for group: {groupKey}')
            return group
        ...
        if add or (remove and delete):
            print(f'Updating group: {groupKey}')
