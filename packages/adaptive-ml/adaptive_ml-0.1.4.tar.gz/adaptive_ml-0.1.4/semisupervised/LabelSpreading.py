...
class LabelSpreading(AdaptiveLearning):
    def __init__(self, X):
        super().__init__()
        ...
        self.label_spreading_model = ls(kernel=self.create_similarity_matrix, n_neighbors=self.n_components)
    ...
    def fit(self, y):
        return self.label_spreading_model.fit(self.X, y)

    def predict(self):
        return self.label_spreading_model.predict(self.X)