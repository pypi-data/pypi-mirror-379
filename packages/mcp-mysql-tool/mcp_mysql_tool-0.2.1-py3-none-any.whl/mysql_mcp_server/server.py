...
def execute_with_custom_connection(config: Dict[str, Any], query: str, params: Optional[tuple] = None) -> Dict[str, Any]:
...
def build_select_query(table: str, columns: List[str] = None, where: str = None, 
    ...
    try:
        ...
        if config:
            ...
            if config.get('database') or os.getenv('MYSQL_DATABASE'):
                conn_config['database'] = config.get('database', os.getenv('MYSQL_DATABASE', ''))
        else:
        ...
        cursor.execute("SELECT VERSION() as mysql_version, DATABASE() as current_database, USER() as `current_user`, CONNECTION_ID() as connection_id")
        ...
        return {
            ...
            'connection_info': {
                ...
                'test_query_result': test_result.get('test_query') if test_result else None
            },
        ...
        return {
            ...
            'error': f"Connection error: {str(e)}",
            'connection_config': {
                'host': conn_config.get('host', 'unknown') if 'conn_config' in locals() else 'unknown',
                'port': conn_config.get('port', 'unknown') if 'conn_config' in locals() else 'unknown',
                'user': conn_config.get('user', 'unknown') if 'conn_config' in locals() else 'unknown',
                'database': conn_config.get('database', 'None') if 'conn_config' in locals() else 'None'
            }
        }
    finally:
        if cursor:
    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                ...
                "required": ["host", "user", "password", "query"]
            }
        ),
        Tool(
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "mysql_query":
        ...
        if not table or not data:
            return [TextContent(
                type="text",
                text="Error: table and data parameters are required"
            )]
        ...
        if not all([host, user, password, query]):
            return [TextContent(
                type="text",
                text="Error: host, user, password, and query parameters are required"
            )]
        ...
        params = arguments.get("params", [])
        result = execute_with_custom_connection(config, query, tuple(params) if params else None)
        return [TextContent(
            type="text",
            text=json.dumps(result, indent=2, default=str)
        )]
        ...
        result = get_db().execute_query("SHOW TABLES")
        return [TextContent(
            type="text",
            text=json.dumps(result, indent=2, default=str)
        )]
        ...
        # Table status
        status = get_db().execute_query(f"SHOW TABLE STATUS LIKE '{table_name}'")
        info['status'] = status
