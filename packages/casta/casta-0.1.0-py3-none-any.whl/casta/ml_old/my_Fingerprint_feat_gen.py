...
def Scalings(msds, dt):
    ...
    def power(x, D, alpha): 
        return 4 * D * (x) ** alpha 
    #def power(x, D, alpha, offset): # old version of power with incluced offset (we chose the one above casue offset didnt work)
...
def GetFeatures(x, y, SL, dt, model):
    ...
    #print(states, "states")  # I added
    #print(SL, "steplenght") # I added
    

    t0, t1, t2, t3 = Time_in(states)
    lifetime = Lifetime(states)
    # this np.arry it the X_fingerprints we get out!
    return np.array(
        [
            alpha,
            beta,
