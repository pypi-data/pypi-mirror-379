...
from tidegear.sentinel.exceptions import HandlerError, LoggedHandlerError, NotReadyError
...
class SentinelCog(Cog):
    ...
    async def cog_load(self) -> None:
        ...
        for old, new in self.moderation_type_migrations.items():
            ...
            for guild in guilds:
                old_config = self.get_type_config(guild, old)
                new_config = self.get_type_config(guild, new)
    ...
    def get_type_config(self, guild: Guild | int, moderation_type: ModerationType | type[ModerationType] | str) -> Group:
    ...
    async def moderate(
        ...
        **kwargs: Any,
        ...
        try:
            if not isinstance(target, (abc.GuildChannel, Thread)):
                kwargs["silent"] = silent if moderation_type.removes_from_guild else True

            moderation = await moderation_handler(cog=self, ctx=ctx, target=target, **kwargs)
    ...
    async def case_embed(self, moderation: Moderation, color: Color | None = None) -> Embed:
        ...
        if moderation.meta:
            for key, value in moderation.meta.items():
                if key == "imported_timestamp":
                    timestamp = datetime.fromisoformat(value)
                    embed.description += f"\n{cf.bold(f'{title(key)}:')} {cf.format_datetime(dt=timestamp)}"
                else:
                    try:
                        if func := moderation.type.history_metadata.get(key, None):
                            value = func(value)  # noqa: PLW2901
                    except RegistryKeyError:
                        pass
            ...
            for moderation in moderations[i : i + pagesize]:
                ...
                if moderation_type:
                    for key, content in moderation.meta.items():
                        if key in moderation.type.history_metadata.keys():
                            func = moderation.type.history_metadata[key]
                            value += f"{cf.bold(title(key) + ':')} {func(content)}\n"
