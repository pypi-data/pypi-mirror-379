...
class ModerationMetadataEntry:
    ...
    Attributes:
        ...
        show_in_history: Whether or not this metadata should be shown within [`history`][tidegear.sentinel.SentinelCog.history].
    ...
    def fetch_from_moderation(self, moderation: "Moderation") -> str | None:
        ...
        """
        meta = moderation.meta
        if value := meta.get(self.key, None):
...
class ModerationType:
    ...
    async def member_target_handler(
        cls, *, cog: "SentinelCog", ctx: commands.Context, target: Member, silent: bool = MISSING, reason: str, **kwargs: Any
    ) -> "Moderation":
    ...
    async def user_target_handler(
        cls, *, cog: "SentinelCog", ctx: commands.Context, target: Member | User, silent: bool = MISSING, reason: str, **kwargs: Any
    ) -> "Moderation":
    ...
    async def channel_target_handler(
        cls, *, cog: "SentinelCog", ctx: commands.Context, target: abc.GuildChannel | Thread, reason: str, **kwargs: Any
    ) -> "Moderation":
