...
def _clean_foreign_keys(obj: dict) -> dict:
    cleaned = {}
    for k, v in obj.items():
        if isinstance(v, dict):
            nested = _clean_foreign_keys(v)
...
class Table(BaseTable):
    ...
    @overload
    def json(self, *, string: Literal[False] = False, indent: bool = ..., options: int = ...) -> bytes: ...

    @overload
    def json(self, *, string: Literal[True], indent: bool = ..., options: int = ...) -> str: ...

    def json(self, *, string: bool = False, indent: bool = False, options: int = 0) -> bytes | str:
        ...
        dump = orjson.dumps(self, default=self._json_serializer, option=options)
    ...
    def _json_serializer(obj: object) -> object:
        if isinstance(obj, Table):
            return _clean_foreign_keys(obj.to_dict())
...
class Change(Table):
    ...
    async def moderator(self, /, *, cache: bool = True) -> PartialUser:
        ...
        if isinstance(self.moderator_id, PartialUser):  # support both initialized PartialUser objects and raw ints
...
class Moderation(Table):
    ...
    async def resolver(self, /, *, cache: bool = True) -> PartialUser:
        ...
        if isinstance(self.resolver_id, PartialUser):  # support both initialized PartialUser objects and raw ints
