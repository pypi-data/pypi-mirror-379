...
class ModerationType:
    ...
    Attributes:
        ...
        removes_from_guild: Whether this type's handler removes the target from the guild,
            ...
            **Moderation types that remove users from guilds are responsible for contacting users using the
            [`SentinelCog.contact_user`][tidegear.sentinel.SentinelCog.contact_user] method *before* removing them from the guild.**
            Defaults to `False`.
    ...
    @classmethod
    async def member_target_handler(cls, cog: "SentinelCog", ctx: commands.Context, target: Member, **kwargs: Any) -> "Moderation":
    ...
    @classmethod
    async def user_target_handler(cls, cog: "SentinelCog", ctx: commands.Context, target: Member | User, **kwargs: Any) -> "Moderation":
