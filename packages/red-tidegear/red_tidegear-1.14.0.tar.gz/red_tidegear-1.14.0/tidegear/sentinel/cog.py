...
from discord import Color, Embed, Forbidden, Guild, Interaction, Member, Message, NotFound, Permissions, Role, Thread, User, abc, ui
...
class SentinelCog(Cog):
    ...
    async def cog_load(self) -> None:
        ...
        with set_env(key="PICCOLO_CONF", value="tidegear.sentinel.db.piccolo_conf", logger=self._sentinel_logger):
            self.db = await register_cog(
                cog_instance=self._db_path, tables=[Change, Moderation, PartialChannel, PartialGuild, PartialRole, PartialUser], trace=True
            )
    ...
    @staticmethod
    def is_in_guild(partial_object: PartialTargetable, guild: Guild) -> bool:
        """Check if a partial is part of a discord Guild.
        ...
        if isinstance(partial_object, PartialRole):
            return bool(guild.get_role(partial_object.role_id))
    ...
    async def sentinel_case_embed(self, moderation: Moderation, color: Color | None = None) -> Embed:
        ...
        try:
            ...
            for metadata in moderation_type.metadata:
                if value := metadata.fetch_from_moderation(moderation):
    ...
    async def sentinel_history_menu(
        ...
        *,
        targets: list[abc.User | PartialUser | GuildChannel | PartialChannel] | None = None,
            ...
            for moderation in moderations[i : i + pagesize]:
                ...
                if moderation_type:
                    for meta in [meta for meta in moderation_type.metadata if meta.show_in_history]:
                        if meta_value := meta.fetch_from_moderation(moderation):
