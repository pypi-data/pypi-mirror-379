...
class SwidgeEvmTransactionDetails(BaseModel):
    ...
    maxFeePerGas: int = Field(..., description="Max fee per gas")
    maxPriorityFeePerGas: int = Field(..., description="Max priority fee per gas")
...
class SwidgeTransactionStep(BaseModel):
    ...
    transactionDetails: SwidgeEvmTransactionDetails | SwidgeSolanaTransactionDetails = (
...
SwidgeUnsignedStep = SwidgeTransactionStep | SwidgeSignatureStep
...
class SwidgeQuoteData(BaseModel):
    ...
    assetRecieve: SwidgeQuoteAsset = Field(
        validation_alias=AliasChoices("assetRecieve", "assetReceive"),
