        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if client is None or client.is_closed:
                    logger.info(f"Creating new persistent client for proxy: {proxy_state.address}")
                    client = httpx.AsyncClient(proxy=proxy_state.address, follow_redirects=True)
                    self._clients[proxy_state.address] = client

                logger.debug(f"Attempt {attempt + 1}/{self.max_retries}: Fetching {url} via {proxy_state.address}")

                # --- Now, use the persistent client ---
                ...
                last_exception = e
                logger.error(f"Attempt {attempt + 1} failed for {url} with proxy {proxy_state.address}: {e}")
