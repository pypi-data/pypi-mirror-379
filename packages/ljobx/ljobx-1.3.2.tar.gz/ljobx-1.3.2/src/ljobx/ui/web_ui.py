...
def main():
    ...
    if st.session_state.results is not None:
        with results_placeholder.container():
            ...
            # This logic is now correct because it will run even for an empty list
            results_list = st.session_state.results
            flat_results = [_flatten_dict(res) for res in results_list]

            if flat_results:
                # (The rest of the logic to display the dataframe remains unchanged)
                all_keys = set(key for res in flat_results for key in res.keys())
                all_keys.discard('recruiter')
                ...
                # This now correctly handles the "No jobs found" case
                st.dataframe([])
