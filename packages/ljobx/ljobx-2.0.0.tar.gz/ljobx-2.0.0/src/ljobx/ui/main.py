...
def ensure_streamlit_config():
    """Checks for a .streamlit/config.toml and creates one from a template if not found."""
    cwd = Path.cwd()
    streamlit_config_dir = cwd / ".streamlit"
    config_file = streamlit_config_dir / "config.toml"

    if not config_file.exists():
        print("-> No local .streamlit/config.toml found. Creating one from default.")
        package_path = Path(__file__).parent
        default_config = package_path / ".streamlit" / "config.toml"

        if default_config.exists():
            streamlit_config_dir.mkdir(exist_ok=True)
            shutil.copy2(default_config, config_file)
        else:
            print("-> Warning: Default config not found in the package. App will use Streamlit defaults.")
