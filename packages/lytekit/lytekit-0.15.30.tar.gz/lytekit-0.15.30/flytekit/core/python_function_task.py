...
class PythonFunctionTask(PythonAutoContainerTask[T]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.runtime_info_dir = os.getenv("latch_runtime_info_dir")
        ...
        def _hash_function(task_func: Callable):
            ...
            for node in ast.walk(module):
                ...
                if len(node.body) != 0:
                    docstr = node.body[0]
                    if isinstance(docstr, ast.Expr) and isinstance(
                        docstr.value, ast.Str
                    ):
                        node.body.pop(0)
    ...
    def execute(self, **kwargs) -> Any:
        ...
        if self.execution_mode == self.ExecutionBehavior.DEFAULT:
            try:
                if self.enable_memory_profiling and self.runtime_info_dir is not None:
                    with (Path(self.runtime_info_dir) / "memory_profile.txt").open(
                        "w"
                    ) as f:
                        from memory_profiler import profile

                        return exception_scopes.user_entry_point(
                            profile(self._task_function, stream=f)
                        )(**kwargs)

                return exception_scopes.user_entry_point(self._task_function)(**kwargs)
            except exception_scopes.FlyteScopedException as e:
                if self.runtime_info_dir is not None:
                    (Path(self.runtime_info_dir) / "traceback.txt").write_text(
                        e.verbose_message
                    )
    ...
    def compile_into_workflow(
        ...
        with FlyteContextManager.with_context(ctx.with_compilation_state(cs)):
            ...
            self._wf = PythonFunctionWorkflow(
                task_function, metadata=workflow_metadata, default_metadata=defaults
            )
