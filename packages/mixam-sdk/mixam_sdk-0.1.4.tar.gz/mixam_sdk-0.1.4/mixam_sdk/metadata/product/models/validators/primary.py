...
class PrimaryComponentValidator(DefaultComponentValidator):
    ...
    def validate(self, productMetadata: ProductMetadata, itemSpecification: ItemSpecification, component: ComponentSupport, result: ValidationResult, base_path: str) -> None:
        ...
        try:
            if component.has_custom_size() and productMetadata.custom_size_metadata is None:
                result.add_error(
        ...
        try:
            st = next((t for t in productMetadata.substrate_types if t.id == component.substrate.type_id), None)
            valid_combo = False
            if st is not None:
        ...
        try:
            if productMetadata.auto_orientable and productMetadata.default_orientation is not None:
                if component.orientation != productMetadata.default_orientation:
                    result.add_error(
                        path=f"{base_path}.orientation",
                        message=(
                            f"This product is auto orientable and only supports {productMetadata.default_orientation.name} orientation. "
                            "(Note: Both portrait & landscape artwork files are auto detected and processed accordingly)"
                        ),
                        code="orientation.not_supported",
                        required=productMetadata.default_orientation.name,
                    )
        except Exception:
            pass
        ...
        try:
            ...
            if not isinstance(component, BoundComponent):
                options = productMetadata.pre_drilled_holes_metadata.pre_drilled_hole_options
        ...
        try:
            if isinstance(component, IFoiledComponent):
                ...
                if front_has:
                    if not productMetadata.foiling_metadata.front_foiling:
                        result.add_error(
                        ...
                        if isinstance(component, ILaminatedComponent):
                            ...
                            if lam != Lamination.NONE:
                                ok = any(lam in opt.supported_laminations for opt in productMetadata.foiling_metadata.front_foiling)
                                if not ok:
                                    result.add_error(
                                        path=f"{base_path}.foiling",
            ...
            if isinstance(component, ITwoSidedComponent):
                ...
                try:
                    ...
                    back_opts = lam_meta.back_options if lam_meta is not None else []
                    # If no back options configured and a back lamination is set, it's unsupported
                    if not back_opts and back_lam != Lamination.NONE:
                        result.add_error(
                        ...
                        if same_as_front_available:
                            ...
                            if back_lam != Lamination.NONE:
                                allowed_values = {int(opt.value) for opt in back_opts}
                ...
                if back_has:
                    if not productMetadata.foiling_metadata.back_foiling:
                        result.add_error(
                        ...
                        if lam != Lamination.NONE:
                            ok = any(lam in opt.supported_laminations for opt in productMetadata.foiling_metadata.back_foiling)
                            if not ok:
                                result.add_error(
                                    path=f"{base_path}.backFoiling",
