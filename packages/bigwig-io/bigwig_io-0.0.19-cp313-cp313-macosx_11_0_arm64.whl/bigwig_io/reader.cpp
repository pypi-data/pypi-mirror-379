...
public:
    ...
    std::future<void> read_headers() {
        return std::async(std::launch::async, [this]() {
            ...
            auto_sql = read_auto_sql(*file, main_header.auto_sql_offset);
    ...
    std::vector<float> read_values(
        ...
        if (type != "bigwig") throw std::runtime_error("read_values only for bigwig");
    ...
    std::vector<float> quantify(
        ...
        if (type != "bigwig") throw std::runtime_error("quantify only for bigwig");
    ...
    std::vector<float> profile(
        ...
        if (type != "bigwig") throw std::runtime_error("profile only for bigwig");
