...
class BouncingLinearFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        """
        global BWRD_TRANSFER_STREAM, BWRD_GRAD_TRANSFER_STREAM, TRANSFER_BACKWARD_FINISHED_EVENT, TRANSFER_GRAD_BACKWARD_FINISHED_EVENT, COMPUTE_BACKWARD_START_EVENT, COMPUTE_BACKWARD_DONE_EVENT, BACKWARD_BUFFER_CLK, W_BWRD_BUFFERS, W_GRAD_BUFFERS, B_GRAD_BUFFERS
...
class CPUBouncingLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, device="cuda"):
        ...
        self.weight = nn.Parameter(
            torch.empty(out_features, in_features, device="cpu")
            .share_memory_()
            .pin_memory()
        )
        self.bias = (
            nn.Parameter(
                torch.empty(out_features, device="cpu").share_memory_().pin_memory()
            )
            if bias
            else None
        )
