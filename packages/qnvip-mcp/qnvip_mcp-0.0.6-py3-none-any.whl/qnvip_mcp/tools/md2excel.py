...
class MarkdownConverter:
    ...
    @staticmethod
    def convert_md_to_excel(md_file_path: str, output_excel_path: str):
        """
        ...
        try:
            ...
            if not df.empty:
                ...
                print(f"成功将 '{md_file_path}' 转换为 '{output_excel_path}'")
            else:
    ...
    @staticmethod
    def convert_md_to_excel_for_mcp(md_file_path: str, output_excel_path: str) -> str:
        """
        为MCP stdio模式优化的Markdown转Excel方法。
        :param md_file_path: 输入的Markdown文件路径。
        :param output_excel_path: 输出的Excel文件路径。
        :return: 转换结果消息
        """
        try:
            ...
            if not df.empty:
                ...
                return f"成功将 '{md_file_path}' 转换为 '{output_excel_path}'，共转换了 {len(df)} 行数据"
            else:
                return f"在 '{md_file_path}' 中未找到有效的表格数据"

        except FileNotFoundError:
            return f"错误: 文件 '{md_file_path}' 未找到"
        except Exception as e:
            return f"转换失败: {str(e)}"
