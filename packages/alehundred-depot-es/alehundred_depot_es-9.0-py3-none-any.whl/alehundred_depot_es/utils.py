...
def get_disk_usage():
    ...
    if code == 0:
        ...
        if len(lines) > 1:
            ...
            percent = parts[2]
            return f"{used_gb:.6f}G / {int(total_gb)}G ({percent} Usado)"
...
def get_mem_usage():
    ...
    if code == 0:
        ...
        if len(lines) > 1:
            parts = lines[1].split()
            return f"{parts[2]}MB / {parts[1]}MB Usado"
    return "No disponible"
...
def get_cpu_usage():
    ...
    if code == 0:
        ...
        if match:
            return f"{match.group(1)}% (Usuario)"
    return "No disponible"
...
def ejecutar_y_yield_salida(comando: list, con_sudo: bool = False):
    try:
        ...
        yield ('LOG', f"Error inesperado al ejecutar comando: {e}")
        yield ('RETURN_CODE', 1)
