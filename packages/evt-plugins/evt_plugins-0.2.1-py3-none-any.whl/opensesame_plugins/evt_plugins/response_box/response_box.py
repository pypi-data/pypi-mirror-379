...
class ResponseBox(Item):
    ...
    def prepare(self):
        ...
        if self.var.device == u'Keyboard':
            ...
            try:
                ...
                for d in device_list:
                    sleep(1) # without a delay, the device will not always be there.
                    composed_string = d['product_string'] + " s/n: " + d['serial_number']
                    open_rsp_devices[composed_string] = EventExchanger()
                    # Get evt device handle:
                    open_rsp_devices[composed_string].attach_id(d['path'])
                    ...
                    oslogger.info('        ...  and with device ID: {}'.format(
                        open_rsp_devices[composed_string]))
            except:
        ...
        if self.current_device is None:
            ...
            open_rsp_devices[self.current_device].write_lines(0) # clear lines
    ...
    def run(self):
        ...
        if self.var.device != u'Keyboard':
            ...
            self.var.response, self.var.end_time = \
                    open_rsp_devices[self.current_device].wait_for_event(
        ...
        if self.var.close_device == 'yes':
            for dkey in open_rsp_devices:
                try:
                    open_rsp_devices[dkey].close()
                    oslogger.info('Device: {} successfully closed!'.format(open_rsp_devices[dkey]))
                except:
                    oslogger.warning('Device {} for closing not found!'.format(open_rsp_devices[dkey]))
