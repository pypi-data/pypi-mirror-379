...
# global var
open_evt_devices = {} # store open device handles.
...
class EvtTrigger(Item):
    ...
    def prepare(self):
        ...
        if self.var.device == u'DUMMY':
            ...
            try:
                ...
                for d in device_list:
                    ...
                    open_evt_devices[composed_string] = EventExchanger()
                    device_output_value[composed_string] = 0 # create device output state storage
                    # Get evt device handle:
                    open_evt_devices[composed_string].attach_id(d['path'])
                    ...
                    oslogger.info('        ...  and with device ID: {}'.format(
                        open_evt_devices[composed_string]))
            except:
        ...
        if self.current_device is None:
            ...
            oslogger.info('Preparing device: {}'.format(open_evt_devices[self.current_device]))
            #open_evt_devices[self.current_device].write_lines(0) # clear lines
    ...
    def run(self):
        ...
        if self.var.device == u'DUMMY':
            ...
            if self.var.outputmode == u'Clear output lines':
                ...
                open_evt_devices[self.current_device].write_lines(device_output_value[self.current_device])
                oslogger.info('{}: send byte code {}'.format(
                    open_evt_devices[self.current_device], device_output_value[self.current_device]))
            elif self.var.outputmode == u'Write output lines':
                ...
                open_evt_devices[self.current_device].pulse_lines(
                ...
                oslogger.info('{}: send byte code {} for the duration of {} ms'.format(
                    open_evt_devices[self.current_device],
                    device_output_value[self.current_device] ^ self.var.mask, self.var.duration))
        # close the device?
        if self.var.close_device == 'yes':
            for dkey in open_evt_devices:
                try:
                    open_evt_devices[dkey].close()
                    oslogger.info('Device: {} successfully closed!'.format(open_evt_devices[dkey]))
                except:
                    oslogger.warning('Device {} for closing not found!'.format(open_evt_devices[dkey]))
