"""
Display and show functionality for the backend.

This module provides the complex show() method logic for displaying data with
...
def process_columns_with_display_funcs(columns, rows_to_show):
    """
...
def expand_wildcard_columns(
    columns: List[Tuple], rows_to_show: List[Dict]
) -> List[Tuple]:
...
def show_data(
    ...
    # Display the results
    if not rows_to_show or (isinstance(rows_to_show, list) and len(rows_to_show) == 0):
        print("No rows to display")
    else:
        fixed_rows = list(rows_to_show)
        if columns is not None:
