...
class ListBackend:
    ...
    def __init__(self, rows: Union[List[Dict], pd.DataFrame]) -> None:
        ...
        if isinstance(rows, pd.DataFrame):
            # Avoid unnecessary copy - use fillna for null handling
    ...
    def show(self, columns=None, limit=None):
        """Display the data with optional column selection and limit."""
        show_data(self.rows, columns, limit, getattr(self, "_default_limit", None))
        return self
