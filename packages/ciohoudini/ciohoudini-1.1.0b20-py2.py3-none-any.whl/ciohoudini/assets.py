...
def collect_scene_hdas():
        ...
        for node in hou.node("/").allSubChildren():
            try:
                ...
                if definition:
                    ...
                    logger.info(f"Found user HDA: {node.path()} -> {library_path}")
    ...
    if filtered_hdas:
        logger.info(f"Found {len(filtered_hdas)} user HDA file(s) in scene")
        for hda in filtered_hdas:
            logger.info(f"  User HDA: {os.path.basename(hda)}")
            logger.debug(f"    Path: {hda}")
    else:
        logger.info("No user HDA files found in scene")
...
def collect_scene_hdas_strict():
    ...
    if hda_list:
        logger.info(f"Found {len(hda_list)} user HDA file(s) in scene (strict mode)")
        for hda in hda_list:
            logger.info(f"  User HDA: {os.path.basename(hda)}")
