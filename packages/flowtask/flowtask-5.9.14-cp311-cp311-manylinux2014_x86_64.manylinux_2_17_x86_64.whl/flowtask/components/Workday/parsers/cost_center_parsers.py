...
def parse_integration_id_data(integration_data: Union[List, Dict, None]) -> Dict[str, Any]:
    ...
    # The Integration_ID_Data contains a list of ID elements with System_ID attributes
    # Structure: <wd:Integration_ID_Data><wd:ID wd:System_ID="WD-I">2502$3</wd:ID>...
    id_list = integration_data.get("ID", [])
    if not isinstance(id_list, list):
        id_list = [id_list] if id_list else []
    
    for id_item in id_list:
        if isinstance(id_item, dict):
            # Get the ID value (could be in _value_1 or direct)
            id_value = id_item.get("_value_1") or str(id_item.get("ID", "")).strip()
            system_id = id_item.get("System_ID")
            
            if id_value:
                result["integration_ids"].append(id_value)
                
                # Use WD-WID system ID as external integration ID if available
                if system_id == "WD-WID" and not result["external_integration_id"]:
                    result["external_integration_id"] = id_value
        elif isinstance(id_item, str):
            # Sometimes it's just a direct string value
            result["integration_ids"].append(id_item)
...
def parse_organization_data(org_data: Dict) -> Dict[str, Any]:
    ...
    return {
        ...
        "organization_visibility": safe_get_nested(org_data, "Organization_Visibility_Reference", "ID", 0, "_value_1"),
...
def parse_organization_type_data(type_data: Dict) -> Dict[str, Any]:
    ...
    org_type_id_list = org_type_ref.get("ID", []) if isinstance(org_type_ref, dict) else []
    
    # Organization Subtype Reference
    org_subtype_ref = type_data.get("Organization_Subtype_Reference", {})
    org_subtype_id_list = org_subtype_ref.get("ID", []) if isinstance(org_subtype_ref, dict) else []
...
def parse_organization_container_data(container_data: Dict) -> Dict[str, Any]:
    ...
    if isinstance(container_ref, dict):
        ...
        # Try to get Organization_Reference_ID first, then Custom_Organization_Reference_ID
        container_id = (extract_by_type(id_list, "Organization_Reference_ID") or 
                       extract_by_type(id_list, "Custom_Organization_Reference_ID"))
...
def parse_cost_center_reference(cc_ref: Dict) -> Dict[str, str]:
    ...
    id_list = cc_ref.get("ID", [])
    if isinstance(id_list, list):
        result["cost_center_wid"] = extract_by_type(id_list, "WID")
        # Try to get Cost_Center_Reference_ID first, then Organization_Reference_ID
        result["cost_center_id"] = (extract_by_type(id_list, "Cost_Center_Reference_ID") or 
                                  extract_by_type(id_list, "Organization_Reference_ID"))
...
def parse_cost_center_data(cost_center: Dict) -> Dict[str, Any]:
    ...
    cc_data = cost_center.get("Cost_Center_Data", {})
    ...
    integration_data = parse_integration_id_data(cc_data.get("Integration_ID_Data"))
    
    # Effective Date
    effective_date = cc_data.get("Effective_Date")
    
    # Combine all parsed data
    parsed_data = {
        # Cost Center Reference
        **cc_ref_data,
        "cost_center_name": cc_reference.get("Descriptor"),
