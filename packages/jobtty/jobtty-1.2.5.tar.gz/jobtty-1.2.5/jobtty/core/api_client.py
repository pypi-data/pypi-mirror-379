...
class JobttyAPI:
    ...
    def search_jobs(self, source: str, search_params: Dict) -> List[Dict]:
        ...
        if jobs_data:
            ...
            # CLIENT-SIDE FILTERING: Since API doesn't properly filter by 'q' parameter
            query = clean_params.get('q', '').lower().strip()
            if query:
                filtered_jobs = []
                for job in jobs:
                    # Search in title, description, company name, and requirements
