...
Provides easy-to-use CLI for starting yLLM servers with different configurations.
...
@click.version_option(version="0.1.0", prog_name="yLLM")
def main(
    model: str,
    ...
    """
    yLLM - Lightweight LLM Server for Testing

    Start a lightweight LLM server using real tiny models for testing and development.

    Examples:
        # Basic usage
        yllm --model Qwen/Qwen3-0.6B

        # Custom port and served model name
        yllm --model Qwen/Qwen3-0.6B --port 8001 --name "TinyLlama/TinyLlama-1.1B-Chat-v1.0"

        # Use vLLM backend
        yllm --model Qwen/Qwen3-0.6B --backend vllm

        # Force CPU usage
        yllm --model Qwen/Qwen3-0.6B --device cpu
    """
    # Set log level
    ...
    click.echo("🚀 Starting yLLM server...")
...
def quickstart(model: str):
    ...
    click.echo("🚀 Quick starting yLLM with default settings...")
