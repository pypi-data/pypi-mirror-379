    ...
    def hook(features: type[Features]) -> None:
        ...
        def resolver_factory():
            def fn(inp: DataFrame):
                result = model_version.model.predict(inp[[c for c in inp.columns if c != pkey_string]].to_pyarrow())
            ...
            return OnlineResolver(
                function_definition="",
                filename="",
                fqn=f"__chalk__inference__resolver__namespace__{output_feature.namespace}__name__{output_feature.name}",
                doc=None,
