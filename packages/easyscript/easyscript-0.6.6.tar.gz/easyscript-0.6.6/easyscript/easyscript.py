...
class EasyScriptEvaluator:
    ...
    def _handle_substitute_expression(self, text: str, substitute_expr: str) -> str:
        """
        Handle Perl-style substitute expression s/pattern/replacement/flags

        Args:
            text: The string to perform substitution on
            substitute_expr: The substitute expression (e.g., "s/pattern/replacement/gi")

        Returns:
            The modified string after substitution
        """
        # Parse the substitute expression
        if not substitute_expr.startswith('s/'):
            raise ValueError("Substitute expression must start with 's/'")

        # Remove the 's/' prefix
        expr = substitute_expr[2:]

        # Find the delimiter (should be '/')
        # Handle escaped delimiters in pattern and replacement
        parts = []
        current_part = ""
        escaped = False
        delimiter_count = 0
        ...
        while i < len(expr):
            ...
            if escaped:
                ...
                current_part = ""
                delimiter_count += 1
                if delimiter_count >= 2:  # We have pattern and replacement
                    # Rest is flags (everything after this delimiter)
                    flags_str = expr[i + 1:]
        ...
        # Handle case where we reached the end without finding the third delimiter
        if delimiter_count == 2 and len(parts) == 2:
            # No flags provided
            parts.append("")

        if delimiter_count < 2:
            raise ValueError("Invalid substitute expression: missing delimiters (expected s/pattern/replacement/flags)")
        ...
        # Convert Perl-style group references ($1, $2) to Python-style (\1, \2)
        ...
        for flag in flags_str:
            if flag == 'i':
                ...
                global_replace = True
            elif flag and flag.strip():  # Only error on non-empty, non-whitespace flags
                raise ValueError(f"Unknown regex flag: '{flag}'")

        try:
            # Compile the pattern with flags
            pattern_obj = re.compile(pattern, flags)

            # Perform substitution
            if global_replace:
                # Replace all occurrences
                result = pattern_obj.sub(replacement, text)
            else:
                # Replace only first occurrence
                result = pattern_obj.sub(replacement, text, count=1)
    ...
    def _parse_unquoted_substitute(self) -> str:
        """Parse unquoted substitute expression starting with 's'"""
        # Consume the 's' token
        if self.current_token().value != 's':
            raise ValueError("Expected 's' token for substitute expression")
        substitute_expr = 's'
        self.consume_token()

        # Parse the rest of the substitute expression until we reach a delimiter that ends it
        # We need to collect tokens until we find a reasonable stopping point
        paren_depth = 0
        while (self.current_token_index < len(self.tokens) and
            ...
            # Stop at certain tokens that would end the substitute expression
            if (paren_depth == 0 and
                token.type in [TokenType.KEYWORD] and
                token.value in ['then', 'else', 'and', 'or']):
                break
    ...
    def parse_comparison(self) -> Any:
        ...
        while self.current_token().type == TokenType.OPERATOR and self.current_token().value in ['>', '<', '>=', '<=', '==', '!=', '~']:
            ...
            if op == '~':
                # Regex matching or substitution: left ~ right
                if not isinstance(left, str):
                    raise TypeError(f"Regex match operator '~' requires string on left side, got '{type(left).__name__}'")

                # Check if next token is unquoted 's' for substitute expression
                if (self.current_token().type == TokenType.IDENTIFIER and
                    self.current_token().value == 's' and
                    ...
                    # Unquoted substitute expression s/pattern/replacement/flags
                    right_substitute = self._parse_unquoted_substitute()
                    left = self._handle_substitute_expression(left, right_substitute)
                else:
                    # Parse normally for quoted strings or other expressions
                    right = self.parse_additive()
                    if isinstance(right, str) and right.startswith('s/'):
                        # Quoted substitute expression
                        left = self._handle_substitute_expression(left, right)
                    elif isinstance(right, str) and right.startswith('s') and len(right) > 1 and right[1] != '/':
                        # Invalid quoted substitute expression - starts with 's' but not 's/'
                        raise ValueError(f"Invalid substitute expression: '{right}' (substitute expressions must start with 's/')")
                    elif isinstance(right, str):
                        # Quoted string - regular regex matching (returns boolean)
                        try:
                            left = bool(re.search(right, left))
                        except re.error as e:
                            raise ValueError(f"Invalid regex pattern '{right}': {e}")
                    else:
                        # Non-string right side
    ...
    def parse_function_call(self, function_name: str) -> Any:
        ...
        if function_name == 'len':
            ...
            if len(args) != 1:
                raise TypeError(f"abs() takes exactly one argument ({len(args)} given)")
            value = args[0]
            ...
            if len(args) < 2:
                raise TypeError(f"min() expected at least 2 arguments, got {len(args)}")
            try:
                return min(args)
            except TypeError as e:
                raise TypeError(f"min() arguments must be comparable: {e}")
        elif function_name == 'max':
            if len(args) < 2:
                raise TypeError(f"max() expected at least 2 arguments, got {len(args)}")
            try:
                return max(args)
            except TypeError as e:
                raise TypeError(f"max() arguments must be comparable: {e}")
        elif function_name == 'round':
            if len(args) < 1 or len(args) > 2:
                raise TypeError(f"round() takes 1 or 2 arguments ({len(args)} given)")
            value = args[0]
            if not isinstance(value, (int, float)):
                raise TypeError(f"round() argument must be a number, not '{type(value).__name__}'")
            if len(args) == 1:
                return round(value)
            else:
                digits = args[1]
                if not isinstance(digits, int):
                    raise TypeError(f"round() second argument must be an integer, not '{type(digits).__name__}'")
                return round(value, digits)
        elif function_name == 'random':
            # random() -> float in [0.0, 1.0)
            # random(upper) -> float in [0.0, upper)
            # random(lower, upper) -> float in [lower, upper)
            if len(args) == 0:
                return random.random()
            elif len(args) == 1:
                upper = args[0]
                if not isinstance(upper, (int, float)):
                    raise TypeError(f"random() single argument must be a number, not '{type(upper).__name__}'")
                return random.random() * upper
            elif len(args) == 2:
                lower, upper = args[0], args[1]
                if not isinstance(lower, (int, float)) or not isinstance(upper, (int, float)):
                    raise TypeError("random() arguments must be numbers")
                return lower + random.random() * (upper - lower)
            else:
                raise TypeError(f"random() takes 0, 1 or 2 numeric arguments ({len(args)} given)")
        else:
