...
def get_frame_traceback(stack_offset=1):
    stack = inspect.stack()
    stack_offset = (
        len(stack) - 1 if stack_offset >= len(stack) else stack_offset
    )
...
def get_frame_module_traceback(
    stack_offset=1,
...
class TracebackInfo:
    ...
    def parse(cls, frame) -> "TracebackInfo":
        ...
        stacktrace.func_name = (
            frame.function if frame.function != "<module>" else None
        )
    ...
    def __eq__(self, __value: object) -> bool:
        assert isinstance(
            __value, TracebackInfo
        ), f"cannot compare {__value} as a {TracebackInfo}"
        return (
