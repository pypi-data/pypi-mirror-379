...
def plot_MSE(self, isSaving=True):
    ...
    Parameters:
        isSaving: bool, whether to save the plot to disk (default: True).

    Returns:
        None
    """
    if not hasattr(self, 'MSE') or not self.MSE:
        raise ValueError("MSE is empty or not calculated. Please run calculateMSE() first.")
    ...
    if isSaving:
        # Gestion robuste du chemin de sauvegarde
        if not hasattr(self, 'saveDir') or self.saveDir is None:
            print("Warning: self.saveDir is not defined. Using current directory.")
            self.saveDir = os.getcwd()  # Dossier courant par défaut

        # Crée les dossiers 'results' s'ils n'existent pas
        results_path = os.path.join(self.saveDir, 'results')
        os.makedirs(results_path, exist_ok=True)  # Crée le dossier si nécessaire

        # Sauvegarde du plot
        now = datetime.now()
        date_str = now.strftime("%Y_%m_%d_%H%M%S")  # Format plus précis (ajout des heures/minutes)
        save_path = os.path.join(results_path, f'MSE_plot_{date_str}.png')

        try:
            plt.savefig(save_path, dpi=300)
            print(f"MSE plot saved to {save_path}")
        except Exception as e:
            print(f"Error while saving MSE plot: {type(e).__name__}: {e}")
            print("The plot will only be displayed, not saved.")
    ...
    def _AlgebraicReconPython(self,withTumor):
        ...
        if withTumor:
            ...
            if self.optimizer.value == OptimizerType.MLEM.value:
                ...
                self.reconPhantom = self._LS(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withTumor, withTumor=withTumor)
            elif self.optimizer.value == OptimizerType.LS_TV.value:
                self.reconPhantom = self._LS_Regularized(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withTumor, withTumor=withTumor)
            else:
            ...
            if self.optimizer.value == OptimizerType.MLEM.value:
                ...
                self.reconLaser = self._LS(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withoutTumor, withTumor=withTumor)
            elif self.optimizer.value == OptimizerType.LS_TV.value:
                self.reconLaser = self._LS_Regularized(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withoutTumor, withTumor=withTumor)
            else:
                raise ValueError(f"Only MLEM and LS are supported for simple algebraic reconstruction. {self.optimizer.value} need Bayesian reconstruction")
    ...
    def _LS(self, SMatrix, y, withTumor):
        ...
        if self.isGPU:
            if check_gpu_memory(config.select_best_gpu(), required_memory):
                result = LS._LS_GPU_basic(SMatrix=SMatrix, y=y, numIterations=self.numIterations, isSavingEachIteration=self.isSavingEachIteration, withTumor=withTumor)
            else:
                warnings.warn("Insufficient GPU memory for single GPU LS. Falling back to CPU.")

        if result is None and self.isMultiCPU:
            result = LS._LS_CPU_multi(SMatrix=SMatrix, y=y, numIterations=self.numIterations, isSavingEachIteration=self.isSavingEachIteration, withTumor=withTumor)

        if result is None:
            result = LS._LS_CPU_opti(SMatrix=SMatrix, y=y, numIterations=self.numIterations, isSavingEachIteration=self.isSavingEachIteration, withTumor=withTumor)
            if result is None:
                warnings.warn("Optimized LS failed. Falling back to basic CPU LS.")
                result = LS._LS_CPU_basic(SMatrix=SMatrix, y=y, numIterations=self.numIterations, isSavingEachIteration=self.isSavingEachIteration, withTumor=withTumor)
    ...
    def _LS_Regularized(self,SMatrix, y, withTumor):
        ...
        if check_gpu_memory(config.select_best_gpu(), required_memory):
            ...
            return LS._LS_TV_GPU(SMatrix= SMatrix, y=y,  numIterations=self.numIterations, isSavingEachIteration=self.isSavingEachIteration, withTumor=withTumor, lambda_tv=self.lambda_reg, L_Factor=self.L_Factor)
        else:
