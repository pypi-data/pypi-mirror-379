...
class AlgebraicRecon(Recon):
    ...
    def plot_MSE(self, isSaving=True):
        ...
        if isSaving and self.saveDir is not None:
            ...
            if self.optimizer == OptimizerType.MLEM:
                SavingFolder = os.path.join(self.saveDir, 'results', f'MSE_plot_MLEM{date_str}.png')
            elif self.optimizer == OptimizerType.LS:
                SavingFolder = os.path.join(self.saveDir, 'results', f'MSE_plot_LS{date_str}.png')
            elif self.optimizer == OptimizerType.LS_TV:
                SavingFolder = os.path.join(self.saveDir, 'results', f'MSE_plot_LS_TV_Lambda_{self.LS_TV_lambda}_LFactor_{self.LS_TV_LFactor}_{date_str}.png')
    ...
    def show_MSE_bestRecon(self, isSaving=True):
        ...
        im0 = axs[0].imshow(best_recon / np.max(best_recon),
        ...
        im1 = axs[1].imshow(self.experiment.OpticImage.phantom / np.max(self.experiment.OpticImage.phantom),
                            extent=(self.experiment.params.general['Xrange'][0]*1000, self.experiment.params.general['Xrange'][1]*1000,
                                    self.experiment.params.general['Zrange'][1]*1000, self.experiment.params.general['Zrange'][0]*1000),
                            cmap='hot', aspect='equal', vmin=0, vmax=1)
        ...
        im2 = axs[2].imshow(lastRecon / np.max(lastRecon),
                            extent=(self.experiment.params.general['Xrange'][0]*1000, self.experiment.params.general['Xrange'][1]*1000,
                                    self.experiment.params.general['Zrange'][1]*1000, self.experiment.params.general['Zrange'][0]*1000),
                            cmap='hot', aspect='equal', vmin=0, vmax=1)
        ...
        if isSaving and self.saveDir is not None:
            ...
            if self.optimizer == OptimizerType.MLEM:
                ...
                namePath = f'{self.SMatrix.shape[3]}_SCANS_comparison_MSE_BestANDLastRecon_LS_TV_Lambda_{self.LS_TV_lambda}_LFactor_{self.LS_TV_LFactor}_Date_{date_str}.png'
