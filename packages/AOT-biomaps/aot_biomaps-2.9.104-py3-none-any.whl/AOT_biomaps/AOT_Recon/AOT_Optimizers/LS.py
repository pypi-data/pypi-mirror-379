...
def _LS_GPU_basic(SMatrix, y, numIterations, isSavingEachIteration, withTumor):
    """
    Least Squares using GPU — fully flattened variables.
    """
    try:
        ...
        A_normalized = A_flat / torch.norm(A_flat, dim=0, keepdim=True).clamp(min=1e-8)
        y_normalized = y_flat / torch.norm(y_flat).clamp(min=1e-8)
        ...
        with torch.no_grad():
            for _ in trange(numIterations, desc=description):
                ...
                for _ in range(2):  # 2 itérations internes de gradient conjugué
                    Ap = A_normalized @ p
