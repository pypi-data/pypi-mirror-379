...
class Experiment(ABC):
    ...
    def reduceDims(self, mode='avg'):
        """
        Réduit les dimensions T, Z, X d'une matrice (T, Z, X, N) par 2 en utilisant une convolution.
        Args:
            matrix (torch.Tensor ou np.ndarray): Matrice d'entrée de forme (T, Z, X, N).
            mode (str): 'avg' (moyenne) ou 'max' (max-pooling).
        Returns:
            torch.Tensor: Matrice réduite de forme (T//2, Z//2, X//2, N).
        """
        for i, field in enumerate(self.AcousticFields):
            # Conversion en tenseur PyTorch si nécessaire
            if isinstance(field.field, np.ndarray):
                field.field = torch.from_numpy(field.field)

            # Permutation des axes pour adapter à conv3d: (N, 1, T, Z, X)
            x = field.field.permute(3, 0, 1, 2).unsqueeze(1)  # Ajoute un canal fictif
