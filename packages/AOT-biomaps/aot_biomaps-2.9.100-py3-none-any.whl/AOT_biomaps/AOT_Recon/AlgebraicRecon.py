from AcoustoOpticTomography.AOT_biomaps.AOT_Recon.AOT_Optimizers import LS
    ...
    def _AlgebraicReconPython(self,withTumor):
        ...
        if withTumor:
            ...
            if self.optimizer.value == OptimizerType.MLEM.value:
                ...
                self.reconPhantom = LS._LS(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withTumor, withTumor=withTumor)
            else:
            ...
            if self.optimizer.value == OptimizerType.MLEM.value:
                ...
                self.reconLaser = LS._LS(SMatrix=self.SMatrix, y=self.experiment.AOsignal_withoutTumor, withTumor=withTumor)
            else:
                raise ValueError(f"Only MLEM is supported for simple algebraic reconstruction. {self.optimizer.value} need Bayesian reconstruction")
