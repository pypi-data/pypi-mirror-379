...
class Tomography(Experiment):
    ...
    def plot_angle_frequency_distribution(self):
        ...
        # Inclure num_elements dans les divs (pour les patterns "tous activés")
        divs = [d for d in range(2, num_elements + 1) if num_elements % d == 0 and d % 2 == 0]
        if num_elements not in divs:  # Ajouter manuellement si pair
            divs.append(num_elements)
        divs.sort()  # Trier par ordre croissant
        ...
        for p in self.patterns:
            ...
            if np.all(bits == 1):  # Cas spécial : pattern "tous activés"
                freqs.append(num_elements)
                continue

            # Chercher le block_size correspondant
            found = False
            for block_size in divs:
                ...
                if any(np.array_equal(np.roll(pattern_check, shift), bits) for shift in range(block_size)):
                    ...
                    break
            if not found:
                freqs.append(None)  # Pattern non reconnu
        ...
        # Créer des bins centrés sur chaque div, avec une largeur uniforme
        bin_edges = np.concatenate([[divs[0] - 1], np.array(divs) - 0.5, [divs[-1] + 0.5]])
        axes[1].hist(freqs, bins=bin_edges, color='salmon', edgecolor='black', rwidth=0.8)
        ...
        axes[1].set_xticks(divs)  # Placer les ticks au centre des bins
        axes[1].set_xticklabels(divs)  # Labels = valeurs des divs
    ...
    def _generate_patterns(self, N):
        ...
        results = set()  # Utiliser un ensemble pour éviter les doublons automatiquement

        # 2. Générer jusqu'à N patterns uniques
        while len(results) < N:
    ...
    def _check_patterns(self, patterns):
        # 1. Vérifier les doublons exacts (même hex ET même angle)
