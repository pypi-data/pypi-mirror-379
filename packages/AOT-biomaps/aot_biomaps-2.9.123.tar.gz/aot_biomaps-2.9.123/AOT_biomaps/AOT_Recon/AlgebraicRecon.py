...
class AlgebraicRecon(Recon):
    ...
    def plot_MSE(self, isSaving=True):
        ...
        if isSaving and self.saveDir is not None:
            ...
            if self.optimizer == OptimizerType.MLEM:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_MLEM{date_str}.png')
            elif self.optimizer == OptimizerType.LS:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_LS{date_str}.png')
            elif self.optimizer == OptimizerType.LS_TV:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_LS_TV_Lambda_{self.lambda_reg}_LFactor_{self.L_Factor}_{date_str}.png')
    ...
    def plot_SSIM(self, isSaving=True):
        ...
        if isSaving and self.saveDir is not None:
            ...
            date_str = now.strftime("%Y_%d_%m_%y")
            if self.optimizer == OptimizerType.MLEM:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_SSIM_plot_MLEM{date_str}.png')
            elif self.optimizer == OptimizerType.LS:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_SSIM_plot_LS{date_str}.png')
            elif self.optimizer == OptimizerType.LS_TV:
                SavingFolder = os.path.join(self.saveDir, f'{self.SMatrix.shape[3]}_SCANS_SSIM_plot_LS_TV_Lambda_{self.lambda_reg}_LFactor_{self.L_Factor}_{date_str}.png')
            plt.savefig(SavingFolder, dpi=300)
