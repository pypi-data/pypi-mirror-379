...
class AlgebraicRecon(Recon):
    ...
    def show_MSE_bestRecon(self, isSaving=True):
        ...
        im0 = axs[0].imshow(best_recon / lambda_max,
                            extent=(self.experiment.params.general['Xrange'][0], self.experiment.params.general['Xrange'][1],
                                    self.experiment.params.general['Zrange'][1], self.experiment.params.general['Zrange'][0]),
        ...
        if self.experiment.OpticImage.laser.intensity.shape != lastRecon.shape:
            lastRecon = lastRecon.T  # Transpose si les dimensions ne correspondent pas
        ...
        if isSaving:
            if self.saveDir is None:
                raise ValueError("saveDir is not set. Please provide a valid save directory.")
            now = datetime.now()
            date_str = now.strftime("%Y_%d_%m_%y")
            savePath = os.path.join(self.saveDir, 'results')
            if not os.path.exists(savePath):
                os.makedirs(savePath)
            SavingFolder = os.path.join(savePath, f'comparison_MSE_BestANDLastRecon{date_str}.png')
            plt.savefig(SavingFolder, dpi=300)
            print(f"MSE plot saved to {SavingFolder}")
