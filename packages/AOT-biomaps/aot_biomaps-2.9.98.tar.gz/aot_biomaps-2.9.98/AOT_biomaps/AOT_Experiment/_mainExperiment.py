...
class Experiment(ABC):
    ...
    def reduceDims(self, mode='avg'):
        """
        Réduit les dimensions T, Z, X d'une matrice (T, Z, X, N) ou (T, Z, X) par 2 en utilisant une convolution.
        Met à jour les paramètres numériques, y compris les valeurs en notation scientifique.
        """
        for i in trange(len(self.AcousticFields),
                        desc="Downsampling Acoustic Fields (T, Z, X → T//2, Z//2, X//2)"):
            # Conversion en tenseur PyTorch si nécessaire
            if isinstance(self.AcousticFields[i].field, np.ndarray):
                self.AcousticFields[i].field = torch.from_numpy(self.AcousticFields[i].field)

            # Vérification de la forme du tenseur
            if self.AcousticFields[i].field.dim() == 3:
                # Cas (T, Z, X) : ajout d'une dimension fictive N=1
                x = self.AcousticFields[i].field.unsqueeze(-1)  # (T, Z, X, 1)
            elif self.AcousticFields[i].field.dim() == 4:
                # Cas (T, Z, X, N) : permutation normale
                x = self.AcousticFields[i].field
            else:
                raise ValueError(f"Forme non supportée : {self.AcousticFields[i].field.shape}")

            # Permutation des axes pour adapter à conv3d: (N, 1, T, Z, X)
            x = x.permute(3, 0, 1, 2).unsqueeze(1)  # (N, 1, T, Z, X)
            ...
            if mode == 'avg':
                ...
                raise ValueError("Mode invalide. Utilisez 'avg' ou 'max'.")

            # Retour à l'ordre original (T//2, Z//2, X//2, N)
            self.AcousticFields[i].field = x_down.squeeze(1).permute(1, 2, 3, 0)

        # Mise à jour des paramètres numériques
        def convert_and_update(param_dict, key, operation):
            """Convertit une valeur en float et applique une opération."""
            if isinstance(param_dict[key], str):
                param_dict[key] = float(param_dict[key])
            param_dict[key] = operation(param_dict[key])

        # Mise à jour de f_saving (division par 2)
        convert_and_update(self.params.acoustic, 'f_saving', lambda x: x / 2)

        # Mise à jour de dx, dy, dz (multiplication par 2)
