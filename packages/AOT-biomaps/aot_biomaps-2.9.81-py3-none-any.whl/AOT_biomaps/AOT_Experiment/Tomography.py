...
class Tomography(Experiment):
    ...
    def _generate_patterns(self, N):
        ...
        def bits_to_hex(bits):
            ...
            return f"{int(bit_string, 2):0{self.params.acoustic['num_elements']//4}x}"
        ...
        divs = [d for d in range(2, num_elements + 1) if num_elements % d == 0 and d % 2 == 0]
        if not divs:
            print(f"Aucun diviseur pair valide trouvé pour num_elements = {num_elements}")
            return []

        angle_choices = list(range(-20, 21))
        results = set()

        # 2. Générer des patterns pour chaque diviseur (y compris num_elements)
        for block_size in divs:
            if block_size == num_elements:
                # Cas spécial : pattern "tous activés" (un seul motif, mais pour tous les angles)
                all_on_bits = np.ones(num_elements, dtype=int)
                all_on_hex = bits_to_hex(all_on_bits)
                for angle in angle_choices:
                    pair = f"{all_on_hex}_{format_angle(angle)}"
                    results.add(pair)
            else:
                # Cas général : patterns équilibrés
                half_block = block_size // 2
                block = np.array([0] * half_block + [1] * half_block)
                reps = num_elements // block_size
        ...
        # 3. Convertir en liste et mélanger
        results = list(results)
        np.random.shuffle(results)

        # 4. Retourner les N premiers (ou compléter si nécessaire)
        if len(results) >= N:
            return results[:N]
        else:
            # Si pas assez de patterns, compléter avec des doublons aléatoires
            extra_needed = N - len(results)
            return results + list(np.random.choice(results, extra_needed, replace=True))
    ...
    def _check_patterns(self, patterns):
        for p in patterns:
            ...
            bits = np.array([int(b) for b in bin(int(hex_part, 16))[2:].zfill(self.params.acoustic['num_elements'])])
            if len(bits) != self.params.acoustic['num_elements']:
                print(f"Erreur longueur: {p}")
                return False
            if np.all(bits == 1):
                continue
            if np.sum(bits) != self.params.acoustic['num_elements'] // 2:
                print(f"Erreur équilibre 0/1: {p}")
                return False
            valid = False
            for block_size in range(2, self.params.acoustic['num_elements']+1, 2):
                if self.params.acoustic['num_elements'] % block_size != 0:
                    continue
                half_block = block_size // 2
                block = np.array([0]*half_block + [1]*half_block)
                reps = self.params.acoustic['num_elements'] // block_size
