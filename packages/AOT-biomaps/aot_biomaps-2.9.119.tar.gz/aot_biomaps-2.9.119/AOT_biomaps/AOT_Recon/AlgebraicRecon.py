    ...
    def plot_MSE(self, isSaving=True):
        ...
        if isSaving and self.saveDir is not None:
            ...
            if self.optimizer == OptimizerType.MLEM:
                SavingFolder = os.path.join(self.saveDir, 'results', f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_MLEM{date_str}.png')
            elif self.optimizer == OptimizerType.LS:
                SavingFolder = os.path.join(self.saveDir, 'results', f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_LS{date_str}.png')
            elif self.optimizer == OptimizerType.LS_TV:
                SavingFolder = os.path.join(self.saveDir, 'results', f'{self.SMatrix.shape[3]}_SCANS_MSE_plot_LS_TV_Lambda_{self.lambda_reg}_LFactor_{self.L_Factor}_{date_str}.png')
