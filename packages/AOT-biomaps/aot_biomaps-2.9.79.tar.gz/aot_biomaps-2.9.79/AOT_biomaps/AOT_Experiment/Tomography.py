...
class Tomography(Experiment):
    ...
    def _generate_patterns(self, N):
        ...
        def bits_to_hex(bits):
            ...
            return f"{int(bit_string, 2):0{params.acoustic['num_elements']//4}x}"
    ...
    def _check_patterns(self, patterns):
        for p in patterns:
            ...
            bits = np.array([int(b) for b in bin(int(hex_part, 16))[2:].zfill(params.acoustic['num_elements'])])
            if len(bits) != params.acoustic['num_elements']:
                print(f"Erreur longueur: {p}")
                return False
            if np.all(bits == 1):
                continue
            if np.sum(bits) != params.acoustic['num_elements'] // 2:
                print(f"Erreur équilibre 0/1: {p}")
                return False
            valid = False
            for block_size in range(2, params.acoustic['num_elements']+1, 2):
                if params.acoustic['num_elements'] % block_size != 0:
                    continue
                half_block = block_size // 2
                block = np.array([0]*half_block + [1]*half_block)
                reps = params.acoustic['num_elements'] // block_size
