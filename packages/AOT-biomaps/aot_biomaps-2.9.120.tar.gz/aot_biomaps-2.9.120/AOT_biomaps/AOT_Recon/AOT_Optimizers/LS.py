...
def _LS_TV_GPU(SMatrix, y, numIterations, isSavingEachIteration, withTumor, lambda_tv=1e-3, L_Factor=1.0, renormalize_output=True):
    ...
    # Normalisation (optionnelle, à ajuster selon tes données)
    ...
    theta_history = []
    mse_history = []  # Pour suivre l'évolution du MSE
    ...
    for k in trange(numIterations, desc=description):
        ...
        # Contrainte de positivité (optionnelle, à commenter si non souhaitée)
        ...
        # Calcul du MSE actuel (pour suivi)
        current_mse = torch.nn.functional.mse_loss(A_flat @ theta_flat, y_flat).item()
        mse_history.append(current_mse)

        if isSavingEachIteration:
            theta_history.append(theta_flat.reshape(Z, X).clone())

    # Renormalisation finale (optionnelle, pour l'affichage)
    if renormalize_output and A_norm > 0 and y_norm > 0:
        theta_flat = theta_flat * (y_norm / (A_norm + 1e-8))

    # Renvoi des résultats
    if isSavingEachIteration:
        return [t.cpu().numpy() for t in theta_history], mse_history
    else:
        return theta_flat.reshape(Z, X).cpu().numpy(), mse_history[-1]  # Retourne aussi le MSE final
