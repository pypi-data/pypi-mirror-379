...
class Tomography(Experiment):
    ...
    def plot_angle_frequency_distribution(self, patterns, num_elements=192):
        """
    ...
    def _generate_patterns(self, N):
        ...
        # 1. Générer le pattern "tous les éléments activés" pour tous les angles
        ...
        # 2. Calculer combien de patterns équilibrés il reste à générer
        remaining_N = N - len(all_on_pairs)
        if remaining_N <= 0:
            return list(results)[:N]

        # 3. Trouver tous les diviseurs de num_elements (sauf 1 et num_elements)
        divs = [d for d in range(2, num_elements) if num_elements % d == 0]
        angle_choices = list(range(-20, 21))
        nb_freq = len(divs)
        max_per_block = int(np.ceil(remaining_N / nb_freq)) if nb_freq > 0 else 0
        ...
        if len(results) < N:
            ...
            results += list(np.random.choice(results, extra_needed, replace=True))
