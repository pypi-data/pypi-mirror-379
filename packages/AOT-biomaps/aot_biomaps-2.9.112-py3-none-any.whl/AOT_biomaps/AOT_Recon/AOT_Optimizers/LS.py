...
def _LS_GPU_basic(SMatrix, y, numIterations, isSavingEachIteration, withTumor):
    """
    Least Squares using GPU — version corrigée pour SMatrix(511,175,200,21) et y(511,21)
    """
    try:
        device = torch.device(f"cuda:{config.select_best_gpu()}")
        T, Z, X, N = SMatrix.shape  # 511, 175, 200, 21
        ZX = Z * X  # 175 * 200 = 35000
        TN = T * N  # 511 * 21 = 10731
        ...
        A_flat = torch.from_numpy(SMatrix).to(device=device, dtype=torch.float32)
        A_flat = A_flat.permute(0, 3, 1, 2).reshape(TN, ZX)  # (10731, 35000)
        y_flat = torch.from_numpy(y).to(device=device, dtype=torch.float32).reshape(TN)  # (10731,)

        # Initialisation de theta
        theta_flat = torch.zeros(ZX, dtype=torch.float32, device=device)  # (35000,)

        # Préparation pour sauvegarde équitable (1000 itérations max)
        if isSavingEachIteration:
            ...
            if numIterations <= 1000:
                ...
                save_indices = {i * step for i in range(1000)}
                save_indices.add(numIterations - 1)
            save_indices = sorted(save_indices)
        ...
        with torch.no_grad():
            ...
            # Ajout de la dernière itération si elle n'est pas déjà incluse
            if isSavingEachIteration and numIterations - 1 not in save_indices:
                matrix_theta_torch.append(theta_flat.reshape(Z, X).clone())
...
def _LS_TV_GPU(SMatrix, y, numIterations, isSavingEachIteration, withTumor, lambda_tv=1e-3, L_Factor=1.0):
    ...
    if isSavingEachIteration:
        theta_history = []
        # Calcul des indices des itérations à sauvegarder
        if numIterations <= 1000:
            save_indices = list(range(numIterations))  # Sauvegarde toutes les itérations
        else:
            step = max(1, numIterations // 1000)  # Évite step=0
            save_indices = {i * step for i in range(1000)}
            save_indices.add(numIterations - 1)  # Inclure la dernière itération
        save_indices = sorted(save_indices)  # Tri pour itération ordonnée
    ...
    for k in trange(numIterations, desc=description):
        ...
        theta_new = torch.clamp(theta_new, min=0.0)  # Positivité forcée
    ...
    if isSavingEachIteration:
        # Ajout de la dernière itération si elle n'est pas déjà incluse
        if numIterations - 1 not in save_indices:
            theta_history.append(theta_flat.reshape(Z, X).clone())
        # Retourne exactement 1000 éléments (ou moins si numIterations < 1000)
