...
class Tomography(Experiment):
    ...
    def plot_angle_frequency_distribution(self):
        ...
        # Histogramme des angles (inchangé)
        ...
        # Utiliser des positions discrètes pour les bins (0, 1, 2, ...)
        unique_freqs, freq_counts = np.unique(freqs, return_counts=True)
        x_pos = np.arange(len(divs))  # Positions des bins : 0, 1, 2, ...

        # Tracer les barres aux positions discrètes
        for freq, count in zip(unique_freqs, freq_counts):
            idx = divs.index(freq)
            axes[1].bar(x_pos[idx], count, color='salmon', edgecolor='black', width=0.8)

        # Configurer l'axe des x pour afficher les labels des divs
