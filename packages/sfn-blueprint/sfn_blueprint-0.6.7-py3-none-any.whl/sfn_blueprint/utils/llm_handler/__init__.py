...
class SFNAIHandler:
    ...
    def route_to(self, llm_provider, configuration, model,db_url=None):
        ...
        kwargs_key = {"model": model}
        if "temperature" in configuration:
            kwargs_key["temperature"] = configuration["temperature"]
        if "max_tokens" in configuration:
            kwargs_key["max_tokens"] = configuration["max_tokens"]
        if "top_p" in configuration:
            kwargs_key["top_p"] = configuration["top_p"]
        if session:
            kwargs_key["session"] = session
