...
from .config import BlankLineConfig, config as globalConfig, setConfig
...
def _processFile(filepath, args, config):
  ...
  changeDetails = FileProcessor.processFile(filepath, config, checkOnly=checkOnly, returnDetails=args.dry_run and args.verbose)
...
def main():
  ...
  parser.add_argument(
    '--dry-run', action='store_true', help='Show what changes would be made without applying them'
  )
  parser.add_argument(
    '--verbose', action='store_true', help='Show detailed output for --check or --dry-run mode'
  )
  ...
  for pathStr in args.paths:
    ...
    if path.is_file() and path.suffix == '.py':
      processedCount += 1
      changed, fileExitCode = _processFile(path, args, config)
      ...
      for pyFile in path.rglob('*.py'):
        processedCount += 1
        changed, fileExitCode = _processFile(pyFile, args, config)
