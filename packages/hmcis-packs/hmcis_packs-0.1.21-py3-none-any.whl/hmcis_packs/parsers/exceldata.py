...
class ExcelParser:
    ...
    def __enter__(self):
        ...
        try:
            ...
            logger.info("Sheets count: %s, names: %s",
                        len(self._wb.sheetnames), self._wb.sheetnames)
            return self
        ...
        if isinstance(self.sheet, int):
            try:
                if self.sheet < 0 or self.sheet >= len(wb.sheetnames):
                    raise IndexError(f"Sheet index {self.sheet} is out of range (0-{len(wb.sheetnames) - 1})")

                sheet_name = wb.sheetnames[self.sheet]
                sheet = wb[sheet_name]
                logger.info("Resolved sheet by 0-based index %d: %s", self.sheet, sheet.title)
                return sheet
            ...
            if len(matches) > 1:
                logger.warning("Multiple sheets matched '%s': %s; using the first: %s",
                               name_sub, matches, matches[0])
        ...
        for i, sheet_name in enumerate(self._wb.sheetnames):
            try:
                ...
                if sheet.max_row == 1 and sheet.max_column == 1:
                    ...
                    range_address = f"A1:{openpyxl.utils.get_column_letter(col_count)}{row_count}"
...
if __name__ == "__main__":
    ...
    try:
        with ExcelParser(
                "example_file.xlsx",  # Замените на реальный путь
            ...
            for info in sheets_info:
                print(f"Лист: {info.name}, строк: {info.row_count}, столбцов: {info.col_count}")
