...
try:
    from genebot.strategies.strategy_config import StrategyConfigManager
    STRATEGY_CONFIG_AVAILABLE = True
    STRATEGY_CONFIG_IMPORT_ERROR = None
except ImportError as e:
    StrategyConfigManager = None
    STRATEGY_CONFIG_AVAILABLE = False
    STRATEGY_CONFIG_IMPORT_ERROR = str(e)

try:
    from src.exchanges.ccxt_adapter import CCXTAdapter
    CCXT_ADAPTER_AVAILABLE = True
    CCXT_ADAPTER_IMPORT_ERROR = None
except ImportError as e:
    CCXTAdapter = None
    CCXT_ADAPTER_AVAILABLE = False
    CCXT_ADAPTER_IMPORT_ERROR = str(e)
...
class MonitorCommand(BaseCommand):
    ...
    def execute(self, args: Namespace) -> CommandResult:
        ...
        dependency_result = self._check_monitoring_dependencies()
    ...
    def _check_monitoring_dependencies(self) -> CommandResult:
        """Check monitoring component dependencies"""
        warnings = []
        
        # Check trade logger dependencies (optional but recommended)
        trade_logger_result = self.check_trade_logger_dependencies(required=False)
        if trade_logger_result.status.value == 'warning':
            warnings.extend(trade_logger_result.suggestions)
        
        # Check strategy config availability
        if not STRATEGY_CONFIG_AVAILABLE:
            warnings.extend([
                "Strategy configuration not available - strategy info limited",
                f"Import error: {STRATEGY_CONFIG_IMPORT_ERROR}"
            ])
        
        # Check CCXT adapter availability
        if not CCXT_ADAPTER_AVAILABLE:
            warnings.extend([
                "CCXT adapter not available - exchange info limited",
                f"Import error: {CCXT_ADAPTER_IMPORT_ERROR}",
                "Install exchange support: pip install ccxt"
            ])
        
        # Check monitoring dependencies (optional)
        monitoring_result = self.check_monitoring_dependencies(required=False)
        if monitoring_result.status.value == 'warning':
            warnings.extend(monitoring_result.suggestions)
        
        if warnings:
            return CommandResult.warning(
                "Monitoring available with limited functionality",
                suggestions=warnings
            )
        
        return CommandResult.success("Monitoring dependencies available")
