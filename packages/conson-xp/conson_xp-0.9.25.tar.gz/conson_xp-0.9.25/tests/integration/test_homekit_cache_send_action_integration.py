...
class TestHomeKitCacheServiceSendActionIntegration:
    ...
    def test_send_action_with_event_telegram_integration(self, mock_telegram_service_class, mock_conbus_service_class):
        ...
        with patch.object(cache_service, 'received_event') as mock_received_event:
            ...
            mock_conbus_service.send_action.assert_called_once_with("2113010000", 1, ActionType.PRESS)

            # Verify telegram service parsed the raw telegram
            mock_telegram_service.parse_event_telegram.assert_called_once_with("<E14L01I02MAK>")
    ...
    @patch('xp.services.homekit_cache_service.ConbusOutputService')
    def test_send_action_with_event_telegram_integration_2(self, mock_conbus_service_class):
        """Integration test for send_action method when receiving EventTelegram."""
        # Setup mocks
        mock_conbus_service = Mock()
        mock_conbus_service_class.return_value = mock_conbus_service
        ...
        mock_response = Mock()
        mock_response.success = True
        mock_response.received_telegrams = [mock_received_telegram_reply, mock_received_telegram_event]
        mock_conbus_service.send_action.return_value = mock_response
    ...
    @patch('xp.services.homekit_cache_service.ConbusOutputService')
    @patch('xp.services.homekit_cache_service.TelegramService')
    def test_send_action_failed_response_integration(self, mock_telegram_service_class, mock_conbus_service_class):
    ...
    @patch('xp.services.homekit_cache_service.ConbusOutputService')
    @patch('xp.services.homekit_cache_service.TelegramService')
    def test_send_action_no_received_telegrams_integration(self, mock_telegram_service_class, mock_conbus_service_class):
