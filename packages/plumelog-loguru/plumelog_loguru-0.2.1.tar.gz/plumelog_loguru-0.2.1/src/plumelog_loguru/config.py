...
class OverflowStrategy(str, Enum):
    """队列溢出策略枚举"""
    DROP_NEW = "drop_new"  # 丢弃新日志
    DROP_OLD = "drop_old"  # 丢弃最旧的日志
    BUFFER = "buffer"  # 使用临时缓存（默认）
...
class PlumelogSettings(BaseSettings):
    ...
    temp_buffer_max_size: int = Field(
        default=2000, ge=1, description="临时缓存最大容量"
    )

    # 溢出策略配置
    overflow_strategy: OverflowStrategy = Field(
        default=OverflowStrategy.BUFFER,
        description="队列满时的处理策略：drop_new(丢弃新日志)、drop_old(丢弃旧日志)、buffer(使用临时缓存)"
    )
