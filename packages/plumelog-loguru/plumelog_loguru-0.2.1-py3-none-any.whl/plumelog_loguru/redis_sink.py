...
from .config import PlumelogSettings, OverflowStrategy
...
class RedisSink:
    ...
    async def _async_handle_log(self, log_record: LogRecord) -> None:
        ...
        try:
            ...
            # 使用 put_nowait 避免阻塞，队列满时根据策略处理
            try:
                self._log_queue.put_nowait(log_record)
            except asyncio.QueueFull:
                # 根据溢出策略处理
                if self.config.overflow_strategy == OverflowStrategy.DROP_NEW:
                    print(f"[RedisSink] 队列已满 ({self.config.queue_max_size})，根据策略丢弃新日志")
                    # 直接丢弃，不做任何操作
                elif self.config.overflow_strategy == OverflowStrategy.DROP_OLD:
                    print(f"[RedisSink] 队列已满 ({self.config.queue_max_size})，丢弃最旧的日志")
                    try:
                        # 尝试移除最旧的日志
                        ...
                        # 如果无法移除或添加，退回到临时缓存
                        self._store_to_temp_buffer(log_record)
                else:  # OverflowStrategy.BUFFER (默认)
                    print(f"[RedisSink] 队列已满 ({self.config.queue_max_size})，日志暂存到临时缓存")
                    self._store_to_temp_buffer(log_record)
    ...
    async def _try_recover_from_temp_buffer(self) -> None:
        """尝试将临时缓存中的日志恢复到队列"""
        if not self._log_queue or self._log_queue.full():
            return
        ...
        with self._temp_buffer_lock:
            ...
            # 计算可恢复的数量
            available_space = self._log_queue.maxsize - self._log_queue.qsize()
            recover_count = min(len(self._temp_buffer), available_space)
            ...
            for _ in range(recover_count):
                recovered.append(self._temp_buffer.popleft())

        # 在锁外进行队列操作
        recovered_count = 0
        for log_record in recovered:
            try:
                self._log_queue.put_nowait(log_record)
                recovered_count += 1
            except asyncio.QueueFull:
                # 重新放回临时缓存
                with self._temp_buffer_lock:
                    self._temp_buffer.appendleft(log_record)
                break

        if recovered_count > 0:
            print(f"[RedisSink] 从临时缓存恢复了 {recovered_count} 条日志到队列")

    async def _log_consumer(self) -> None:
        """后台消费者任务，持续从队列中获取日志并发送到Redis"""
        assert self._log_queue is not None, "队列未初始化"

        last_recovery_time = asyncio.get_event_loop().time()
        recovery_interval = 5.0  # 每5秒尝试恢复临时缓存的日志
        ...
        while self._running or not self._log_queue.empty():
            try:
                ...
                if current_time - last_recovery_time >= recovery_interval:
                    await self._try_recover_from_temp_buffer()
                ...
                if success:
                    ...
                    for _ in batch:
                        self._log_queue.task_done()
                    # 发送失败时，暂存到临时缓存而不是重新入队，避免死锁
                    print(f"[RedisSink] Redis发送失败，{len(batch)} 条日志暂存到临时缓存")
                    for log in batch:
                        self._store_to_temp_buffer(log)
