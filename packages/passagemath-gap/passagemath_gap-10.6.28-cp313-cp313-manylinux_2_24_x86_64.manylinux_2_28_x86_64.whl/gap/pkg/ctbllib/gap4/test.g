...
##    two character tables of the symmetric group of degree <M>n</M>
##    (from the &ATLAS; and from the <Package>SpinSym</Package> package)
##    are consistent.
##    The interesting values of <A>n</A> are in <C>[ 5 .. 13 ]</C>.
##  </Item>
##  <#/GAPDoc>
##
CTblLib.Test.TablesOfSymmetricGroup:= function( n )
    local result, t, tt, trans, classperm, pi, p, tmod, ttmod, pclassperm,
          prowperm;
    ...
    if Permuted( CharacterParameters( t ), trans.rows ) <>
       CharacterParameters( tt ) then
      Print( "#E  n = ", n, ": problem with character parameters\n" );
      result:= false;
    fi;
      ...
      if ttmod = fail then
        ...
        if not HasCharacterParameters( tmod ) then
          Print( "#I  character parameters missing in ", tmod, ",\n",
                 ...
                 Permuted( CharacterParameters( ttmod ), prowperm^-1 ) ),
                 " ", "" ), ")\n" );
        elif Permuted( CharacterParameters( tmod ), prowperm ) <>
             CharacterParameters( ttmod ) then
          Print( "#E  n = ", n, ", p = ", p,
                 ": problem with character parameters\n" );
          result:= false;
        fi;
      fi;
    od;
