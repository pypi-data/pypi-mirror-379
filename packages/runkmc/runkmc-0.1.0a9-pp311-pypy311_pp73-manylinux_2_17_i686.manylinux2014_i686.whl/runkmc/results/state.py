...
class SequenceData:
    ...
    def _from_df(df: pd.DataFrame, monomer_names: Optional[List[str]]) -> SequenceData:
        ...
        if monomer_names is None or len(monomer_names) == 0:
            ...
            for col in df.columns:
                if col.startswith("MonomerCount_"):
                    monomer_names.append(col.replace("MonomerCount_", ""))
        ...
        return SequenceData(
            ...
            monomer_count={
                name: df[f"MonomerCount_{name}"].to_numpy(np.uint64)
                for name in monomer_names
            },
            sequence_count={
                name: df[f"SequenceCount_{name}"].to_numpy(np.uint64)
                for name in monomer_names
            },
            sequence_length2={
                name: df[f"SequenceLengths2_{name}"].to_numpy(np.float64)
                for name in monomer_names
            },
            _raw_data=df,
    ...
    def from_csv(filepath: Path | str, metadata: Metadata) -> SequenceData:
        ...
        monomer_names = list(metadata.get_monomer_map().values())
