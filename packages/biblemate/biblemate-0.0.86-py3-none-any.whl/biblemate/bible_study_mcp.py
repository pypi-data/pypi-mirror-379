...
def treasury(reference:str) -> dict:
    """Treasury of Scripture Knowledge (Enhance); usage examples: `//treasury/John 3:16`, `//treasury/Deut 6:4`"""
...
def lexicons() -> str:
    """UBA Lexicons; usage examples: `//lexicon/G25`, `//lexicon/TBESH/G25`, `//lexicon/TBESH/H3478`"""
    resources = json.loads(run_uba_api(".resources"))
    return "\n".join([f"- `{r}`" for r in resources["lexiconList"]])

@mcp.resource("lexicon://{module}/{entry}")
def lexicon(module:str, entry:str) -> str:
    """UBA Lexicon; ; usage examples: `//lexicon/G25`, `//lexicon/TBESH/G25`, `//lexicon/TBESH/H3478`"""
    import re
...
def retrieve_bible_chapter(request:str) -> str:
    ...
    refs = BibleVerseParser(False).extractAllReferencesReadable(command)
    if not refs:
        return "Please provide a valid Bible reference to complete your request."
    return run_uba_api(f"CHAPTER:::{config.default_bible}:::{refs}")
