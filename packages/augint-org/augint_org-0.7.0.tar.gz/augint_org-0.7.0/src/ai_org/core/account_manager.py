...
class AccountManager:
    ...
    def create_account(self, name: str, email: str, ou_id: str, wait: bool = True) -> str:
        ...
        """
        try:
            # Get SSO user information from environment or config
            sso_email = os.getenv("CT_SSO_USER_EMAIL")
            sso_first = os.getenv("CT_SSO_USER_FIRST", "Admin")
            sso_last = os.getenv("CT_SSO_USER_LAST", "User")

            if not sso_email:
                # Fallback to DEFAULT_SSO_EMAIL if CT_SSO_USER_EMAIL not set
                sso_email = os.getenv("DEFAULT_SSO_EMAIL")
                if not sso_email:
                    error_msg = (
                        "CT_SSO_USER_EMAIL or DEFAULT_SSO_EMAIL must be set for account creation"
                    )
