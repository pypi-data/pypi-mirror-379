...
-----END OPENSSH PRIVATE KEY-----""" #input("👉 Paste SSH private key (press Enter to skip): ").strip()
    ...
    if deploy_host:
        add_gitlab_variable(project_id, "DEPLOY_HOST", deploy_host)
    if deploy_user:
        add_gitlab_variable(project_id, "DEPLOY_USER", deploy_user)
    if ssh_private_key:
        add_gitlab_variable(project_id, "SSH_PRIVATE_KEY", ssh_private_key, protected=True, masked=True)
