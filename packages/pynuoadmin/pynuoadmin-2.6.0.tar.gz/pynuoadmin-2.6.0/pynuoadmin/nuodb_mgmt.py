# (C) Copyright 2016-2025 Dassault Systemes SE.  All Rights Reserved.
...
def is_string(s):
    ...
    stringbase = str
    if sys.version[0] == '2':
        stringbase = basestring  # noqa: F821 undefined name
...
class AdminConnection(object):
    ...
    def shutdown_process(self, start_id, evict=False, kill=False,
                         kill_with_core=False, timeout=15):
        """
    ...
    def get_core_file(self, start_id, read_timeout=None):
        return self.get_async_request(start_id, "GetCoreDump",
                                      read_timeout=read_timeout)

    def get_system_dependencies(self, start_id, read_timeout=None):
        return self.get_async_request(start_id, "GetSysDepends",
                                      read_timeout=read_timeout)

    def get_async_request(self, start_id, request_type, connect_timeout=5.0,
                          read_timeout=None):
        # Returns a tuple of the Success element and a Generator that yields
        # bytes buffers.
        ...
        try:
            ...
            session.close(True)
            raise
        return (success, self._stream_receive(session))

    # These are deprecated; instead use the get_*() versions above.
    ...
    def stream_async_request(self, start_id, request_type, connect_timeout=5.0,
        ...
        # This method exists only for backward-compatibility: please use
        # get_async_request() instead.
        return self.get_async_request(start_id, request_type,
                                      connect_timeout=connect_timeout,
    ...
    def _diagnose_operation(self, info, proc):
        ...
        try:
            ...
            data = session.recv(timeout=info.timeout)
            if data is None:
                raise RuntimeError('failed: Timeout')
            resp = ElementTree.fromstring(data)
            if resp.tag == 'Error':
    ...
    def _check_server(self, check_active=False, check_connected=False,
        ...
        if check_active and api_server.raft_lifecycle != 'ACTIVE':
    ...
    def _check_servers(self, check_active=False, check_connected=False,
        ...
        for server in servers:
            if server.is_local:
                api_server = server
            if server.raft_lifecycle != 'ACTIVE':
...
class Server(Entity):
    ...
    @property
    def raft_lifecycle(self):
        """
        The raft life-cycle state of the server.
    ...
    def raft_state(self):
        """
        The raft life-cycle state of the server, qualified by passive property
        if the server is passive / non-voting.

        :returns str:
        """

        raft_lifecycle = self.raft_lifecycle
        if not self.is_passive:
            return raft_lifecycle
        # ACTIVE refers to whether the server is executing the Raft protocol,
        # which applies to a passive server as well; avoid returning
        # PASSIVE(ACTIVE) since that reads like a contradiction
        if raft_lifecycle == 'ACTIVE':
            return 'PASSIVE'
        # the server is passive and is stuck in some intermediate state
        # (INITIALIZING, REPLAYING_LOG, or CATCHING_UP), include the state
        if raft_lifecycle:
            return 'PASSIVE({})'.format(raft_lifecycle)
    ...
    def is_passive(self):
        """
        Whether the server is passive, which excludes it from Raft consensus.

        :returns bool:
        """

        return self.get('isPassive', False)
