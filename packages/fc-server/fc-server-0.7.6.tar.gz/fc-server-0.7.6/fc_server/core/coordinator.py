...
class Coordinator:
    ...
    async def __set_resource_status_post(self, resource, status, caller, purpose):
        ...
        if framework:
            ...
            if purpose:
                ...
                self.logger.info(
                    "%s(%s): resource acquired by %s", resource, status, owner
                )
        if status == "fc":
            self.logger.info("%s(%s): resource released", resource, caller)

        await self.publisher_queue.put({resource: status})

    def __set_resource_status(self, resource, status, caller="", purpose=""):
        self.__managed_resources_status[resource] = status

        self.logger.info("* %s now belongs to %s", resource, status)
        asyncio.create_task(
            self.__set_resource_status_post(resource, status, caller, purpose)
        )
    ...
    def accept_resource(self, resource, context, purpose=""):
        ...
        self.__set_resource_status(
            resource, context.__module__.split(".")[-1], purpose=purpose
        )
