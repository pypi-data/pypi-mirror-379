...
Since version 1.0.0 s2Downloader uses the new version of element84's catalog for querying through the downloadable data.
If you want to use the old version of the catalog (v0) for any reason, fall back to version 0.4.3 of s2Downloader.
More information about the
`catalog changes <https://www.element84.com/blog/introducing-earth-search-v1-new-datasets-now-available>`_.
...
At the command line, S2Downloader provides the **s2downloader_cli.py**. Run with relative or absolute path to config
json file:
::
...
The package expects a configuration file in ``json`` format, like the
`default_config.json <https://git.gfz-potsdam.de/fernlab/products/s2downloader/-/blob/main/data/default_config.json>`_
in the repository and the sentinel-2 tile grid file in ``zipped shape`` format, like the
`sentinel_2_index_shapefile_attr.zip <https://git.gfz-potsdam.de/fernlab/products/s2downloader/-/blob/main/data/
sentinel_2_index_shapefile_attr.zip?ref_type=heads>`_
(define the path for the grid file in "s2_settings": "tiles_definition_path").

A valid configuration for downloading data might look like follows:
...
Simplified example for using a Polygon instead of a bounding box. The bounding_box option has to stay as empty list when
a polygon is added.
...
**Note:** To get the correct UTM zone, latitude band and grid square for downloading complete tiles either the tile
grid `sentinel_2_index_shapefile_attr.zip <https://git.gfz-potsdam.de/fernlab/products/s2downloader/-/blob/main/data
/sentinel_2_index_shapefile_attr.zip>`_ can be displayed in a GIS or this
`web map <https://eatlas.org.au/data/uuid/f7468d15-12be-4e3f-a246-b2882a324f59>`_ can be used.
