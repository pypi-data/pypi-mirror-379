...
class ChatCLI:
    ...
    async def _generate_code_map_for(self, root: str) -> None:
        ...
        if example_block:
            prompt_lines.append("\nStyle example (not the same project, just for reference):")
                        ...
                        if event == "session.started":
                            ...
                            try:
                                if context_window > 0:
                                    ...
                                    # Show the bar for this reply's usage and include token count (in+out)
                                    self.ui.print(
                                        f"[ctx] cxt used: {int(last_total)} {bar}",
                                        style=self.ui.theme["dim"],
                                        force=True,
                                    )
                            except Exception:
