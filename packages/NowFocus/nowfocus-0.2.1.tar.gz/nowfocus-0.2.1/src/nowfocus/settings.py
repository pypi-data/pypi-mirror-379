...
class SettingsWindow(Gtk.Window):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        show_long_sessions_button.connect("clicked", self.show_long_sessions)
        self.operations_box.add(show_long_sessions_button)

        self.long_sessions_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
        self.operations_box.add(self.long_sessions_box)

        # self.show_long_sessions()
        self.show_all()

        

    def show_long_sessions(self, limit):

        self.long_sessions_box.foreach(lambda child: child.destroy()) 

        long_sessions = db_query(" SELECT * FROM sessions WHERE "+sessions_timeframe_sql()+" order by duration DESC LIMIT 10 ") 
        
        print(long_sessions)

        label = Gtk.Label()
        label.set_markup('<b>Longest 10 sessions of '+conf.user['hours_search_timeframe']+'</b>')

        self.long_sessions_box.add(label)
        ...
        for ls in long_sessions:
            ...
            self.long_sessions_box.add(btn)

        self.long_sessions_box.show_all()
