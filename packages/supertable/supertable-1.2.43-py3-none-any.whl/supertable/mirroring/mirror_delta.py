...
- metaData.format included; schemaString is a valid Spark StructType JSON.
- No latest.json is written.
- Parquet data files are **copied** under the table folder and obsolete ones are deleted.
"""
    ...
    try:
        if hasattr(storage, "ls"):
            for p in storage.ls(table_files_dir) or []:
                ...
                if bn and (bn.endswith(".parquet") or ".parquet" in bn or ".snappy.parquet" in bn):
                    rels.add(("files/" + bn).replace("\\", "/"))
        else:
            # Best effort: if storage.listdir exists
...
def write_delta_table(super_table, table_name: str, simple_snapshot: Dict[str, Any]) -> None:
    ...
    # Delete obsolete co-located files
    for rp in to_remove:
        try:
    ...
    with io.StringIO() as s:
        ...
        commit_info = {
            "commitInfo": {
                ...
                "readVersion": None if version == 0 else max(version - 1, 0),
                "isBlindAppend": False,
                "operationMetrics": {
        ...
        metadata = {
            "metaData": {
                "id": _stable_table_id(super_table.organization, super_table.super_name, table_name),
