...
#
# Additionally, to match the original cloud patch behavior exactly, we also
# **promote any relative file paths found inside JSON metadata** (e.g. lists
# of Parquet files) to full URIs under SUPERTABLE_HOME. This ensures downstream
# engines (like DuckDB) receive absolute cloud URIs instead of local-looking paths.
#
...
def _looks_like_rel_data_path(s: str) -> bool:
    """
    Heuristics to detect relative file/data paths in metadata that should be promoted.
    """
    if not isinstance(s, str):
        return False
    if _is_cloud_uri(s):
        return False
    # Common data path hints
    if s.endswith(".parquet") or s.endswith(".json") or s.endswith(".orc"):
        return True
    if "/data/" in s or s.startswith("lanes/") or s.startswith("tables/"):
        return True
    # Avoid promoting obvious non-paths (URLs, queries, ids)
    if "://" in s or s.strip().startswith(("SELECT", "CREATE", "WITH", "INSERT")):
        return False
    # Has path separators and no drive
    return ("/" in s or "\\" in s) and not os.path.isabs(s)


def _promote_json_paths(obj: Any) -> Any:
    """
    Recursively walk JSON-like structures and promote any string paths to full URIs
    under SUPERTABLE_HOME. This mirrors the 'resolve ALL paths' intent of the
    original cloud patch and fixes cases where query builders hand DuckDB relative
    paths (which then 404).
    """
    # Keys that are likely to contain file/dir paths
    PATHY_KEYS = {
        "files",
        "data_files",
        "paths",
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if isinstance(v, (list, tuple)):
                out[k] = [_to_full_uri(x) if isinstance(x, str) and _looks_like_rel_data_path(x) else _promote_json_paths(x) for x in v]  # type: ignore[list-item]
            elif isinstance(v, str) and (k in PATHY_KEYS or _looks_like_rel_data_path(v)):
                out[k] = _to_full_uri(v)
            else:
                out[k] = _promote_json_paths(v)
        return out
    if isinstance(obj, list):
        return [_to_full_uri(x) if isinstance(x, str) and _looks_like_rel_data_path(x) else _promote_json_paths(x) for x in obj]
    return obj
...
def _ls_read_json(self, path: str, retries: int = 3, backoff: float = 0.1) -> Any:
    """
    Cloud mode: read via fsspec and *promote* any relative paths inside the JSON payload
    so downstream consumers receive absolute URIs.
    """
    if not _cloudy():
    ...
    for _ in range(max(1, retries)):
        try:
            ...
            # Promote any file-like paths inside metadata (critical for DuckDB parquet_scan)
            try:
                payload = _promote_json_paths(payload)
            except Exception:
                # Never fail the read because of promotion logic; log and continue
                logger.debug("Path promotion skipped for %s due to error.", norm)
            return payload
