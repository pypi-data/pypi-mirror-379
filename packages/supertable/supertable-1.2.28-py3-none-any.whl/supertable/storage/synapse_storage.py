...
def activate_synapse(
    ...
    *,
    ...
    # ===================== Patch 2: Locking (blob lock, *this* was missing) =====================
    ...
    # --- resource lock (noop on cloud; we lock per-file) ---
    def _lk_lock_resources(self, resource_id: str) -> None:
        return  # noop; per-file locks suffice on cloud
    ...
    def _lk_self_lock(self, timeout_seconds: int = 60, lock_duration_seconds: int = 30) -> bool:
        # lock file lives inside working_dir (e.g., .../super/.lock.json), but ensure ABFSS
        lock_target = _to_full_uri(os.path.join(self.working_dir, ".lock.json"))
