...
#   - Locking -> blob-based (.lock) exclusive locks; shared reads via storage
#
# Usage:
...
def activate_synapse(
    ...
    *,
    ...
    LOCAL_CACHE_DIR = os.environ.get(
        "SUPERTABLE_LOCAL_CACHE", os.path.join(tempfile.gettempdir(), "supertable_duck_cache")
    )
    ...
    # ===================== Patch 2: Locking (blob lock; robust working_dir fallback) =====================
    ...
    def _acquire_blob_lock(fs, lock_path: str, ttl_seconds: int = 300, retries: int = 300, sleep: float = 0.2) -> None:
        for _ in range(max(1, retries)):
            try:
    ...
    # --- exclusive write: blob lock around writer() -> write_json ---
    ...
    def _lk_self_lock(self, timeout_seconds: int = 60, lock_duration_seconds: int = 30) -> bool:
        """
        Acquire a process-wide ABFSS blob lock for this Locking identity.
        Prefer a lock in `working_dir/.lock.json` if available; otherwise
        fall back to SUPERTABLE_HOME/locks/{identity}.lock.json.
        """
        # Prefer the object’s working_dir if present; some callers don’t set it.
        working_dir = getattr(self, "working_dir", None) or getattr(self, "working_dir_path", None)
        identity = getattr(self, "identity", "global")

        if working_dir:
            lock_rel = os.path.join(working_dir, ".lock.json")
        else:
            lock_rel = os.path.join("locks", f"{identity}.lock.json")
        ...
        setattr(self, "_cloud_lock_state", (fs, norm))
        retries = max(1, int((timeout_seconds / 0.2)))
        log_where = f"working_dir={working_dir!r}" if working_dir else f"fallback='locks/{identity}.lock.json'"
        try:
            from supertable.config.defaults import logger as _slog  # type: ignore
            _slog.debug(f"[cloud-lock] trying {log_where} → {norm}")
        except Exception:
            logger.debug(f"[cloud-lock] trying {log_where} → {norm}")

        _acquire_blob_lock(fs, norm, ttl_seconds=int(lock_duration_seconds), retries=retries, sleep=0.2)
        try:
            from supertable.config.defaults import logger as _slog  # type: ignore
            _slog.debug(f"[cloud-lock] acquired: {norm}")
        except Exception:
            logger.debug(f"[cloud-lock] acquired: {norm}")
        return True
    ...
    def _lk_self_unlock(self) -> None:
        ...
        if st:
            fs, norm = st
            _release_blob_lock(fs, norm)
            try:
                from supertable.config.defaults import logger as _slog  # type: ignore
                _slog.debug(f"[cloud-lock] released: {norm}")
            except Exception:
                logger.debug(f"[cloud-lock] released: {norm}")
