...
#   - LocalStorage.{read_json,write_json,read_bytes,write_bytes,exists,mkdir,size}
...
def _ls_mkdirs(self, path: str, exist_ok: bool = True) -> None:
    if not _cloudy():
        return _ORIG_LOCAL_STORAGE_METHODS["mkdir"](self, path, exist_ok)
    target = _to_full_uri(path)
    fs, norm = _fs_and_path(target)
    try:
...
def activate_synapse(home: Optional[str] = None) -> None:
    ...
    _ORIG_LOCAL_STORAGE_METHODS.update(
        {
            ...
            "mkdir": getattr(_ls.LocalStorage, "mkdir"),  # Fixed: was 'mkdirs'
    ...
    setattr(_ls.LocalStorage, "mkdir", _ls_mkdirs)  # Fixed: was 'mkdirs'
