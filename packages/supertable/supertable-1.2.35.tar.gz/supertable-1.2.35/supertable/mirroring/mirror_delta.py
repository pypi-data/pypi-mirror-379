...
newline-delimited JSON actions.

This implementation exactly mirrors the structure you showed (data/ + _delta_log/),
while being resilient to storage backends that don't implement optional methods
(like `listdir`, `copy`, or `stat`). It also avoids hard failures when a source
parquet path is missing by skipping that file (and logging), so the mirror step
won't abort your write pipeline.

On-disk layout (for <org>/<super>/delta/<table_name>):
  data/
...
Actions order in each JSON log:
  commitInfo, protocol, metaData, remove*, add*

Protocol and metadata are set for Delta compatibility:
  protocol: {"minReaderVersion": 1, "minWriterVersion": 4}
  commitInfo.operation = "WRITE"
  commitInfo.operationParameters.mode = "Overwrite"
  commitInfo.isolationLevel = "Serializable"
  commitInfo.isBlindAppend = false
  commitInfo.operationMetrics: numFiles/numOutputBytes (+numOutputRows if derivable)
  metaData.format = parquet
  metaData.schemaString = Spark-compatible JSON (derived from simple schema)
  metaData.partitionColumns = []
  metaData.configuration = CDF + autoOptimize flags
  add.stats is a compact JSON string if pyarrow is available

Robustness:
- Works even if storage lacks listdir/copy/stat (skips cleanup, copies via bytes).
- If the existing _delta_log is incomplete (missing protocol/metaData), it resets the log.
- If a source parquet file can't be found, it is skipped with a warning (no crash).
"""
...
# If a source parquet file is not found in storage, try reading from local FS.
