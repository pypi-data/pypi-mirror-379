...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("--home", default=os.getenv("SUPERTABLE_HOME", ""),
                   help="Workspace/home path (LOCAL dir or abfss:// for Azure). Writes SUPERTABLE_HOME. "
                        "When abfss:// is provided, Azure settings are inferred automatically.")
    ...
    p.add_argument("--azure-prefix", default=os.getenv("SUPERTABLE_PREFIX", ""),
    ...
    if storage == "LOCAL":
        ...
        if args.create_local_home:
            try:
                ...
                print(f"[OK] Ensured local directory: {Path(kv['SUPERTABLE_HOME']).resolve()}")
            except Exception as e:
                print(f"[WARN] Could not create local directory '{kv['SUPERTABLE_HOME']}': {e}", file=sys.stderr)
    ...
    if need_azure:
        ...
        if args.home and args.home.lower().startswith("abfss://"):
            try:
                account, container, endpoint, prefix = parse_abfss(args.home)
                kv.setdefault("AZURE_STORAGE_ACCOUNT", account)
                kv.setdefault("AZURE_BLOB_ENDPOINT", endpoint)
                kv.setdefault("AZURE_CONTAINER", container)
                if prefix:
                    kv.setdefault("SUPERTABLE_PREFIX", prefix)
                print(f"[INFO] Parsed ABFSS: account={account}, container={container}, endpoint={endpoint}, prefix='{prefix}'")
        ...
        if args.azure_connection_string:
            ...
            if args.azure_container: kv["AZURE_CONTAINER"] = args.azure_container
            if args.azure_prefix:    kv["SUPERTABLE_PREFIX"] = args.azure_prefix

        # Hint that AAD is expected when no secret provided
        if not any(kv.get(k) for k in ("AZURE_STORAGE_KEY", "AZURE_SAS_TOKEN", "AZURE_STORAGE_CONNECTION_STRING")):
    ...
    if need_azure:
        print(f"[INFO] AZURE_ACCOUNT={kv.get('AZURE_STORAGE_ACCOUNT','')}, CONTAINER={kv.get('AZURE_CONTAINER','')}, PREFIX={kv.get('SUPERTABLE_PREFIX','')}")
