...
def add(
    ...
    # Actualizar pypackage.json primero
    for name, spec in parsed:
        pkg.dependencies[name] = spec or ""
    pkg.save()

    # Preparar requisitos para pip
    reqs: list[str] = []
    for name, spec in parsed:
        s = spec.strip()
        if s.startswith("^"):
            # Convertir caret a compatible con pip
            base = s[1:]
            parts = base.split(".")
            try:
                major = int(parts[0])
    ...
    # Fijar versiones exactas para paquetes sin especificador
    modified = False
    if not no_deps:  # Solo si instalamos dependencias
