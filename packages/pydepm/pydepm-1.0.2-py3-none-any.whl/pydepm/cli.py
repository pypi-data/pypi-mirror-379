...
@app.command("list")
def list_cmd():
    """List installed packages and versions in the managed venv."""
    from .installer import freeze_versions
    with console.status("[cyan]Listing installed packages..."):
        versions = freeze_versions()
    if not versions:
        console.print("[yellow]No packages are installed in the managed venv.")
        return
    t = Table(show_header=True, header_style="bold")
    t.add_column("Package")
    t.add_column("Version")
    for name in sorted(versions.keys()):
        console_line = (name, versions[name])
        t.add_row(*console_line)
    console.print(t)
