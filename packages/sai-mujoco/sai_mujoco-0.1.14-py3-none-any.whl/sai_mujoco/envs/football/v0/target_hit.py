...
class KickToTarget_v0(FootballEnv_v0):
    ...
    def _set_camera(self, 
        ...
        center = 0.5 * ball_qpos + 0.5 * goal_pos

        # Distance: use vertical FOV to fit the horizontal span conservatively
        # Treat the XY distance as the "span" to keep within view.
        span_xy = float(np.linalg.norm((ball_qpos - goal_pos)[:2]))
        z_min, z_max = float(min(ball_qpos[2], goal_pos[2])), float(max(ball_qpos[2], goal_pos[2]))
        span_z = z_max - z_min
        ...
        dist = float(np.clip(margin * max(d_h, d_v), 8.0, 12))
