...
def isimported(module_name: str):
    """
    Check if a module name is currently present in the caller's global namespace (like the shell).

    Returns
    -------
    True  -> if module is currently imported into the caller's globals (shell)
    False -> if module exists but has not been imported into the caller's globals
    "Module <name> doesn't exist." -> if module does not exist at all

    Examples:
    ---------
    After restart (nothing imported yet):
        >>> modx.isimported("random")
        False

    After importing random:
        >>> import random
        >>> modx.isimported("random")
        True

    With a non-existent module:
        >>> modx.isimported("hihithrhr")
        'Module hihithrhr doesn't exist.'
    """
    import importlib.util
    import inspect

    # Look at the globals() of the caller (one frame back)
    frame = inspect.currentframe().f_back
    caller_globals = frame.f_globals

    # Case 1: already in caller's globals (shell)
    if module_name in caller_globals:
        return True

    # Case 2: exists on disk but not imported into caller's globals yet
    spec = importlib.util.find_spec(module_name)
    if spec is not None:
        return False

    # Case 3: does not exist at all
