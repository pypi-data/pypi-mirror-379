...
class ModelAdmin:
    ...
    def get_breadcrumbs(self, request, obj=None, **kwargs):
        ...
        qs = getattr(request, "_current_queryset", None) or self.queryset.all()
        ...
        if self.parent_field:
            ...
            if parent_filter:
                try:
                    ...
                    log.warning(
                        f"FieldError in parent filter for model {self.model.__name__}: {e}"
                    )
                    # We use the unfiltered queryset
    ...
    def _count_objects(self, queryset):
        """Count objects using estimated or exact count"""
    ...
    def get_cached_count(self, queryset, timeout=30):
        ...
        try:
            ...
            def _cached_count():
                return self._count_objects(queryset)
