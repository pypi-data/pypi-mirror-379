...
class WhosePIIGuardian:
            ...
            try:
                ...
                if config.is_valid():
                    ...
                    if not local_fallback:
                        raise ValueError(
                            "API key and endpoint ID are required for cloud mode. "
                            "Set ANOTIAI_API_KEY and ANOTIAI_ENDPOINT_ID environment variables "
                            "or enable local_fallback=True"
                        )
                    logger.warning("Cloud credentials not provided, using local mode")
    ...
    def mask_text(self, text: str, confidence_threshold: float = 0.5) -> Tuple[str, Dict[str, Any]]:
        ...
        if self._use_cloud_inference():
            try:
                return self.api_client.mask_text(text, confidence_threshold)
    ...
    def unmask_text(self, masked_text: str, pii_map: Dict[str, Any]) -> str:
        ...
        if self._use_cloud_inference():
            try:
                return self.api_client.unmask_text(masked_text, pii_map)
            except Exception as e:
                logger.warning(f"Cloud inference failed: {e}")
                if self.local_fallback:
                    logger.info("Falling back to local inference")
    ...
    def detect_pii(self, text: str, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        ...
        if self._use_cloud_inference():
            try:
                return self.api_client.detect_pii(text, confidence_threshold)
            except Exception as e:
                logger.warning(f"Cloud inference failed: {e}")
                if self.local_fallback:
                    logger.info("Falling back to local inference")
    ...
    def get_model_version(self) -> str:
        ...
        if self._use_cloud_inference():
            try:
                result = self.api_client.get_model_version()
    ...
    # Local inference methods (legacy)
    # def _mask_text_local(self, text: str, confidence_threshold: float = 0.5) -> Tuple[str, Dict[str, str]]:
    ...
    # def _unmask_text_local(self, masked_text: str, pii_map: Dict[str, Any]) -> str:
