...
class DeploymentUtils:
    ...
    def _group_spec_to_dict(self, group_data: dict) -> dict:
        ...
        for i, resource_data in enumerate(group_data["resources"]):
            ...
            resource_spec = {
                "resource": {
                    "id": i + 1,
                    "cpu": {"units": {"val": str(resource_data["cpu"])}},
                    "memory": {"quantity": {"val": str(resource_data["memory"])}},
                    "storage": storage_specs,
                    "gpu": {"units": {"val": str(resource_data.get("gpu", "0"))}},
                },
    ...
    def submit_manifest_to_provider(
        ...
        try:
            ...
            if use_http:
                return self.akash_client.manifest.send_manifest_http(
                ...
                return self.akash_client.manifest.send_manifest_with_fallback(
                    provider_endpoint=provider_endpoint,
                    lease_id=lease_id,
