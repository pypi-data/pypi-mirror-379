...
class DiscoveryClient(DiscoveryQuery):
    """
    ...
    def __init__(self, akash_client):
        ...
        logger.info(
            "Initialized DiscoveryClient with gRPC-first and HTTP fallback support"
        )
    ...
    def get_provider_resources(
        ...
        try:
            ...
            if status_result["status"] == "success":
                ...
                available_resources = {}
                for resource_type in ["cpu", "memory", "ephemeral_storage", "gpu"]:
                    resource_data = cluster_inventory.get(resource_type, {})
                    if isinstance(resource_data, dict) and "val" in resource_data:
                        available_resources[resource_type] = resource_data["val"]
                    else:
                        available_resources[resource_type] = resource_data
                ...
                if reservations.get("pending", {}).get("resources"):
                    ...
                    for resource_type in ["cpu", "memory", "ephemeral_storage", "gpu"]:
                        ...
                        if isinstance(resource_data, dict) and "val" in resource_data:
                            ...
                            pending_resources[resource_type] = resource_data
        ...
        try:
            ...
            if resources_result["status"] == "success":
                ...
                for resource_type, required_value in required_resources.items():
                    available_value = available.get(resource_type, 0)
                    ...
                    if resource_type == "cpu":
                        if isinstance(available_value, str):
                            ...
                            sufficient[resource_type] = int(available_value) >= int(
                                required_value
                            )
                    elif resource_type in ["memory", "ephemeral_storage"]:
