...
class TestRunner:
    ...
    def run_command(self, cmd: List[str], description: str) -> bool:
        ...
        try:
            result = subprocess.run(cmd, check=True, cwd=self.test_dir.parent)
    ...
    def run_client_unit_tests(self, verbose: bool = False, coverage: bool = False) -> bool:
        """运行客户端单元测试"""
        cmd = ["python", "-m", "pytest", "tests/agent_runtime/client/unit/", "-m", "unit"]
    ...
    def run_client_integration_tests(self, verbose: bool = False) -> bool:
        """运行客户端集成测试"""
        cmd = ["python", "-m", "pytest", "tests/agent_runtime/client/integration/", "-m", "integration"]
