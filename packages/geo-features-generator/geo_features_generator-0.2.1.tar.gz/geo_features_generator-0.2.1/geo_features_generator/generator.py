...
class GeoFeaturesGenerator:
    ...
    enable_squared_distances: bool
        Генерировать ли квадраты расстояний (haversine_m2, equirect_m2, manhattan_m2).
    enable_normed_deltas: bool
        Генерировать ли нормализованные дельты координат в метрах (abs_dlat_m, abs_dlon_m).
    enable_cosine_similarity: bool
        Генерировать ли косинусное сходство и косинусное расстояние между точками.
    enable_kmeans: bool
        Генерировать ли кластеры K-means для каждой точки координат.
    kmeans_n_clusters: int
        Количество кластеров для K-means алгоритма.
    kmeans_random_state: Optional[int]
        Контролируемый случайный генератор для K-means.
    enable_nearest_neighbor: bool
        Генерировать ли расстояние до ближайшего соседа для каждой точки.
    enable_polar_features: bool
        Генерировать ли полярные признаки относительно базовой точки.
    polar_base_point: Optional[Tuple[float, float]]
        Базовая точка для полярных признаков (широта, долгота).
    enable_h3: bool
        Генерировать ли H3 ячейки для каждой геофичи. Требует установленную библиотеку 'h3'.
    h3_resolutions: Optional[list[int]]
        Список разрешений H3 ячеек для генерации признаков. По умолчанию [8, 10].
    """
    ...
    def __init__(
        ...
        # H3 parameters
        self.enable_h3 = bool(enable_h3)
        self.h3_resolutions = list(h3_resolutions) if h3_resolutions is not None else [8, 10]
    ...
    def transform(self, X: ArrayLike) -> ArrayLike:
        ...
        if self.generate_point_features:
            for idx, (lat_col, lon_col) in enumerate(self.coordinate_pairs):
                ...
                # H3 ячейки для каждой геофичи
                if self.enable_h3:
                    try:
                        import h3
                        for resolution in self.h3_resolutions:
                            # Простое применение с использованием pandas аналогичном add_h3_features
                            h3_cells = [h3.latlng_to_cell(lat_val, lon_val, resolution) for lat_val, lon_val in zip(lat, lon)]
                            features[f"{base}_h3_res{resolution}"] = np.array(h3_cells, dtype=object)
                    except ImportError:
                        raise ImportError("Для H3 функций требуется установленная библиотека 'h3'") from None
    ...
    def _build_feature_names(self, X: Optional[ArrayLike]) -> List[str]:
        ...
        if self.generate_point_features:
            for idx, _ in enumerate(self.coordinate_pairs):
                ...
                if self.enable_h3:
                    for resolution in self.h3_resolutions:
                        names.append(f"{base}_h3_res{resolution}")
        if self.generate_pair_features:
            if self.selected_pair_indices is not None:
                indices = list(self.selected_pair_indices)
            else:
