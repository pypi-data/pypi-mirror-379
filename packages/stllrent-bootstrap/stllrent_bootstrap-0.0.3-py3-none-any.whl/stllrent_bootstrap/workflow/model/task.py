...
class TaskModel(Base):
    ...
    workflow_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('workflow.oid', ondelete='CASCADE'), index=True, init=False)
    task_name: Mapped[str] = mapped_column(nullable=True, unique=False, index=False)
    task_metadata: Mapped[dict[str, Any]] = mapped_column(unique=False, nullable=True, index=False)
    ...
    workflow: Mapped["WorkflowModel"] = relationship(
        ...
        init=False # Não incluir no dataclass __init__
    )
