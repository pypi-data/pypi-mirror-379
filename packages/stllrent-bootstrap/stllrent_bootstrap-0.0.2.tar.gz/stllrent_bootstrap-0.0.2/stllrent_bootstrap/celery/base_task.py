...
class BootstrapTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            self._update_monitoring_service(self.request.id, "REJECTED", failure_metadata)
            
            # É CRUCIAL relançar a exceção para que o Celery continue
            # o processo de rejeitar a mensagem no broker.
            raise
        except Retry as re_exc:
            log.critical(f"Retentando [{self.request.id}]: Retry for {re_exc!r}")
            failure_metadata = {
                "exception_type": type(re_exc).__name__,
                "exception_message": str(re_exc)
            }
            self._update_monitoring_service(self.request.id, "RETRY", failure_metadata)
            raise
            ...
            self._update_monitoring_service(self.request.id, "FAILURE", failure_metadata)
            raise
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        self._update_monitoring_service(task_id, "FAILURE", failure_metadata)
    ...
    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        ...
        self._update_monitoring_service(task_id, status, {"output": output_data})

    def _update_monitoring_service(self, task_id, status, metadata):
        """
