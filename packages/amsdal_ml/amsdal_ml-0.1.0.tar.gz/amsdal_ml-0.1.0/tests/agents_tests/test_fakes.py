...
class FakeModel(MLModel):
    ...
    def stream(self, _prompt: str) -> Iterator[str]:
        if self.async_mode:
    ...
    async def astream(self, _prompt: str) -> Coroutine[Any, Any, AsyncIterator[str]]:
        if not self.async_mode:
        ...
        async def _gen() -> AsyncIterator[str]:
            if not self._scripted:
                if False:  # preserve async-generator shape for type checkers
