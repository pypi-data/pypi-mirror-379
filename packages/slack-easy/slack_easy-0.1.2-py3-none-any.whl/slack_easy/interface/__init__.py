...
class BaseSlackMessage(ABC):
    ...
    def send(self,
             ...
             token: str = None,
             usr_name_channel_id_mapping: dict = None,
        ...
        for recipient in recipients:
            if isinstance(recipient, str):
                if channel_id:
                    ...
                    if usr_name_channel_id_mapping and recipient in usr_name_channel_id_mapping:
                        slack_id = usr_name_channel_id_mapping[recipient]
                    else:
                        slack_id = self.slack_bot.id_from_name(recipient)
                    if slack_id is None:
                        log.warning(f"Channel ID not found for recipient '{recipient}'. Skipping.\n"
                                    f"Solutions: \n"
                                    f"1) Use configure_slack() to add user name channel id mapping\n"
                                    f"2) Pass user name channel id mapping in send function\n"
                                    f"3) Set channel_id=True if passing channel ID directly")
                        continue
                ...
                log.warning(f"Recipient must be a string, got type: {type(recipient)}")
