...
class EventMixin:
    ...
    def handle_equipped_event(self: EventMixinProtocol, event: events.EquippedEvent):
        if unit := self.game_state.units.get(event.unitId):
            if unit == self.game_state.player:
                setattr(unit.equipment, event.slot, event.item)
            inv_items = unit.inventory.get(event.item)
            if inv_items:
                unit.inventory[event.item] = inv_items - 1
                if unit.inventory[event.item] <= 0:
                    unit.inventory.pop(event.item, None)
...
class InstanceCharacter(EventMixin):
    ...
    async def handle_event(self, event: BaseEvent, client: "GameClient") -> None:
        match event:
            ...
            case events.EquippedEvent():
                self.handle_equipped_event(event)
            case _:
