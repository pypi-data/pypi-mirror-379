...
from .constants import KEY_SLICE
...
INCOMING = "->"
OUTGOING = "<-"
KEY_SLICE_PACKET_FORMAT = "<q64s32s64s260s"


class KeySlice(Message):
    ...
    def inflate(self, command):
        ...
        data = self._unpack_key_slice_message(self.message_bytes)
        self.peer = command.get("peer")
        self.timestamp = data.get("timestamp")
        self.speaker = data.get("speaker").rstrip(b"\x00").decode("ascii")
        self.slice = data.get("slice")
    ...
    def get_slice(self):
        if not self.slice:
            self.slice = self.db.get_key_slice(self.peer.peer_id)
        return self.slice

    def get_message_bytes(self):
        message_bytes = struct.pack(
            KEY_SLICE_PACKET_FORMAT,
            self.timestamp,
            os.urandom(64),
    ...
    def _unpack_key_slice_message(self, message_bytes):
        int_ts, _, speaker, slice, _ = struct.unpack(
            KEY_SLICE_PACKET_FORMAT, message_bytes
        )
