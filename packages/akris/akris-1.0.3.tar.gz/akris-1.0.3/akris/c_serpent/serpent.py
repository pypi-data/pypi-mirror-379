...
try:
    ...
    try:
        # Try loading from the local c_serpent directory
        package_path = os.path.dirname(os.path.abspath(__file__))
        import glob
        so_files = glob.glob(os.path.join(package_path, "_serpent_cffi*.so"))
        if so_files:
            lib = ctypes.cdll.LoadLibrary(so_files[0])
        else:
            raise OSError("No suitable shared library found")
    except OSError:
        # Fallback to venv site-packages
        venv_site_packages_path = sys.path[4]
        import glob
        so_files = glob.glob(os.path.join(venv_site_packages_path, "_serpent_cffi*.so"))
        if so_files:
            lib = ctypes.cdll.LoadLibrary(so_files[0])
        else:
            raise OSError("No suitable shared library found in venv")
