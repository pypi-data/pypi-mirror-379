...
def test_message_stats(cortes_client):
    ...
    cortes_client.send_command({"command": "direct_text", "args": ["pizarro", "m1"]})
    assert await_message_with_timeout(cortes_client, message)

    cortes_client.send_command({"command": "message_stats", "args": []})
    message_stats = await_messages(1, cortes_client)
