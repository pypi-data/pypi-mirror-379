...
class DataValidator:
    ...
    def validate_data(self, data: pd.DataFrame) -> DataQualityReport:
        ...
        total_issues = (
            total_errors * 2
        ) + total_warnings  # Errors weighted more heavily
        ...
        if rule.name == "price_outliers":
            ...
            for col in columns:
                if col in data.columns:
                    ...
                    outlier_mask = np.abs(data[col] - mean_val) > (
                        sigma_threshold * std_val
                    )
    ...
    def _validate_temporal(
        ...
        if rule.name == "temporal_continuity" and "timestamp" in data.columns:
            expected_interval = timedelta(
                minutes=rule.parameters["expected_interval_minutes"]
            )
    ...
    def _generate_recommendations(
        self, validation_results: List[ValidationResult], quality_score: float
    ) -> List[str]:
        ...
        for result in validation_results:
            if not result.passed:
                if result.rule_name == "price_outliers" and result.warnings_count > 0:
                    ...
                    result.rule_name == "volume_outliers" and result.warnings_count > 0
                ):
                    recommendations.append(
                        "Volume outliers detected. Consider volume-based filtering or normalization."
                    )
                elif (
                    result.rule_name == "temporal_continuity"
                    ...
                    result.rule_name == "ohlc_relationship" and result.errors_count > 0
                ):
                    recommendations.append(
    ...
    def export_validation_report(
        self, report: DataQualityReport, output_path: str
    ) -> None:
        ...
        report_dict = {
            "validation_summary": {
                ...
                "quality_threshold_met": report.quality_score
            ...
            "validation_results": [
                {
                    ...
                    "affected_records_count": (
                        len(result.affected_records) if result.affected_records else 0
                    ),
                }
...
class OutlierDetector:
    ...
    def detect_volume_outliers_iqr(
        data: pd.DataFrame, column: str = "volume", multiplier: float = 2.5
    ) -> pd.Series:
        """
