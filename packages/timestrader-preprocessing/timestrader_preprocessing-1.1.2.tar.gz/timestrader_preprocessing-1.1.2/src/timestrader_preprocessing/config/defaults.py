...
class ParameterManager:
    ...
    async def load_parameters(
        ...
        try:
            ...
            if parameter_data is None and allow_packaged_fallback:
                self.logger.info(
                    "Falling back to packaged Story 1.7 normalization artifact"
                )
    ...
    async def _load_packaged_parameters(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            artifact_path = resources.files(
                "timestrader_preprocessing.config"
            ).joinpath(self._packaged_artifact)
            ...
            if not self._validate_checksum(artifact_path, data.get("checksum")):
                self.logger.error(
                    "Packaged normalization artifact failed checksum validation"
                )
                return None
            ...
            self.logger.error(
                "Packaged normalization artifact not found: %s", self._packaged_artifact
            )
            return None
        except Exception as exc:  # pragma: no cover - defensive logging
    ...
    def _find_parameter_file(self, version: Optional[str] = None) -> Optional[Path]:
        ...
        try:
            if version:
                file_path = (
                    self.parameter_path / f"normalization_params_v{version}.json"
                )
                return file_path if file_path.exists() else None

            parameter_files = list(
                self.parameter_path.glob("normalization_params_v*.json")
            )
        ...
        try:
            ...
            for field in required_fields:
                if field not in data:
                    self.logger.error(
                        f"Missing required field in parameter file: {field}"
                    )
                    return None
    ...
    def detect_parameter_drift(
        self, current_stats: Dict[str, Dict[str, float]], threshold: float = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            for indicator, current_stat in current_stats.items():
                ...
                mean_drift = abs(current_stat["mean"] - training_params["mean"]) / abs(
                    training_params["mean"]
                )
                std_drift = abs(current_stat["std"] - training_params["std"]) / abs(
                    training_params["std"]
                )
            ...
            if drift_results["overall_drift"]:
                alert_msg = (
                    f"Parameter drift detected: {drift_results['drift_indicators']}"
                )
    ...
    async def hot_reload(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                and latest_file.stat().st_mtime <= self._current_version.loaded_at
            ):
                return False
