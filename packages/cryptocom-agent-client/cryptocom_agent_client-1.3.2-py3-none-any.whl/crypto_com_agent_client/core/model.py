...
class Model:
    ...
    def initialize_model(self: Self) -> Union[
        ...
        if self.provider == Provider.OpenAI:
            ...
            # Get environment variables for guardrail and knowledge base
            guardrail_id = self.guardrail_id or os.getenv("AWS_GUARDRAIL_ID")
            guardrail_version = self.guardrail_version or os.getenv("AWS_GUARDRAIL_VERSION", "DRAFT")
            knowledge_base_id = self.knowledge_base_id or os.getenv("AWS_KNOWLEDGEBASE_ID")

            # Prepare ChatBedrock parameters
            bedrock_params = {
                "model_id": self.model or os.getenv("AWS_MODEL_ID", "anthropic.claude-3-haiku-20240307-v1:0"),
            ...
            if knowledge_base_id:
                try:
                    ...
                    if "aws_access_key_id" in aws_config:
                        runtime_config["aws_access_key_id"] = aws_config["aws_access_key_id"]
                    if "aws_secret_access_key" in aws_config:
                        runtime_config["aws_secret_access_key"] = aws_config["aws_secret_access_key"]
                    ...
                    if self.debug_logging:
                        print(f"Warning: Could not initialize knowledge base client: {e}")
    ...
    def query_knowledge_base(
        self, query: str, max_results: int = 5, min_relevance_score: Optional[float] = None
    ) -> Optional[str]:
        """
        ...
        elif hasattr(self, 'min_relevance_score') and self.min_relevance_score is not None:
            ...
            if response and "retrievalResults" in response:
                ...
                for result in response["retrievalResults"]:
                    ...
                    if score >= score_threshold:
                        ...
                        filtered_count += 1
                        if self.debug_logging:
                            print(f"   ⚠️ Filtered out result with low score: {score:.3f} < {score_threshold}")

                if self.debug_logging and filtered_count > 0:
                    print(f"   📊 Filtered {filtered_count}/{total_count} results below threshold ({score_threshold})")

                if results:
                    return "\n".join(results)
                else:
                    if self.debug_logging:
                        print(f"   ❌ All {total_count} results were below threshold ({score_threshold})")
