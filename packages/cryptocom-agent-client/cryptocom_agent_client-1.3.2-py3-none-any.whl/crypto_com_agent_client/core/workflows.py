...
class GraphWorkflow:
    ...
    def _query_vector_db(self: Self, state: AgentState) -> dict[str, Any]:
        ...
        if not hasattr(self.model_handler, 'provider') or self.model_handler.provider != Provider.Bedrock:
            if self.debug_logging:
        ...
        try:
            ...
            kb_results = self.model_handler.query_knowledge_base(human_query, max_results=3)

            if self.debug_logging:
                print(f"📋 VectorDB response: {kb_results[:200] if kb_results else 'None'}...")

            # Check if we have valid results
            if (kb_results and
                not kb_results.startswith("No") and
                not kb_results.startswith("Knowledge base query failed") and
                not kb_results.startswith("Knowledge base query error")):

                if self.debug_logging:
                    print(f"✅ VectorDB context found - {len(kb_results)} chars")

                return {
                    "vectordb_context": kb_results,
                    "vectordb_query": human_query
                }
            else:
                if self.debug_logging:
    ...
    def _call_model(self: Self, state: AgentState) -> dict[str, list[BaseMessage]]:
        ...
        if vectordb_context and vectordb_query:
            ...
            for msg in selected_messages:
                if hasattr(msg, 'type') and msg.type == 'human' and msg.content == vectordb_query:
