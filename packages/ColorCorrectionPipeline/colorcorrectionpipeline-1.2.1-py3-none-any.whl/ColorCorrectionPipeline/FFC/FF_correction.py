...
from ..utils.logger_ import ThrowDlg, log_, match_keywords
...
class FlatFieldCorrection:
    ...
    def __init__(self, img=None, **kwargs):
        ...
        try:
            ...
            MODEL_PATH = os.path.join(
                ...
                "FFC",
                "Models",
                "plane_det_model_YOLO_512_n.pt",
            )

        self.img = img
        self.model_path = kwargs.get(
            "model_path", MODEL_PATH
        )  # Use global MODEL_PATH as default
    ...
    def transform_extremity(
        self, x: np.ndarray, cut_off: float = 1.5, max: float = 2.0
    ):
        x_ = x.flatten()
        mask = x_ > cut_off
    ...
    def detect_and_crop(self):
        ...
        if not self.manual_crop:
            ...
            log_(
                'Select ROI manually\nPress "ENTER" when done selecting ROI',
                "light_blue",
                "italic",
                "info",
            )
    ...
    def fit_model(self, X, y, **kwargs):
        ...
        if fit_method not in options:
            response = ThrowDlg.yesno(
                f"""Fit method '{fit_method}' is not recognized. Try one of: {options}.
                                      Do you want to continue with the default method (Linear regression)?"""
            )
            if response.lower() == "yes":
                fit_method = "linear"
                log_(
                    f"Using the default method '{fit_method}' for fitting",
                    color="orange",
                    font_style="bold",
                    level="WARNING",
                )
                ...
                log_(
                    f"Cancelled fitting using method '{fit_method}'",
                    color="orange",
                    font_style="bold",
                    level="WARNING",
                )
            ...
            log_(
                f"ffc fitting using method '{fit_method}'...",
                color="cyan",
                font_style="italic",
                level="INFO",
            )
