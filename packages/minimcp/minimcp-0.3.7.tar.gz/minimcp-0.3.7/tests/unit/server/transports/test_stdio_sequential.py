...
from minimcp.server.transports.stdio import sequential_transport
...
class TestStdioTransportSimple:
    ...
    def test_sequential_transport_function_exists(self):
        """Test that sequential_transport function exists and is callable."""
        assert callable(sequential_transport)

        # Check function signature
        import inspect

        sig = inspect.signature(sequential_transport)
        assert len(sig.parameters) == 1
        assert "handler" in sig.parameters

    def test_sequential_transport_is_async(self):
        """Test that sequential_transport is an async function."""
        import asyncio

        assert asyncio.iscoroutinefunction(sequential_transport)
    ...
    def test_function_docstring(self):
        ...
        assert sequential_transport.__doc__ is not None
        assert "sequential" in sequential_transport.__doc__
        assert "handler" in sequential_transport.__doc__
    ...
    @pytest.mark.asyncio
    async def test_sequential_transport_requires_handler(self):
        """Test that sequential_transport requires a handler parameter."""
        # This should raise TypeError if called without handler
        with pytest.raises(TypeError):
            await sequential_transport()  # type: ignore

    def test_sequential_transport_handler_signature(self):
