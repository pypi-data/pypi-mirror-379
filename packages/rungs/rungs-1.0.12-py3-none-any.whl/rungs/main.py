    ...
    def do_menu(self, menu_name, options=None):
        ...
        while True:
            ...
            if cmd.startswith('rungs '):
                module_name = 'rungs.main'
                # CD to exactly where this package and module will be found.
                os.chdir(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                vp = [sys.executable, '-m', module_name]
                vp += ['-n'] if self.opts.dry_run else []
                vp += [ cmd[6:] ] # skipping 'rungs '
                result = subprocess.run(vp)
