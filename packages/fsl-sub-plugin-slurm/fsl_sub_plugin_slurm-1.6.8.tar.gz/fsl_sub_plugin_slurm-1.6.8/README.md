...
| smt | False | The CPUs in this partition support symmetric multi-threading (e.g. `Hyperthreading`), it is enabled and SLURM is configured to expose these threads. As SLURM will always give the job an even number of threads setting this to True will ensure that the thread control environment variables are set such that they can take advantage of the additional virtual thread. Leave as False (or don't set) if you don't want to use SMT. |
