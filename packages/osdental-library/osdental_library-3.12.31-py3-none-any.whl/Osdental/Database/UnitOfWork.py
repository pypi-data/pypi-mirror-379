    ...
    async def __call__(self):
        try:
            ...
            raise DatabaseException(
                status_code=Code.INTEGRITY_DB_ERROR_CODE, error=str(ierr)
            )
        except OperationalError as oerr:
            raise DatabaseException(
                status_code=Code.OPERATIONAL_DB_ERROR_CODE, error=str(oerr)
            )
        except (DBAPIError, InvalidRequestError) as db_err:
            raise DatabaseException(
                status_code=Code.DB_UNKNOWN_ERROR_CODE, error=str(db_err)
            )
