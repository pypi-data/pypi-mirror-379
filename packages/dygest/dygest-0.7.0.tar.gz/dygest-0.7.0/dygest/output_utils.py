...
class HTMLWriter(WriterBase):
    ...
    def add_metadata(self):
        ...
        if div_page_container:
            ...
            if self.named_entities:
                ner_count = Counter(
                    [x['ner_tag'] for x in self.named_entities]
                )
                ner_counter_string = (
                    ', '.join(
                        f"{tag} ({count})" for tag, count in ner_count.items()
                    )
                )
                ner_string = (
                    f"{len(self.named_entities)} [{ner_counter_string}]"
                )
                self.format_metadata_entry(
                    parent=div_metadata_content,
                    label="Entities",
    ...
    def add_keywords(self):
        ...
        if div_page_container:
            ...
            keywords_tag.string = (
                ', '.join([key for key in self.keywords])
                )
            # Append new elements
    ...
    def add_main_content(self, content_editable: bool = False):
        ...
        speaker = r'\[(?:SPEAKER_\d+|UNKNOWN)\]'
        pattern = f'{timestamp} {speaker}|{speaker}'
        ...
        # 3. Collect anchor tag events for TOC (using sentence_offsets)
                ...
                for topic in item['topics']:
                    ...
                    if location and location not in inserted_locations:
                        ...
                        if location in self.sentence_offsets:
                            ...
                            # Do not insert a linebreak if speaker tag found
                            if self.has_speaker:
                                continue
        ...
        for event in events:
            ...
            if event_type == 'start_entity':
                ...
                outer_span_tag = self.soup.new_tag(
                    ...
                    attrs={
                        "class": "ner-entity",
                        "data-color": CMAP.get(data, ''),
                        "style": f"background-color: {CMAP.get(data, '')};",
                        "title": data,
                ...
                # Add the current paragraph to the content if it has children
                if p_tag.contents:
    ...
    def transform_markdown_tags(self):
        ...
        if not main_content:
            print("""... No <div class="content"> in HTML.""")
            return
        ...
        for p in transformed_soup.find_all('p'):
            ...
            if (
                ...
                and not p.find(True, recursive=False)
            ):
                p.decompose()
...
class MarkdownWriter(WriterBase):
    ...
    def write(self):
        ...
        if self.export_metadata:
            ...
            md_content.append(
                f"- Tokens: {self.token_count:,.0f}".replace(',', '.')
            )
        ...
        if self.keywords:
            ...
            keywords_str = ', '.join([key for key in self.keywords])
        ...
        if self.toc:
            ...
            for item in self.toc:
                ...
                for topic in item['topics']:
                    location = topic.get('location')
                    if location:
                        topic_str = topic['summary'].rstrip('.,;:?!')
                        topic_loc = f"#location-{location}"
                        md_content.append(f"- [{topic_str}]({topic_loc})")
                    else:
                        md_content.append(
                            f"- {topic['summary'].rstrip('.,;:?!')}"
                        )
        ...
        timestamp = r"\[\d{2}:\d{2}:\d{2}\.\d{3}\]"
        speaker = r"\[(?:SPEAKER_\d+|UNKNOWN)\]"
        pattern = fr"{timestamp} {speaker}|{speaker}"
        content = re.sub(pattern, lambda m: f"\n\n**{m.group(0)}**\n", content)

        # Handle NER entities
        if self.named_entities:
            # Sort entities by start position to avoid position shifts
            ...
            for entity in sorted_entities:
                ...
                tag_start = "<span style='background-color:"
                tag_label = CMAP.get(entity_ner_tag, '')
                tag_end = f"title='{entity_ner_tag}'>{entity_text}</span>"
                formatted_entity = f"{tag_start} {tag_label} {tag_end}"
                content = (
                    content[:entity['start']] +
                    formatted_entity +
                    content[entity['end']:]
                )
...
class WriterFactory:
    ...
    """
    from dygest.core import DygestProcessor
    ...
    def _get_writer_class(self) -> type[WriterBase]:
        ...
        if not writer_class:
            raise ValueError(
                f"... Unknown export format: {self.proc.export_format}"
            )
    ...
    def _get_writer_params(self, writer_class: type[WriterBase]) -> dict:
        ...
        shared_params = {
            ...
            'named_entities': (
                self.proc.entities if self.proc.add_ner else None
            ),
            'toc': self.proc.toc if self.proc.add_toc else None,
            'summary': (
                self.proc.summaries if self.proc.add_summaries else None
            ),
        ...
        # .html
        if (
            self.proc.export_format == ExportFormats.HTML
            and self.proc.html_template_path
        ):
            # Construct the output path with template folder name
            template_dir = self.proc.html_template_path.name
            path_parent = self.proc.output_filepath.parent
            path_output = f"{self.proc.output_filepath.stem}_{template_dir}"
            base_path = path_parent / path_output
        ...
        # .md / Markdown
            ...
            output_filepath = self.proc.output_filepath.with_suffix(
                f'.{suffix}'
            )
    ...
    def _get_format_specific_params(
        self,
        writer_class: type[WriterBase]
    ) -> dict:
        """
        ...
        if writer_class == HTMLWriter:
            return {
                'export_metadata': (
                    self.proc.export_metadata
                    if self.proc.export_metadata
                    else False
                ),
