...
def get_provider_config(model_name: str) -> Tuple[str, str, str]:
    ...
    if len(provider) == 3:
        ...
        typer.secho(
            "... Error: Model name must be in format 'provider/model' "
            "(e.g. 'ollama/qwen2.5:latest') or 'openai/provider/model' "
            "(e.g. 'openai/exampleprovider/qwen2.5:latest')",
            fg=typer.colors.RED
        )
...
def call_llm(
    ...
    response_format = set_llm_response_format(
        model,
        output_format,
        json_schema
    )
    ...
    try:
        ...
        print(
            f"[purple]... {e}. Please check that your provided "
            "model name exists."
        )
        raise typer.Exit(code=1)
