...
def load_tagger(language: str = 'en', precise: bool = False):
    ...
    end_time = time.time()
    print(f'... NER tagger loaded in {end_time - start_time:.2f} seconds.')
    ...
    for text, entity_list in text_to_entities.items():
        ...
        if verbose:
            ...
            if num_matches < num_entities:
                print(
                    ...
                    f"... {num_entities - num_matches} entities will "
                    "... not be updated."
                )
            elif num_matches > num_entities:
                print(
                    f"... Warning: More matches than entities for '{text}'. "
                    f"... {num_matches - num_entities} extra matches will "
                    "... be ignored."
                    )
        ...
        for i, entity in enumerate(entity_list):
            if i < num_matches:
                ...
                if verbose:
                    print(
                        f"... Entity {i+1} with text '{text}' has no "
                        f"... corresponding match in 'doc'."
                        )
