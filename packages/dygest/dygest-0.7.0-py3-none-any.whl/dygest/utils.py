...
def default_html_template(name: str = "tabs") -> Path:
    ...
    if name not in ("tabs", "plain"):
        raise ValueError(
            f"Unknown template name: {name}. Choose 'tabs' or 'plain'."
        )
...
def validate_and_fix_json(
    ...
    # Check if the fixed string is valid json
...
def print_entities(entities: list[dict]) -> None:
    [print(f"... ... {e['text']} → {e['ner_tag']}") for e in entities]
...
def validate_json_input(json_data: dict) -> bool:
    """
    Validate if the JSON input contains all required keys and has the
    correct structure.
    ...
    for chunk_key, chunk_data in json_data['chunks'].items():
        ...
        if 'text' not in chunk_data or 's_ids' not in chunk_data:
            print(
                f"... Error: Chunk '{chunk_key}' missing "
                "required 'text' or 's_ids'"
            )
            return False
