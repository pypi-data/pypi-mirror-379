...
class APIClient:
    ...
    def get_sync_measurements(self, typesToInclude: str = 'trend',
        ...
        if(self.syncMarker_diagnoses + self.syncMarker_dynamic + self.syncMarker_nodes + self.syncMarker_trend == 0):
            ...
            try:
                endpoint_triggerSycnhMarker = f"/v1/deltasync?lastSyncPosition=0&typesToInclude={typesToIncludeValue}&maxNumberOfRecords=1"
                self._make_request("GET", endpoint_triggerSycnhMarker, params=kwargs)   
            except:
                pass
        if(self.syncMarker_diagnoses + self.syncMarker_dynamic + self.syncMarker_nodes + self.syncMarker_trend == 0):
