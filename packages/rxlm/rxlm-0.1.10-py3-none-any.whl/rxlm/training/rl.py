...
class IMPOConfig(TypedDict):
    ...
    advantage_mode: Literal['batch', 'step']
...
class IMPOAlgorithm(RlAlgorithm):
    ...
    def __init__(self, config: Optional[PPOConfig] = None):
        ...
        self.debug_step = 0
        self.advantage_mode = config.get('advantage_mode', 'batch')
    ...
    def calculate_advantages(self, rewards: torch.Tensor, values: torch.Tensor, dones: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_distributed_advantage_norm:
            ...
            if self.advantage_mode == 'batch':
                normalized_advantages = self.normalize_advantages_flatten(advantages, 1 - dones)
            else:
                normalized_advantages = self.normalize_advantages_per_step(advantages, 1 - dones)
        return normalized_advantages, ref_values

    def normalize_advantages_per_step(self, advantages: torch.Tensor, mask: torch.Tensor, eps=1e-8,
                                      clip=10.0) -> torch.Tensor:
        """
        advantages: [T, B]
        mask: [T, B]
