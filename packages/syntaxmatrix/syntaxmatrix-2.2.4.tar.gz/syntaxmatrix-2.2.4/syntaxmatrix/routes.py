        ...
        if request.method == "POST" and "askai_question" in request.form:
            ...
            if df is not None:      
                ...
                with contextlib.redirect_stdout(_buf_out), contextlib.redirect_stderr(_buf_err):
                    ...
                    html_path = os.path.join(MLEARNING_OUTPUTS, f"result_{session_id}.html")
                    with open(html_path, "w", encoding="utf-8") as f:
                        f.write(html_doc)

                    # 2.4 Append a single download button (no template edits needed)
                    dl_html = f"""
                    <div style="margin:14px 0;display:flex;gap:10px;flex-wrap:wrap">
                      <a class="btn" href="/download/result/html/{session_id}"
                        style="padding:8px 12px;border:1px solid #0b6;border-radius:6px;text-decoration:none;">
