...
class TauResult:
    """Result of a TAU clustering run."""
...
class TauClustering:
    ...
    def __init__(self, graph_source: str | Path | nx.Graph, config: Optional[TauConfig] = None):
    ...
    def run(self) -> TauResult:
        ...
        with Pool(
            ...
            for generation in range(1, self.config.max_generations + 1):
                ...
                history.append(best_modularity)
                ...
                print(f'Generation {generation} Top fitness: {best_modularity:.5f}; Average fitness: '
                      ...
                      f'elite_count={elite_count} ; immigrant_count={immigrant_count} ; offspring_count={offspring_count}')

        if best_partition is None:
            raise RuntimeError("TAU clustering failed to produce any solution.")

        best_mod = float(best_partition.fitness or -np.inf)
        return TauResult(
            membership=best_partition.membership.copy(),
            modularity=best_mod,
            history=history,
