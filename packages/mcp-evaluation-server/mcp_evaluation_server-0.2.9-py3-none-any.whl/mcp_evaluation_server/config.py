...
class Settings(SecureSettings):
    ...
    # 数据表名称
    # 注意：实际使用 mcp_test_results 表，保留此字段以保持向后兼容
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # 从环境变量或安全存储获取配置
        self.supabase_url = self.get_secure_api_key("SUPABASE_URL", "supabase_url")
        self.supabase_service_role_key = self.get_secure_api_key("SUPABASE_SERVICE_ROLE_KEY", "supabase_key")
