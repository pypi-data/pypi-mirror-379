...
try:
    ...
    try:
        ...
        SECURE_AVAILABLE = True
        print("✅ 成功导入C扩展配置（备用路径）")
    except ImportError:
        try:
            from .secure_key_manager import SecureKeyManager
            SECURE_AVAILABLE = True
            print("✅ 成功导入SecureKeyManager")
        except ImportError:
    ...
    def _init_secure_manager(self):
        ...
        if SECURE_AVAILABLE:
            try:
                ...
                print(f"Supabase URL: {_secure_config_manager.supabase_url[:30]}...")
                print(f"Supabase Key: {_secure_config_manager.supabase_service_role_key[:20]}...")
            except Exception as e:
                print(f"⚠️  C扩展配置初始化失败: {e}")
                # 回退到SecureKeyManager
                try:
                    from .secure_key_manager import SecureKeyManager
                    ...
                    print(f"⚠️  SecureKeyManager初始化失败: {e2}")
                    self._secure_manager = None
