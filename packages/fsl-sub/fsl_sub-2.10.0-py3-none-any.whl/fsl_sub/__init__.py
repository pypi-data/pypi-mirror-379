...
def submit(
    ...
    # If not already set, define FSLSUB_PARALLEL
    if threads is None:
        if 'FSLSUB_PARALLEL' not in os.environ.keys():
            logger.debug("Setting FSLSUB_PARALLEL to 0")
            os.environ['FSLSUB_PARALLEL'] = '0'
    else:
        logger.debug("Setting FSLSUB_PARALLEL to %s" % threads)
        os.environ['FSLSUB_PARALLEL'] = str(threads)
    update_envvar_list(
        my_export_vars,
        f"FSLSUB_PARALLEL={os.environ['FSLSUB_PARALLEL']}",
        overwrite=False)
    ...
    if mconfig['queues'] is False:
        logger.debug("Method lacks queues, set some defaults")
        queue = None
        split_on_ram = None
        queue_conf = {}
    else:
        if threads is None:
            logger.debug("No threads given, default to one")
            threads = 1
    ...
    logger.debug(
        ", ".join(
            [str(a) for a in [
                command, task_name, threads, queue, jobhold, array_task,
                array_hold, array_limit, array_specifier, parallel_env,
                jobram, jobtime, resources, ramsplit, priority,
