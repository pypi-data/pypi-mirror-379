...
class TestShell(unittest.TestCase):
    def setUp(self):
        yaml = YAML(typ='safe')
        dcf = str(files('fsl_sub').joinpath('default_config.yml'))
        with open(dcf, 'r') as yaml_source:
            yc = yaml.load(yaml_source)
            tvars = yc['thread_control']
        ...
        self.thread_vars = {}
        for tv in tvars:
            self.thread_vars.update({tv: str(self.available_cores)})
    ...
    def test_simple_job(self, mock_spr):
        ...
        env_dict = {
            'FSLSUB_JOBID_VAR': 'SHELL_JOB_ID',
            'FSLSUB_NSLOTS': 'SHELL_NCPUS',
            'SHELL_JOB_ID': f"{jid}",
            'SHELL_NCPUS': str(self.available_cores),
            'FSLSUB_PARALLEL': str(self.available_cores),
        }

        env_dict.update(self.thread_vars)
        mock_spr.assert_called_with(
            job,
            env=env_dict
        )
...
class TestShell__run(unittest.TestCase):
    def setUp(self):
        ...
        for tv in tvars:
            self.thread_vars.update({tv: str(self.available_cores)})
        self.p_env['FSLSUB_JOBID_VAR'] = 'SHELL_JOB_ID'
...
'''.format(self.bash)
    ...
    def test__run_job(self, *args):
        job = [self.job]
        fsl_sub.plugins.fsl_sub_plugin_shell._run_job(
            job, self.SHELL_JOB_ID, self.p_env,
...
''')
    ...
    def test__run_job_stderr(self, *args):
        job = [self.errorjob]
        errout = f'''jobid:{self.SHELL_JOB_ID}
taskid:
start:
end:
step:
'''
    ...
    def test__run_parallel_all(self, mock_gc, *args):
        ...
        fsl_sub.plugins.fsl_sub_plugin_shell._run_parallel(
            jobs, self.SHELL_JOB_ID, self.p_env,
            to_file=True, stdout_file=self.stdout, stderr_file=self.stderr
        )
        job_list = []
        for subjob in (1, 2, 3):
            ...
            child_env['SHELL_JOB_ID'] = self.SHELL_JOB_ID
...
step:{4}
'''.format(self.SHELL_JOB_ID, subjob, 1, 3, 1))
...
'''.format(self.SHELL_JOB_ID, subjob, 1, 3, 1))
    ...
    def test__run_parallel_threadlimited(self, mock_gc, *args):
        ...
        jobs = [self.job, self.job, self.job, ]
        fsl_sub.plugins.fsl_sub_plugin_shell._run_parallel(
            jobs, self.SHELL_JOB_ID, self.p_env, to_file=True,
            stdout_file=self.stdout, stderr_file=self.stderr,
            parallel_limit=2
        )
...
'''.format(self.SHELL_JOB_ID, subjob, 1, 3, 1))
    ...
    def test_submit(self, mock__run_job, mock_getpid, *args):
        ...
        result_environ['FSLSUB_JOBID_VAR'] = 'SHELL_JOB_ID'
        result_environ['FSLSUB_NSLOTS'] = 'SHELL_NCPUS'
        result_environ['SHELL_NCPUS'] = str(self.available_cores)
        result_environ['FSLSUB_PARALLEL'] = str(self.available_cores)
        result_environ.update(self.thread_vars)
        # result_environ['SHELL_JOB_ID'] = str(mock_pid) - mocked so doesn't get set
        with patch.dict(
                'fsl_sub.plugins.fsl_sub_plugin_shell.os.environ',
                test_environ,
                clear=True):
    ...
    def test_quoted_arg_submit(
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            result_environ['SHELL_NCPUS'] = str(self.available_cores)
            result_environ['FSLSUB_NSLOTS'] = 'SHELL_NCPUS'
            result_environ['SHELL_JOB_ID'] = str(mock_pid)
            result_environ['FSLSUB_PARALLEL'] = str(self.available_cores)
