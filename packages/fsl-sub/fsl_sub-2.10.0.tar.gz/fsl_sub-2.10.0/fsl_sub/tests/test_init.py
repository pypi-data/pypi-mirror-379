...
def mock_submit(mocker):
    ...
    plugins['fsl_sub_plugin_shell'] = FakePlugin()
    plugins['fsl_sub_plugin_shell'].submit = MagicMock(name='submit')
    plugins['fsl_sub_plugin_shell'].qtest = MagicMock(name='qtest')
    plugins['fsl_sub_plugin_shell'].qtest.return_value = True
    plugins['fsl_sub_plugin_shell'].queue_exists = MagicMock(
        name='queue_exists')
    plugins['fsl_sub_plugin_shell'].queue_exists.return_value = True
    plugins['fsl_sub_plugin_shell'].BadSubmission = BadSubmission
...
def test_set_fslsub_parallel_shell(mock_submit, monkeypatch):
    monkeypatch.delenv('FSLSUB_PARALLEL', raising=False)
    test_conf = copy.deepcopy(mock_submit['config'])
    test_conf['method'] = 'shell'
    set_config(mock_submit, test_conf)
    ...
    fsl_sub.submit(
        ...
        queue='vs.q')
    assert 'FSLSUB_PARALLEL' in os.environ
    assert os.environ['FSLSUB_PARALLEL'] == '0'
    monkeypatch.delenv('FSLSUB_PARALLEL')


def test_set_fslsub_parallel_grid(mock_submit, monkeypatch):
    monkeypatch.delenv('FSLSUB_PARALLEL', raising=False)
    fsl_sub.submit(
        'mytasks',
        array_task=True,
        ...
        queue='vs.q')
    assert 'FSLSUB_PARALLEL' in os.environ
    assert os.environ['FSLSUB_PARALLEL'] == '1'
    monkeypatch.delenv('FSLSUB_PARALLEL')


def test_set_fslsub_parallel2(mock_submit, monkeypatch, mocker):
    mocker.patch(
    ...
    fsl_sub.submit(
        ...
        threads=2)
    assert 'FSLSUB_PARALLEL' in os.environ
    assert os.environ['FSLSUB_PARALLEL'] == '2'
    monkeypatch.delenv('FSLSUB_PARALLEL')


def test_set_fslsub_parallel3(mock_submit, monkeypatch, mocker):
    mocker.patch(
        "fsl_sub.parallel.process_pe_def",
        autospec=True,
        return_value=('openmp', 2)),
    monkeypatch.setenv('FSLSUB_PARALLEL', '4')
    fsl_sub.submit(
        'mytasks',
        array_task=True,
        validate_command=False,
...
def test_set_fslsub_parallel4(mock_submit, monkeypatch):
    monkeypatch.setenv('FSLSUB_PARALLEL', '4')
    fsl_sub.submit(
        './mytask',
        validate_command=False,
...
def test_set_fslsub_parallel5(mock_submit, monkeypatch, mocker):
    mocker.patch(
        "fsl_sub.parallel.process_pe_def",
        autospec=True,
        return_value=('openmp', 4)),
    monkeypatch.setenv('FSLSUB_PARALLEL', '2')
    fsl_sub.submit(
        'mytasks',
        array_task=True,
        validate_command=False,
