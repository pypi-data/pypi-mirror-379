...
class WeChatAutomator:
    ...
    def check_and_wait(self, reply_counter, threshold=5, wait_time=5):
        """
        检查回复数量，如果达到阈值则等待指定时间
        :param threshold:5条回复
        :param wait_time:5分钟
        :return:
        """
        try:
            if reply_counter % threshold == 0 and reply_counter != 0:
    ...
    def video_item_comment(self,next_sibling,video_data,comment_list:list=None,comment_key:list=None):
        ...
        while True:
            try:
                self.check_and_wait(replay_count)
                """
