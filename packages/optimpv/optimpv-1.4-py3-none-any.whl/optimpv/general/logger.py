...
# Replace with local ClassDecorator or import if available
...
OPTIMPV_ROOT_LOGGER_NAME = "optimpv"
DEFAULT_LOG_LEVEL: int = logging.INFO
T = TypeVar("T")


class OptimpvOutputNameFilter(logging.Filter):
...
def get_logger(
    ...
    Args:
        ...
        force_name: If set to false and the module specified
            is not ultimately a descendent of the `optimpv` module
            specified by `name`, "optimpv." will be prepended to `name`

    Returns:
        The logging.Logger object.
    """
    if not force_name and not re.search(rf"^{OPTIMPV_ROOT_LOGGER_NAME}(\.|$)", name):
        name = f"{OPTIMPV_ROOT_LOGGER_NAME}.{name}"
    logger = logging.getLogger(name)
    logger.setLevel(level)
    logger.addFilter(OptimpvOutputNameFilter())
    return logger


def build_stream_handler(level: int = DEFAULT_LOG_LEVEL) -> logging.StreamHandler:
    """Build the default stream handler used for most optimpv logging. Sets
...
def set_optimpv_logger_levels(level: int) -> None:
    """Set the log level for all optimpv loggers, such that logs of given level
    are printed to STDERR by the root logger
    """

    for logger in logging.Logger.manager.loggerDict.values():
        if isinstance(logger, logging.Logger) and logger.name.startswith(
            OPTIMPV_ROOT_LOGGER_NAME
        ):
            logger.setLevel(level)
...
"""Sets up optimpv's root logger to not propagate to Python's root logger and
use the default stream handler.
"""
ROOT_LOGGER: logging.Logger = get_logger(OPTIMPV_ROOT_LOGGER_NAME)
