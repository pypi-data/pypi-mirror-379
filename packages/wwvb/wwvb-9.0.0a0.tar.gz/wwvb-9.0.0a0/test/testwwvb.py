...
def replace_am(minute: wwvb.WWVBTimecode, replaced: dict[int, int]) -> wwvb.WWVBTimecode:
    """Change AM data at arbitrary positions, returning a fresh timecode object"""
    am = tuple(wwvb.AmplitudeModulation(replaced.get(i, ai)) for i, ai in enumerate(minute.am))
    return dataclasses.replace(minute, am=am)
...
class WWVBRoundtrip(unittest.TestCase):
    ...
    def test_decode(self) -> None:
        ...
        for _ in range(20):
            ...
            decoded: wwvb.WWVBTimecodeAM | None = None
    ...
    def test_noise2(self) -> None:
        ...
        for position in uwwvb.always_mark:
            for noise in (0, 1):
                test_input = replace_am(timecode, {position: noise})
        ...
        for i in range(8):
            if i in (0b101, 0b010):  # Test the 6 impossible bit-combos
                continue
            test_input = replace_am(timecode, {36: i & 1, 37: (i >> 1) & 1, 38: (i >> 2) & 1})
            decoded = wwvb.WWVBMinute.from_timecode_am(test_input)
            self.assertIsNone(decoded)
        # Invalid year-day
        test_input = replace_am(timecode, {22: 1, 23: 1, 25: 1})
        decoded = wwvb.WWVBMinute.from_timecode_am(test_input)
        self.assertIsNone(decoded)
    ...
    def test_noise3(self) -> None:
        ...
        for poslist in [
            ...
            with self.subTest(test=poslist):
                test_input = replace_am(timecode, dict.fromkeys(poslist, 1))
                decoded = wwvb.WWVBMinute.from_timecode_am(test_input)
                self.assertIsNone(decoded)
    ...
    def test_timecode_str(self) -> None:
        ...
        timecode_am = minute.as_timecode_am()
        self.assertEqual(
            repr(timecode_am),
            "<WWVBTimecodeAM 210100000200100001120001010002001000010201100100120010011112>",
        )
        timecode_pm = minute.as_timecode_pm()
        self.assertEqual(
            repr(timecode_pm),
            "<WWVBTimecodePhase vv^^^v^^v^vvvvv^v^^vv^^^vv^^vvv^v^vv^v^^^^^v^^v^^^^^^vv^vvvv>",
        )
    ...
    def test_invalid_minute(self) -> None:
        ...
        am = list(minute.am)
        wwvb._put_am_bcd(am, 61, 1, 2, 3, 5, 6, 7, 8)  # valid BCD, invalid minute
        minute = dataclasses.replace(minute, am=am)
    ...
    def test_invalid_hour(self) -> None:
        ...
        minute = base_minute.as_timecode()
        am = list(minute.am)
        wwvb._put_am_bcd(am, 29, 12, 13, 15, 16, 17, 18)  # valid BCD, invalid hour
        minute = dataclasses.replace(minute, am=am)
        decoded_minute = wwvb.WWVBMinute.from_timecode_am(minute)
        assert decoded_minute is None
    ...
    def test_invalid_bcd_day(self) -> None:
        ...
        base_minute = wwvb.WWVBMinute(2021, 1, 1, 0, 0)
        minute = base_minute.as_timecode()
        minute = replace_am(minute, dict.fromkeys(range(30, 34), 1))
        decoded_minute = wwvb.WWVBMinute.from_timecode_am(minute)
        assert decoded_minute is None
    ...
    def test_invalid_mark(self) -> None:
        ...
        base_minute = wwvb.WWVBMinute(2021, 1, 1, 0, 0)
        minute = base_minute.as_timecode()
        minute = replace_am(minute, {57: wwvb.AmplitudeModulation.MARK})
        decoded_minute = wwvb.WWVBMinute.from_timecode_am(minute)
        assert decoded_minute is None

    def test_unset_value(self) -> None:
        """Check that the _require_not_none utility function works as designed"""
        with self.assertRaises(ValueError):
            wwvb._require_not_none(None)
