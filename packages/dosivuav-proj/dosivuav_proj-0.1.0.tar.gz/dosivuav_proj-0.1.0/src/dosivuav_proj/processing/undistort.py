...
def _load_calibration_data() -> tuple[np.ndarray, np.ndarray]:
    ...
    This function loads the camera matrix and distortion coefficients from
    the calibration data file specified in the config module.
    ...
    if not os.path.exists(utils.CALIBRATION_DATA_PATH):
        log.error("Camera calibration data not found. Calibrating now...")
...
def undistort_image(image: MatLike) -> MatLike:
    ...
    matrix, dist_coeffs = _load_calibration_data()
    if not os.path.exists(utils.CALIBRATION_DATA_PATH):
