...
def clear_output_data() -> None:
    ...
    # Remove output directories if they are empty
    if os.path.exists(outputs_dir) and os.path.isdir(outputs_dir):
        try:
            shutil.rmtree(outputs_dir)
            log.info("Output directories removed.")
        except OSError as e:
            log.error(f"Error removing output directories: {e}")
    else:
        log.info("No output directories to remove.")
...
def validate_base_name(image_name: str | None, video_name: str | None) -> bool:
    ...
    if image_name:
        if not os.path.isfile(IMAGE_TO_UNDISTORT.format(name=image_name)):
            raise FileNotFoundError(f"Image file not found: {image_name}")
        return True

    if video_name:
        if not os.path.isfile(PROJECT_VIDEO_PATH.format(name=video_name)):
