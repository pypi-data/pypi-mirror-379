...
"""
    ...
    def plot_residuals_analysis(self):
        """Análisis de residuos completo con residuos normalizados"""
        if not self.processor or not self.processor.model_results:
            messagebox.showwarning("Advertencia", "No hay resultados para visualizar")
            return
        ...
        try:
            ...
            # Calcular residuos normales
            residuals = y_true - y_pred
            
            # Calcular residuos normalizados (1 - pred/real)
            # Evitar division por cero
            y_true_safe = y_true.copy()
            y_true_safe[y_true_safe == 0] = 1e-10  # Reemplazar ceros con valor muy pequeño
            normalized_residuals = 1 - (y_pred / y_true_safe)
            ...
            # 1. Residuos NORMALIZADOS vs Predicciones
            ax1 = self.plot_figure.add_subplot(gs[0, 0])
            ax1.scatter(y_pred, normalized_residuals, alpha=0.6, s=30, color='blue')
            ax1.axhline(y=0, color='red', linestyle='--', linewidth=2)
            ax1.set_xlabel('Predicciones')
            ax1.set_ylabel('Residuos Normalizados (1 - pred/real)')
            ax1.set_title('Residuos Normalizados vs Predicciones')
            ax1.grid(True, alpha=0.3)
            
            # Añadir línea de tendencia de residuos normalizados
            z = np.polyfit(y_pred, normalized_residuals, 1)
            p = np.poly1d(z)
            ax1.plot(y_pred, p(y_pred), "r-", alpha=0.8, linewidth=1)
            
            # Añadir estadísticas del gráfico
            mean_norm_error = np.mean(np.abs(normalized_residuals))
            std_norm_error = np.std(normalized_residuals)
            ax1.text(0.02, 0.98, f'Error Norm Medio: {mean_norm_error:.4f}\nStd: {std_norm_error:.4f}', 
                    transform=ax1.transAxes, verticalalignment='top', 
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
            
            # 2. Q-Q plot (normalidad de residuos normalizados)
            ax2 = self.plot_figure.add_subplot(gs[0, 1])
            from scipy import stats
            stats.probplot(normalized_residuals, dist="norm", plot=ax2)
            ax2.set_title('Q-Q Plot - Residuos Normalizados')
            ax2.grid(True, alpha=0.3)
            
            # 3. Residuos NORMALIZADOS vs Valores Reales
            ax3 = self.plot_figure.add_subplot(gs[1, 0])
            ax3.scatter(y_true, normalized_residuals, alpha=0.6, s=30, color='green')
            ax3.axhline(y=0, color='red', linestyle='--', linewidth=2)
            ax3.set_xlabel('Valores Reales')
            ax3.set_ylabel('Residuos Normalizados (1 - pred/real)')
            ax3.set_title('Residuos Normalizados vs Valores Reales')
            ax3.grid(True, alpha=0.3)
            
            # 4. Distribución de residuos normalizados
            ax4 = self.plot_figure.add_subplot(gs[1, 1])
            n, bins, patches = ax4.hist(normalized_residuals, bins=30, alpha=0.7, color='orange', 
                                    density=True, edgecolor='black')
            
            # Estadísticas de residuos normalizados
            mean_norm = np.mean(normalized_residuals)
            std_norm = np.std(normalized_residuals)
            
            # Líneas de referencia
            ax4.axvline(x=0, color='red', linestyle='--', linewidth=2, label='Error Norm = 0')
            ax4.axvline(x=mean_norm, color='green', linestyle='-', linewidth=2, 
                    label=f'Media = {mean_norm:.4f}')
            ax4.axvline(x=mean_norm + std_norm, color='purple', linestyle=':', alpha=0.7, 
                    label=f'±1σ = ±{std_norm:.4f}')
            ax4.axvline(x=mean_norm - std_norm, color='purple', linestyle=':', alpha=0.7)
            
            # Superponer distribución normal para comparación
            x = np.linspace(normalized_residuals.min(), normalized_residuals.max(), 100)
            ax4.plot(x, stats.norm.pdf(x, mean_norm, std_norm), 'b-', linewidth=2, 
                    alpha=0.7, label=f'Normal({mean_norm:.3f}, {std_norm:.3f})')
            
            ax4.set_xlabel('Residuos Normalizados')
            ax4.set_ylabel('Densidad')
            ax4.set_title('Distribución de Residuos Normalizados')
            ax4.legend(fontsize=8)
            ax4.grid(True, alpha=0.3)
            
            # Añadir texto con métricas adicionales
            max_norm_error = np.max(np.abs(normalized_residuals))
            percentile_95 = np.percentile(np.abs(normalized_residuals), 95)
            
            stats_text = f'''Métricas de Residuos Normalizados:
    Error Norm Medio: {mean_norm_error:.4f}
    Std Error Norm: {std_norm_error:.4f}
    Error Norm Máximo: {max_norm_error:.4f}
    Percentil 95: {percentile_95:.4f}
            
    Interpretación:
    • Valores cercanos a 0: predicción precisa
    • Valores > 0: subestimación (pred < real)  
    • Valores < 0: sobreestimación (pred > real)'''
            
            ax4.text(0.02, 0.98, stats_text, transform=ax4.transAxes, 
                    verticalalignment='top', fontsize=8,
                    bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
            
            self.plot_figure.suptitle(f'Análisis de Residuos Normalizados - {model_display}', 
                                    fontsize=14, fontweight='bold')
